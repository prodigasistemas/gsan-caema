...
//--------------------------------------------------------------------------------
	...
	/**
	 * [UC0617] - Consultar Resumo das Acoes de Cobranca Eventual Popup de
	 *  Resumo do Retorno de fiscalizacao
	 * 
	 ...
	 * @throws ErroRepositorioException
	 */
	public Collection consultarCobrancaAcaoSituacaoAcaoAnoMesEventual(
			Integer idCobrancaAcao,
			Integer idCobrancaAcaoSituacao,
			InformarDadosGeracaoResumoAcaoConsultaEventualHelper informarDadosGeracaoResumoAcaoConsultaEventualHelper,
			String anoMes)
		...
		try {
		  ...
		  consulta = "select new gcom.gerencial.bean.ResumoCobrancaAcaoSituacaoAcaoDetalhesHelper( fisu.id, " 
						+ " fisu.descricaoFiscalizacaoSituacao, "
						+ " count(cbdo.id), sum(cbdo.valorDocumento)) "
						+ " from OrdemServico os "
						+ " INNER JOIN os.cobrancaDocumento  cbdo "
						+ " INNER JOIN cbdo.cobrancaAcaoAtividadeComando cacm "
						+ " INNER JOIN cacm.cobrancaAcao cbac "
						+ " INNER JOIN cbdo.cobrancaAcaoSituacao castu "
						+ " INNER JOIN os.fiscalizacaoSituacao fisu"
						+ " WHERE cacm.id in   (select distinct(re.cobrancaAcaoAtividadeComando.id) "
											+ " from ResumoCobrancaAcaoEventual re "
										    + " inner join re.cobrancaAcao cbac "
										    + " inner join re.cobrancaAcaoSituacao castu " 
										    + " WHERE  cacm.id = re.cobrancaAcaoAtividadeComando.id ";
										    
										    consulta = consulta
											+ this
													.criarCondicionaisResumosEventuaisHQL(informarDadosGeracaoResumoAcaoConsultaEventualHelper);
		    consulta = consulta + ")"
		    					+ "and cbac.id = :idCobrancaAcao ";
					...
					if (anoMes != null) {
						consulta = consulta + "and  to_char(os.dataFiscalizacaoSituacao, 'YYYYMM') = '"
								+ anoMes + "'";
					}
					
			consulta = consulta + "group by fisu.id, fisu.descricaoFiscalizacaoSituacao "
								+ "order by fisu.id ";
	 ...
	 * @throws ErroRepositorioException
	 */
	public Collection consultarAnoMesRetornoResumoFiscalizacaoEventual(
			Integer idCobrancaAcao,
			Integer idCobrancaAcaoSituacao,
			InformarDadosGeracaoResumoAcaoConsultaEventualHelper informarDadosGeracaoResumoAcaoConsultaEventualHelper)
			throws ErroRepositorioException {
		...
		try {
		  ...
		  consulta = "select new gcom.gerencial.bean.ResumoAnoMesRetornoFiscalizacaoHelper( " 
						+ " to_char(os.dataFiscalizacaoSituacao, 'YYYYMM'), " 
						+ " count(cbdo.id), sum(cbdo.valorDocumento) )"
						+ " from OrdemServico os "
						+ " INNER JOIN os.cobrancaDocumento  cbdo "
						+ " INNER JOIN cbdo.cobrancaAcaoAtividadeComando cacm "
			...
			consulta = consulta + "group by to_char(os.dataFiscalizacaoSituacao, 'YYYYMM') "
								+ "order by to_char(os.dataFiscalizacaoSituacao, 'YYYYMM') ";
	...
	public Collection consultarCobrancaAcaoEventual(
		...
		try {
			consulta = " select new gcom.gerencial.bean.CobrancaAcaoHelper (cbac.id, cbac.descricaoCobrancaAcao, "
					+ " cbac.servicoTipo.id, min(re.tempoRealizacaoEmitir), max(re.tempoRealizacaoEncerrar)," 
					+ " sum(CASE WHEN castu.id <> :idCobrancaAcaoSit THEN re.quantidadeDocumentos WHEN castu.id = :idCobrancaAcaoSit " 
					+ " AND re.fiscalizacaoSituacao.id IN (SELECT fiz.id FROM FiscalizacaoSituacao fiz) THEN re.quantidadeDocumentos END), "
					+ " sum(CASE WHEN castu.id <> :idCobrancaAcaoSit THEN re.valorDocumentos WHEN castu.id = :idCobrancaAcaoSit " 
					+ " AND re.fiscalizacaoSituacao.id IN (SELECT fiz.id FROM FiscalizacaoSituacao fiz) THEN re.valorDocumentos END),  "
					+ " cbac.numeroDiasRemuneracaoTerceiro) "
					+ " from ResumoCobrancaAcaoEventual re"
					+ " inner join re.cobrancaAcao cbac"
					+ " inner join re.cobrancaAcaoSituacao castu "
					+ " where 1 = 1 "
			...
			if (informarDadosGeracaoResumoAcaoConsultaEventualHelper
				...
				retorno = session.createQuery(consulta).setTimestamp(
						...
						"dataFinal",
						informarDadosGeracaoResumoAcaoConsultaEventualHelper
								.getDataFinalEmissao()).setInteger(
									"idCobrancaAcaoSit", CobrancaAcaoSituacao.FISCALIZADA).list();
			} else {
				retorno = session.createQuery(consulta).setInteger(
					"idCobrancaAcaoSit", CobrancaAcaoSituacao.FISCALIZADA).list();
			}
	...
	public Collection consultarCobrancaAcaoSituacaoEventual(
		...
		try {
			consulta = " select new gcom.gerencial.bean.CobrancaAcaoSituacaoHelper(castu.id, castu.descricao, " 
					+ " sum(CASE WHEN castu.id <> :idCobrancaAcaoSit THEN re.quantidadeDocumentos WHEN castu.id = :idCobrancaAcaoSit " 
					+ " AND re.fiscalizacaoSituacao.id IN (SELECT fiz.id FROM FiscalizacaoSituacao fiz) THEN re.quantidadeDocumentos END), "
					+ " sum(CASE WHEN castu.id <> :idCobrancaAcaoSit THEN re.valorDocumentos WHEN castu.id = :idCobrancaAcaoSit " 
					+ " AND re.fiscalizacaoSituacao.id IN (SELECT fiz.id FROM FiscalizacaoSituacao fiz) THEN re.valorDocumentos END))  "
	...
	public Collection consultarCobrancaAcaoDebitoEventual(
		...
		try {
			consulta = " select new gcom.gerencial.bean.CobrancaAcaoDebitoHelper (cdst.id, cdst.descricao, " 
					+ " sum(CASE WHEN castu.id <> :idCobrancaAcaoSit THEN re.quantidadeDocumentos WHEN castu.id = :idCobrancaAcaoSit " 
					+ " AND re.fiscalizacaoSituacao.id IN (SELECT fiz.id FROM FiscalizacaoSituacao fiz) THEN re.quantidadeDocumentos END), "
					+ " sum(CASE WHEN castu.id <> :idCobrancaAcaoSit THEN re.valorDocumentos WHEN castu.id = :idCobrancaAcaoSit " 
					+ " AND re.fiscalizacaoSituacao.id IN (SELECT fiz.id FROM FiscalizacaoSituacao fiz) THEN re.valorDocumentos END)) "
			...
			if (informarDadosGeracaoResumoAcaoConsultaEventualHelper
				...
				retorno = session.createQuery(consulta).setInteger(
						"idCobrancaAcao", idCobrancaAcao).setInteger(
						"idCobrancaAcaoSituacao", idCobrancaAcaoSituacao).setInteger(
						"idCobrancaAcaoSit", CobrancaAcaoSituacao.FISCALIZADA)
						.list();
			}
	...
	public List getDebitosACobrarGerencia(int idQuadra)
		...
		try {
			String hql =
				...
				"  inner join dc.lancamentoItemContabil as lanc " + //RM7720
				...
				" and qua.id = :idQuadra" +
				" and (lanc.id <> 2 or (lanc.id = 2 and dc.anoMesRefContabilizacao < 201203 )) " + //RM7720
				"  order by" +
				"  imo.id, dc.id";
