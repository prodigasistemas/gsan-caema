...
//				 INDICE 89 - Quantidade Economias, Grupo Subcategoria 101				    
	...
	/**
	 * [UC1307] - Gerar Relatório Kit CAS Serviços
	 * 
	 * @author Hugo Azevedo
	 * @date 28/03/2012 
	 * 
	 */
	public Collection obterColecaoUnidadeNegocioGerencial() throws ErroRepositorioException{
		...
		try{
			...
			sql = " select gUnidade " +
				  " from GUnidadeNegocio gUnidade " +
				  " where gUnidade.indicadorUso = :indicadorUso " +
				  " order by gUnidade.nome";
	 ...
	 * 
	 */
	public boolean validarReferenciaGerencial(String anoMesReferencia) throws ErroRepositorioException{
		...
		try{
			...
			//inexistência de ocorrências na tabela micromedicao.UN_RES_INS_HIDR com  REIH_AMREFERENCIA =ano/mês informado
			sql = " select count(*) as resultado" +
				  " from micromedicao.UN_RES_INS_HIDR " +
				  " where REIH_AMREFERENCIA = :anoMesReferencia";
			
			Integer result1 = (Integer)session.createSQLQuery(sql)
					          .addScalar("resultado", Hibernate.INTEGER)
					          .setInteger("anoMesReferencia", new Integer(anoMesReferencia))
					          .uniqueResult();
							  
			
			//inexistência de ocorrências na tabela cadastro.UN_RES_LIG_ECON  com RELE_AMREFERENCIA=ano/mês informado
			sql = " select count(*) as resultado" +
				  " from cadastro.UN_RES_LIG_ECON " +
				  " where RELE_AMREFERENCIA = :anoMesReferencia";
			
			Integer result2 = (Integer)session.createSQLQuery(sql)
					          .addScalar("resultado", Hibernate.INTEGER)
					          .setInteger("anoMesReferencia", new Integer(anoMesReferencia))
					          .uniqueResult();
			
			
			//inexistência de ocorrências na tabela UN_RESUMO_FATURAMENTO com REFA_AMREFERENCIA=ano/mês informado
			sql = " select count(*) as resultado" +
				  " from faturamento.UN_RESUMO_FATURAMENTO " +
				  " where REFA_AMREFERENCIA = :anoMesReferencia";
			
			Integer result4 = (Integer)session.createSQLQuery(sql)
					          .addScalar("resultado", Hibernate.INTEGER)
					          .setInteger("anoMesReferencia", new Integer(anoMesReferencia))
					          .uniqueResult();
			
			//inexistência de ocorrências na tabela micromedicao.UN_RES_LT_ANORM com RELT_AMREFERENCIA=ano/mês informado
			sql = " select count(*) as resultado" +
				  " from micromedicao.UN_RES_LT_ANORM " +
				  " where RELT_AMREFERENCIA = :anoMesReferencia";
			
			Integer result5 = (Integer)session.createSQLQuery(sql)
					          .addScalar("resultado", Hibernate.INTEGER)
					          .setInteger("anoMesReferencia", new Integer(anoMesReferencia))
					          .uniqueResult();
						
			//inexistência de ocorrências na tabela micromedicao.UN_RESUMO_CONSUMO_AGUA com RECA_AMREFERENCIA=ano/mês informado
			sql = " select count(*) as resultado" +
				  " from micromedicao.UN_RESUMO_CONSUMO_AGUA " +
				  " where RECA_AMREFERENCIA = :anoMesReferencia";
			
			Integer result6 = (Integer)session.createSQLQuery(sql)
					          .addScalar("resultado", Hibernate.INTEGER)
					          .setInteger("anoMesReferencia", new Integer(anoMesReferencia))
					          .uniqueResult();
			
			//inexistência de ocorrências na tabela atendimentopublico.UN_RES_KIT_CAS_SERVICO  com RkcA_AMREFERENCIA=ano/mês informado
			sql = " select count(*) as resultado" +
				  " from atendimentopublico.UN_RES_KIT_CAS_SERVICO " +
				  " where RKCA_AMREFERENCIA = :anoMesReferencia";
			
			Integer result7 = (Integer)session.createSQLQuery(sql)
					          .addScalar("resultado", Hibernate.INTEGER)
					          .setInteger("anoMesReferencia", new Integer(anoMesReferencia))
					          .uniqueResult();
			
			if(
				result1.compareTo(zero) > 0
			&&  result2.compareTo(zero) > 0
			&&  result4.compareTo(zero) > 0
			&&  result5.compareTo(zero) > 0
			&&  result6.compareTo(zero) > 0
			&&  result7.compareTo(zero) > 0
					){
				retorno = true;
			}
	 ...
	 * 
	 */
	public Collection obterDadosKitCASInstalacaoHidrometro(
			FiltrarResumoDadosCasHelper helper, Integer idCategoria, Integer idPerfilImovel) throws ErroRepositorioException{
		
		 Session session = HibernateUtil.getSessionGerencial();
         Collection retorno = null;
         String consulta = null;
         
         Map query = totalizadorResumoDadosCas(helper);
         ...
         try {
         	consulta = "select  "+
					   "COALESCE(sum(COALESCE(r.REIH_QTHIDR_SUBSTITUIDO_RAMAL,0)),0) as hidrSubstituidosRamal, "+
					   "COALESCE(sum(COALESCE(r.REIH_QTHIDR_SUBSTITUIDO_POCO,0)),0) as hidrSubstituidosPoco, "+
					   "COALESCE(sum(COALESCE(r.REIH_QTHIDR_INSTALADO_RAMAL,0)),0) as hidrInstaladosRamal, "+
					   "COALESCE(sum(COALESCE(r.REIH_QTHIDR_INSTALADO_POCO,0)),0) as hidrInstaladosPoco, "+
         			    ...
         			    if(tipo != null && tipo.equals("totalizadorPorEstado")){
         			    	consulta += "and r.reih_amreferencia = :referencia where "; 
         			    }else{
         			    	consulta += "where r.reih_amreferencia = :referencia and ";
         			    }
         			    
         			    if(idCategoria != null)
         			    	consulta += " r.catg_id = :idCategoria ";
         			    else if(idPerfilImovel != null)
         			    	consulta += " r.iper_id = :idPerfilImovel ";
            ...
            Query q = session.createSQLQuery(consulta)
             .addScalar("hidrSubstituidosRamal", Hibernate.INTEGER)
             .addScalar("hidrSubstituidosPoco", Hibernate.INTEGER)
             .addScalar("hidrInstaladosRamal", Hibernate.INTEGER)
             .addScalar("hidrInstaladosPoco", Hibernate.INTEGER)
	         ...
	         .setInteger("referencia",helper.getAnoMesReferencia());
            
            if(idCategoria != null)
            	q.setInteger("idCategoria", idCategoria);
            else if(idPerfilImovel != null)
            	q.setInteger("idPerfilImovel", idPerfilImovel);
	 ...
	 * 
	 */
	public Collection obterDadosKitCASLigacaoEconomia(
			FiltrarResumoDadosCasHelper helper, Integer idCategoria, Integer idPerfilImovel) throws ErroRepositorioException{
         ...
         try {
         	consulta = "select  "+
         				  "COALESCE(sum(COALESCE(r.RELE_QTLIGACOESCORTESMES,0)),0) as corExec, "+
         		          "COALESCE(sum(COALESCE(r.RELE_QTLIGACOESRELIGADASMES,0)),0) as relig, "+
         		          "COALESCE(sum(COALESCE(r.RELE_QTLIGACOESNOVASAGUA,0)),0) as ligImpAgua, "+
         		          "COALESCE(sum(COALESCE(r.RELE_QTLIGACOESNOVASESGOTO,0)),0) as ligImpEsgoto, "+      
	 			          gerenciaRegionalID + " AS gerenciaRegionalID, " +
	            		  gerenciaRegional + " AS gerenciaRegional, " +
	 			          unidadeNegocioID + " AS unidadeNegocioID, " +
	            		  unidadeNegocio + " AS unidadeNegocio, " +
         			    ...
         			    if(tipo != null && tipo.equals("totalizadorPorEstado")){
         			    	consulta += "and r.rele_amreferencia = :referencia where "; 
         			    }else{
         			    	consulta += "where r.rele_amreferencia = :referencia and ";
         			    }
            ...
            Query q = session.createSQLQuery(consulta)
             .addScalar("corExec", Hibernate.INTEGER)
             .addScalar("relig", Hibernate.INTEGER)
             .addScalar("ligImpAgua", Hibernate.INTEGER)
             .addScalar("ligImpEsgoto", Hibernate.INTEGER)
             .addScalar("gerenciaRegionalID", Hibernate.STRING)
	         .addScalar("gerenciaRegional", Hibernate.STRING)
	         .addScalar("unidadeNegocioID", Hibernate.STRING)
	         .addScalar("unidadeNegocio", Hibernate.STRING)
	 ...
	 * 
	 */
	public Collection obterDadosKitCASServicos(
			FiltrarResumoDadosCasHelper helper, Integer idCategoria, Integer idPerfilImovel) throws ErroRepositorioException{
         ...
         try {
         	consulta = "select  "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTSUPRESSOESEXECUTADAS,0)),0) as supExec, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTRESTABELECIMENTOS,0)),0) as restab, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTFISCORIUNDASCORTADOS,0)),0) as fiscCort, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTFISCORIUNDASSUPRIMIDOS,0)),0) as fiscSup, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTAVISOSCORTEEMITIDOS,0)),0) as avCorEm, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTRAPENDCOMERCNOPRAZO,0)),0) as raPenComPr, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTRAPENDCOMERCFORAPRAZO,0)),0) as raPenComForPr, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTRAPENDOPERACNOPRAZO,0)),0) as raPenOpPr, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTRAPENDOPERACFORAPRAZO,0)),0) as raPenOpForPr, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTFATURASREVISAO,0)),0) as fatRev, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTCARTASNEGATIVEMITIDAS,0)),0) as carNegEm, " +
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTANORMALIDADESINFORMADA,0)),0) as anInform, " +
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTREGISTROSRECEBIDOS,0)),0) as regRec, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTIMOVECONOMIAACRESC,0)),0) as qtdImovEconCres, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTECONOMIASACRESC,0)),0) as qtdEconCres, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTIMOVECONOMIADECRESC,0)),0) as qtdImovEconDecres, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTECONOMIASDECRESC,0)),0) as qtdEconDecres, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTALTERACAOCATEGORIA,0)),0) as qtdAlteracaoCategoria, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTIMOVINCLTARIFASOCIAL,0)),0) as qtdImovIncTarSocial, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTIMOVEXCLTARIFASOCIAL,0)),0) as qtdImovExcTarSocial, "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTFATURAMENTOSUSPENSO,0)),0) as qtdFaturamentoSuspenso, "+
	 			          gerenciaRegionalID + " AS gerenciaRegionalID, " +
	            		  gerenciaRegional + " AS gerenciaRegional, " +
	 			          unidadeNegocioID + " AS unidadeNegocioID, " +
	            		  unidadeNegocio + " AS unidadeNegocio, " +
         			     ...
         			     "from atendimentopublico.UN_RES_KIT_CAS_SERVICO r " +
         			     join;
         			    
         			    if(tipo != null && tipo.equals("totalizadorPorEstado")){
         			    	consulta += "and r.rkca_amreferencia = :referencia where "; 
         			    }else{
         			    	consulta += "where r.rkca_amreferencia = :referencia and ";
         			    }
         	...
         	Query q = session.createSQLQuery(consulta)
             .addScalar("supExec", Hibernate.INTEGER)
             .addScalar("restab", Hibernate.INTEGER)
             .addScalar("fiscCort", Hibernate.INTEGER)
             .addScalar("fiscSup", Hibernate.INTEGER)
             .addScalar("avCorEm", Hibernate.INTEGER)
             .addScalar("raPenComPr", Hibernate.INTEGER)
             .addScalar("raPenComForPr", Hibernate.INTEGER)
             .addScalar("raPenOpPr", Hibernate.INTEGER)
             .addScalar("raPenOpForPr", Hibernate.INTEGER)
             .addScalar("fatRev", Hibernate.INTEGER)
             .addScalar("carNegEm", Hibernate.INTEGER)
             .addScalar("anInform", Hibernate.INTEGER)
             .addScalar("regRec", Hibernate.INTEGER)
             .addScalar("gerenciaRegionalID", Hibernate.STRING)
	         .addScalar("gerenciaRegional", Hibernate.STRING)
	         .addScalar("unidadeNegocioID", Hibernate.STRING)
	         .addScalar("unidadeNegocio", Hibernate.STRING)
	         ...
	         .addScalar("municipioID", Hibernate.STRING)
	         .addScalar("municipio", Hibernate.STRING)
	         .addScalar("qtdImovEconCres", Hibernate.INTEGER)
	         .addScalar("qtdEconCres", Hibernate.INTEGER)
	         .addScalar("qtdImovEconDecres", Hibernate.INTEGER)
	         .addScalar("qtdEconDecres", Hibernate.INTEGER)
	         .addScalar("qtdAlteracaoCategoria", Hibernate.INTEGER)
	         .addScalar("qtdImovIncTarSocial", Hibernate.INTEGER)
	         .addScalar("qtdImovExcTarSocial", Hibernate.INTEGER)
	         .addScalar("qtdFaturamentoSuspenso", Hibernate.INTEGER)
	         .setInteger("referencia",helper.getAnoMesReferencia());
	 ...
	 * 
	 */
	public Collection obterDadosKitCASResumoFaturamento(
			FiltrarResumoDadosCasHelper helper, Integer idCategoria, Integer idPerfilImovel) throws ErroRepositorioException{
         ...
         try {
         	consulta = "select  "+
         				"COALESCE(sum(COALESCE(r.REFA_QTCONTASEMITIDAS,0)),0) as contEmit, "+ 
	 			          gerenciaRegionalID + " AS gerenciaRegionalID, " +
	            		  gerenciaRegional + " AS gerenciaRegional, " +
	 			          unidadeNegocioID + " AS unidadeNegocioID, " +
	            		  unidadeNegocio + " AS unidadeNegocio, " +
         			    ...
         			    if(tipo != null && tipo.equals("totalizadorPorEstado")){
         			    	consulta += "and r.refa_amreferencia = :referencia where "; 
         			    }else{
         			    	consulta += "where r.refa_amreferencia = :referencia and ";
         			    }
         	...
         	Query q = session.createSQLQuery(consulta)
             .addScalar("contEmit", Hibernate.INTEGER)
             .addScalar("gerenciaRegionalID", Hibernate.STRING)
	         .addScalar("gerenciaRegional", Hibernate.STRING)
	         .addScalar("unidadeNegocioID", Hibernate.STRING)
	         .addScalar("unidadeNegocio", Hibernate.STRING)
	 ...
	 * 
	 */
	public Collection obterDadosKitCASDesempenhoMicromedicao(
			FiltrarResumoDadosCasHelper helper, Integer idCategoria, Integer idPerfilImovel) throws ErroRepositorioException{
         ...
         try {
         	consulta = "select  "+
	         		   "COALESCE(SUM( " +
	       			   "        case when (r.reca_ichidrometro = 1) then " +
	       			   "          COALESCE(r.reca_qtligacoes,0) " +
	       			   "        else " +
	       			   "          0 " +
	       			   "        end " +
	       			   "        ),0) as ligHidr, "+
	       		       "COALESCE(sum(COALESCE(r.reca_qtligacoes,0)),0) as ligAt, "+
	 			          gerenciaRegionalID + " AS gerenciaRegionalID, " +
	            		  gerenciaRegional + " AS gerenciaRegional, " +
	 			          unidadeNegocioID + " AS unidadeNegocioID, " +
	            		  unidadeNegocio + " AS unidadeNegocio, " +
         			    ...
         			    if(tipo != null && tipo.equals("totalizadorPorEstado")){
         			    	consulta += "and r.reca_amreferencia = :referencia where "; 
         			    }else{
         			    	consulta += "where r.reca_amreferencia = :referencia and ";
         			    }
         	...
         	consulta = consulta + " and r.reca_icligacaofaturada = 1 ";
            ...
            Query q = session.createSQLQuery(consulta)
             .addScalar("ligHidr", Hibernate.INTEGER)
             .addScalar("ligAt", Hibernate.INTEGER)
             .addScalar("gerenciaRegionalID", Hibernate.STRING)
	         .addScalar("gerenciaRegional", Hibernate.STRING)
	         .addScalar("unidadeNegocioID", Hibernate.STRING)
	         .addScalar("unidadeNegocio", Hibernate.STRING)
	 ...
	 * 
	 */
	public Collection obterDadosKitCASLeituraAnormalidade(
			FiltrarResumoDadosCasHelper helper, Integer idCategoria, Integer idPerfilImovel) throws ErroRepositorioException{
         ...
         try {
         	consulta = "select  "+
	         		   " COALESCE(SUM( " +
	      			   "             case when (r.LTST_ID IN (1, 3, 4)) then " +
	      			   "               COALESCE(r.RELT_QTLEITURAS,0) " +
	      			   "             else " +
	      			   "               0 " +
	      			   "             end " +
	      			   "           ),0) AS leitEfetuada, " +
	      			   " COALESCE(SUM( " +
	      			   "             case when (r.ltan_id            <> 0) then " +
	      			   "               COALESCE(r.RELT_QTLEITURAS,0) " +
	      			   "             else " +
	      			   "               0 " +
	      			   "             end " +
	      			   "           ),0) AS leitAnorm, " + 
         			     ...
         			     "from micromedicao.UN_RES_LT_ANORM r " +
         			     join;
         	...
         	if(tipo != null && tipo.equals("totalizadorPorEstado")){
			    	...
			    	consulta += "where r.relt_amreferencia = :referencia and ";
            ...
            Query q = session.createSQLQuery(consulta)
             .addScalar("leitEfetuada", Hibernate.INTEGER)
             .addScalar("leitAnorm", Hibernate.INTEGER)
             .addScalar("gerenciaRegionalID", Hibernate.STRING)
	         .addScalar("gerenciaRegional", Hibernate.STRING)
	         .addScalar("unidadeNegocioID", Hibernate.STRING)
	         .addScalar("unidadeNegocio", Hibernate.STRING)
	 ...
	 * 
	 */
	public Collection obterDadosKitCASConsumoAgua(
			FiltrarResumoDadosCasHelper helper, Integer idCategoria, Integer idPerfilImovel) throws ErroRepositorioException{
         ...
         try {
        	 consulta = "select  "+
        			  "COALESCE(sum(COALESCE(r.RECA_QTECONOMIAS,0)),0) as ecAtivas, "+
			          gerenciaRegionalID + " AS gerenciaRegionalID, " +
           		  gerenciaRegional + " AS gerenciaRegional, " +
			          unidadeNegocioID + " AS unidadeNegocioID, " +
           		  unidadeNegocio + " AS unidadeNegocio, " +
    			   ...
    			   if(idCategoria != null)
   			    	consulta += "and r.catg_id = :idCategoria ";
   			    else if(idPerfilImovel != null)
   			    	consulta += "and r.iper_id = :idPerfilImovel ";
    	...
    	consulta += where +	                        
            ...
            Query q = session.createSQLQuery(consulta)
             .addScalar("ecAtivas", Hibernate.INTEGER)
             .addScalar("gerenciaRegionalID", Hibernate.STRING)
	         .addScalar("gerenciaRegional", Hibernate.STRING)
	         .addScalar("unidadeNegocioID", Hibernate.STRING)
	         .addScalar("unidadeNegocio", Hibernate.STRING)
	 ...
	 * 
	 */
	public Collection obterDadosKitCASConsumoAgua2(
			FiltrarResumoDadosCasHelper helper, Integer idCategoria, Integer idPerfilImovel) throws ErroRepositorioException{
         ...
         try {
        	 consulta = "select  "+
        		      "COALESCE(sum(COALESCE(r.RECA_QTLIGACOES,0)),0) as ligAtivas, "+
			          gerenciaRegionalID + " AS gerenciaRegionalID, " +
           		  gerenciaRegional + " AS gerenciaRegional, " +
			          unidadeNegocioID + " AS unidadeNegocioID, " +
           		  unidadeNegocio + " AS unidadeNegocio, " +
    			    ...
    			    consulta += "where r.reca_amreferencia = :referencia " +
			    			" and r.RECA_ICLIGACAOFATURADA = 1 "; 
    	...
    	consulta += where +	                        
            ...
            Query q = session.createSQLQuery(consulta)
             .addScalar("ligAtivas", Hibernate.INTEGER)
             .addScalar("gerenciaRegionalID", Hibernate.STRING)
	         .addScalar("gerenciaRegional", Hibernate.STRING)
	         .addScalar("unidadeNegocioID", Hibernate.STRING)
	         .addScalar("unidadeNegocio", Hibernate.STRING)
	 ...
	 * 
	 */
	public Collection obterDadosKitCASLigacaoInativaEconomia(
			FiltrarResumoDadosCasHelper helper, Integer idCategoria, Integer idPerfilImovel) throws ErroRepositorioException{
         ...
         try {
         	consulta = "select  "+
         			 	  " COALESCE(sum(COALESCE(r.RELE_QTECONOMIAS,0)),0) as econInat, "+
         			 	  " COALESCE(sum(COALESCE(r.RELE_QTLIGACOES,0)),0) as ligInat, "+
	 			          gerenciaRegionalID + " AS gerenciaRegionalID, " +
	            		  gerenciaRegional + " AS gerenciaRegional, " +
	 			          unidadeNegocioID + " AS unidadeNegocioID, " +
	            		  unidadeNegocio + " AS unidadeNegocio, " +
            ...
            Query q = session.createSQLQuery(consulta)
             .addScalar("econInat", Hibernate.INTEGER)
             .addScalar("ligInat", Hibernate.INTEGER)
             .addScalar("gerenciaRegionalID", Hibernate.STRING)
	         .addScalar("gerenciaRegional", Hibernate.STRING)
	         .addScalar("unidadeNegocioID", Hibernate.STRING)
	         .addScalar("unidadeNegocio", Hibernate.STRING)
	 ...
	 * 
	 */
	public Collection obterDadosKitCASServicosTipoRevisao(
			FiltrarResumoDadosCasHelper helper, Integer idCategoria, Integer idPerfilImovel, Integer idTipoRevisao) throws ErroRepositorioException{
         ...
         try {
         	consulta = "select  "+
	         		   " COALESCE(sum(COALESCE(r.RKCA_QTFATURASREVISAO,0)),0) as fatRev, "+
	            		  ...
	            		  municipioID + " AS municipioID, " +
	            		  municipio + " AS municipio, " +
	            		" COALESCE(sum(COALESCE(r.RKCA_QTIMOVEISFATURASREV,0)),0) as imoveisFatuRev "+
         			   ...
         			   consulta += "and r.CMRV_ID = :idTipoRevisao ";		
	         ...
	         .addScalar("municipioID", Hibernate.STRING)
	         .addScalar("municipio", Hibernate.STRING)
	         .addScalar("imoveisFatuRev", Hibernate.INTEGER)
	         .setInteger("referencia",helper.getAnoMesReferencia());
         	...
         	q.setInteger("idTipoRevisao",idTipoRevisao);
	 ...
	 * 
	 */
	public Collection obterDadosKitCASServicosCartasNegativacao(
			FiltrarResumoDadosCasHelper helper, Integer idCategoria, Integer idPerfilImovel) throws ErroRepositorioException{
         ...
         try {
         	consulta = "select  "+
	         		   " COALESCE(sum(COALESCE(r.RNEG_QTINCLUSOES,0)),0) as qtdInc, "+
	 			          gerenciaRegionalID + " AS gerenciaRegionalID, " +
	            		  gerenciaRegional + " AS gerenciaRegional, " +
	 			          unidadeNegocioID + " AS unidadeNegocioID, " +
	            		  unidadeNegocio + " AS unidadeNegocio, " +
         			     ...
         			     "from cobranca.RESUMO_NEGATIVACAO r " +
         			     join;
         			    
         			  	consulta += "where to_number(concat(to_char(r.rneg_dtprocessamentoenvio,'YYYY')," +
         			  				"to_char(r.rneg_dtprocessamentoenvio,'mm'))) = :referencia and ";
         			   ...
         			   consulta += " and r.RNEG_NNEXECRESUMONEGAT = :resumoNegativacao ";		
         	...
         	Query q = session.createSQLQuery(consulta)
             .addScalar("qtdInc", Hibernate.INTEGER)
             .addScalar("gerenciaRegionalID", Hibernate.STRING)
	         .addScalar("gerenciaRegional", Hibernate.STRING)
	         .addScalar("unidadeNegocioID", Hibernate.STRING)
	         .addScalar("unidadeNegocio", Hibernate.STRING)
	         ...
	         .addScalar("municipio", Hibernate.STRING)
	         .setInteger("referencia",helper.getAnoMesReferencia())
         	 .setInteger("resumoNegativacao",sistemaParametro.getNumeroExecucaoResumoNegativacao());
	...
	public Map totalizadorResumoDadosCas(FiltrarResumoDadosCasHelper filtro){
    	...
    	retorno.put("gerenciaRegionalID", "coalesce(sum(0),0) ");
        retorno.put("gerenciaRegional",   "coalesce(sum(0),0) ");
        retorno.put("unidadeNegocioID", "coalesce(sum(0),0) ");
        retorno.put("unidadeNegocio",   "coalesce(sum(0),0) ");
        retorno.put("localidadeID",     "coalesce(sum(0),0) ");
        retorno.put("localidade",       "coalesce(sum(0),0) ");
        retorno.put("municipioID",      "coalesce(sum(0),0) ");
        retorno.put("municipio",        "coalesce(sum(0),0) ");
	...
	public void gerarResumoPendenciaBD() throws ErroRepositorioException, SQLException {
		...
		try {
			...
			sql = " call cobranca.sp_gerar_res_pendencia() ";
