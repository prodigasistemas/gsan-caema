...
public class ConsultarArquivoTextoLeituraAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
			...
			//Verificar se a empresa inserida contém o indicador PPP			
			boolean indicadorEmpresaPpp = false;
			FiltroEmpresa filtroEmpresa = new FiltroEmpresa();
			filtroEmpresa.adicionarParametro(new ParametroSimples(FiltroEmpresa.ID, empresaID));
			...
			if(empresa.getIndicadorEmpresaPPP() != null && empresa.getIndicadorEmpresaPPP().equals(ConstantesSistema.SIM)){
				indicadorEmpresaPpp = true;
			}
			
			consultarArquivoTextoRoteiroEmpresaHelper.setIndicadorEmpresaPpp(indicadorEmpresaPpp);
			...
			Boolean indicadorLocalidadePpp = false;
			...
			"" + ConstantesSistema.NUMERO_NAO_INFORMADO) && !localidadeID.equals("")){
				...
				//Verificar Indicador PPP Localidade				
				...
				if(colecaoLocalidade != null){
					...
					if(localidade.getIndicadorContratoPpp() == 1 && empresa.getIndicadorEmpresaPPP() == 1){
						indicadorLocalidadePpp = true;
					}else if(empresa.getIndicadorEmpresaPPP() != 2){
						throw new ActionServletException("atencao.localidade.contratoPPP");
					}					
				}
			}
			consultarArquivoTextoRoteiroEmpresaHelper.setIndicadorLocalidadePpp(indicadorLocalidadePpp);
			...
			if(!consultarArquivoTextoLeituraActionForm.getServicoTipoCelular().equals("2")){
				httpServletRequest.setAttribute( "impressaoSimultania", "SIM" );
			}else{
				httpServletRequest.setAttribute( "impressaoSimultania", "NAO" );
			}
