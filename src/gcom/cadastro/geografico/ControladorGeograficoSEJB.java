...
public class ControladorGeograficoSEJB implements SessionBean {
	...
	public void atualizarBairro(Bairro bairro, Collection colecaoBairroArea,
		...
		//[FS0007] - Logradouro bloqueado para alteração
		if(bairro.getMunicipio() != null && 
			bairro.getMunicipio().getIndicadorLogradouroBloqueado().equals(ConstantesSistema.SIM)){
			
			sessionContext.setRollbackOnly();
			throw new ControladorException("atencao.bairro_bloqueado_atualizacao_cadastral",null,"atualizar");
	...
	public Integer inserirMunicipio(Municipio municipio, Usuario usuarioLogado)
		...
		if(municipio.getClientePrefeitura().getId() != null && !municipio.getClientePrefeitura().getId().equals("")){
			filtroMunicipio.limparListaParametros();
			
			filtroMunicipio = new FiltroMunicipio();


			filtroMunicipio.adicionarParametro(new ParametroSimples(
					FiltroMunicipio.ID_CLIENTE_PREFEITURA, municipio.getClientePrefeitura().getId()));
			
			filtroMunicipio.adicionarParametro(new ParametroSimplesDiferenteDe(
				FiltroMunicipio.ID, municipio.getId()));

			colecaoMunicipio = getControladorUtil().pesquisar(
					filtroMunicipio, Municipio.class.getName());

			if (colecaoMunicipio != null && !colecaoMunicipio.isEmpty()) {
				sessionContext.setRollbackOnly();
				throw new ControladorException("atencao.senha.invalida",null,"Município "+ ((Municipio)Util.retonarObjetoDeColecao(colecaoMunicipio)).getId() +" já está com o cliente responsável informado.");
			}
		}
	...
	public Integer inserirBairro(Bairro bairro, Collection colecaoBairroArea,
		...
		if(bairro.getMunicipio() != null && 
			...
			sessionContext.setRollbackOnly();
			throw new ControladorException("atencao.bairro_bloqueado_atualizacao_cadastral",null,"inserir");
