...
public class RepositorioEnderecoHBM implements IRepositorioEndereco {
	...
	public Collection pesquisarLogradouroCompleto(String codigoMunicipio,
		...
		try {
			...
			// se foi informado o nome popular do logradouro
			if (loteamentoLogradouro != null && !loteamentoLogradouro.trim().equals("")) {
				if (tipoPesquisaPopular != null
						&& tipoPesquisaPopular
								.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA
										.toString())) {
					condicional = condicional + " logr.loteamento like '%"
							+ loteamentoLogradouro.toUpperCase() + "%' and ";
				} else {
					condicional = condicional + " logr.loteamento like '"
							+ loteamentoLogradouro.toUpperCase() + "%' and ";
				}
...
//				}
	...
	public Integer pesquisarLogradouroCompletoCount(String codigoMunicipio,
		...
		try {
			...
			// se foi informado o loteamento do logradouro
			if (loteamentoLogradouro != null && !loteamentoLogradouro.trim().equals("")) {
				if (tipoPesquisaPopular != null
						&& tipoPesquisaPopular
								.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA
	...
	/**
	 * 
	 * Pesquisar o cep pelo id do cep
	 * 
	...
	public Cep pesquisarCepId(Integer idCep) throws ErroRepositorioException {
		Cep retorno = null;
		Session session = HibernateUtil.getSession();
		String consulta = null;
		try {
			consulta = "select cep " + "from Cep cep "
					+ "where cep.id = :idCep";
			retorno = (Cep) session.createQuery(consulta)
					.setInteger("idCep", idCep).setMaxResults(1)
					.uniqueResult();
	 ...
	 * @return Collection
	 */
	public Collection pesquisarEnderecoCepMunicipioFormatado(
		...
		try {
			consulta = " select logradouro.nome," // 0
					...
					+ " logradouro.id, " // 3
					+ " logradouroCep.id," // 4
					+ " logradouroTipo.descricaoAbreviada, " // 5
					+ " logradouroTitulo.descricaoAbreviada " // 6
					+ " from LogradouroCep logradouroCep "
					+ " left join logradouroCep.cep cep "
					+ " left join logradouroCep.logradouro logradouro "
					+ " left join logradouro.logradouroTipo logradouroTipo "
					+ " left join logradouro.logradouroTitulo logradouroTitulo "
					+ " where logradouroCep.id = :id";
			retorno = session
					.createQuery(consulta)
					.setInteger("id",
							id.intValue()).list();
