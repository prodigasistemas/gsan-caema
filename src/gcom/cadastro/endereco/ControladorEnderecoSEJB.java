...
public class ControladorEnderecoSEJB implements SessionBean {
	...
	public Integer inserirLogradouro(Logradouro logradouro,
		...
		//[FS0013] - Logradouro bloqueado para alteração
		if(logradouro.getMunicipio() != null && 
			logradouro.getMunicipio().getIndicadorLogradouroBloqueado().equals(ConstantesSistema.SIM)){
			
			sessionContext.setRollbackOnly();
			throw new ControladorException("atencao.logradouro_bloqueado_atualizacao_cadastral",null,"inserir");
	...
	public void atualizarLogradouro(
		...
		filtroLogradouro.adicionarParametro(
			new ParametroSimples(FiltroLogradouro.ID, logradouro.getId()));

		// Procura o logradouro na base
		Collection colecaoLogradouro = 
			getControladorUtil().pesquisar(filtroLogradouro, Logradouro.class.getName());
		...
		//[FS0014] - Logradouro bloqueado para alteração
		if(logradouro.getMunicipio() != null && 
			logradouro.getMunicipio().getIndicadorLogradouroBloqueado().equals(ConstantesSistema.SIM)){
			
			sessionContext.setRollbackOnly();
			throw new ControladorException("atencao.logradouro_bloqueado_atualizacao_cadastral",null,"atualizar");
			...
			if (arrayEndereco[20] != null) {
				...
				if (arrayEndereco[19] != null) {
					...
					if (arrayEndereco[33] != null){
						logradouro.setLoteamento("" + arrayEndereco[33]);
					}
				}
				...
				if (arrayEndereco[20] != null) {
					...
					if (arrayEndereco[19] != null) {
						...
						if (arrayEndereco[34] != null){
							logradouro.setLoteamento("" + arrayEndereco[34]);
						}
					}
					LogradouroTipo logradouroTipo = null;
	...
	public Collection pesquisarLogradouroCompleto(String codigoMunicipio,
			String codigoBairro, String nome, String nomePopular, String loteamentoLogradouro,
		...
		try {
			return repositorioEndereco.pesquisarLogradouroCompleto(
					codigoMunicipio, codigoBairro, nome, nomePopular, loteamentoLogradouro,
...
//					indicadorUso, tipoPesquisa, tipoPesquisaPopular);
	...
	public String pesquisarEnderecoRegistroAtendimentoFormatado(
		...
		// Caso logradouroCep esteja vazio, busca o primeiro LogradouroCep de acordo com o logrdaouro do bairro
		if(arrayEndereco[18] == null && arrayEndereco[19] != null){
			Integer idLogrBairro = (Integer) arrayEndereco[19];
			
			FiltroLogradouroBairro filtro = new FiltroLogradouroBairro(FiltroLogradouroBairro.ID);
			filtro.adicionarParametro(new ParametroSimples(FiltroLogradouroBairro.ID, idLogrBairro));
			Collection colLogrBairro = this.getControladorUtil().pesquisar(
					filtro,	LogradouroBairro.class.getName());
			LogradouroBairro logrBairro = (LogradouroBairro) Util.retonarObjetoDeColecao(colLogrBairro);
			if( logrBairro != null){
				try {
					Collection<LogradouroCep> colLogrCep = repositorioEndereco.pesquisarAssociacaoLogradouroCepPorLogradouro(logrBairro.getLogradouro().getId());
					if (colLogrCep != null && !colLogrCep.isEmpty()) {
	
						Iterator cepIterator = colLogrCep.iterator();
	
						LogradouroCep logrCep = (LogradouroCep) cepIterator.next();	
						arrayEndereco[18] = logrCep.getId();
						cepBairro = repositorioEndereco.pesquisarCepId(logrCep.getCep().getCepId());
					
						colecaoEndereco = repositorioEndereco
							.pesquisarEnderecoCepMunicipioFormatado(logrCep.getId());
					}
				} catch (ErroRepositorioException ex) {
					throw new ControladorException("erro.sistema", ex);
				}
	
				Iterator enderecoCepIterator = colecaoEndereco.iterator();
	
				Object[] arrayCepEndereco = (Object[]) enderecoCepIterator.next();
				//Nome logradouro
				arrayEndereco[0] =arrayCepEndereco[0];
				//Id logradouro
				arrayEndereco[17] =arrayCepEndereco[3];
				//Logr tipo abreviado
				arrayEndereco[20] =arrayCepEndereco[5];
				//Logr tipo descricao
				arrayEndereco[1] =arrayCepEndereco[1];
				// Logr titulo descricao
				arrayEndereco[21]=arrayCepEndereco[2];
			}
		}
	...
	public String pesquisarBairroLogradouroRegistroAtendimentoSolicitante(
		...
		if(arrayEndereco[4] != null && arrayEndereco[1] != null && arrayEndereco[0] != null && arrayEndereco[22] != null){
