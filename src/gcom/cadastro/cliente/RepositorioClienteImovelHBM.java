...
//					+ " AND imovPerfil.id = " + ImovelPerfil.TARIFA_SOCIAL.toString()
	 ...
	 * @throws ErroRepositorioException
	 */
	public Collection<Object[]> obterImoveisAssociados(
			Integer codigoClienteAnterior) throws ErroRepositorioException {
		...
		try {
			String consulta = "select imov_id AS imovel, "
					+ "   clim_id AS idRegistro, "
					+ "   clim_icnomeconta AS indicadorNomeConta, "
					+ "   crtp_id AS clieRelacaoTipo "
					+ " from cadastro.cliente_imovel "
					+ " where clim_dtrelacaofim is null "
					+ "   and clie_id = :codigoClienteAnterior ";
			
			
			retorno = (Collection<Object[]>) 
					session.createSQLQuery(consulta)
					.addScalar("imovel",Hibernate.INTEGER)
					.addScalar("idRegistro",Hibernate.INTEGER)
					.addScalar("indicadorNomeConta",Hibernate.SHORT)
					.addScalar("clieRelacaoTipo",Hibernate.INTEGER)
					.setInteger("codigoClienteAnterior", codigoClienteAnterior)
					.list();
	 ...
	 * @return void
	 */
	public void atualizarClienteImovel(Integer idRegistroAtualizar)
			throws ErroRepositorioException {
		...
		try {
			consulta = "update gcom.cadastro.cliente.ClienteImovel "
				+ " set clim_dtrelacaofim = :dataAtual, clim_tmultimaalteracao = :ultimaAlteracao, "
				+ " cifr_id = :motivoFimRelacao "
				+ " where clim_id = :idRegistroAtualizar ";
		
			session.createQuery(consulta)
					.setDate("dataAtual", new Date())
					.setTimestamp("ultimaAlteracao", new Date())
					.setInteger("motivoFimRelacao", ClienteImovelFimRelacaoMotivo.DUPLICIDADE_DO_CLIENTE)
					.setInteger("idRegistroAtualizar", idRegistroAtualizar)
					.executeUpdate();
