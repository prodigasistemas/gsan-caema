...
public class ClienteAtualizacaoCadastral extends ObjetoTransacao {
	...
	private Date dataRelacaoFim;
	
	private String descricaoRelacaoTipoCliente;
	
	private String dataRelacaoFimFormatada;
	
	private Date dataRelacaoInicio;
	
	//criado para exibição do popup de "Consultar Dados do Imóvel no Ambiente Virtual 2"
	...
	public Date getDataRelacaoFim() {
		return dataRelacaoFim;
	}

	public void setDataRelacaoFim(Date dataRelacaoFim) {
		this.dataRelacaoFim = dataRelacaoFim;
	}

	public String getDescricaoRelacaoTipoCliente() {
		
		if ( getIdClienteRelacaoTipo().equals(ClienteRelacaoTipo.PROPRIETARIO.intValue())){
			descricaoRelacaoTipoCliente = "PROPRIETARIO";
		} else if ( getIdClienteRelacaoTipo().equals(ClienteRelacaoTipo.RESPONSAVEL.intValue()) ) {
			descricaoRelacaoTipoCliente = "RESPONSAVEL";
		} else if ( getIdClienteRelacaoTipo().equals(ClienteRelacaoTipo.USUARIO.intValue()) ) {
			descricaoRelacaoTipoCliente = "USUARIO";
		}
		
		return descricaoRelacaoTipoCliente;
	}

	public String getDataRelacaoFimFormatada() {
		
		dataRelacaoFimFormatada = Util.formatarData(getDataRelacaoFim());
		
		return dataRelacaoFimFormatada;
	}

	public Date getDataRelacaoInicio() {
		return dataRelacaoInicio;
	}

	public void setDataRelacaoInicio(Date dataRelacaoInicio) {
		this.dataRelacaoInicio = dataRelacaoInicio;
	}
	...
	public String getRgFormatado(){
		...
		if(getDsAbreviadaOrgaoExpedidorRg() != null){
			retorno = retorno + " " + getDsAbreviadaOrgaoExpedidorRg();
		}
		
		if(getDsUFSiglaOrgaoExpedidorRg() != null){
			retorno = retorno + "/" + getDsUFSiglaOrgaoExpedidorRg();
		}
		return retorno;
	}
	...
	public String getSexoFormatado(){
		String retorno = "";
		if(getIdPessoaSexo() != null){
			if(getIdPessoaSexo().equals(new Integer("1"))){
				retorno = "MASCULINO";
			}else if(getIdPessoaSexo().equals(new Integer("2"))){
				retorno = "FEMININO";
			}
		}
		return retorno;
	...
	public String getEnderecoFormatadoAbreviado() {
		...
		if (this.getIdLogradouro() != null
				&& !this.getIdLogradouro().equals(
						new Integer("0"))) {
			
			// verifica se o logradouro tipo do imovel é diferente de null
			if (this.getIdLogradouroTipo() != null
					&& !this.getIdLogradouroTipo().equals("")) {
				if (this.getDsLogradouroTipo() != null
						&& !this.getDsLogradouroTipo().equals("")) {
					// concatena o logradouro tipo do imovel
					endereco = this.getDsLogradouroTipo().trim();
				}
			}
			
			// verifica se o logradouro titulo do imovel é diferente de null
			if (this.getIdLogradouroTitulo() != null
					&& !this.getIdLogradouroTitulo().equals("")) {
				if (this.getDsLogradouroTitulo() != null
						&& !this.getDsLogradouroTitulo().equals("")) {
					// concatena o logradouro titulo do imovel
					endereco = endereco
							+ " "
							+ this.getDsLogradouroTitulo().trim();
				}
			}

			// concatena o logradouro do imovel
			endereco = endereco + " "
					+ this.getDescricaoLogradouro().trim();

//			if (this.getIdEnderecoReferencia() != null
//					&& !this.getIdEnderecoReferencia().equals("")) {
//				if (this.getDsEnderecoReferencia() != null
...
//			}
			...
			if (this.getIdBairro() != null
					&& this.getIdBairro().intValue() != 0) {
				endereco = endereco + " - "
						+ this.getNomeBairro().trim();				
			}
			
			if (this.getIdMunicipio() != null
					&& this.getIdMunicipio().intValue() != 0) {
				endereco = endereco
						+ " "
						+ this.getNomeMunicipio().trim();
				
				if (this.getDsUFSiglaMunicipio() != null) {
					endereco = endereco
							+ " "
							+ this.getDsUFSiglaMunicipio().trim();
				}
			}

			if (this.getCodigoCep() != null) {
				// concatena o cep formatado do imóvel
				endereco = endereco + " "  
						+ Util.formatarCEP(this.getCodigoCep().toString());;
			}
