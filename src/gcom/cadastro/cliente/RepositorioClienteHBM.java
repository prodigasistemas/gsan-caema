...
public class RepositorioClienteHBM implements IRepositorioCliente {
	...
	public Collection filtrarCliente(String codigo, String cpf, String rg,
			...
			String nomeClienteReceitaFederal, String tipoPesquisaNomeCliente)
			throws ErroRepositorioException {
		...
		try {
			...
			//nomeClienteReceitaFederal
			// nome
			if (nomeClienteReceitaFederal != null && !nomeClienteReceitaFederal.equals("")) {
				if (tipoPesquisaNomeCliente != null
						&& tipoPesquisaNomeCliente
								.equals(ConstantesSistema.TIPO_PESQUISA_COMPLETA
										.toString())) {
					consulta = consulta + " upper(cliente.nomeClienteReceitaFederal) like '%"
							+ nomeClienteReceitaFederal.toUpperCase() + "%'  and  ";
				} else {
					consulta = consulta + " upper(cliente.nomeClienteReceitaFederal) like '"
							+ nomeClienteReceitaFederal.toUpperCase() + "%'  and  ";
				}
	...
	public Object filtrarQuantidadeCliente(String codigo, String cpf,
			...
			String tipoPesquisaNomeCliente)
			throws ErroRepositorioException {
	...
	public Object[] obterDadosCliente(Integer idImovel, 
		...
		try {
			...
			consulta = " select cliente.clie_id as idCliente, " + //0
					   ...
					   " oerg.oerg_id as orgaoExp, " + //7
					   " unfe.unfe_id as unidFederativa, " + //8
...
//					   " cled_dscomplementoendereco as complemento, " + //23
					   " ratv_id as idRamoAtividade, " + //15
					   " cltp.cltp_icpessoafisicajuridica as indicadorPessoaFisicaJuridica," + //16
					   " clienteImovel.clim_dtrelacaoinicio as dataInicioRelacao," +//17
					   " clienteImovel.clim_dtrelacaofim as dataRelacaoFim,"+ //18
					   " cliente.clie_nmabreviado as nomeAbreviado"+//19
...
//					   " left join cadastro.bairro bairro on (lgbr.bair_id = bairro.bair_id)" + 
			...
			retornoConsulta = (Object[])session.createSQLQuery(consulta)
					...
					.addScalar("orgaoExp", Hibernate.INTEGER)
					.addScalar("unidFederativa", Hibernate.INTEGER)
...
//					.addScalar("complemento", Hibernate.STRING)	
					...
					.addScalar("dataInicioRelacao", Hibernate.DATE)
					.addScalar("dataRelacaoFim", Hibernate.DATE)
		...
		try {
			...
			if (retornoConsulta.size() > 0) {
				...
				while (clienteFoneIter.hasNext()) {
					...
					if (element[7] != null){
						clienteFone.setContato((String) element[7]);
					}
	...
	public ClienteFone pesquisarDadosClienteFonePadrao(Integer idCliente) 
		...
		try {
			...
			retorno = (ClienteFone) crit.uniqueResult();
	 ...
	 * @throws ErroRepositorioException
	 */
	public Collection<ClienteVirtual> pesquisarClienteVirtual(Date periodoInicial , Date periodoFinal , int numeroPagina, String idImovel, String situacaoCliente, String tempoCadastro) throws ErroRepositorioException {
		...
		try {
			...
			consulta = "select cliente from ClienteVirtual cliente "
					+ "left join cliente.imovel imo "
					+ "where imo.id = cliente.imovel.id ";
					
			if(periodoInicial != null && periodoFinal != null){
				consulta += "and cliente.dataCadastro >= :periodoInicial "
						 +	"and cliente.dataCadastro <= :periodoFinal ";
				parameters.put("periodoInicial", periodoInicial);
				parameters.put("periodoFinal", periodoFinal);
			}
			
			if(idImovel != null && !idImovel.equals("")){
				consulta += "and imo.id = :idImovel ";
				parameters.put("idImovel", Integer.parseInt(idImovel));
			}
			
			if(situacaoCliente != null && !situacaoCliente.equals("")){
				consulta += "and cliente.codigoSituacao = :codigoSituacao ";
				parameters.put("codigoSituacao", Short.parseShort(situacaoCliente));
			}
			
			if(tempoCadastro != null && tempoCadastro.equals("1")){
				Date data = Util.subtrairNumeroDiasDeUmaData(new Date(), 30);
				consulta += "and cliente.dataCadastro > :data ";
				parameters.put("data", data);
			}
			
			consulta += "order by cliente.dataCadastro , cliente.nome";
			...
			clienteVirtual = (Collection<ClienteVirtual>) query.setFirstResult(10*numeroPagina)
					.setMaxResults(10).list();
	 ...
	 * @throws ErroRepositorioException
	 */
	public Integer pesquisarQuantidadeClienteVirtual(Date periodoInicial , Date periodoFinal, String idImovel, String situacaoCliente, String tempoCadastro) throws ErroRepositorioException {
		...
		try {
			...
			// Monta o HQL
			consulta = "select count(cliente.id) from ClienteVirtual cliente "
					 + "left join cliente.imovel imo "
					 + "where imo.id = cliente.imovel.id ";
			
			if(periodoInicial != null && periodoFinal != null){
				consulta += "and cliente.dataCadastro >= :periodoInicial and " 
					+ "cliente.dataCadastro <= :periodoFinal ";
				parameters.put("periodoInicial", periodoInicial);
				parameters.put("periodoFinal", periodoFinal);
			}
	...
	public Collection<ClienteVirtual> pesquisarSituacaoClienteCadastradoPortal(Integer idImovel) 
		...
		try{
			...
			consulta = "select cliente from ClienteVirtual cliente "
					 ...
					 + "and cliente.codigoSituacao <> :codigoSituacao "
					 + "order by cliente.dataCadastro desc ";
			
			clienteVirtual = (Collection<ClienteVirtual>) 
				session.createQuery(consulta).
					setInteger("idImovel", idImovel).
					setShort("codigoSituacao", new Short(ConstantesSistema.CODIGO_SITUACAO_CADASTRO_ATUALIZADO)).
					list();
	...
	public Collection<ClienteVirtual> pesquisarClienteVirtual(Date periodoInicial, Date periodoFinal, String idImovel,String situacaoCliente, String tempoCadastro) 
		throws ErroRepositorioException {
		...
		try {
			...
			consulta = "select cliente from ClienteVirtual cliente "
					...
					+ "left join fetch cliente.orgaoExpedidorRg rg "
					+ "left join fetch cliente.unidadeFederacao uf "
					+ "left join fetch cliente.pessoaSexo sexo "
					+ "left join fetch cliente.ramoAtividade ramo "
					+ "left join fetch cliente.foneTipo foneTipo "
					+ "where cliente.imovel.id = imo.id "; 
			...
			consulta += "order by cliente.codigoSituacao, cliente.nome";
			...
			clienteVirtual = (Collection<ClienteVirtual>) query.list();
	 ...
	 * @throws ErroRepositorioException
	 */
	public Cliente pesquisarDadosClientePrefeitura(Integer idCliente)
			throws ErroRepositorioException {
		...
		try {
			...
			consulta = " select cli.nome from Cliente cli " 
					+ " where cli.id = :idCliente " 
					+ " and cli.nome like '%PREFEITURA%' ";
			
			
			/* consulta = "select cl.clie_nmcliente as nome "+
					" from cadastro.cliente cl "+
					" JOIN cadastro.cliente_tipo ct "+
					" ON ct.cltp_id = cl.cltp_id "+
					" where clie_nmcliente like 'PREFEITURA%' "+
					" AND ct.cltp_icpessoafisicajuridica = 1 "+
					" AND cl.clie_id = :idCliente ";*/

			// Executa o HQL
			nomeCliente = (String) session.createQuery(consulta)
			.setInteger("idCliente", idCliente)
			.uniqueResult();
			
			retorno.setNome(nomeCliente);
	...
	public Collection pesquisarClienteFonePadrao(Integer idCliente)
		...
		try {
			...
			hql = "SELECT cliFone " 
					+ " FROM ClienteFone cliFone "
					+ " INNER JOIN cliFone.cliente cli "
					+ " WHERE cli.id = :idCliente "
					+ " AND cliFone.indicadorTelefonePadrao = :indicadorFonePadrao " 
					+ " ORDER BY ultimaAlteracao desc";

			clienteFone = (Collection) session.createQuery(hql)
					.setInteger("idCliente", idCliente)
					.setShort("indicadorFonePadrao", ClienteFone.INDICADOR_FONE_PADRAO)
					.list();
	...
	/**
	 * [UC1388] - Consultar Quantidade de Acessos a base da Receita Federal
	 * 
	 ...
	 * @throws ErroRepositorioException
	 */
	public Collection <QuantidadeAcessosReceitaFederal> consultarQuantidadeAcessosReceitaFederal(Date dataInicial, Date dataFinal, String mensagemRetorno) throws ErroRepositorioException {		
		Session session = HibernateUtil.getSession();		
		try {						
			String hql = "SELECT count(mrrf_id) FROM SEGURANCA.consulta_receita_federal mrrf_id " +
						 "WHERE mref.imovel.id = :idImovel and mref.anoMesReferencia = :anoMesReferencia";						
			return session.createQuery(hql)			        												
					.setTimestamp("dataInicial", new Date())
					.setTimestamp("dataFinal", new Date())
					.setString("mensagemRetorno", mensagemRetorno)
					.list();		
			} catch (HibernateException e) {			
				throw new ErroRepositorioException(e, "Erro no Hibernate");		
			} finally {			
	...
	public Collection<QuantidadeAcessosReceitaFederal> filtrarQuantidadeAcessosReceitaFederal
				(FiltrarQuantidadeAcessosReceitaFederalHelper helper)throws ErroRepositorioException{
		Collection<QuantidadeAcessosReceitaFederal> retorno = new ArrayList<QuantidadeAcessosReceitaFederal>();
		Session session = HibernateUtil.getSession();
		String consulta = null;
		Query query = null;
		try {
			consulta = "SELECT "+
					" mrrf.mrrf_dsmensagemretorno as msgRetorno , "+ 
					" COUNT(*)  AS quantAcesso , crf.mrrf_id as idMsgRetorno" +
					" FROM SEGURANCA.ACESSO_RECEITA_FEDERAL crf "+
					" JOIN CADASTRO.MSG_RET_RECEITA_FEDERAL mrrf ON crf.MRRF_ID = mrrf.MRRF_ID "+
					" WHERE crf.ARFD_DTCONSULTA BETWEEN :dtInical and :dtFinal";
					
					if(helper.getMensagemRetorno() != null && !helper.getMensagemRetorno().equals("-1")){
						consulta = consulta + " and crf.MRRF_ID = :idMsgRetorno";
					}
					
					consulta = consulta + " GROUP BY crf.MRRF_ID, mrrf.mrrf_dsmensagemretorno ";
					
					query = session.createSQLQuery(consulta)						
							.addScalar("msgRetorno", Hibernate.STRING)
							.addScalar("quantAcesso", Hibernate.INTEGER)
							.setTimestamp("dtInical", Util.formatarDataInicial(helper.getDataInicial()))
							.setTimestamp("dtFinal", Util.formatarDataFinal(helper.getDataFinal()));
					
					
					if(helper.getMensagemRetorno() != null && !helper.getMensagemRetorno().equals("-1")){
						query.setParameter("idMsgRetorno", helper.getMensagemRetorno());
					}
	...
	*/
	...
	public Collection pesquisarClientesValidacao(int numeroPagina) throws ErroRepositorioException{
		...
		String consulta = " SELECT c.clie_id as idCliente, " +
				 " c.clie_nmcliente as nome, " +
		         " c.CLIE_NMABREVIADO as nomeAbreviado, " +
				 
		         " c.clie_nncpf as cpf, " +
		         " c.clie_nncnpj as cnpj, " +
                 " c.CLIE_NNRG as rg, " +  
		         
                 " c.CLIE_DTRGEMISSAO as dataEmissao, " +
                 " c.CLIE_DTNASCIMENTO as dataNascimento, " +
                 " c.CLIE_DSEMAIL as email, " +
                 
                 " c.CLIE_ICUSO as uso, " +
                 " c.OERG_ID as orgaoExpedidor, " +
                 " c.CLIE_CDCLIENTERESPONSAVEL as clienteResponsavel, " +
                 
                 " c.PSEX_ID as sexo, " +
                 " c.PROF_ID as profissao, " +
                 " c.UNFE_ID as unidFed, " +
                 
                 " c.CLTP_ID as tipPro, " +
                 " c.CLIE_ICUSONOMEFANTASIACONTA as usoFantasia, " +
                 " c.RATV_ID as ratv, " +
                 
                 " c.clie_tmultimaalteracao as ultimaAlteracao, " +
               
                 " c.clie_iccobrancaacrescimos as icAcrescimo, " +     
                 " c.CLIE_ICGERAARQUIVOTEXTO as icArquivoTexto, " +
                 " c.CLIE_ICVENCIMENTOMESSEGUINTE as icVencimentoMesSeguinte, " +
                 " c.CLIE_ICGERAFATURAANTECIPADA as icGeraFaturaAntecipada, " +
                 " c.CLIE_ICUSONOMEFANTASIACONTA as icNomeFantasiaConta, " +
                 " c.CLIE_ICPERMITENEGATIVACAO as icPermiteNegativacao, " +
                 " c.CLIE_ICNEGATIVACAOPERIODO as icNegativacaoPeriodo, " +
                 " c.CLIE_ICBLOQUEARDESPESASPOSTAIS as icBloquearDespesasPostais, " +
                 " c.CLIE_ICBLOQUEARALTERARIMOVEIS as icBloquearAlterarImoveis, " +
                 
                 " c.CLIE_NMCLIENTERECEITAFEDERAL as ncReceitaFederal, " +
                 
                 " c.clie_cdclienteresponsavel as responsavel " +
                 
		         " FROM cadastro.cliente c " +
		         
		         " INNER JOIN cadastro.cliente_tipo tipo on (tipo.cltp_id = c.cltp_id " + 
                 " AND (c.clie_nncpf IS NOT NULL AND tipo.CLTP_ICPESSOAFISICAJURIDICA = 1) " + 
                 " AND epod_id NOT IN (1, 2, 3)) " +
           
                " WHERE c.clie_icuso = 1 " +
		        " AND NOT EXISTS (SELECT clie_id FROM seguranca.consulta_receita_federal crf where crf.crfd_nncpf = c.clie_nncpf)" +
                ...
                " INNER JOIN cadastro.cliente_tipo tipo on (tipo.cltp_id = c.cltp_id " + 
                " AND (c.clie_nncnpj IS NOT NULL AND tipo.CLTP_ICPESSOAFISICAJURIDICA = 2) " + 
                " AND epod_id NOT IN (1, 2, 3)) " +

                " WHERE c.clie_icuso = 1 " +
                " AND NOT EXISTS (SELECT clie_id FROM seguranca.consulta_receita_federal crf where crf.crfd_nncnpj = c.clie_nncnpj) ";
		...
		if (numeroPagina == 0){ 
			...
			retorno = session.createSQLQuery(consulta)
					...
					.addScalar("dataNascimento", Hibernate.DATE)
					.addScalar("email", Hibernate.STRING)
					
					.addScalar("uso", Hibernate.SHORT)
					.addScalar("orgaoExpedidor", Hibernate.INTEGER)
					.addScalar("clienteResponsavel", Hibernate.INTEGER)
					
					.addScalar("sexo", Hibernate.INTEGER)
					.addScalar("profissao", Hibernate.INTEGER)
					.addScalar("unidFed", Hibernate.INTEGER)
					
					.addScalar("tipPro", Hibernate.INTEGER)
					.addScalar("usoFantasia", Hibernate.SHORT)
					.addScalar("ratv", Hibernate.INTEGER)
					
					.addScalar("ultimaAlteracao", Hibernate.DATE)
					
					.addScalar("icAcrescimo", Hibernate.SHORT)
					.addScalar("icArquivoTexto", Hibernate.SHORT)
					.addScalar("icVencimentoMesSeguinte", Hibernate.SHORT)
					.addScalar("icGeraFaturaAntecipada", Hibernate.SHORT)
					.addScalar("icNomeFantasiaConta", Hibernate.SHORT)
					.addScalar("icPermiteNegativacao", Hibernate.SHORT)
					.addScalar("icNegativacaoPeriodo", Hibernate.SHORT)
					.addScalar("icBloquearDespesasPostais", Hibernate.SHORT)
					.addScalar("icBloquearAlterarImoveis", Hibernate.SHORT)
					
					.addScalar("ncReceitaFederal", Hibernate.STRING)
					
					.addScalar("responsavel", Hibernate.INTEGER)
					
					.setFirstResult(10).setMaxResults(20).list();
		} else {
			...
			retorno = session.createSQLQuery(consulta)
					...
					.setFirstResult(10*numeroPagina).setMaxResults(20).list();
	...
	*/
	...
	public int pesquisarQuantidadeClientesValidacao() throws ErroRepositorioException{
		...
		String consulta = 
				...
				" SELECT count (c.clie_id) AS quantidadeClientes " + 
				" FROM cadastro.cliente c " +
				" INNER JOIN cadastro.cliente_tipo tipo on (tipo.cltp_id = c.cltp_id and epod_id NOT IN (1, 2, 3)) " + 
				" WHERE clie_icuso = 1 " +
				" AND (clie_nncpf IS NOT NULL OR clie_nncnpj IS NOT NULL) " +
				" AND NOT EXISTS (SELECT clie_id FROM seguranca.consulta_receita_federal crf where crf.clie_id = c.clie_id ) ";
		
		retorno = (Integer) session.createSQLQuery(consulta).addScalar("quantidadeClientes", Hibernate.INTEGER).uniqueResult();
	 ...
	 * @throws ErroRepositorioException
	 */
	public String obterNomeCliente(Integer idCliente) 
			throws ErroRepositorioException {
		...
		try {
			String consulta = "select clie_nmcliente AS nome "
					+ " from cadastro.cliente "
					+ " where clie_id = :idCliente ";
	...
	public void atualizarClienteUnificado(Integer idCliente)
		...
		try {
			consulta = "update gcom.cadastro.cliente.Cliente "
				+ " set clie_icuso = :indicadorInativo, clie_tmultimaalteracao = :ultimaAlteracao "
				+ " where clie_id = :idCliente ";
		
			session.createQuery(consulta)
					.setShort("indicadorInativo", ConstantesSistema.NAO)
					.setTimestamp("ultimaAlteracao", new Date())
					.setInteger("idCliente", idCliente)
					.executeUpdate();
	...
	* @throws ErroRepositorioException
	*/
    public Collection gerarRelatorioClientesValidadosReceitaFederal(Collection colecaoClientesParametro) throws ErroRepositorioException{
		...
		String idReceitaFederal = "";
		Iterator iterator = colecaoClientesParametro.iterator();
		
		while (iterator.hasNext()) {
			Object array = (Object) iterator.next();
			if ((array != null) && (idReceitaFederal == "")){
				idReceitaFederal = String.valueOf(array);
			} else 
			if ( array != null ) {
				idReceitaFederal = idReceitaFederal + "," + String.valueOf(array);
			}	
		}
		
		String consulta = " select crf.CLIE_ID as id, crf.CRFD_NMCLIENTE as nome, crf.CRFD_NNCPF as cpf, " +
				"crf.CRFD_NNCNPJ as cnpj, mrf.MRRF_DSMENSAGEMRETORNO as mensagem " +  

		" FROM seguranca.consulta_receita_federal crf " +

		" LEFT JOIN CADASTRO.MSG_RET_RECEITA_FEDERAL mrf on (mrf.MRRF_ID = crf.MRRF_ID) " +
		
		" WHERE crf.CRFD_ID IN (" + idReceitaFederal + ")";
		...
		.addScalar("cpf", Hibernate.STRING)
		.addScalar("cnpj", Hibernate.STRING)
		.addScalar("mensagem", Hibernate.STRING)
	...
	public Cliente pesquisarClienteArquivoTextoRetornoClienteCobrancaSmartphone(
			String numeroCpfOuCnpj, Boolean numeroCpf, Integer idCliente) 
		throws ErroRepositorioException {
