...
public class RepositorioQuadraHBM implements IRepositorioQuadra {
	...
	public QuadraFace pesquisarQuadraFace(Integer idImovel)throws ErroRepositorioException {
	  ...
	  * @date: 17/01/2012 
     */
    public Boolean existeQuadra(Integer idSetorComercial, Integer numQuadra) throws ErroRepositorioException {
        ...
        try {
        	...
        	String consulta = " SELECT quadra.id FROM Quadra as quadra " +
        			" WHERE quadra.setorComercial.id = :idSetorComercial AND quadra.indicadorUso = :indicadorUso " +
        			" AND quadra.numeroQuadra = :numQuadra";
        
            retornoConsulta = session.createQuery(consulta)
            				 .setInteger("idSetorComercial", idSetorComercial)
            				 .setInteger("indicadorUso", ConstantesSistema.INDICADOR_USO_ATIVO)
            				 .setInteger("numQuadra", numQuadra)
            				 .list();
	...
	public Integer pesquisarQuantidadeQuadrasForaDoGrupo(Integer idSetorComercial, Integer numeroQuadraInicial,
			Integer numeroQuadraFinal, Integer idGrupoFaturamento) throws ErroRepositorioException {
		
		Session session = HibernateUtil.getSession();
		Integer retorno = 0;
		try {

			String consulta = "select count(quadra.qdra_id) as contador from cadastro.quadra quadra "
					+ " inner join cadastro.setor_comercial setor on setor.stcm_id = quadra.stcm_id "
					+ " inner join micromedicao.rota rota on rota.rota_id = quadra.rota_id "
					+ " where setor.stcm_id = :idSetorComercial "
					+ " and quadra.qdra_nnquadra BETWEEN :numeroInicial AND :numeroFinal "
					+ " and rota.ftgr_id <> :idGrupoFaturamento";

			retorno = (Integer) session.createSQLQuery(consulta).addScalar("contador", Hibernate.INTEGER)
			.setInteger("idSetorComercial", idSetorComercial)
			.setInteger("numeroInicial", numeroQuadraInicial)
			.setInteger("numeroFinal", numeroQuadraFinal)
     ...
     * @date: 06/05/2014
     */
    public Integer pesquisarRotaParaEncaixarQuadra(
    		Collection<Integer> idsRotas, Integer numeroQuadra) throws ErroRepositorioException {
        ...
        try {
            consulta = 
            	...
            	"FROM Quadra q " + 
            	"WHERE " + 
            		"q.rota.id IN (:idsRotas) and q.indicadorUso = 1 " + 
            	"GROUP BY " + 
            		"q.rota.id " +
            	"Having " +
            		"MIN(q.numeroQuadra) <= :numeroQuadra and MAX(q.numeroQuadra) >= :numeroQuadra";
        
            retorno =  (Integer) session.createQuery(consulta)
            	.setParameterList("idsRotas", idsRotas)
            	.setParameter("numeroQuadra", numeroQuadra)
            	.uniqueResult();
	...
	public int pesquisarMinimoCodigoQuadra(Integer idSetorComercial)
		...
		try {
			consulta = "SELECT min(q.numeroQuadra) " + "FROM Quadra q "
					+ "INNER JOIN q.setorComercial sc "
					+ "WHERE sc.id = :idSetorComercial ";
