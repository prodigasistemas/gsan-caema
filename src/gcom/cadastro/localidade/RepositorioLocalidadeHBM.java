...
public class RepositorioLocalidadeHBM implements IRepositorioLocalidade {
	...
	public Localidade pesquisarLocalidade(Integer idLocalidade) throws ErroRepositorioException{
		...
		try{
			...
			return (Localidade) session.createQuery("SELECT loc FROM Localidade loc WHERE loc.id = :idLoc and loc.indicadorUso = :indicadorUso")
	...
	/**
	 * 
	 * Pesquisar as localidades do município da região metropolitana
	 * 
	 * @author Ana Maria
	 * @date 18/10/2012
	 * 
	 */
	public Collection pesquisarLocalidadesMunicipioRegiaoMetropolitana() 
			throws ErroRepositorioException {
		Collection retorno = null;
		Session session = HibernateUtil.getSession();
		String consulta = null;
		try {
			consulta = "select distinct loc.id "
					  ...
					  +"inner join setor.localidade loc "
					  +"inner join setor.municipio mun "
					  +"where mun.nome in('RECIFE', 'OLINDA', 'JABOATAO', 'PAULISTA', 'IGARASSU', 'ABREU E LIMA', "
					  +"'CAMARAGIBE', 'CABO DE SANTO AGOSTINHO', 'SAO LOURENCO DA MATA', 'ARACOIABA', 'ITAMARACA', 'ITAPISSUMA', 'MORENO')";
	 ...
	 * 
	 */
	public Collection pesquisarListaDeLocalidadesParaComandosAcaoCobrancaEventual(Integer idComando) throws ErroRepositorioException{
		Session sessao = HibernateUtil.getSession();
		Collection retorno = null;
		String consulta = null;
		
		try{
			consulta = " select distinct cdloca.loca_id as idLocalidade, cdloca.loca_nmlocalidade as descricao from cadastro.localidade  cdloca "
					  +" inner join cobranca.cob_acao_atv_cmd_loca cobr on cobr.loca_id = cdloca.loca_id "
					  +" where cobr.cacm_id = :idComando";
	...
	public Collection pesquisarLocalidadeParaComandoAcaoCobrancaEventual(Integer idComando)throws ErroRepositorioException{
		Session sessao = HibernateUtil.getSession();
		Collection retorno = null;
		String consulta = null;
		
		try{
			consulta = " select distinct cdloca.loca_id as idLocalidade, cdloca.loca_nmlocalidade as dsLocalidade " 
					  +" from cadastro.localidade cdloca "
					  +" inner join cobranca.cobranca_acao_ativ_cmd cbacao on cbacao.loca_id = cdloca.loca_id "
					  +" where cbacao.cacm_id = :idComando ";
			
			retorno = sessao.createSQLQuery(consulta)
					  .addScalar("idLocalidade", Hibernate.INTEGER)
					  .addScalar("dsLocalidade", Hibernate.STRING)
					  .setInteger("idComando", idComando)
					  .list();
	 ...
	 * @throws ErroRepositorioException
	 */
	public Integer obterQuantidadeImoveisLocalidadeSistemaAbastecimento(Integer idLocalidade, Integer idSistemaAbastecimento) throws ErroRepositorioException{
		
		Session session = HibernateUtil.getSession();
		String consulta = "";
		Integer retornoConsulta;
		
		try{
			consulta = 	"SELECT count(*) as quantidade "
					 +	"FROM cadastro.imovel imov "
					 +	"WHERE imov.loca_id = :idLocalidade AND imov.sabs_id = :idSistemaAbastecimento ";

			retornoConsulta = (Integer) session.createSQLQuery(consulta)
					.addScalar("quantidade", Hibernate.INTEGER)
					.setInteger("idLocalidade", idLocalidade)
					.setInteger("idSistemaAbastecimento", idSistemaAbastecimento)
					.setMaxResults(1)
					.uniqueResult();
