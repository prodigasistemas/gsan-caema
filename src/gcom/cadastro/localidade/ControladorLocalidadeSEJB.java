...
public class ControladorLocalidadeSEJB implements SessionBean {
    ...
    public Quadra validarQuadra(InserirQuadraHelper helper) throws ControladorException{
		...
		quadraInserir.setIndicadorAtualizacaoCadastral(new Short(helper.getIndicadorAtualizacaoCadastral()));
		...
		if (sistemaParametro.getIndicadorQuadraFace().equals(ConstantesSistema.NAO)){
			 ...
			 * "REDE_PARCIAL = 3" o campo subsistema passa a ser obrigatório
			 */
			SubSistemaEsgoto objetoSubSistemaEsgoto = new SubSistemaEsgoto();
			...
			if (quadraInserir.getIndicadorRedeEsgoto().equals(Quadra.COM_REDE) || 
				...
				else if (helper.getSubsistemaID() == null || helper.getSubsistemaID().equalsIgnoreCase("")) {
					
					throw new ControladorException("atencao.sistema_esgoto_nao_contem_bacia");
				} 
				else {
					
					objetoSubSistemaEsgoto.setId(new Integer(helper.getSubsistemaID()));
					quadraInserir.setSubSistemaEsgoto(objetoSubSistemaEsgoto);
				}
			} 
			else if (helper.getSubsistemaID() != null && !helper.getSubsistemaID().equalsIgnoreCase("") && 
					!helper.getSubsistemaID().equalsIgnoreCase(String.valueOf(ConstantesSistema.NUMERO_NAO_INFORMADO))) {
	...
	/**
	  * [UC 0711] Filtro Para Geração de Ordens de Serviço Seletivas
	  * 
	  *  Metodo que verifica a existencia da quadra
	  *  dentro do Setor Comercial
	  *  
	  * @author: Raimundo Martins
	  * @date: 17/01/2012 
	  */
	public Boolean existeQuadra(Integer idSetorComercial, Integer numQuadra) throws ControladorException{
		try {
			return repositorioQuadra.existeQuadra(idSetorComercial, numQuadra);
		} catch (ErroRepositorioException ex) {			
			throw new ControladorException("erro.sistema", ex);
		}
	}
	...
	/**
	 * Pesquisa os ids das localidaes dos municipios da região metropolitana
	 *
	 * @author Ana Maria
	 * @date 18/10/2012
	 *
	 ...
	 * @throws ControladorException
	 */
	public Collection<Integer> pesquisarLocalidadesMunicipioRegiaoMetropolitana() 
			throws ControladorException {
		try {
			return repositorioLocalidade.pesquisarLocalidadesMunicipioRegiaoMetropolitana();
		} catch (ErroRepositorioException ex) {
			sessionContext.setRollbackOnly();
			throw new ControladorException("erro.sistema", ex);
		}
	...
	/**
	  * [UC0934] Informar Subdivisões de Rota
	  *  
	  * @author: Flavio Ferreira
	  * @date: 12/11/2013 
	  */
  
	public  Integer pesquisarQuantidadeQuadrasForaDoGrupo(Integer idSetorComercial, Integer numeroQuadraInicial, Integer numeroQuadraFinal,
			Integer idGrupoFaturamento) throws ControladorException {

		try {
			return repositorioQuadra.pesquisarQuantidadeQuadrasForaDoGrupo(idSetorComercial, numeroQuadraInicial, numeroQuadraFinal, idGrupoFaturamento);
		} catch (ErroRepositorioException ex) {
			throw new ControladorException(	"erro.sistema",ex);
		}
	 ...
	 * 
	 */
	public Collection pesquisarListaDeLocalidadesParaComandosAcaoCobrancaEventual(Integer idComando) throws ControladorException{
		Collection<Localidade> colecaoRetorno = new ArrayList<Localidade>();
		
		try{
			
			Collection colecao = repositorioLocalidade.pesquisarListaDeLocalidadesParaComandosAcaoCobrancaEventual(idComando);
			
			Iterator colecaoLocalidadeArrayIt = colecao.iterator();
			Object[] localidadeArray;
			Localidade localidade;
			while (colecaoLocalidadeArrayIt.hasNext()) {
				localidadeArray = (Object[]) colecaoLocalidadeArrayIt.next();
				
				Integer id = (Integer) localidadeArray[0];
				String descricao = (String) localidadeArray[1];
				localidade = new Localidade();
				localidade.setId(id);
				localidade.setDescricao(descricao);
				colecaoRetorno.add(localidade);
			}
	...
	public Collection<Localidade> pesquisarLocalidadeParaComandoAcaoCobrancaEventual(Integer idComando) throws ControladorException{
		Collection<Localidade> colecaoRetorno = new  ArrayList<Localidade>();
		
		try{
			Collection colecao = repositorioLocalidade.pesquisarLocalidadeParaComandoAcaoCobrancaEventual(idComando);
	 ...
	 /**
     * pesquisar rota para encaixar quadra
     * 
     * Este método irá encontrar a qual rota a quadra deve ser associada, através das faixas de quadras das rotas do setor.
     * 
     * Retorna a Rota
     * 
     ...
     * @date: 06/05/2014
     */
    public Rota pesquisarRotaParaEncaixarQuadra(
    		SetorComercial objetoSetorComercial, Integer numeroQuadra) throws ControladorException{
    	try{
	    	Integer idRotaPesquisada = null;
	    	Rota rotaPesquisada = null;
	        Collection colecaoQuadraRota = new ArrayList<Quadra>();
	        FiltroQuadra filtroQuadraRota = new FiltroQuadra();
	        
	        FiltroRota filtroRota = new FiltroRota();
	        Collection<Rota> colecaoRota = new ArrayList<Rota>();
	        filtroRota.adicionarParametro(new ParametroSimples(FiltroRota.SETOR_COMERCIAL_ID, objetoSetorComercial.getId()));
	        ...
	        if(colecaoRota != null && !colecaoRota.isEmpty()){
		        ...
		        for (Rota r : colecaoRota) {
					idsRotas.add(r.getId());
		        }
		        
		        idRotaPesquisada = this.repositorioQuadra.pesquisarRotaParaEncaixarQuadra(idsRotas, numeroQuadra);
		        
		        if (idRotaPesquisada == null){
		        	Integer maiorQuadra = this.pesquisarMaximoCodigoQuadra(objetoSetorComercial.getId());
			        Integer menorQuadra = this.pesquisarMinimoCodigoQuadra(objetoSetorComercial.getId());
			        
			        if(numeroQuadra.compareTo(maiorQuadra) == 1){
			        	idRotaPesquisada = this.repositorioQuadra.pesquisarRotaParaEncaixarQuadra(idsRotas, maiorQuadra);
			        }else{
			        	idRotaPesquisada = this.repositorioQuadra.pesquisarRotaParaEncaixarQuadra(idsRotas, menorQuadra);
			        }
			        
			        if(idRotaPesquisada == null){
			        	FiltroRota filtroRotaNull = new FiltroRota();
			        	filtroRotaNull.adicionarParametro(new ParametroSimples(FiltroRota.CODIGO_ROTA, 1));
			        	filtroRotaNull.adicionarParametro(new ParametroSimples(FiltroRota.INDICADOR_USO, 1));
			        	filtroRotaNull.adicionarParametro(new ParametroSimples(FiltroRota.LOCALIDADE_ID, objetoSetorComercial.getLocalidade().getId()));
			        	filtroRotaNull.adicionarParametro(new ParametroSimples(FiltroRota.SETOR_COMERCIAL_ID, objetoSetorComercial.getId()));
			        	
			            Collection colecaoRotaNull = getControladorUtil().pesquisar(filtroRotaNull, Rota.class.getName());
			            
			            if(!colecaoRotaNull.isEmpty()){
			            	Rota rotaNull = (Rota)colecaoRotaNull.iterator().next();
			            	
			            	idRotaPesquisada = rotaNull.getId();
			            }else{
		        ...
		        filtroRota.adicionarParametro(new ParametroSimples(FiltroRota.ID_ROTA, idRotaPesquisada));
		        filtroRota.adicionarCaminhoParaCarregamentoEntidade(FiltroRota.LEITURA_TIPO);
		        colecaoRota = getControladorUtil().pesquisar(filtroRota, Rota.class.getName());
		        rotaPesquisada = (Rota)Util.retonarObjetoDeColecao(colecaoRota);
    ...
    /**
	 * Método que retorna o Menor número da quadra de um setor comercial
	 * 
	 * @author Flávio Leonardo
	 * @date 09/05/2014
	 * 
	...
	public int pesquisarMinimoCodigoQuadra(Integer idSetorComercial) 
			throws ControladorException{
		int numeroQuadra = 0;
		try {
			numeroQuadra = repositorioQuadra.pesquisarMinimoCodigoQuadra(idSetorComercial);
		} catch (ErroRepositorioException e) {
	 ...
	 * @throws ControladorException
	 */
	public void verificarImovelAssociadoLocalidadeSistemaAbastecimento(String idLocalidade, String idSistemaAbastecimentoPrincipal,
			String[] idsSistemasAbastecimentoSecundarios) throws ControladorException{
		
		//SISTEMAS DE ABASTECIMENTO SELECIONADOS
		Integer[] idsSistemasAbastecimento = new Integer[idsSistemasAbastecimentoSecundarios.length + 1];
		idsSistemasAbastecimento[0] = Integer.valueOf(idSistemaAbastecimentoPrincipal);
		
		for (int i = 0; i < idsSistemasAbastecimentoSecundarios.length; i++) {
			idsSistemasAbastecimento[i + 1] = Integer.valueOf(idsSistemasAbastecimentoSecundarios[i]);
		}
		
		//SELECIONANDO OS SISTEMAS DE ABASTECIMENTO QUE EXISTEM HOJE CADASTRADOS NA BASE
		FiltroLocalidadeSistemaAbastecimento filtroLocalidadeSistemaAbastecimento = new FiltroLocalidadeSistemaAbastecimento();
		filtroLocalidadeSistemaAbastecimento.adicionarCaminhoParaCarregamentoEntidade("sistemaAbastecimento");
		
		filtroLocalidadeSistemaAbastecimento.adicionarParametro(new ParametroSimples(FiltroLocalidadeSistemaAbastecimento.LOCALIDADE_ID, 
			Integer.valueOf(idLocalidade)));
		
		Collection<LocalidadeSistemaAbastecimento> colecaoBase = getControladorUtil().pesquisar(filtroLocalidadeSistemaAbastecimento,
			LocalidadeSistemaAbastecimento.class.getName());
		
		if (colecaoBase != null && !colecaoBase.isEmpty()){
			
			Iterator<LocalidadeSistemaAbastecimento> it = colecaoBase.iterator();
			boolean verificar = true;
			
			while (it.hasNext()){
				
				LocalidadeSistemaAbastecimento localidadeSistemaAbastecimento = (LocalidadeSistemaAbastecimento) it.next();
				
				verificar = true;
				
				for (int i = 0; i < idsSistemasAbastecimento.length; i++) {
				
					if (localidadeSistemaAbastecimento.getSistemaAbastecimento().getId().equals(idsSistemasAbastecimento[i])){
						verificar = false;
						break;
					}
				}
				...
				if (verificar){
					...
					try {
						...
						Integer quantidadeImoveis = repositorioLocalidade.obterQuantidadeImoveisLocalidadeSistemaAbastecimento(Integer.valueOf(idLocalidade), 
							localidadeSistemaAbastecimento.getSistemaAbastecimento().getId());
						
						if (quantidadeImoveis > 0){
							throw new ControladorException("atencao.localidade_sistema_abastecimento_utilizado");
						}
