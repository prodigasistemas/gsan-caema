...
//				+ "AND art.id = :idAtendimentoTipo ";
    ...
    public void atualizarUnidadeOrganizacional(UnidadeOrganizacional unidadeOrganizacional, Integer idAntigo)
		throws ErroRepositorioException {
		Session session = HibernateUtil.getSession();
		String update;
		...
		Integer unidadeTramitacao = null;
		if(unidadeOrganizacional.getUnidadeTramitacao() != null){
			unidadeTramitacao = unidadeOrganizacional.getUnidadeTramitacao().getId().intValue();	
		}
		...
		if(unidadeOrganizacional.getMeioSolicitacao() !=null){
			idMeioSolicitacao = unidadeOrganizacional.getMeioSolicitacao().getId().intValue();
		}

		try {
			update = " update gcom.cadastro.unidade.UnidadeOrganizacional"
				   + " set unid_idsuperior = "+unidadeSuperior	
				   + " ,unid_idunidadetramitacao = " + unidadeTramitacao 
			...
			if (unidadeOrganizacional.getUnidadeRepavimentadora() != null
				...
				update += ", unid_idrepavimentadora = " + unidadeOrganizacional.getUnidadeRepavimentadora().getId().toString()
						+ " where unid_id = :idAntigo ";
			}
			
			update += " where unid_id = :idAntigo ";
    ...
    public UnidadeOrganizacional pesquisarUnidadeOrganizacional(Integer idUnidade)
		...
		try {
			consulta = " select unid"
					 ...
					 + " left join fetch unid.unidadeTramitacao untram"
				   	 + " where unid.id = :idUnidade";
	...
	/**
	 * 
	 * Retorna a unidade organizacional superior a partir de uma localidade
	 * 
	 * @author Hugo Azevedo
	 * @date 06/02/2012
	 * 
	 * */
	public UnidadeOrganizacional pesquisarUnidadeOrganizacionalSuperior(Integer idLocalidade) throws ErroRepositorioException{
		...
		try{
			...
			sql = " SELECT unidade.unid_id as idUnidade "
					+ " FROM cadastro.unidade_organizacional unidade " 
					+ " INNER JOIN cadastro.localidade localidade ON localidade.uneg_id = unidade.uneg_id "
	...
	/**
	 * 
	 * Retorna a unidade organizacional sugerida para trâmite
	 * 
	 * @author Hugo Azevedo
	 * @date 10/02/2012
	 * 
	 * */
	public LocalidadeSolicTipoGrupo pesquisarLocalidadeSolicTipoGrupo(Integer idLocalidade,
			Integer idSituacao) throws ErroRepositorioException{
		
		Session session = HibernateUtil.getSession();
		String sql = "";
		
		LocalidadeSolicTipoGrupo retorno = null;
		
		try{
			
			sql = " SELECT lstg " +
				  " FROM LocalidadeSolicTipoGrupo lstg " +
				  " INNER JOIN FETCH lstg.unidadeOrganizacional unidade " +
				  " INNER JOIN lstg.localidade loca " +
				  " INNER JOIN lstg.solicitacaoTipoGrupo stg " +
				  " WHERE loca.id = :idLocalidade " +
				  " AND stg.id = :idSituacao";
			
			retorno = 
					(LocalidadeSolicTipoGrupo)session.createQuery(sql)
					.setInteger("idLocalidade",idLocalidade)
	 ...
	 * @date 11/05/2012
	 */
	public UnidadeOrganizacional pesquisarUnidadeOrganizacionalPorEmpresa(Integer idEmpresa)
		throws ErroRepositorioException {
		...
		try {
			...
			consulta = "SELECT DISTINCT unidade "
				+ "FROM UnidadeOrganizacional unidade  "
				+ "WHERE unidade.empresa.id = :idEmpresa ";
			
			retornoConsulta = 
				(UnidadeOrganizacional) session.createQuery(consulta).setInteger("idEmpresa", idEmpresa).setMaxResults(1).uniqueResult();
	...
	/**
	 * Método que retorna o maior id da Unidade Organizacionl
	 * 
    ...
    public int pesquisarMaximoIdUnidadeOrganizacional()
	throws ErroRepositorioException {
    	int retorno = 0;
