...
public class ControladorUnidadeSEJB implements SessionBean {
	...
	public void atualizarUnidadeOrganizacional(
			UnidadeOrganizacional unidadeOrganizacional, Usuario usuario,Integer idAntigo)
			throws ControladorException {

		String idUnidade = "" + unidadeOrganizacional.getId();

		validaAtualizacaoUnidadeOrganizacional(unidadeOrganizacional,idAntigo);
		...
		if (unidadeAtualizada != null && !unidadeAtualizada.getId().equals(idAntigo)) {
		...
		// Seta o filtro para buscar a unidade organizacional na base
		filtroUnidadeOrganizacional.adicionarParametro(new ParametroSimples(
				FiltroUnidadeOrganizacional.ID, idAntigo));
		...
		try {
			repositorioUnidade
					.atualizarUnidadeOrganizacional(unidadeOrganizacional,idAntigo);
		} catch (ErroRepositorioException ex) {
			ex.printStackTrace();
			throw new ControladorException("erro.sistema", ex);
		}
	...
	private void validaAtualizacaoUnidadeOrganizacional(
		...
		if (colecaoUnidade != null && !colecaoUnidade.isEmpty()) {
			...
			if (!unidadeOrganizacional.getId()
					...
					&& !idAntigo.equals(unidadePesquisada.getId())
					) {
				sessionContext.setRollbackOnly();
				throw new ControladorException(
						"atencao.inserir_unidade_organizacional_descricao_ja_existe",
		...
		if (unidadeOrganizacional.getUnidadeSuperior() != null
			...
			if (unidadeSuperior == null || unidadeSuperior.equals("")) {
				throw new ControladorException(
						"atencao.unidade.organizacional.inexistente");
			}
	 ...
	 * 
	 * */
	public UnidadeOrganizacional pesquisarUnidadeOrganizacionalSuperior(Integer idLocalidade) throws ControladorException{
		try {
			return repositorioUnidade
					.pesquisarUnidadeOrganizacionalSuperior(idLocalidade);
		} catch (ErroRepositorioException e) {
			sessionContext.setRollbackOnly();
			throw new ControladorException("erro.sistema", e);
		}
	...
	public LocalidadeSolicTipoGrupo pesquisarLocalidadeSolicTipoGrupo(Integer idLocalidade,
			Integer idSituacao) throws ControladorException{
		
		try {
			return repositorioUnidade
					.pesquisarLocalidadeSolicTipoGrupo(idLocalidade,idSituacao);
		} catch (ErroRepositorioException e) {
			sessionContext.setRollbackOnly();
			throw new ControladorException("erro.sistema", e);
		}
	 ...
	 * @date 11/05/2012
	 */
	public UnidadeOrganizacional pesquisarUnidadeOrganizacionalPorEmpresa(
			Integer idEmpresa) throws ControladorException {

		try {
			return repositorioUnidade
					.pesquisarUnidadeOrganizacionalPorEmpresa(idEmpresa);
		} catch (ErroRepositorioException ex) {
			ex.printStackTrace();
			throw new ControladorException("erro.sistema", ex);
		}
	...
	/**
	 * Método que retorna o maior id da Unidade Organizacional
	 * 
	...
	public int pesquisarMaximoIdUnidadeOrganizacional() throws ControladorException {
		try {
			return repositorioUnidade.pesquisarMaximoIdUnidadeOrganizacional();
		} catch (ErroRepositorioException ex) {
			sessionContext.setRollbackOnly();
			throw new ControladorException("erro.sistema", ex);
		}
