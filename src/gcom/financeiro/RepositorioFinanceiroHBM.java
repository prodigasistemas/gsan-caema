...
/*	public Collection obterParametrosContabilArrecadacao(Integer idCategoria,Integer idItemLancamentoContabil,
	...
	*/
	...
	public Collection pesquisarGerarIntegracaoContabilidade(String idLancamentoOrigem, String anoMes) throws ErroRepositorioException{
		...
		try {
			consulta = 
				"SELECT contaContabil.cnct_nnconta AS numeroConta,"
				+ "CASE contaContabil.cnct_iccusto WHEN 1 THEN localidade.loca_cdcentrocusto WHEN 2 THEN localidade.loca_cdcentrocustoesgoto END AS centroCusto,"
				+ "CASE WHEN lancamentoContabilItem.lcti_icdebitocredito=2 THEN lancamentoContabilItem.lcti_vllancamento END AS valorDebito,"
				+ "CASE WHEN lancamentoContabilItem.lcti_icdebitocredito=1 THEN lancamentoContabilItem.lcti_vllancamento END AS valorCredito,"
				+ "localidade.loca_nmlocalidade AS nomeLocalidade,"
				+ "localidade.loca_id           AS idLocalidade,"
				+ "localidade.muni_idprincipal  AS idMunicipioPrincipal,"
				+ "localidadeAlpha.localidade_alpha AS nomeLocalidadeAlpha, "
				+ "municipioAlpha.municipio_alpha AS nomeMunicipioAlpha "
				+ "FROM financeiro.lancamento_contabil_item lancamentoContabilItem "
				+ "INNER JOIN financeiro.lancamento_contabil lancamentoContabil ON lancamentoContabilItem.lcnb_id=lancamentoContabil.lcnb_id "
				+ "INNER JOIN financeiro.lancamento_origem lancamentoOrigem ON lancamentoContabil.lcor_id=lancamentoOrigem.lcor_id "
				+ "INNER JOIN financeiro.conta_contabil contaContabil ON lancamentoContabilItem.cnct_id=contaContabil.cnct_id "
				+ "INNER JOIN cadastro.localidade localidade ON lancamentoContabil.loca_id=localidade.loca_id "
				+ "LEFT JOIN alpha.localidades_alpha localidadeAlpha on localidade.loca_id = localidadeAlpha.id_localidade_gsan "
				+ "LEFT JOIN alpha.municipios_alpha municipioAlpha on localidade.muni_idprincipal = municipioAlpha.muni_id "
				
				+ "WHERE lancamentoContabil.lcnb_amlancamento = :anoMes "
				+ "AND lancamentoOrigem.lcor_id =:idLancamentoOrigem "
				+ "ORDER BY localidade.loca_id";

			retorno = session.createSQLQuery(consulta).
				addScalar("numeroConta", Hibernate.STRING).
				addScalar("centroCusto", Hibernate.STRING).
				...
				addScalar("idMunicipioPrincipal", Hibernate.INTEGER).
				addScalar("nomeLocalidadeAlpha", Hibernate.STRING).
				addScalar("nomeMunicipioAlpha", Hibernate.STRING).
	...
	public Collection<ParametrosDevedoresDuvidososItem> pesquisaParametrosDevedoresDuvidososItem(
		...
		try {
			consulta = "SELECT parametroItem " 
					...
					+ "LEFT JOIN FETCH parametroItem.cobrancaSituacao cobranca "
...
//				+ "rdd.unidadeNegocio.id, "//10
	...
	public Collection<Object[]> pesquisarDadosContasCategoriaValorAguaEsgoto(
		...
		Date dataRetificacao = Util.obterUltimaDataMes(Util.obterMes(anoMesReferenciaContabil), Util.obterAno(anoMesReferenciaContabil));
		...
		try {
			...
			consulta = "SELECT loca.greg_id as idGerencia, loca.uneg_id as idUnidadeNegocio, loca.loca_id as idLocalidade, "
					...
					+ " :situacaoParcelada, :situacaoDebitoPrescrito ) and ( conta.dcst_idanterior is null or ( conta.cnta_amreferenciaconta <= :anoMesReferenciaContabil and conta.dcst_idanterior not in (:situacaoIncluida) " 
					+ " and conta.cnta_dtretificacao is not null and conta.cnta_dtretificacao <= :dataRetificacao ) ) ) ) "
			...
			retorno = session.createSQLQuery(consulta).addScalar("idGerencia",
					...
					DebitoCreditoSituacao.NORMAL).setInteger("situacaoRetificada",
						DebitoCreditoSituacao.RETIFICADA).setInteger(
					"situacaoIncluida", DebitoCreditoSituacao.INCLUIDA).setInteger(
							"situacaoCancelada",
					...
					.setDate("dataRetificacao", dataRetificacao).list();
        ...
        try {
            ...
            retorno = session.createSQLQuery(consulta).addScalar("idGerencia",
					...
					.setInteger("situacaoParcelada",
							...
							"servico", FinanciamentoTipo.SERVICO_NORMAL)
				    .setDate("dataRetificacao", dataRetificacao).list();
    ...
    public Collection<Object[]> pesquisarDadosDebitoACobrarCategoriaServico(
        ...
        try {
        	...
        	consulta = "SELECT loca.greg_id as idGerencia, loca.uneg_id as idUnidadeNegocio, loca.loca_id as idLocalidade, "
                    ...
                    + " and ( dbac.fntp_id = :servico ) AND "
                    + " (dbac.lict_id <> 2 or (dbac.lict_id = 2 and dbac.dbac_amrefcontabilizacao < 201203 )) "
     ...
     * 
     * Modificações adicionadas pela RM 7720: Filtrar tambem por lançamento item contábil diferente de acréscimos 
     * por impontualidade ou (lançamento item contábil igual à acréscimos por impontualidade e ano mês de contabilização 
     * menor que 201203 da tabela  DEBITO_A_COBRAR. Amelia Pessoa.
     *   
     * @author Rafael Corrêa
     * @date 08/11/2007
     *
    ...
    public Collection<Object[]> pesquisarDadosDebitoACobrarCategoriaFinancimentosCurtoPrazo(
        ...
        try {
        	...
        	consulta = " SELECT loca.greg_id as idGerencia, loca.uneg_id as idUnidadeNegocio, loca.loca_id as idLocalidade, "
                    ...
                    + " INNER JOIN faturamento.deb_a_cobrar_catg dbacCat "
                    + " on dbacCat.dbac_id = dbac.dbac_id "
                    + " INNER JOIN financeiro.lancamento_item_contabil lanc on dbac.lict_id = lanc.lict_id " //RM7720
                    + " WHERE loca.loca_id = :idLocalidade "
                    + " and ( ( dbac.dbac_amreferenciacontabil <= :anoMesReferenciaContabil "
                    + " and dbac.dcst_idatual in ( :situacaoNormal, "
                    + " :situacaoIncluida, :situacaoRetificada ) ) "
                    ...
                    + " and (dbac.lict_id <> 2 or (dbac.lict_id = 2 and dbac.dbac_amrefcontabilizacao < 201203 )) "
                    + " GROUP BY loca.greg_id, loca.uneg_id, loca.loca_id, dbaccat.catg_id "
                    + " ORDER BY idGerencia, idUnidadeNegocio, idLocalidade, idCategoria ";
     ...
     * curto prazo pela gerência, localidade e categoria
     *
     * Modificações adicionadas pela RM 7720: Filtrar tambem por ano mês de contabilização 
     * menor que 201203 da tabela  DEBITO_A_COBRAR. Amelia Pessoa.
     * 
     * @author Rafael Corrêa
     * @date 08/11/2007
    ...
    public Collection<Object[]> pesquisarDadosDebitoACobrarCategoriaJurosCobrados(
        ...
        try {
        	...
        	consulta = " SELECT loca.greg_id as idGerencia, loca.uneg_id as idUnidadeNegocio, loca.loca_id as idLocalidade, "
                    ...
                    + " and dbac.dbac_amrefcontabilizacao < 201203 " //RM4071
                    + " GROUP BY loca.greg_id, loca.uneg_id, loca.loca_id, dbaccat.catg_id "
                    + " ORDER BY idGerencia, idUnidadeNegocio, idLocalidade, idCategoria ";
...
//	*******************************Caern	
	...
	public Collection<Object[]> pesquisarContasAReceberParaResumo(
		...
		try {
			...
			consulta = " SELECT \n " 
					  + " idGerencia, idUnidadeNegocio, idLocalidade, idImovelPerfil, \n "
					  + " COALESCE( cltp.epod_id, 4 )   AS idEsferaPoder, \n " 
					  + " idCategoria, idDocumentoTipo, dataVencimento, \n "
					  + " COUNT( dadosbasicos.cnta_id ) AS quantidadeDocumentos, \n "
					  + " SUM ( valorAgua )             AS valorAguaCategoria, \n "
					  + " SUM ( valorEsgoto )           AS valorEsgotoCategoria, \n "
					  + " SUM ( valorDebitos )          AS valorDebitoCategoria, \n "
					  + " SUM ( valorCreditos )         AS valorCreditoCategoria, \n "
					  + " SUM ( valorImpostos )         AS valorImpostos, \n "
					  ...
					  + " loca.uneg_id                         AS idUnidadeNegocio, \n "
					  + " loca.loca_id                         AS idLocalidade, \n "
					  + " COALESCE( conta.iper_id, 5 )         AS idImovelPerfil, \n "
					  + " COALESCE( \n " 
					  + " ( \n "
					  + "    SELECT ctcg.catg_id \n "
					  + "    FROM faturamento.conta_categoria ctcg \n "
					  + "     WHERE ctcg.cnta_id = conta.cnta_id AND ROWNUM = 1 \n "
					  + "     GROUP BY ctcg.catg_id, ctcg.ctcg_qteconomia \n "
					  + "     HAVING ctcg.ctcg_qteconomia = Max( ctcg.ctcg_qteconomia ) \n " 
					  + "   ), 1 )                             AS idCategoria, \n "
					  ...
					  + " COALESCE(conta.cnta_vlagua,0)        AS valorAgua, \n "
					  + " COALESCE(conta.cnta_vlesgoto,0)      AS valorEsgoto, \n "
					  + " COALESCE(conta.cnta_vldebitos,0)     AS valorDebitos, \n "
					  + " COALESCE(conta.cnta_vlcreditos,0)    AS valorCreditos, \n "
					  + " COALESCE(conta.cnta_vlimpostos,0)    AS valorImpostos \n "
					  + " from \n "
					  + " cadastro.localidade loca \n "
					  
					  + " INNER JOIN faturamento.conta conta  ON loca.loca_id = conta.loca_id \n "
					  + " ) dadosbasicos  \n "
					  + " LEFT JOIN cadastro.cliente_conta clct          ON dadosbasicos.cnta_id = clct.cnta_id AND clct.crtp_id = 3 \n "
					  ...
					  + " LEFT JOIN financeiro.faixa_docs_a_receber fdrc ON \n "
					  + "     fdrc.fdrc_vlfaixainicial <= (valorAgua + valorEsgoto + valorDebitos - ( valorCreditos + valorImpostos ) ) \n "
					  + " AND fdrc.fdrc_vlfaixafinal   >= (valorAgua + valorEsgoto + valorDebitos - ( valorCreditos + valorImpostos ) ) \n "
					  + " WHERE \n "
					  + " idLocalidade = :idLocalidadeParam AND \n " 
					  + " ( \n "
					  + " ( \n "
					  + "   dadosbasicos.cnta_amreferenciacontabil <= :anoMesReferenciaContabil AND \n " 
					  + "   dadosbasicos.dcst_idatual IN ( "+DebitoCreditoSituacao.NORMAL+", "+DebitoCreditoSituacao.INCLUIDA+", \n "
					  + DebitoCreditoSituacao.RETIFICADA+" ) \n " 
					  + " ) OR \n "
					  + " ( dadosbasicos.cnta_amreferenciacontabil > :anoMesReferenciaContabil AND \n " 
					  + "   dadosbasicos.dcst_idatual IN ( "+DebitoCreditoSituacao.CANCELADA+", "+DebitoCreditoSituacao.CANCELADA_POR_RETIFICACAO+", \n "
					  + " "+DebitoCreditoSituacao.PARCELADA+", "+DebitoCreditoSituacao.DEBITO_PRESCRITO+") AND \n " 
					  + "   ( dadosbasicos.dcst_idanterior IS NULL OR \n "
					  + "     ( dadosbasicos.cnta_amreferenciaconta <= :anoMesReferenciaContabil AND \n " 
					  + "       dadosbasicos.dcst_idanterior not in (1,2) \n " 
					  + "     ) \n " 
					  + "   ) \n " 
					  + " ) \n "
					  + " ) \n "					 
					  
					  + " GROUP BY idGerencia, idUnidadeNegocio, idLocalidade, idImovelPerfil, \n "
					  + " COALESCE( cltp.epod_id, 4 ),  idCategoria, idDocumentoTipo, dataVencimento, fdrc.fdrc_id \n "
					  
					  + " ORDER BY idGerencia, idUnidadeNegocio, idLocalidade, idImovelPerfil, \n "
					  + " COALESCE( cltp.epod_id, 4 ),  idCategoria, idDocumentoTipo, dataVencimento, fdrc.fdrc_id \n ";
			/*
			...
			*/
			...
			retorno = session.createSQLQuery(consulta)
					...
					.setInteger("idLocalidadeParam", idLocalidade)
					.setInteger("anoMesReferenciaContabil", anoMesReferenciaContabil)
	...
	public Collection<Object[]> pesquisarGuiasPagamentoAReceberParaResumo(
		...
		try {
			...
			consulta = " SELECT \n " 
					  ...
					  + " COALESCE( cltp.epod_id, 4 )   AS idEsferaPoder, \n " 
					  + " idCategoria, idDocumentoTipo, dataVencimento, \n "
					  + " COUNT( dadosbasicos.gpag_id ) AS quantidadeDocumentos, \n "	
					  + " SUM ( valorDebitos )          AS valorDebitoCategoria, \n "
					  + " fdrc.fdrc_id                  AS idFaixa \n "
					  + " FROM ( \n "
					  + " SELECT \n "
					  ...
					  + " loca.uneg_id                         AS idUnidadeNegocio, \n "
					  + " loca.loca_id                         AS idLocalidade, \n "
					  + " COALESCE( imov.iper_id, 5 )         AS idImovelPerfil, \n " 					  
					  + " COALESCE( \n " 
					  + " ( \n "
					  + "    SELECT ctcg.catg_id \n "
					  + "    FROM faturamento.conta_categoria ctcg \n "
					  ...
					  + " gpag.gpag_id , \n "
					  + " gpag_amreferenciacontabil, \n "
					  + " gpag.dcst_idatual, \n "
					  + " gpag.dcst_idanterior, \n "
					  + " gpag.gpag_vldebito, \n "
					  + " COALESCE(gpag.gpag_vldebito,0)     AS valorDebitos \n " 
					  + " from \n "
					  + " cadastro.localidade loca \n "					  
					  ...
					  + " INNER JOIN faturamento.conta conta  ON loca.loca_id = conta.loca_id \n "
					  + " ) dadosbasicos  \n "				  
					  + " LEFT JOIN cadastro.cliente_guia_pagamento clgp on dadosbasicos.gpag_id = clgp.gpag_id and clgp.crtp_id = "
					  + ClienteRelacaoTipo.RESPONSAVEL.toString()					  
					  + " LEFT JOIN cadastro.cliente resp                ON clgp.clie_id = resp.clie_id \n "
					  + " LEFT JOIN cadastro.cliente_tipo cltp           ON resp.cltp_id = cltp.cltp_id \n "
					  + " LEFT JOIN financeiro.faixa_docs_a_receber fdrc ON fdrc.fdrc_vlfaixainicial <= coalesce(dadosbasicos.gpag_vldebito,0) "
					  + "  AND fdrc.fdrc_vlfaixafinal >= coalesce(dadosbasicos.gpag_vldebito,0) "					  
					  + " WHERE \n "
					  + " idLocalidade = :idLocalidadeParam AND \n " 
					  + " ( \n "
					  + " ( \n "
					  + "   dadosbasicos.gpag_amreferenciacontabil <= :anoMesReferenciaContabil AND \n " 
					  + "   dadosbasicos.dcst_idatual IN ( "+DebitoCreditoSituacao.NORMAL+", "+DebitoCreditoSituacao.INCLUIDA+", \n "
					  + DebitoCreditoSituacao.RETIFICADA+" ) \n " 
					  + " ) OR \n "
					  + " ( dadosbasicos.gpag_amreferenciacontabil > :anoMesReferenciaContabil AND \n " 
					  + "   dadosbasicos.dcst_idatual IN ( "+DebitoCreditoSituacao.CANCELADA+", "+DebitoCreditoSituacao.CANCELADA_POR_RETIFICACAO+", \n "
					  + " "+DebitoCreditoSituacao.PARCELADA+", "+DebitoCreditoSituacao.DEBITO_PRESCRITO+") AND \n " 
					  + "   ( dadosbasicos.dcst_idanterior IS NULL ))) \n "					  
					  + " GROUP BY idGerencia, idUnidadeNegocio, idLocalidade, idImovelPerfil, \n "
					  + " COALESCE( cltp.epod_id, 4 ),  idCategoria, idDocumentoTipo, dataVencimento, fdrc.fdrc_id \n "					  
					  + " ORDER BY idGerencia, idUnidadeNegocio, idLocalidade, idImovelPerfil, \n "
					  + " COALESCE( cltp.epod_id, 4 ),  idCategoria, idDocumentoTipo, dataVencimento, fdrc.fdrc_id \n ";
	...
	public Collection<Object[]> pesquisarDebitosACobrarAReceberParaResumo(
		...
		try {
			...
			consulta = "SELECT loca.greg_id as idGerencia, loca.uneg_id as idUnidadeNegocio, "//0, 1
					...
					//+ " AND (lanc.lict_id <> 2 OR ((lanc.lict_id = 2) AND (dbac.dbac_amrefcontabilizacao < 201203))) " //RM7720
			...
			*/
			...
			consulta = " SELECT \n " 
					  ...
					  + " cadastro.localidade loca \n "					  
					  + " INNER JOIN faturamento.debito_a_cobrar dbac on loca.loca_id = dbac.loca_id \n " 
					  + " AND (dbac.lict_id <> 2 OR ((dbac.lict_id = 2) AND (dbac.dbac_amrefcontabilizacao < 201203))) " //RM7720
					  + " INNER JOIN financeiro.lancamento_item_contabil lanc on dbac.lict_id = lanc.lict_id \n " //RM7720
					  + " LEFT JOIN cadastro.imovel imov on imov.imov_id = dbac.imov_id \n "					  
					  + " INNER JOIN faturamento.conta conta  ON loca.loca_id = conta.loca_id \n "
					  + " ) dadosbasicos  \n "
					  + " LEFT JOIN cadastro.cliente_conta clct          ON dadosbasicos.cnta_id = clct.cnta_id AND clct.crtp_id = 3 \n "
					  + " LEFT JOIN cadastro.cliente resp                ON clct.clie_id = resp.clie_id \n "
	...
	public Collection<Object[]> pesquisarCreditosARealizarAReceberParaResumo(
		...
		try {
			...
			consulta = " SELECT \n " 
					  ...
					  + " conta.cnta_dtvencimentoconta         AS dataVencimento, \n "
					  + " conta.cnta_id, \n "
					  + " crar.crar_amreferenciacontabil, \n "
					  + " crar.dcst_idatual, \n "
					  + " crar.dcst_idanterior, \n "
					  + " cnta_amreferenciaconta, \n "
					  + " COALESCE(conta.cnta_vlagua,0)        AS valorAgua, \n "
					  + " COALESCE(conta.cnta_vlesgoto,0)      AS valorEsgoto, \n "
					  + " COALESCE(conta.cnta_vldebitos,0)     AS valorDebitos, \n "
					  ...
					  + " LEFT JOIN cadastro.imovel imov on imov.imov_id = crar.imov_id \n "					  
					  + " INNER JOIN faturamento.conta conta  ON loca.loca_id = conta.loca_id \n "
					  + " ) dadosbasicos  \n "
					  + " LEFT JOIN cadastro.cliente_conta clct          ON dadosbasicos.cnta_id = clct.cnta_id AND clct.crtp_id = 3 \n "
					  + " LEFT JOIN cadastro.cliente resp                ON clct.clie_id = resp.clie_id \n "
					  ...
					  + " ( \n "
					  + " ( \n "
					  + "   dadosbasicos.crar_amreferenciacontabil <= :anoMesReferenciaContabil AND \n " 
					  + "   dadosbasicos.dcst_idatual IN ( "+DebitoCreditoSituacao.NORMAL+", "+DebitoCreditoSituacao.INCLUIDA+", \n "
					  + DebitoCreditoSituacao.RETIFICADA+" ) \n " 
					  + " ) OR \n "
					  + " ( dadosbasicos.crar_amreferenciacontabil > :anoMesReferenciaContabil AND \n " 
					  + "   dadosbasicos.dcst_idatual IN ( "+DebitoCreditoSituacao.CANCELADA+", "+DebitoCreditoSituacao.CANCELADA_POR_RETIFICACAO+", \n "
					  + DebitoCreditoSituacao.PARCELADA+") \n " 
					  +	" OR ( dadosbasicos.dcst_idatual = "+DebitoCreditoSituacao.DEBITO_PRESCRITO+" AND \n " 
					  + " dadosbasicos.dcst_idanterior IS NULL ) \n "
					  + "   ) \n " 
					  + " ) \n "
					 // + " ) \n "					 
...
//					  + " (pag.pgst_idatual <> 10 or pag.pgst_idatual <> 0)"
	...
	public BigDecimal obterValorTotalContasDevedoresDuvidosos( int anoMesReferenciaContabil, Integer idLocalidade, Integer idQuadra, String anoMesString, 
		...
		try {
			consulta =	" select sum(coalesce(cnta.cnta_vlagua,0) + coalesce(cnta.cnta_vlesgoto,0) " 
					+	"+ coalesce(cnta.cnta_vldebitos,0) - coalesce(cnta.cnta_vlcreditos,0) ) as valorContas "
	...
	public Collection<Object[]> pesquisarValorAguaAgrupadoPorCategoriaDevedoresDuvidosos(int anoMesReferenciaContabil, 
		...
		try {
			String referencia = "";
			if ( idTipoPerda.equals(PerdasTipo.PROVISAO_PERDAS_SOCIETARIAS)) {
				referencia = " cnta.cnta_amreferenciabaixasocial ";
			} else {
				
				if(idTipoPerda.equals(PerdasTipo.PERDAS_FISCAIS)){
					where = " and cnta.cnta_amreferenciabaixasocial is null ";
				}
		...
		consulta = "select sum(ctcg.ctcg_vlagua) as valorAgua, "
			...
			+ " and cnta.qdra_id = :idQuadra"
			+ " and "+referencia+" = :anoMesReferencia" + where
	...
	public Collection<Object[]> pesquisarValorBaixadoAgrupadoPorLocalidadeResumoDevedoresDuvidosos( int anoMesReferenciaContabil )  throws ErroRepositorioException {
		...
		+ 	" group by loca_id " ;
			...
			retorno = session.createSQLQuery(consulta)
					...
					.setInteger("anoMesReferencia", anoMesReferenciaContabil)
					.setInteger("perdaTipo", PerdasTipo.PERDAS_FISCAIS)
					.list();
	 ...
	 * @throws ErroRepositorioException
	 */	
	public void retiraBaixaDasContasPerdasSocietarias(int anoMesReferenciaContabil, int localidadeId)	throws ErroRepositorioException {
		...
		try {
			...
			update =  "update faturamento.conta set "
				...
				+ "petp_id = null " 
				+ "where petp_id = ? "
				+ "and cnta_amreferenciabaixasocial = ? " 
				+ "and loca_id = ?" ; 
	...
	public void atualizaContaPerdasSocietarias(int anoMesReferenciaContabil , Integer anoMesInicial, Integer anoMesFinal, Integer anoMesMenosNumeroDeMesesInformados,
		...
		try {
			...
			update =	"update faturamento.conta c1 "
					+   "set cnta_amreferenciabaixasocial = ? , "
					+   "petp_id = ? "
					+	"WHERE c1.loca_id = ? "
				    +   "and c1.qdra_id = ? " 
					+   "and c1.imov_id in (select distinct(c2.imov_id) "
										+	"from faturamento.conta c2 "
										+	"inner join faturamento.conta_categoria cc on (c2.cnta_id=cc.cnta_id) "
										+	"inner join cadastro.cliente_imovel clim on clim.imov_id = c2.imov_id "
										+	"inner join cadastro.cliente clie on clie.clie_id = clim.clie_id "
										+	"inner join cadastro.cliente_tipo cltp on cltp.cltp_id = clie.cltp_id "
										+	"where c2.cnta_amreferenciabaixacontabil is not null "
										+	"and c2.cnta_amreferenciaconta  < ? "
										+	"and cc.catg_id in ( " +colecaoCategoria + " ) "
										+	"and not exists ( select pag.cnta_id from arrecadacao.pagamento pag where pag.cnta_id = c2.cnta_id ) " 
										+	"and clim.crtp_id = ? and clim.clim_dtrelacaofim is null  "
										+	"and cltp.epod_id in ( " + colecaoEsferaPoder +" )) "
					+	"and c1.dcst_idatual = ? "
					+	"and c1.cnta_amreferenciabaixacontabil is null "
					+	"and c1.cnta_amreferenciabaixasocial is null "
					+	"and c1.cnta_amreferenciaconta  between ? and ? ";
			...
			st.setInt(5, anoMesMenosNumeroDeMesesInformados);
			st.setInt(6, ClienteRelacaoTipo.USUARIO);
			st.setInt(7, DebitoCreditoSituacao.NORMAL);
			st.setInt(8, anoMesInicial);
			st.setInt(9, anoMesFinal);
	 ...
	 * @throws ErroRepositorioException
	 */	
	public void retiraBaixaDasContasPerdasOrgaoPublico(int anoMesReferenciaContabil, int idLocalidade)	throws ErroRepositorioException {
...
Collection retorno;
		...
		try{
			...
			consulta.append("resReceita.rrec_vlreceita as valor, ");
...
//					.setInteger("categId", Categoria.PUBLICO)
	...
	public void gerarContasAReceberContabilAnaliticoTabelaAuxiliar() throws ErroRepositorioException, SQLException {
		...
		try {
			...
			sql = " CALL FINANCEIRO.SP1_GERAR_CONTA_REC_CTB() ";
	...
	public void gerarContasAReceberContabilAnalitico() throws ErroRepositorioException, SQLException {
		...
		try {
			con = session.connection();
			sql = " CALL FINANCEIRO.SP2_GERAR_CONTA_REC_CTB() ";
	...
	public void gerarContasAReceberContabilSintetico() throws ErroRepositorioException, SQLException {
		...
		try {
			con = session.connection();
			sql = " CALL FINANCEIRO.SP3_GERAR_CONTA_REC_CTB() ";
	...
	public void inserirParametrosPerdaSocietarias(Integer anoMesReferencia) throws ErroRepositorioException {
		...
		try {
			...
			insert = "INSERT "
					+"INTO financeiro.param_perdas_societarias( "
					+"ppso_id, ppso_amrefenciacontabil, ppso_amreferenciabaixainicial, "
		            +"ppso_amreferenciabaixafinal, ppso_nnmesesreferenciainferior, "
		            +"ppso_icgeracaoreal, ppso_iccategoriaresidencial, ppso_iccategoriacomercial, "
		            +"ppso_iccategoriaindustrial, ppso_iccategoriapublica, ppso_icesferaparticular, "
		            +"ppso_icesferamunicipal, ppso_icesferaestadual, ppso_icesferafederal, "
		            +"ppsp_tmultimaalteracao) "         
		+"( SELECT "
		+"financeiro.seq_param_perdas_societarias.nextval, "
		+ anoMesReferencia + ", "
		+"ppso_amreferenciabaixainicial, "
		+"ppso_amreferenciabaixafinal, "
		+"ppso_nnmesesreferenciainferior, "
		+"1, "
		+"ppso_iccategoriaresidencial, "
		+"ppso_iccategoriacomercial, "
		+"ppso_iccategoriaindustrial, "
		+"ppso_iccategoriapublica, ppso_icesferaparticular, "
		+"ppso_icesferamunicipal, ppso_icesferaestadual, ppso_icesferafederal, "
		+"sysdate "
		+"FROM financeiro.param_perdas_societarias "
		+"where  ppso_amrefenciacontabil = (select max( ppso_amrefenciacontabil) from financeiro.param_perdas_societarias))";
	 ...
	 * @throws ErroRepositorioException
	 */
	public ParametrosDevedoresDuvidosos obterParametrosDevedoresDuvidosos() throws ErroRepositorioException {
		...
		try {
			consulta = "SELECT pded "
					+ "FROM ParametrosDevedoresDuvidosos pded " 
					+ "ORDER BY pded.anoMesReferenciaContabil DESC ";
	 ...
	 * @throws ErroRepositorioException
	 */
	public Collection<Object[]> pesquisarValorTransicaoPerdaSocietariaParaFiscal(int anoMesReferenciaContabil, 
			Integer idLocalidade, Integer idQuadra) throws ErroRepositorioException {
		...
		consulta = "Select "+
					"sum((coalesce(vl_agua,0) + coalesce(vl_esgoto,0) + coalesce(vl_debito,0)) - (coalesce(vl_credito,0))) as valorTransicao, "+
					"categoria as categoria "+
					"from ( "+
					"select "+
					"catg_id            as categoria, "+
					"sum(ctcg_vlagua)   as vl_agua, "+
					"sum(ctcg_vlesgoto) as vl_esgoto, "+
					"0                  as vl_debito, "+
					"0                  as vl_credito "+
					"from faturamento.conta ct "+
					"inner join faturamento.conta_categoria cc on cc.cnta_id = ct.cnta_id "+
					"where ct.cnta_amreferenciabaixasocial is not null "+
					      "and ct.cnta_amreferenciabaixacontabil = :anoMesReferencia "+
					      "and ct.petp_id = 1 "+
					      "and ct.loca_id = :idLocalidade "+
						  "and ct.qdra_id = :idQuadra "+
					"GROUP BY catg_id, 0, 0   "+
					"UNION ALL "+
					"select "+
					"catg_id, "+
					"0, "+
					"0, "+
					"sum(DCCG_VLCATEGORIA), "+
					"0 "+
					"from faturamento.conta ct "+
					"inner join faturamento.debito_cobrado dc on dc.cnta_id = ct.cnta_id and dc.fntp_id  in ( 1,2,3,4,5,6,7,8,9 ) "+ 
					"inner join faturamento.debito_cobrado_categoria dcc on dcc.dbcb_id = dc.dbcb_id "+
					"where ct.cnta_amreferenciabaixasocial is not null "+
					      "and ct.cnta_amreferenciabaixacontabil = :anoMesReferencia "+
					      "and ct.petp_id = 1 "+
					      "and ct.loca_id = :idLocalidade "+
					...
					"0, "+
					"0, "+
					"sum(crcg_vlcategoria) "+ 
					"from faturamento.conta ct "+
					"inner join faturamento.credito_realizado cr on cr.cnta_id = ct.cnta_id "+ 
					"inner join faturamento.cred_realizado_catg crc on crc.crrz_id = cr.crrz_id "+
					"where ct.cnta_amreferenciabaixasocial is not null "+
					      "and ct.cnta_amreferenciabaixacontabil = :anoMesReferencia "+
					      "and ct.petp_id = 1 "+
					      "and ct.loca_id = :idLocalidade "+
			...
			retorno = session.createSQLQuery(consulta)
					.addScalar("valorTransicao",Hibernate.BIG_DECIMAL)
					.addScalar("categoria", Hibernate.INTEGER)
					.setInteger("idLocalidade", idLocalidade)
					.setInteger("anoMesReferencia", anoMesReferenciaContabil)
					.setInteger("idQuadra", idQuadra)
	 ...
	 * @throws ErroRepositorioException
	 */
	public void atualizarValorBaixadoParametrosDevedoresDuvidosos(Integer anoMesReferenciaContabil) throws ErroRepositorioException {
		...
		try {
			...
			update = " update financeiro.param_deved_duvid pdd "
					+ "	set pdd.pded_vlbaixado = (select " 
													+"sum(case "
														+"when lctp_id  in (1,2) or ( RDED_NNSEQUENCIATIPOLANCAMENTO in (1000,1300,1500) and lctp_id not in (1,2) ) "
														+"then coalesce(RDED_VLVALORBAIXADO,0) "
														+"when RDED_NNSEQUENCIATIPOLANCAMENTO = 1400 then (coalesce(RDED_VLVALORBAIXADO,0)*-1) "
													+"end) as vlr_baixado "
													+"from financeiro.resumo_deved_duvid "
													+"where RDED_AMREFERENCIACONTABIL = " + anoMesReferenciaContabil  
													+" and petp_id = " +PerdasTipo.PERDAS_FISCAIS+ ") "
					+ " ,pded_tmultimaalteracao = ? "
					+ "WHERE pdd.pded_amreferenciacontabil = " + anoMesReferenciaContabil;
