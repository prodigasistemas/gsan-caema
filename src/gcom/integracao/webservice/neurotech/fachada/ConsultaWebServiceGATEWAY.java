...
public class ConsultaWebServiceGATEWAY {
	...
	public ConsultarReceitaFederal consultarPessoaFisica(String cpf, Usuario usuario, Cliente cliente, ClienteFone fonePrincipal, ClienteEndereco endereco)
		...
		//tenta realizar a conexao com o mesmo cliente pelo menos duas vezes
		if (consultarGatewayResponse.get_return().length != 1
			...
			consultarGatewayResponse = stub.consultarGateway(consultarGateway);	
		}
		...
		if (consultarGatewayResponse.get_return().length == 1
			...
			if(nomePessoaFisica.length() > 100){
				nomePessoaFisica = nomePessoaFisica.substring(0, 99);
			}
		...
		if ( consultaRF!= null ) {
			AcessoReceitaFederal acessoReceitaFederal = new AcessoReceitaFederal(new Date(), new Date(), consultaRF.getMensagemRetornoReceitaFederal());
			Fachada.getInstancia().inserir(acessoReceitaFederal);
		}
	...
	public ConsultarReceitaFederal consultaPessoaJuridica(String cnpj,
		...
		if (consultarGatewayResponse.get_return().length == 1
			...
			if(nomePessoaJuridica.length() > 100){
				nomePessoaJuridica = nomePessoaJuridica.substring(0, 99);
			}
			...
			if(nomeFantasia.length() > 100){
				nomeFantasia = nomeFantasia.substring(0, 99);
			}
			...
			if (cep != null && !cep.contains("*") && !cep.equals("")) {
