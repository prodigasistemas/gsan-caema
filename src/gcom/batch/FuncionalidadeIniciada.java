...
//				    }							
				...
				case Funcionalidade.EXECUTAR_COMANDO_DE_NEGATIVACAO :
					...
					if (nComando.getIndicadorSimulacao() == NegativacaoComando.NAO_SIMULACAO) { 
						...
						this.getControladorSpcSerasa().gerarArquivoNegativacao(nComando.getId(), (Integer)obj[3], neg.getId(), true);
				...
				case Funcionalidade.BATCH_GERAR_MOTIVOS_NAO_GERACAO_CONTAS_IMOVEIS_COB_EMPRESA :
					...
					Collection<Object[]> colecaoComandos = (Collection<Object[]>) tarefaBatch
						.getParametro("colecaoComandos");

						if (colecaoComandos != null && !colecaoComandos.isEmpty()) {
		
							Iterator<Object[]> it = colecaoComandos.iterator();
							...
							while (it.hasNext()) {
								...
								//1.1.4. O sistema deverá atualizar o indicador de inclusão de motivos de não geração par ao comando de cobrança 
								//1.1.4. [SB0010 - Atualizar Comando de Cobrança].
								getControladorCobranca().atualizarComandoEmpresaCobrancaConta(
									(Integer)comando[0]);
