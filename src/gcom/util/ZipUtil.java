...
//		zipFile.closeEntry();
	...
	public static byte[] zipBytes(String filenameArqTxt, byte[] arquivoTexto, String filenameArqFoto,byte[] arquivoFotos) throws IOException {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		ZipOutputStream zos = new ZipOutputStream(baos);
		
		ZipEntry entry = new ZipEntry(filenameArqTxt);
		entry.setSize(arquivoTexto.length);
		zos.putNextEntry(entry);
		zos.write(arquivoTexto);
		
		
		ZipEntry entryFoto = new ZipEntry(filenameArqFoto);
		entry.setSize(arquivoFotos.length);
		zos.putNextEntry(entryFoto);
		zos.write(arquivoFotos);
