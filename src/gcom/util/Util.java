...
public class Util {
	...
	/**
	 * Converte a data passada em string no formato dd/MM/yyyy
	 * 
...
/**
	...
	public static boolean compararAnoMesReferenciaSemBarra(String anoMesReferencia1,
			String anoMesReferencia2, String sinal) {
	...
	public static BigDecimal formatarPopulacaoparaBigDecimal(String valor) {
		BigDecimal bigDecimalFormatado = new BigDecimal("0");
		...
		if (valor != null) {
			...
			boolean temCasaDecimal = false;
			if (valor.length() > 4
					&& (valor.substring(valor.length() - 5, valor.length() - 4)
							.equals(".") || valor.substring(valor.length() - 5,
							valor.length() - 4).equals(","))) {
				temCasaDecimal = true;
			}
			...
			if (temCasaDecimal) {
				int tamanho = valorSemPontuacao.length();
				valorSemPontuacao = valorSemPontuacao.substring(0, tamanho - 4)
						+ "."
						+ valorSemPontuacao.substring(tamanho - 4, tamanho);
			}
			if (negativo) {
				valorSemPontuacao = "-" + valorSemPontuacao;
			}
	...
	public static Date converteStringParaDateHora(String data) {
		...
		try {
			...
			new IllegalArgumentException(data
					+ " não tem o formato dd/MM/yyyy HH:mm:ss.");
		}
		return retorno;
	}
	...
	/**
	 * Valida o mês/ano de referência sem a barra
	 * retornando true se a data for inválida e false se a data for válida
	 * 
	...
	public static String formatarMoedaRealNumber(BigDecimal valor) {
		/**
		 * Símbolos especificos do Real Brasileiro
		 */
		DecimalFormatSymbols REAL = new DecimalFormatSymbols(new Locale("pt",
		...
		// return DINHEIRO_REAL.format(valor);
		if (valor != null && !"".equals(valor)) {
			DecimalFormat DINHEIRO_REAL = new DecimalFormat("###,###,##0",
					REAL);
			return DINHEIRO_REAL.format(valor);
		} else {
			return "";
    ...
    /**
     * Método que recebe uma string e verifica se a string só tem numeros ou se o seu valor é zero.
     * 
     * @param data
     * @autor Breno Santos
     * @date 10/09/2010
	...
	public static boolean ehDiaUtil(Date dataAnalisada,Collection<NacionalFeriado> colecaoNacionalFeriado,
		...
		// Verifica se eh Feriado
			...
			String mesDiaDataAnalisada = recuperaMesDiaDaData(dataAnalisada);
			String mesDiaDataFeriado = "";
			...
			if(colecaoNacionalFeriado != null && !colecaoNacionalFeriado.isEmpty()){
				...
				while (itera.hasNext()) {
					...
					if(nacionalFeriado.getIndicadorPerene().equals(ConstantesSistema.NAO)){
						if(nacionalFeriado.getData().compareTo(dataAnalisada) == 0){
							ehDiaUtil = false;
							break;
						}
					}else{
						mesDiaDataFeriado = recuperaMesDiaDaData(nacionalFeriado.getData()); 
						
						if(mesDiaDataAnalisada.equals(mesDiaDataFeriado)){
							ehDiaUtil = false;
							break;
						}
					}
			...
			if(ehDiaUtil){
				...
				if(colecaoMunicipioFeriado != null && !colecaoMunicipioFeriado.isEmpty()){
					...
					while (itera.hasNext()) {
						...
						if(municipioFeriado.getIndicadorPerene().equals(ConstantesSistema.NAO)){
							if(municipioFeriado.getDataFeriado().compareTo(dataAnalisada) == 0){
								ehDiaUtil = false;
								break;
							}
						}else{
							mesDiaDataFeriado = recuperaMesDiaDaData(municipioFeriado.getDataFeriado());
	 ...
	 * @param ano Ano
	 * @param colecaoDatasFeriados Coleção de Datas dos Feriados
	 * 
	...
	public static Date subtrairNumeroMesesDeUmaData(Date data, int numeroMeses) {
		...
		// subtrai o nº de meses INFORMADO da data
	...
	/**
	 * Author: Sávio Luiz Data: 08/01/2006
	 * 
	 * Adiciona nº de dias para uma data
	 * 
	 * @param numeroDias
	 ...
	 * @return data menos o nº de dias informado
	 */
	public static Date adicionarHorasDatas(Date data, int numeroHoras) {
		// cria uma instância de GregorianCalendar para manipular a data
		Calendar c = GregorianCalendar.getInstance();
	...
	/**
	 * O metódo completa uma string com espaços em branco (ex: passa a string
	 * "12.36" e o tamanho máximo 10 e retorna " 12.36" ) apenas se a string não exceder o tamanho máximo
	 * 
	 * 
	...
	/**
	 * Recupera o MesDia da Data Entrada: Uma data(da base) Saída: MMDD
	 * 
	 * @author Sávio Luiz
	 * @date 22/04/2006
	 * 
	 * @param data
	 * @return Uma string no formato MMDD
	 */
	public static String recuperaMesDiaDaData(Date data) {
		String mes = "" + getMes(data);
		if (mes.length() == 1) {
			mes = "0" + mes;
		}
	...
	private static String numero(long n) {
		...
		String u[] = { "", "UM", "DOIS", "TRÊS", "QUATRO", "CINCO", "SEIS",
	...
	/**
	 * Verifica se o parametro informado é diferente de null
	...
	/**
	 * Adiciona zeros a esqueda do número truncando informado tamamho máximo campo 6
	 * Número 16 retorna 000016
	 * 
	 * @param tamanhoMaximoCampo
	 *            Descrição do parâmetro
	 ...
	 * Data: 21/11/2008
	 * Adiciona nº de dias uteis para uma data
	 */
	...
	*/
	...
	/**
	 * Método que recebe o nome do arquivo e retorna a extensão do mesmo.
	...
	public static String obterExtensaoDoArquivo(String nomeArquivo){
		...
		if ( nomeArquivoPartido.length>1 && nomeArquivoPartido[1] != null ){
	...
	/**
	 * Retorna true se o combo(parametro campo) não é branco nem nulo
	 * e se não é igual a ConstantesSistema.NUMERO_NAO_INFORMADO.
	 *
	 *@since 06/10/2009
	 *@author Marlon Patrick
	 */
	 ...
	 * @param encode código do encode para conversão
	 ...
	 * 
	 * @param str String
	 * @param tm Tamanho máximo da string
	 * @return
	 */
	...
	/**
	 * Recebe uma string no formato ddmmaaaa e validar o dia, mês e ano, incluindo validação para
	...
	/**
	 * Verifica se duas strings são iguais, considerando também quando tiverem valor nulo.
	 * 
	...
	*/
	 ...
	 * válida e está no formato correto
	 * 
	 * @author Raimundo Martins
	 * @date 25/11/2011
	 * */
	...
	/**
	 * Retorna um número aleatório a partir do intervalo de valores passado como parâmetro.  
	 * 
	...
	/**
	 * Obter a extensao do arquivo de um FormFile
	 * 
	 * @author Davi Menezes
	 * @date 14/06/2012
	 */
	public static String retornarExtensaoArquivo(FormFile formFile) {
    ...
    /**
     * Metodo responsavel por formatar ate 12 casas decimais. Utilizado para formatar as latitudes e longitudes.
     * @param valor
     * @return
     */
	...
	/**
	 * Retorna o DDD do telefone, caso seja passado no formado '(99) 9999-9999'
	 * 
	 * @param telefone
	 * @return ddd
	 */
	public static int obterDDDTelefone(String telefone) {

		int ddd = new Integer(telefone.substring(1, 3)).intValue();

		return ddd;
	}

	/**
	 * Retorna o número do telefone, caso seja passado no formado '(99) 9999-9999'
	 * 
	 * @param telefone
	 * @return ddd
	 */
	public static int obterNumeroTelefone(String telefone) {

		int numero = new Integer(
					telefone.substring(4, 8)
						.concat(telefone.substring(9, 13))
				).intValue();
	...
	/**
	 * Método que recebe o nome do arquivo e retorna a extensão do mesmo, buscando pelo ultimo ponto.
	 * @author Fernanda Almeida
	 * @date 15/12/2012
	 * @param String
	 * @return String
	 */
	public static String obterExtensaoDoArquivoUltimoPonto(String nomeArquivo){
	...
	/**
	 * Verifica se uma String é um BigDecimal
	 * 
	...
	/**
	 * Verifica se uma String é um Long
	 * 
	 * @author Carlos Chaves
	 * @date 12/12/2012
	 * */
	 ...
	 * @return
	 */
	public static String montarString(Collection colecao) {
		String retorno = "";
	...
	/**
	 * Retira a acentuacao de uma string
	 * 
	 * @author Carlos Chaves
	 * @date 21/02/2013
	 * */
	public static String retirarAcentuacao(String frase){
	...
	/**
	 * Validar um número de processo judicial de formato
	 * NNNNNNN-DD.AAAA.JTR.OOOO
	 * 
	 * @author Hugo Azevedo
	 * @date 26/03/2013
	 * */
	public static boolean validarNumeroProcessoJudicial(long NNNNNNN, long DD, long AAAA, long JTR, long OOOO) { 
		String valor1 = ""; 
		long resto1 = 0; 
		String valor2 = ""; 
		long resto2 = 0; 
	...
	public static BigDecimal formatarMoedaRealparaBigDecimalComErro(String valor) {
	...
	public static boolean validarMesReferenciaComBarra(String mesAnoReferencia) {
		boolean anoMesInvalido = true;
	...
	/**
	 * 
	 * Método que recebe duas coleções e verifica se são iguais.
	 * Seus objetos são comparados através do método equals, sendo necessário
	 * sua sobreescrita em caso de objetos de tipo não-nativo.
	 * 
	 **/
	public static boolean isEqualCollection(Collection col1, Collection col2){
	 ...
	 * @return Descrição do retorno
	 */
	public static String[] converterColecaoParaArray(Collection<String> colecao) {
		...
		if (colecao != null && !colecao.isEmpty()) {
			...
			while (iterator.hasNext()) {
				retorno[i] = (String) iterator.next();
				i++;
			}
	...
	/**
	 * Este metodo verifia se o CEP passado pertence a um determinado Municipio  
	 * @autor Flavio Ferreira
	 * @date 11/12/2013
	 **/
	public static boolean validarCepParaMunicipio(String municipio, String cep){
		
		boolean retorno = false;
		String cepSemFormatacao = retirarFormatacaoCEP(cep);
		FiltroMunicipio filtroMunicipio = new FiltroMunicipio();
		filtroMunicipio.adicionarParametro(new ParametroSimples(filtroMunicipio.ID, municipio));
		
		Collection colecaoPesquisa = Fachada.getInstancia().pesquisar(filtroMunicipio, Municipio.class.getName());
		Municipio objetoMunicipio = (Municipio) retonarObjetoDeColecao(colecaoPesquisa);
		
		if(Integer.parseInt(cepSemFormatacao) >= objetoMunicipio.getCepInicio() && Integer.parseInt(cepSemFormatacao) <= objetoMunicipio.getCepFim()){
			retorno = true;
		}
	 ...
	 * @return retorna inscricao estadual PE formatada
	 */
	public static String formatarInscricaoEstadualPE(String inscricaoEstadual){
		String inscricaoFormatada = null;
		// VERIFICA SE EH DIFERENTE DE NULL
		if(inscricaoEstadual.length()==14){
			inscricaoFormatada = inscricaoEstadual.substring(0, 2)+".";
			inscricaoFormatada += inscricaoEstadual.substring(2, 3)+".";
			inscricaoFormatada += inscricaoEstadual.substring(3, 6)+".";
			inscricaoFormatada += inscricaoEstadual.substring(6, 13)+"-";
			inscricaoFormatada += inscricaoEstadual.substring(13, 14);
		}
		// RETORNO
		return inscricaoFormatada;
	}
	
	public static String formatarMoedaReal3Casas(BigDecimal valor) {
		/**
		 * Símbolos especificos do Real Brasileiro
		 */
		DecimalFormatSymbols REAL = new DecimalFormatSymbols(new Locale("pt",
		 ...
		 */
		if (valor != null && !"".equals(valor)) {
			DecimalFormat DINHEIRO_REAL = new DecimalFormat("###,###,##0.000",
					REAL);
			return DINHEIRO_REAL.format(valor);
		} else {
			return "";
	...
	public static void preencherParametrosQuery(HashMap<String, Object> parametros,
			Query query) {
		...
		while (iterMap.hasNext()) {
			...
			if (parametros.get(key) instanceof Set) {
				Set setList = (HashSet) parametros.get(key);
				query.setParameterList(key, setList);
			} else if (parametros.get(key) instanceof Collection) {
				Collection collection = (ArrayList) parametros.get(key);
	...
	/**
	 * Método que retorna o IP original da máquina que fez a requisição ao servidor
	 * 
	 * Para que esse método funcione o proxy por onde a informação trafega deve suportar 
	 * X-Forwarded-For, para maiores informações, see: http://en.wikipedia.org/wiki/X-Forwarded-For
	 * 
	 * @author Bruno Barros
	 * @since 16/07/2014
	 * @param httpServletRequest Requisição utilizada para acesso a máquina  
	 * @return String com o ip da máquina
	 */
	public static String getLocalMachineIP( HttpServletRequest httpServletRequest ){	
		
		// Tentamos pegar o IP diretamente do Header
		String ip = httpServletRequest.getHeader("X-Real-IP");
		
		if ( null == ip || "".equals(ip.trim()) || "unknown".equalsIgnoreCase(ip) ) {
			
			// Caso não seja possível, pegamos o IP que veio no Header da requisição via "X-Forwarded-For"
			ip = httpServletRequest.getHeader("X-Forwarded-For");
			
			// Para cada proxy ou nó passado pela requisição, um novo IP é adicionado com "," sendo a primeira sempre o IP inicial 
			if ( null != ip && !"".equals(ip.trim()) && !"unknown".equalsIgnoreCase(ip)) {
