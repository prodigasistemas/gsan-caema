...
public class RepositorioUtilHBM implements IRepositorioUtil {
		...
		try {
			...
			if (ids.size() > 999) {
				...
				for (int i = 0; i < indice; i++) {
					...
					// pesquisa a coleção de atividades e atribui a variável "retorno"
					retornoConsulta = new ArrayList(new CopyOnWriteArraySet(session.createQuery(
	...
	/**
	 * Método resposável por verificar se uma tabela no banco de dados Oracle existe.
	 * 
	 ...
	 * @throws ErroRepositorioException
	 */
	public Boolean verificarTabelaBancoOracleExistente(String nomeTabela) throws ErroRepositorioException {
