...
/**
	 ...
	 * [SB0001] - Obter Situação da Conta
	 * @author Hugo Azevedo
	 * @date 18/01/2012
	 */
	public boolean verificarRevisaoContratoParcelamento(Integer id) throws ErroRepositorioException{
		...
		try{
			...
			sql = " select count(*) as contador " +
				  ...
				  " inner join faturamento.conta_motivo_revisao cmr on cmr.cmrv_id = conta.cmrv_id " +
				  " where conta.cnta_id = :idConta " +
				  " and cmr.cmrv_id = :idMotivoRevisao ";
			
				  
			
			Integer qtdContas = (Integer)session.createSQLQuery(sql)
								.addScalar("contador",Hibernate.INTEGER)
								.setInteger("idConta",id)
								.setInteger("idMotivoRevisao",ContaMotivoRevisao.CONTA_EM_CONTRATO_PARCELAMENTO)
								.uniqueResult();
			
			sql = 	  " select count(*) as contador " +
					  " from faturamento.conta_historico contaH " +
					  " inner join faturamento.conta_motivo_revisao cmr on cmr.cmrv_id = contaH.cmrv_id " +
					  " where contaH.cnta_id = :idConta " +
					  " and cmr.cmrv_id = :idMotivoRevisao ";
			
			Integer qtdContasHistorico = (Integer)session.createSQLQuery(sql)
										.addScalar("contador",Hibernate.INTEGER)
										.setInteger("idConta",id)
										.setInteger("idMotivoRevisao",ContaMotivoRevisao.CONTA_EM_CONTRATO_PARCELAMENTO)
										.uniqueResult();
			...
			Integer qtdContasTotal = Util.somaInteiros(qtdContas, qtdContasHistorico);
			
			if(qtdContasTotal.compareTo(new Integer(0)) > 0){
				retorno = true;
			}
	 ...
	 * [SB0005] - Consultar Dados Contrato Parcelamento por Cliente Responsável
	 * 
	 * @author Hugo Azevedo, Mariana Victor
	 * @date 19/01/2012, 16/02/2012
	 */
	public Collection pesquisarContratosParcelamentoCliente(Integer id, boolean parcelasVencidas) throws ErroRepositorioException{
		...
		try{
			...
			sql = " select " +
				  " distinct cp.cpar_nncontrato as numeroContrato, " +
				  " cpp.cppr_nnprestacao as numeroPrestacao, " +
				  " cp.cpar_nnprestacoes as totalPrestacoes, " +
				  " cpp.cppr_vlprestacao as valorPrestacao, " +
				  " cpp.cppr_dtvencimento as dataVencimento, " +
				  "	cli.clie_id              AS idCliente, "+ //5
				  "	cli.clie_nmcliente      AS nome "+ //6
				  " from cobranca.contrato_parcel cp " +
				  " inner join cobranca.contrato_parcel_cliente cpc on cpc.cpar_id = cp.cpar_id " +
				  " inner join cobranca.contrato_parcel_prest cpp on cpp.cpar_id = cp.cpar_id " +
				  "	inner join cadastro.cliente cli on cli.clie_id = cpc.clie_id "+
				  " where cpc.clie_id = :idCliente " +
				  " and cpc.cpcl_idsuperior is null " + 
				  " and cp.pcst_id = :situacaoContrato " +
				  " and cpp.cppr_dtpagamento is null ";
			
			if (parcelasVencidas) {
				  sql = sql +
					  " and cpp.cppr_dtvencimento < :dataAtual ";
			} else {
				  sql = sql +
					  " and cpp.cppr_dtvencimento >= :dataAtual ";
			}
			...
			sql = sql + 
				" order by cli.clie_nmcliente, cp.cpar_nncontrato, cpp.cppr_nnprestacao ASC";
			
			retorno = session.createSQLQuery(sql)
					  .addScalar("numeroContrato", Hibernate.STRING)
					  .addScalar("numeroPrestacao", Hibernate.INTEGER)
					  .addScalar("totalPrestacoes", Hibernate.INTEGER)
					  .addScalar("valorPrestacao", Hibernate.BIG_DECIMAL)
					  .addScalar("dataVencimento", Hibernate.DATE)
					  .addScalar("idCliente", Hibernate.INTEGER)
					  .addScalar("nome", Hibernate.STRING)
					  .setInteger("idCliente", id)
					  .setInteger("situacaoContrato",ParcelamentoSituacao.NORMAL)
	 ...
	 * @date 19/01/2012
	 */
	public Collection pesquisarContratosParcelamentoTodosOsClientes(Integer id) throws ErroRepositorioException{
		...
		try{
			...
			sql = "SELECT cp.cpar_nncontrato AS numeroContrato, "+ //0
			    "	  cpp.cppr_nnprestacao    AS numeroPrestacao, "+ //1
				"	  cp.cpar_nnprestacoes    AS totalPrestacoes, "+ //2
				"	  cpp.cppr_vlprestacao    AS valorPrestacao, "+ //3
				"	  cpp.cppr_dtvencimento   AS dataVencimento, "+ //4
				"	  cli.clie_id              AS idCliente, "+ //5
				"	  cli.clie_nmcliente      AS nome "+ //6
				"	FROM cobranca.contrato_parcel cp "+
				"	INNER JOIN cobranca.contrato_parcel_cliente cpc "+
				"	ON cpc.cpar_id = cp.cpar_id "+
				"	INNER JOIN cobranca.contrato_parcel_prest cpp "+
				"	ON cpp.cpar_id = cp.cpar_id "+
				"	INNER JOIN cadastro.cliente cli "+
				"	ON cli.clie_id in  "+
				"	( "+
				"	  select clie_id from cobranca.contrato_parcel_cliente cpc2 where cpc2.cpar_id = cpc.cpar_id "+
				"	) "+
				"	WHERE cpc.clie_id         = :idCliente "+
				"	AND cpp.cppr_dtpagamento IS NULL "+
				"	AND cp.pcst_id            = :situacaoContrato "+
				"	ORDER BY cli.clie_id,cp.cpar_nncontrato, "+
				"	cpp.cppr_nnprestacao ASC ";
			
			retorno = session.createSQLQuery(sql)
					  .addScalar("numeroContrato", Hibernate.STRING) //0
					  .addScalar("numeroPrestacao", Hibernate.INTEGER) //1
					  .addScalar("totalPrestacoes", Hibernate.INTEGER) //2
					  .addScalar("valorPrestacao", Hibernate.BIG_DECIMAL) //3
					  .addScalar("dataVencimento", Hibernate.DATE) //4
					  .addScalar("idCliente",Hibernate.INTEGER)//5
					  .addScalar("nome",Hibernate.STRING)//6
					  .setInteger("idCliente", id)
					  .setInteger("situacaoContrato",ParcelamentoSituacao.NORMAL)
					  .list();
	 ...
	 * [UC1139] Atualizar Contrato de Parcelamento por Cliente
	 * 
	 * Atualiza o número e a data do contrato
	 * 
	 * @author Mariana Victor
	 * @data 27/03/2012
	 * 
	 ...
	 * @return void
	 */
	public void atualizarContratoParcelamentoPorCliente(Integer idContrato, String numero, Date dataContrato)
			throws ErroRepositorioException {
		...
		try {
			consulta = "update gcom.cobranca.contratoparcelamento.ContratoParcelamento "
				+ " set cpar_nncontrato = :numero, cpar_dtcontrato = :dataContrato, cpar_tmultimaalteracao = :ultimaAlteracao "
				+ " where cpar_id = :idContrato ";
			...
			session.createQuery(consulta)
					...
					.setDate("dataContrato", dataContrato)
					.executeUpdate();
		} catch (HibernateException e) {
			throw new ErroRepositorioException(e, "Erro no Hibernate");
		} finally {
