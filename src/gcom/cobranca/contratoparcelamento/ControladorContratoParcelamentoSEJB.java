...
//				 ------------ REGISTRAR TRANSAÇÃO ----------------
	...
	public List<RelatorioManterContratoParcelamentoPorClienteBean> gerarRelatorioManterContratoParcelamentoPorCliente(FiltroContratoParcelamentoCliente filtroContrato) throws ControladorException
	{
		...
		for (ContratoParcelamentoCliente contratoCliente : contratosPesquisa) {
			...
			beanContrato.setNumeroContrato(contratoCliente.getContrato().getNumero().toUpperCase());
			if(contratoCliente.getContrato().getContratoAnterior() != null){
				beanContrato.setNumeroContratoAnterior(contratoCliente.getContrato().getContratoAnterior().getNumero().toUpperCase());
    ...
    public Integer inserirContratoParcelamentoPorCliente(ContratoParcelamento contrato, Usuario usuarioLogado, 
        ...
        try{
            ...
            if(contrato.getNumero() == null || contrato.getNumero().trim().equals("")) {
            	contrato.setNumero(contrato.getId().toString());
            	this.getControladorUtil().atualizar(contrato);
            }
            ...
            for (DebitosClienteHelper debitosClienteHelper : listaDebitos) {
                ...
                if (debitosClienteHelper.getTipoDocumento().compareTo(DocumentoTipo.CONTA) == 0) {
	                ...
	                getControladorFaturamento().colocarRevisaoConta(colecaoContas, conta.getId()+"", motivoRevisao, usuarioLogado, true);
...
//			------------ REGISTRAR TRANSAÇÃO ----------------
	...
	public List<RelatorioEmitirContratoParcelamentoPorClienteBean> pesquisarDadosRelatorioContratoParcelamentoPorCliente(
		...
		try {
			...
			if (helper.getContratoParcelamento() != null
				...
				bean.setNumeroContrato(helper.getContratoParcelamento().getNumero().toUpperCase());
			}
	
			// 1.2.2.1. No. Contrato Anterior
			if (helper.getContratoParcelamento().getContratoAnterior() != null) {
				bean.setNumeroContratoAnt(helper.getContratoParcelamento().getContratoAnterior().getNumero().toUpperCase());
			}
			...
			if ((helper.getColecaoContaItens() != null 
				...
				while(iteratorConta.hasNext()) {
					...
					if (contratoParcelamentoItem.getIndicadorItemCancelado() != null
						...
						ContaHistorico conta = contaGeral.getContaHistorico();
						...
						// 1.2.1.6.	Valor Débitos 
						subBean.setValorDebitos(conta.getValorDebitos());
						...
						if (conta.getValorDebitos() != null) {
							valorConta = valorConta.add(conta.getValorDebitos());
						}
						if (conta.getValorCreditos() != null) {
							valorConta = valorConta.subtract(conta.getValorCreditos());
						}
			...
			if(helper.getColecaoItensDebitoACobrar() != null && !helper.getColecaoItensDebitoACobrar().isEmpty()){
				...
				while(iteratorDebitoACobrar.hasNext()){
					...
					if(debitoGeral.getDebitoACobrar().getValorDebito() != null){
						...
						String parcelasACobrar = (debitoGeral.getDebitoACobrarHistorico().getNumeroPrestacaoDebitoMenosBonus()-debitoGeral.getDebitoACobrarHistorico().getPrestacaoDebito()) + "";
						subBean.setParcelasACobrar(parcelasACobrar);
    ...
    public void atualizaContratoParcelamentoPorCliente(ContratoParcelamento contrato, Usuario usuarioLogado, 
        ...
        try{
            ...
            for (int i = 1; i < listaDeParcelas.size(); i++) {
                prestacaoContratoParcelamento = listaDeParcelas.get(i);Date dataVencimento = null;
                if (i == 1) {
	                int dia = contrato.getNumeroDiasEntreVencimentoParcel();
	                int mes = Util.getMes(dataVencPacel);
	                int ano = Util.getAno(dataVencPacel);
...
//                 ------------ REGISTRAR TRANSAÇÃO ----------------
	 ...
	 * @date 18/01/2012
	 */
	public boolean verificarRevisaoContratoParcelamento(Integer id) throws ControladorException {
		
		try {
			return repositorioContratoParcelamento.verificarRevisaoContratoParcelamento(id);
		} catch (ErroRepositorioException ex) {
			ex.printStackTrace();
			throw new ControladorException("erro.sistema", ex);
		}
	 ...
	 * @date 19/01/2012
	 */
	public Collection<ContratoParcelamentoClieRespHelper> pesquisarContratosParcelamentoCliente(
			Integer idCliente)  throws ControladorException {
		
			Collection<ContratoParcelamentoClieRespHelper> retorno = new ArrayList<ContratoParcelamentoClieRespHelper>();
			Collection retornoQuery = null;
			
			try{
				
				retornoQuery =  repositorioContratoParcelamento.pesquisarContratosParcelamentoCliente(idCliente, true);
			...
			if(retornoQuery != null && !retornoQuery.isEmpty()){
				...
				while(it.hasNext()){
					...
					ContratoParcelamentoClieRespHelper helper = new ContratoParcelamentoClieRespHelper();
					
					helper.setContrato((String)obj[0]);
					helper.setNumeroParcela(((Integer)obj[1]).toString());
					helper.setTotalParcela(((Integer)obj[2]).toString());
					helper.setValor((BigDecimal)obj[3]);
					helper.setDataVencimento(Util.formatarData((Date)obj[4]));
					
					helper.setIdCliente((Integer)obj[5]);
					helper.setNomeCliente((String)obj[6]);
	...
	/**
	 * [UC0203] Consultar Débitos
	 * [SB0006] - Consultar Dados Contrato Parcelamento por Cliente Responsável (Parcelas a Vencer)
	 * 
	 * @author Mariana Victor
	 * @date 17/02/2012
	 */
	public Collection<ContratoParcelamentoClieRespHelper> pesquisarContratosParcelamentoClienteParcelasAVencer(
			Integer idCliente)  throws ControladorException {
			...
			try{
				...
				retornoQuery =  repositorioContratoParcelamento.pesquisarContratosParcelamentoCliente(idCliente, false);
	...
	public void atualizarContratoParcelamentoPorCliente(Integer idContrato, String numero, Date dataContrato) 
		...
		try {
			repositorioContratoParcelamento.atualizarContratoParcelamentoPorCliente(idContrato, numero, dataContrato);
		} catch (ErroRepositorioException ex) {
			ex.printStackTrace();
			throw new ControladorException("erro.sistema", ex);
		}
