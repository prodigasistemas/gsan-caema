...
//				arquivo.append( primeiroRegistro + "\n" );
			...
			if( retorno != null && ( retorno.getRelatorioConsistenciaProcessamento() != null || retorno.getMensagemComunicacaoServidorCelular() !=null ) ){
				...
				//out.flush();
				 ...
				 String mensagem = retorno.getMensagemComunicacaoServidorCelular();
    ...
    private void receberFoto(DataInputStream din, HttpServletResponse response, OutputStream out) throws IOException {
    	try{
	    	...
	    	//Lemos o tipo de Foto
	    	int tipoFoto = din.readInt();
	    	
	    	//Lemos o tipo de medição 
	    	int tipoMedicao = din.readInt();
	    	
	    	//Lemos o código da anormalidade de consumo
	    	int idConsumoAnormalidade = din.readInt();
			 ...
			 * Data: 10/09/2013
			 * [UC1359] Requisicao Fotos Anormalidade
			 * Observacao: Caso o tipoFoto = FotoSituacaoOrdemServico.NOTIFICACAO_CONEXAO_ESGOTO
			 */
			if(tipoFoto==FotoSituacaoOrdemServico.NOTIFICACAO_CONEXAO_ESGOTO){
				getFachada().inserirFotoConexaoEsgoto(numeroImovel, bytesFoto, tipoFoto);
			}else{
				getFachada().inserirFotoMovimentoRoteiroEmpresa(numeroImovel, idLeituraAnormalidade, anoMesReferencia,
					tipoFoto, tipoMedicao, idConsumoAnormalidade, bytesFoto);
			}
