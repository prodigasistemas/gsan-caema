...
public class ProcessarRequisicaoDispositivoMovelAcompanhamentoServicoAction extends GcomAction {
	...
	private static final int ATUALIZAR_INDICADOR_TRANSMISSAO_EQUIPE = 13;
	private static final int ATUALIZAR_INDICADOR_TRANSMISSAO_OS = 14;
	private static final int ENCERRAR_OS_PREFEITURA = 15;
	private static final int BAIXAR_APK_PREFEITURA = 16;
	private static final int VERIFICAR_VERSAO_PREFEITURA = 17;
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		try {
			...
			case BAIXAR_MENSAGEM:
				...
				break;				
			case RECEBER_FOTO:
				this.receberFoto(imei, din, response, out);
				break;
			case MUDAR_SITUACAO_ORDEM_SERVICO:
				atualizarSituacaoProgramacaoOrdemServico(din, response, out,imei);
				break;
			case ENCERRAR_OS:
				encerrarOS(din, response, out,imei, false);
				break;
			...
			case BAIXAR_APK:
				out = response.getOutputStream();
				this.baixarNovaVersaoAcompanhamento(din, response, out, SistemaAndroid.GSANAS);
				break;	
			case BAIXAR_APK_PREFEITURA:
				out = response.getOutputStream();
				this.baixarNovaVersaoAcompanhamento(din, response, out, SistemaAndroid.GSANAS_PREFEITURA);
				break;	
			case VERIFICAR_VERSAO:
				this.verificarVersao(din,response,out,SistemaAndroid.GSANAS);
				break;
			case VERIFICAR_VERSAO_PREFEITURA:
				this.verificarVersao(din,response,out,SistemaAndroid.GSANAS_PREFEITURA);
				break;
			case
			ATUALIZAR_INDICADOR_TRANSMISSAO_EQUIPE:
				this.atualizarIndicadorTransmissaoEquipe(imei,response,out);
				break;
			case ATUALIZAR_INDICADOR_TRANSMISSAO_OS:
				this.atualizarIndicadorTransmissaoOs(imei, din, response,out);
				break;
			case ENCERRAR_OS_PREFEITURA:
				this.encerrarOS(din, response, out, imei, true);
				break;
			default:
				break;
			}
	...
	private void verificarVersao(DataInputStream din, HttpServletResponse response, OutputStream out, int sistema ) throws IOException{
		Fachada fachada = Fachada.getInstancia();
        
        try{
        	Object[] obj = fachada.baixarNovaVersaoApk( sistema );
	...
	private void baixarNovaVersaoAcompanhamento(DataInputStream din, HttpServletResponse response, OutputStream out, int sistema) throws IOException{
		Fachada fachada = Fachada.getInstancia();
	...
	public void baixarArquivo( long imei, HttpServletResponse response, OutputStream out ) throws IOException{		
		...
		try{
			// Libera os arquivos antes de baixar
			// Caso haja arquivo em execução, é enviado para pendente
			fachada.liberarArqTextoAcompServico(imei);
	 ...
	 * @throws IOException
	 */
	public void receberFoto( long imei, DataInputStream data, HttpServletResponse response, OutputStream out ) throws IOException{		
		...
		try{
			...
			Double coordenadaX = null;
			Double coordenadaY = null;
			...
			try{
				coordenadaX = data.readDouble();
				String latitude = coordenadaX.toString();
				if(latitude.length() > 24){
					coordenadaX = null;
				}
				
				coordenadaY = data.readDouble();				
				String longitude = coordenadaY.toString();
				if(longitude.length() > 24){
					coordenadaY = null;
				}
			}catch(Exception ex ){
				System.out.println("");
			...
			// inserimos na base
			Fachada.getInstancia().inserirFotoOrdemServico( numeroOS, tipoFoto, bytesFoto,coordenadaX,coordenadaY );		
			...
			System.out.println("Erro ao receber foto");
			response.setContentLength( 1 );
	...
	public void atualizarSituacaoProgramacaoOrdemServico( 
			...
			HttpServletResponse response, 
			OutputStream out, long imei ) throws IOException{		
		try{		
			// Lemos o número da OS para qual essa foto pertence
			int numeroOS = data.readInt();
			...
			Fachada.getInstancia().atualizarSituacaoProgramacaoOrdemServico( numeroOS , situacao,imei);
			
			System.out.println("Atualizando situação" + numeroOS);
	 ...
	 * @throws IOException
	 */
	public void encerrarOS( DataInputStream data, HttpServletResponse response, OutputStream out,long imei, boolean prefeitura) throws IOException{		
		try{
			...
			// Faz substituição de hidrômetro	
			Object[] dadosRetorno = Fachada.getInstancia().substituirHidrometro(buffer,imei,prefeitura);
			...
			if(dadosRetorno != null){
				...
				Fachada.getInstancia().retornoAtualizarOrdemServicoAcompanhamentoServicoSubstituicao( dataProgramacao,idEquipe, prefeitura );
			}	else{
				System.out.println("Ocorreu um problema na procura pela OSProgramacao.");
			}
	 ...
	 * 
	 * Atualiza o indicador de transmissão de todo o arquivo para SIM, apenas ao receber o retorno afirmativo do celular
	 * 
	 ...
	 * @param imei
	 */
	private void atualizarIndicadorTransmissaoEquipe(long imei,	HttpServletResponse response, OutputStream out) throws IOException{		
		
		try{
			
			fachada.atualizarIndicadorTransmissaoEquipeAcompanhamentoServico( imei );
	 ...
	 * [UC1199] Acompanhamento de Arquivos de Roteiro
	 * 
	 * Atualiza o indicador de transmissão da OS para SIM, apenas ao receber o retorno afirmativo do celular
	 * 
	 * @author Fernanda Almeida
	 * @date 26/10/12
	 * 
	 ...
	 * @param out 
	 */
	private void atualizarIndicadorTransmissaoOs(long imei,	DataInputStream data, HttpServletResponse response, OutputStream out) throws IOException{		
		
		try{
			// Lemos o número da OS para qual essa atualização pertence
			int numeroOS = data.readInt();
			
			fachada.atualizarIndicadorTransmissaoOsAcompanhamentoServico( imei, numeroOS );
