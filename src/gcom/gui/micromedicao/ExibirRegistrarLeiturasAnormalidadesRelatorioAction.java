...
public class ExibirRegistrarLeiturasAnormalidadesRelatorioAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if(form.getIndicadorAnormalidadeImpactaLeitura() == null){
				form.setIndicadorAnormalidadeImpactaLeitura("2");
		}
...
/*	private void pesquisarLeiturista(String leituristaId, 
	...
	private void pesquisarLocalidade(String inscricaoTipo,
		...
		if (inscricaoTipo.equalsIgnoreCase("origem")) {
			...
			httpServletRequest.getSession(false).removeAttribute("bloquearSetorComercial");
			...
			//Bloquear Setores comerciais, caso seja diferente da Localidade Inicial
			if(!form.getLocalidadeDestinoID().equals(form.getLocalidadeOrigemID())){
				...
				httpServletRequest.getSession(false).setAttribute("bloquearSetorComercial", "sim");
			}
			else{
				httpServletRequest.getSession(false).removeAttribute("bloquearSetorComercial");
			}
	...
	private void pesquisarSetorComercial(String inscricaoTipo,
		...
		if (inscricaoTipo.equalsIgnoreCase("origem")) {
			...
			httpServletRequest.getSession(false).removeAttribute("bloquearRota");
			...
			//Bloquear Rota, caso o SC inicial seja diferente do Final
			if(!form.getSetorComercialOrigemID().equals(form.getSetorComercialDestinoID())){
				httpServletRequest.getSession(false).setAttribute("bloquearRota","sim");
				form.setRotaInicial("");
				form.setRotaFinal("");
			}
			else{
