...
public class InserirAnormalidadeLeituraAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		String indicadorAnormalidadeImpactaLeitura = form.getIndicadorAnormalidadeImpactaLeitura();
		...
		if(Util.formatarMoedaRealparaBigDecimal(fatorComLeitura).compareTo(new BigDecimal("0")) <= 0){
		...
		if(Util.formatarMoedaRealparaBigDecimal(fatorSemLeitura).compareTo(new BigDecimal("0")) <= 0){
			throw new ActionServletException(	"atencao.invalido_zero",
					null,
		 ...
		 if (form.getTipoSolicitacao() != null && !form.getTipoSolicitacao().equals("-1")){
	        ...
	        if (form.getTipoSolicitacao() != null && !form.getTipoSolicitacao().equals("-1")){
	            ...
	            if (idTipoSolicitacao.equals(ConstantesSistema.NUMERO_NAO_INFORMADO)) {
	            	...
	            	anormalidadeLeituraInserir.setSolicitacaoTipoEspecificacao(null);
	            } else {

	            	solicitacaoTipoEspecificacao.setId(Integer.valueOf(form.getEspecificacao()));
	            	anormalidadeLeituraInserir.setSolicitacaoTipoEspecificacao(solicitacaoTipoEspecificacao);
	            }
	        }
		...
		// Anormalidade impacta na leitura/faturamento é obrigatório.
		if (indicadorAnormalidadeImpactaLeitura == null || indicadorAnormalidadeImpactaLeitura.equalsIgnoreCase("")) {
			throw new ActionServletException(	"atencao.required",
												null,
												"Anormalidade impacta na leitura/faturamento");
		}
		...
		anormalidadeLeituraInserir.setIndicadorEmissaoOrdemServico(ConstantesSistema.NAO);
		...
		anormalidadeLeituraInserir.setIndicadorAnormalidadeImpactaLeitura(new Short(indicadorAnormalidadeImpactaLeitura));
		anormalidadeLeituraInserir.setIndicadorFotoObrigatoria(new Short(indicadorFotoObrigatoria));
		
		
		if (tipoServico != null && !tipoServico.equals("")&& !form.getTipoServico()
		...
		if(idEmpresa != null && !idEmpresa.equals("")){
			...
			anormalidadeLeituraInserir.setEmpresa(empresa);
		}else{
			anormalidadeLeituraInserir.setEmpresa(null);
		}
