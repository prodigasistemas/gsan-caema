...
public class ProcessarRequisicaoDispositivoMovelFiscalizacaoAnormalidadeAction extends GcomAction {
	...
	private void atualizarMovimentacao(DataInputStream din, HttpServletResponse response, OutputStream out) throws IOException {
		...
		try{
			...
			e.printStackTrace();
			System.out.println("ERROR: Ao atualizar o arquivo de fiscalizacao de anormalidade");
			response.setContentLength(1);
			out.write(RESPOSTA_ERRO);
			out.flush();
		}
	...
	private void baixarArquivo(long imei, HttpServletResponse response, OutputStream out) throws IOException  {
		try{
			...
			if(retorno != null && retorno.length > 0){
				...
				System.out.println("INICIO: Baixando arquivo texto fiscalizacacao ID: "+arquivoTextoVisitaCampo.getId());
				...
				System.out.println("FIM: Baixando arquivo texto fiscalizacacao ID: "+arquivoTextoVisitaCampo.getId());
			}
			
			else{
				System.out.println("ERROR: Nao existe arquivo de fiscalizacacao de anormalidade.");
				response.setContentLength(1);
	...
	private void receberFoto(DataInputStream din, HttpServletResponse response, OutputStream out) throws IOException {
		try {
			...
			//inserimos na base
			System.out.println("INICIO: Recebendo foto fiscalizacao anormalidade OS:"+numeroOS+" TIPO FOTO:"+tipoFoto);
			
			getFachada().inserirFotoOrdemServico(numeroOS, tipoFoto, bytesFoto,null,null);
			
			System.out.println("FIM: Recebendo foto fiscalizacao anormalidade OS:"+numeroOS+" TIPO FOTO:"+tipoFoto);
