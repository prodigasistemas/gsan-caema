...
public class ExibirAtualizarCriterioCobrancaAction extends GcomAction {
		...
		if (reload == null || reload.equals("")) {
			...
			if (sessao.getAttribute("colecaoCobrancaCriterioLinha") == null
				...
				if (collectionCobrancaCriterio != null
			       ...
			       Integer idRDSelecionada = null;
			        //Seta a resolucao diretoria
			        if(cobrancaCriterio.getResolucaoDiretoria() != null){
			        	
			        	idRDSelecionada = cobrancaCriterio.getResolucaoDiretoria().getId();
			        	criterioCobrancaActionForm.setIdResolucaoDiretoria(idRDSelecionada.toString());
			        }
			        
			        Collection<ResolucaoDiretoria> colecaoResolucaoDiretoria = this.getResolucaoDiretoria(idRDSelecionada);
	 ...
	 * @return Collection
	 * */
	private Collection<ResolucaoDiretoria> getResolucaoDiretoria(Integer idRDSelecionada){
		Collection<ResolucaoDiretoria> colecaoResolucaoDiretoria = Fachada.getInstancia().
				pesquisarResolucaoDiretoriaComIndicadorValidoAcaoCobranca(idRDSelecionada);
				//new ArrayList();
