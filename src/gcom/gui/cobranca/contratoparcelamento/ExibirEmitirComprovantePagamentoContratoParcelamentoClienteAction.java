	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if(idContrato != null && !idContrato.equals("")){
			...
			//Rotina para buscar o cliente superior se a ocorrência não for única
				...
				Iterator<ContratoParcelamentoCliente> it = collectionContratoParcelamentoCliente.iterator();
				while(it.hasNext()){
			...
			if(collectionPrestacaoContratoParcelamento.size() != 0){
				...
				//2.1.1.1.	Caso o número do contrato seja provisório 
				if(contratoParcelamentoCliente.getContrato().getNumero().compareTo(contratoParcelamentoCliente.getContrato().getId().toString()) == 0){
					emitirComprovantePagamento.setIdNumeroContrato("SEM NÚMERO");
				} else {
					emitirComprovantePagamento.setIdNumeroContrato(contratoParcelamentoCliente.getContrato().getNumero());
				}
				...
				Iterator<PrestacaoContratoParcelamento> it = collectionPrestacaoContratoParcelamento.iterator();
				int marcador = 0;
				...
				sessao.setAttribute("quantidadeParcelas",collectionPrestacaoContratoParcelamento.size());
					...
					throw new ActionServletException("atencao.contrato.pagamento.inexistente");
				}
			}
			else{
				throw new ActionServletException("atencao.numero.contrato.nao.existe");
