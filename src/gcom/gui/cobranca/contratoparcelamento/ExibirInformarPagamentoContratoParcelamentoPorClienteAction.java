		...
		if (idCliente != null && !idCliente.trim().equals("")) {
			...
			if (colecaoCliente != null && !colecaoCliente.isEmpty()) {
				...
				while (iterator.hasNext()) {
					...
					String idContrato = ((Integer) dadosContrato[0]).toString();
					String numeroContrato = (dadosContrato[1]).toString();
					helper.setIdContrato(idContrato);
					if (numeroContrato.compareTo(idContrato) == 0) {
						helper.setNumeroContrato("SEM NÚMERO");
					} else {
						helper.setNumeroContrato(numeroContrato);
					}
