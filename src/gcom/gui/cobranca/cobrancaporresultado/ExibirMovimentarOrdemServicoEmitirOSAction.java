...
public class ExibirMovimentarOrdemServicoEmitirOSAction extends GcomAction {
    ...
    public ActionForward execute(ActionMapping actionMapping,
		...
		if (httpServletRequest.getParameter("limparTotalizacao") != null
			...
			this.getSessao(httpServletRequest).removeAttribute("gerarArquivoTexto");
		}
			...
			if (movimentarOrdemServicoEmitirOSHelper != null) {
				...
				if (agruparPorImovel) {
					...
					if (colecaoDados != null && !colecaoDados.isEmpty()) {
						...
						Collection<Integer> colecaoImoveis = new ArrayList();
						...
						if (colecaoEmpresaCobrancaFaixa != null && !colecaoEmpresaCobrancaFaixa.isEmpty()) {
								...
								while (iteratorColecaoDados.hasNext()) {
									...
									if (dados[0] != null){
										...
										if (qnt >= numeroMinimoContas
											...
											if(dados[3] != null){
												colecaoImoveis.add((Integer) dados[3]);
											}
										}
									}
							...
							if (!colecaoQtdeContas.isEmpty()
								...
								this.getSessao(httpServletRequest).setAttribute("colecaoImoveis", colecaoImoveis);
								this.getSessao(httpServletRequest).setAttribute("gerarArquivoTexto", true);
		...
		if(Util.isVazioOrNulo((Collection<?>) this.getSessao(httpServletRequest).getAttribute("colecaoUnidadeNegocio"))){
			...
			Collection<?> colUnidadeNegocio = this.getFachada().pesquisar(filtroUnidadeNegocio, UnidadeNegocio.class.getName());
			if(!Util.isVazioOrNulo(colUnidadeNegocio)){
				httpServletRequest.setAttribute("colecaoUnidadeNegocio", colUnidadeNegocio);
			}
		}
