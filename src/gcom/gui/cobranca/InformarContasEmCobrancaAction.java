...
import gcom.cobranca.ComandoEmpresaCobrancaContaAcaoCobranca;
import gcom.cobranca.ComandoEmpresaCobrancaContaAcaoCobrancaPK;
import gcom.cobranca.ComandoEmpresaCobrancaContaCobrancaSituacao;
import gcom.cobranca.ComandoEmpresaCobrancaContaCobrancaSituacaoPK;
import gcom.cobranca.ComandoEmpresaCobrancaContaFaixa;
import gcom.cobranca.ComandoEmpresaCobrancaContaFaixaPK;
...
public class InformarContasEmCobrancaAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		String[] idsAcoesCobrancaNaoGeracao = informarContasEmCobrancaActionForm.getIdsAcoesCobrancaNaoGeracao();
		String[] idsCobrancaSituacao = informarContasEmCobrancaActionForm.getIdsCobrancaSituacao();
		...
		comandoEmpresaCobrancaConta.setIndicadorInclusaoNaoGeracao(ConstantesSistema.NAO.intValue());
		...
		if (quantidadeContasInicial != null && !quantidadeContasInicial.trim().equals("")) {
			...
			if (quantidadeMenorFaixa != null 
				...
				throw new ActionServletException(
						"atencao.quantidade_inicial.inferior.quantidade_menor_faixa", quantidadeMenorFaixa.toString());
			}
			
			Integer quantidadeMaiorFaixa = Fachada.getInstancia()
					.pesquisarQuantidadeContasMaiorFaixa(new Integer(informarContasEmCobrancaActionForm.getIdEmpresa()));
			
			if (quantidadeMaiorFaixa != null 
					&& quantidadeMaiorFaixa.compareTo(qtdContasFinal) > 0) {

				throw new ActionServletException(
						"atencao.quantidade_final.inferior.quantidade_maior_faixa", quantidadeMaiorFaixa.toString());
			}
		...
		//Caso tenha sido informada a quantidade de imóveis para o comando, caso contrário nulo
		if(informarContasEmCobrancaActionForm.getQtdImoveisMaximaComando() != null 
				&& !informarContasEmCobrancaActionForm.getQtdImoveisMaximaComando().equals("")){
			comandoEmpresaCobrancaConta.setQtdImoveisComando(new Integer(informarContasEmCobrancaActionForm.getQtdImoveisMaximaComando()));
		}
		else{
			comandoEmpresaCobrancaConta.setQtdImoveisComando(null);
		}
		...
		//2.24. Caso a empresa tenha faixa de valor informada por faixa com proporcionalidade, 
		//      o sistema deverá incluir as faixas com o percentual da proporcionalidade
		if(informarContasEmCobrancaActionForm.getQtdImoveisMaximaComando() != null 
				&& informarContasEmCobrancaActionForm.getPercentualImoveisFaixa() != null){
			Collection colecao = (Collection)this.getSessao(httpServletRequest).getAttribute("colecaoEmprCobFai");
			Object[] arrayEmpCobFai = colecao.toArray();
			
			//[FS0019] - Verificar percentual informado
			validarPercentualImoveisFaixa(informarContasEmCobrancaActionForm.getPercentualImoveisFaixa(),
					informarContasEmCobrancaActionForm.getQtdImoveisMaximaComando());
			
			for(int i = 0; i < informarContasEmCobrancaActionForm.getPercentualImoveisFaixa().length; i++ ){
				EmpresaCobrancaFaixa ecf = (EmpresaCobrancaFaixa)arrayEmpCobFai[i];
				
				if(informarContasEmCobrancaActionForm.getPercentualImoveisFaixa()[i] != null && 
						!informarContasEmCobrancaActionForm.getPercentualImoveisFaixa()[i].equals("")){
				
					String percentual = (informarContasEmCobrancaActionForm.getPercentualImoveisFaixa()[i]).replace(",", ".");
					
					ComandoEmpresaCobrancaContaFaixaPK pk = new ComandoEmpresaCobrancaContaFaixaPK();
					pk.setComandoEmpresaCobrancaContaId(idComandoEmpresaCobrancaConta);
					pk.setEmpresaCobrancaFaixaId(ecf.getId());
					
					ComandoEmpresaCobrancaContaFaixa ceccf = new ComandoEmpresaCobrancaContaFaixa();
					ceccf.setComp_id(pk);
					ceccf.setPercentualInformadoFaixa(new BigDecimal(percentual));
					ceccf.setUltimaAlteracao(new Date());
					
					fachada.inserir(ceccf);
				
				}	
			}
		}
		
		//1.1. Caso o usuário tenha selecionado pelo menos uma ação de cobrança, a cada ação de cobrança selecionada
		if(informarContasEmCobrancaActionForm.getIdsAcoesCobrancaNaoGeracao() != null &&
				informarContasEmCobrancaActionForm.getIdsAcoesCobrancaNaoGeracao().length > 0){
			
			for(int i = 0; i < informarContasEmCobrancaActionForm.getIdsAcoesCobrancaNaoGeracao().length; i++ ){
				String idAcaoCobranca = (String)informarContasEmCobrancaActionForm.getIdsAcoesCobrancaNaoGeracao()[i];
				
				ComandoEmpresaCobrancaContaAcaoCobrancaPK pk = new ComandoEmpresaCobrancaContaAcaoCobrancaPK();
				pk.setAcaoCobrancaId(new Integer(idAcaoCobranca));
				pk.setComandoEmpresaCobrancaContaId(idComandoEmpresaCobrancaConta);
				
				ComandoEmpresaCobrancaContaAcaoCobranca ceac = new ComandoEmpresaCobrancaContaAcaoCobranca();
				ceac.setComp_id(pk);
				ceac.setUltimaAlteracao(new Date());
				
				fachada.inserir(ceac);
			}
		}
		
		//TODO ????? 1.1. Caso o usuário tenha selecionado pelo menos uma ação de cobrança, a cada ação de cobrança selecionada
		if(idsCobrancaSituacao != null &&
				idsCobrancaSituacao.length > 0){
			
			for(int i = 0; i < idsCobrancaSituacao.length; i++ ){
				String idCobrancaSituacaoInformada = (String) idsCobrancaSituacao[i];
				
				ComandoEmpresaCobrancaContaCobrancaSituacaoPK pk = new ComandoEmpresaCobrancaContaCobrancaSituacaoPK();
				pk.setCobrancaSituacaoId(new Integer(idCobrancaSituacaoInformada));
				pk.setComandoEmpresaCobrancaContaId(idComandoEmpresaCobrancaConta);
				
				ComandoEmpresaCobrancaContaCobrancaSituacao ceca = new ComandoEmpresaCobrancaContaCobrancaSituacao();
				ceca.setComp_id(pk);
				ceca.setUltimaAlteracao(new Date());
				
				fachada.inserir(ceca);
			}
		}
	...
	//[FS0019] - Verificar percentual informado
	private void validarPercentualImoveisFaixa(String[] percentualImoveisFaixa, String qtdImoveisMaximaComando) {
		
		BigDecimal somaPercentual = new BigDecimal(0);
		if(qtdImoveisMaximaComando != null && !qtdImoveisMaximaComando.equals("")){
			for(int i = 0; i < percentualImoveisFaixa.length; i++ ){
				
				if(percentualImoveisFaixa[i] != null && 
						!percentualImoveisFaixa[i].equals("")){
				
					String percentual = (percentualImoveisFaixa[i]).replace(",", ".");		
					//Caso o valor informado do percentual seja inválido ou maior que 100,00
					if(Util.validarValorNaoNumericoComoBigDecimal(percentual) || 
					   (new BigDecimal(percentual)).compareTo(new BigDecimal(100)) > 0){
						
						throw new ActionServletException(
								"atencao.valor_percentual_invalido");
					}
					else{
						somaPercentual = somaPercentual.add(new BigDecimal(percentual));
					}
				}
				
				//Caso não seja informado percentual em algum dos campos
				else{
					throw new ActionServletException(
							"atencao.campo_selecionado.obrigatorio", null, "Percentual");
				}
			}
			
			
			//Caso o valor da soma dos percentuais informados seja diferente de 100,00
			if(somaPercentual.compareTo(new BigDecimal(100)) != 0){
				throw new ActionServletException(
						"atencao.soma_valor_percentual_invalido");
			}
		}
	}
