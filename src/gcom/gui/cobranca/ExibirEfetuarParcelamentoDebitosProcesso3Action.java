...
public class ExibirEfetuarParcelamentoDebitosProcesso3Action extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if (valorDebitoACobrarParcelamentoImovel != null && indicadorDebitosACobrar.equals("1")){
...
//			BigDecimal debitoTotalAtualizado = new BigDecimal("0.00");
		...
		// Caso tenha acionado o botão calcular da aba 3
			...
			sessao.setAttribute("valorAcrescimosImpontualidadePagamentoAVista", opcoesParcelamento.getValorTotalAcrescimosPagamentoAVista());
		...
		valorTotalDoacoesConta = obterValorDoacoesDasContasDoParcelamento(colecaoContaValoresNegociacao);
...
//        }
		...
		efetuarParcelamentoDebitosActionForm.set("valorTotalDoacoes",Util.formatarMoedaReal(valorTotalDoacoesConta));
	...
	private BigDecimal obterValorDoacoesDasContasDoParcelamento(Collection colecaoContas){
		...
		if (colecaoContas != null && !colecaoContas.isEmpty()) {
			...
			while (contas.hasNext()) {
				...
				if (contaValoresHelper.getIndicadorContasDebito() == null || !contaValoresHelper.getIndicadorContasDebito().equals(new Integer("2"))) {
					valorTotalDoacoes = valorTotalDoacoes.add(Fachada.getInstancia().obterValorDoacoesConta(contaValoresHelper.getConta().getId()));
				}
			}
		}
