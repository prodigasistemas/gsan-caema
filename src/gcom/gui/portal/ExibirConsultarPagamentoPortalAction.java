...
public class ExibirConsultarPagamentoPortalAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		ActionForward retorno = actionMapping.findForward("consultarPagamentoDebitosPortal");
		
		ConsultarPagamentoDebitosPortalActionForm form = (ConsultarPagamentoDebitosPortalActionForm) actionForm;
		...
		if ( sistemaParametro.getIndicadorImprimirExtratoSorteio().equals(ConstantesSistema.SIM) ) {
				httpServletRequest.setAttribute("exibirBotaoImprimirExtrato", true);
		} else {
				httpServletRequest.removeAttribute("exibirBotaoImprimirExtrato");
		}
		
		
		if(method != null && method.equalsIgnoreCase("pesquisarImovel")) {
			
			if ( form.getMatriculaImovel() != null && !form.getMatriculaImovel().equals("") ) {
		
				Imovel imovel = fachada.pesquisarImovel(Integer.valueOf(form.getMatriculaImovel()));
		
				if ( imovel != null && imovel.getId() != null ) {
					//Montar o cabeÁalho
					matricula = imovel.getId();	
					Cliente cliente = fachada.pesquisarClienteUsuarioImovel(matricula);
					form.setNomeCliente(formatarNome(cliente.getNome()));
					...
					//[FS0003] ‚Äì Verificar existÍncia de parcelamento ativo
					Collection<Parcelamento> parcelamentos = null;//= fachada.pesquisarParcelamentosSituacaoNormal(matricula);
//								
//					if(Util.isVazioOrNulo(parcelamentos)){
						// [FS0012] Verificar existÍncia de parcelamento no mÍs
						...
						if(Util.isVazioOrNulo(parcelamentos)){
							...
							//[UC0067] Obter DÈbito do ImÛvel ou Cliente - [FS0014] Verificar existÍncia de dÈbitos para o imÛvel
							if(this.obterDebitosImovel(matricula)){
								
									
									
										//As trÍs situaÁıes onde pode ter reativaÁ„o da situaÁ„o de ·gua.
										if(imovel.getLigacaoAguaSituacao().getId().intValue() == LigacaoAguaSituacao.SUPRIMIDO ||
											...
											/*
											 * Caso tenha sido a primeira vez que a p·gina foi carrega, isto È: que
										...
										/*
										 * Caso n„o esteja dentro das trÍs situaÁıes acima, o indicador de restabelecimento ser·
										 * por padr„o 2.
										 */
										}else{
											form.setIndicadorRestabelecimento("2");
...
//										if(method != null && method.equalsIgnoreCase("pesquisarImovel")){
											...
											//ValidaÁıes relativas ao cpf/cliente.
											ParcelamentoPerfil parcelamentoPerfil = this.obterPerfilParcelamentoImovel(imovel, form);
											//Verifica se o imÛvel possui perfil de parcelamento
...
//					}
					...
					httpServletRequest.setAttribute("matriculaInvalido", true);
					return retorno;
				}
			} else {
				form.setMatriculaImovel("");
		...
		/*
		 * Se houver n„o houver erro no parcelamento e n„o houver uma mesangem de reativaÁ„o do ramal
		 * (quando o efetuar parcelamento for bem sucedido e tiver religaÁ„o de ·gua).
		 */
	...
	private boolean obterDebitosImovel(Integer matricula){
		...
		this.colecaoDebitoImovel = Fachada.getInstancia().obterDebitoImovelOuCliente(
				1,// Indicador dÈbito imÛvel 
				matricula.toString(), // MatrÌcula do imÛvel
				null, // CÛdigo do cliente
				null, // Tipo de relaÁ„o do cliento com o imÛvel
				"000101", // ReferÍncia inicial do dÈbito
				"999912", // ReferÍncia final do dÈbito
				...
				1, // Indicador dÈbito a cobrar
				1, // Indicador crÈdito a realizar
				1, // Indicador notas promissÛrias
				1, // Indicador guias de pagamento
				1, // Indicador acrÈscimos por impontualidade
	...
	private void calcularDebitos(ObterDebitoImovelOuClienteHelper debitos, Imovel imovel, ConsultarPagamentoDebitosPortalActionForm form,
		 ...
		 * tenha digitado, n„o ser· necess·rio calcular os dÈbitos novamente, apenas o novo valor do
		 * parcelamento.
		 */
		if(!recalcularOpcaoParcelamento){

			// Para o c·lculo do DÈbito Total Atualizado
			...
			Integer periodoContaInicial = null;
			Integer periodoContaFinal = null;
			String periodo = null;
			// Dados do DÈbito do ImÛvel - Contas
			Collection<ContaValoresHelper> colecaoContaValores = debitos.getColecaoContasValores();
			//[SB0011] Verificar √önica Fatura
//			fachada.verificarUnicaFatura(colecaoContaValores,parcelamentoPerfil);
			...
			if (!Util.isVazioOrNulo(colecaoContaValores)) {
				...
				while (contaValores.hasNext()) {
					...
					if ( periodoContaInicial != null ) {
						if ( Util.compararAnoMesReferencia(periodoContaInicial, contaValoresHelper.getConta().getReferencia(), ">") ){
							periodoContaInicial = contaValoresHelper.getConta().getReferencia();
						}
					} else {
						periodoContaInicial = contaValoresHelper.getConta().getReferencia();
					}
					
					if ( periodoContaFinal != null ) {
						if ( Util.compararAnoMesReferencia(periodoContaFinal, contaValoresHelper.getConta().getReferencia(), "<") ){
							periodoContaFinal = contaValoresHelper.getConta().getReferencia();
						}
					} else {
						periodoContaFinal = contaValoresHelper.getConta().getReferencia();
					}
					
					periodo = Util.formatarAnoMesParaMesAno(periodoContaInicial) + " a " +  Util.formatarAnoMesParaMesAno(periodoContaFinal); 
...
//
//					// Caso nenhuma ocorrÍncia tenha sido selecionada passar para a prÛxima conta
...
//
//						//Determina o percentual de desconto por antiguidade do dÈbito
...
//						 * o motivo de revis„o informado N√ÉO entrar„o no parcelamento.
//						 */
...
//					}
				...
				form.setPeriodoDebitos(periodo);
				form.setQuantidadeDebitos(quantidadeContas.toString());
			...
			if (!Util.isVazioOrNulo(guiasPagamentoValores)){
				...
				// Seta as Guias de Pagamento em DÈbito
			...
			//----------------------------- DÈbitos a cobrar -----------------------------//
			//[FS0022]-Verificar existÍncia de juros sobre imÛvel
		...
		/*
		 * Caso o usu·rio tenha digitado um novo valor ou seja a primeira vez que esteja calculando os dÈbitos do imÛvel,
		 * ser· necess·rio obter as opÁıes de parcelamento. A coleÁ„o de conta valores ser· setada na sessao dentro do
		 * calcular dÈbitos. Logo acima.
		 */
		...
		sessao.setAttribute("formPagamento", form);
	}


	//MÈtodo auxiliar que vai retornar o perfil do parcelamento
	private ParcelamentoPerfil obterPerfilParcelamentoImovel(Imovel imovel, 
			ConsultarPagamentoDebitosPortalActionForm form){
		Fachada fachada =  Fachada.getInstancia();

		ParcelamentoPerfil parcelamentoPerfil;

		//[FS004] Verificar existÍncia da situaÁ„o do imÛvel.
...
//		if(!Util.isVazioOrNulo(colecaoResolucaoDiretoria)){
		...
		if ( resolucaoDiretoria != null ) {
			...
			if (imovelSituacao != null) {
				...
				if(parcelamentoPerfil == null){
					//CondiÁ„o 1 - iper_id = iper_id do imovel e scat_id = scat_id do imovel
					parcelamentoPerfil = fachada.obterPerfilParcelamento(imovel.getId(),imovelSituacao.getImovelSituacaoTipo().getId(),
					...
					if (parcelamentoPerfil == null) {
						...
						//CondiÁ„o 2 - iper_id = iper_id do imovel e scat_id = null do imovel
						if (parcelamentoPerfil == null) {
							parcelamentoPerfil = fachada.obterPerfilParcelamento(imovel.getId(),imovelSituacao.getImovelSituacaoTipo().getId(),
							...
							if (parcelamentoPerfil == null) {
								...
								//CondiÁ„o 3 - iper_id = null do imovel e scat_id = scat_id do imovel
								if (parcelamentoPerfil == null) {
									parcelamentoPerfil = fachada.obterPerfilParcelamento(imovel.getId(),imovelSituacao.getImovelSituacaoTipo().getId(),
									...
									if(parcelamentoPerfil == null){
										...
										//CondiÁ„o 4 - iper_id = null do imovel e scat_id = null
										if (parcelamentoPerfil == null) {
											parcelamentoPerfil = fachada.obterPerfilParcelamento(imovel.getId(),imovelSituacao.getImovelSituacaoTipo().getId(), 
	...
	private void obterOpcaoParcelamento(Collection<ContaValoresHelper> colecaoContaValores, Collection<GuiaPagamentoValoresHelper> colecaoGuiaPagamento,
			Imovel imovel, ConsultarPagamentoDebitosPortalActionForm form, HttpSession sessao, HttpServletRequest request){
		...
		ResolucaoDiretoria resolucaoDiretoria = this.obterResolucaoDiretoria(valorDebitoTotalAtualizado);
		...
		/*
		 * Caso seja uma mudanÁa no indicador de restabelecimento o valor entrada informado vai ter
		 * null, pois o sitema vai calcular o valor mÌnimo e atribuir ao valor informado
		 */
		...
		/*
		 * [SB0002] - Obter OpÁıes de Parcelamento de acordo com a entrada informada
		 * CARREGANDO O HELPER COM AS INFORMA√á√ïES DO PARCELAMENTO
		 */
		...
		if (valorEntradaInformado != null && valorEntradaInformado.compareTo(valorEntradaMinima.setScale(
			...
			// Limpa os EP da ColeÁ„o de Contas
			...
			// Limpando a opÁ„o de parcelamento
			...
			// Verifica se o valor do dÈbito menos o valor dos descontos È menor que o valor minimo da parcela
			...
			resultadoDiferenca = valorDebitoTotalAtualizado.subtract(valorTotalDescontos);
				...
				// O valor do dÈbito È menor que o valor da parcela mÌnima permitida.
				// Utilizar a opÁ„o Pagamento √† Vista. 
	...
	private boolean validarPerfilParcelamento(ConsultarPagamentoDebitosPortalActionForm form,
...
//            		descricao = Util.removerUltimosCaracteres(descricao, 2);
	...
	private String formatarNome(String nome){
		...
		if ( nome.length() > 40 ) {
			nomeFormatado = nome.substring(0, 40);
		} else {
			nomeFormatado = nome;
		}
	...
	private ResolucaoDiretoria obterResolucaoDiretoria(BigDecimal valorDebitoTotalAtualizado) {
		
		///Configurando os par√¢metros para obter opÁıes de parcelamento
		FiltroResolucaoDiretoria filtroResolucaoDiretoria = new FiltroResolucaoDiretoria();
		if ( valorDebitoTotalAtualizado.compareTo(new BigDecimal(5000)) <= 0 ) {
			filtroResolucaoDiretoria.adicionarParametro( new ParametroSimples(FiltroResolucaoDiretoria.CODIGO, new Integer(23)));
		} else  {
			filtroResolucaoDiretoria.adicionarParametro( new ParametroSimples(FiltroResolucaoDiretoria.CODIGO, new Integer(25)));
		}
		
		Collection<ResolucaoDiretoria> colecaoResolucaoDiretoria = Fachada.getInstancia().pesquisar(filtroResolucaoDiretoria, ResolucaoDiretoria.class.getName());
