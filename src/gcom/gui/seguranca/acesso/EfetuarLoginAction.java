...
import gcom.seguranca.acesso.usuario.bean.EfetuarLoginHelper;
		...
		if(visualizacaoRAUrgencia == null || !visualizacaoRAUrgencia.equals("sim")){
			...
			if (login == null || login.trim().equals("")) {
				...
				if (!this.verificarExistenciaLogin(login)) {
					...
					* Amélia Pessoa - 05/01/2012 */
					String[] erro = Fachada.getInstancia().liberarAcesso(usuarioLogado);
					if (erro!=null){
						if (erro[0].equals("1")){
							throw new ActionServletException("atencao.usuario_afastado_indefinidamente", null, usuarioLogado.getLogin());
						} else {
							throw new ActionServletException("atencao.usuario_temporariamente_bloqueado", usuarioLogado.getLogin(), erro[1]);
						}
					}
					
					/* RM 3892.2 - Verificar se usuário está na situação "Senha nao revalidada" 
					 * Amelia Pessoa */
					if (usuarioLogado!=null && usuarioLogado.getUsuarioSituacao()!=null && usuarioLogado.getUsuarioSituacao().getId().intValue()==UsuarioSituacao.SENHA_NAO_REVALIDADA.intValue()){
						throw new ActionServletException("atencao.usuario_senha_nao_revalidada", null, usuarioLogado.getLogin());
					}
					...
					if (usuarioLogado == null) {
						 ...
						 * incrementa o nÂº de tentativas e joga esse nÂº na sessão
						 * verifica se o nÂº de tentativas é maior que a permitida se
						...
						if (loginUsuarioSessao.equals(login)) {
							...
							//Zera o nÂº de tentativas de acesso e joga o login do usuário na sessão
							numeroTentativas = 0;
						...
						if (disponibilizarAlteracaoSenha) {
							...
							//recupera o nÂº de dias que falta para expirar a semha do usuário
						...
						usuarioLogado.setIpLogado(Util.getLocalMachineIP(httpServletRequest));
						...
						// Cria uma instÃ¢ncia do menu para gerar a arvóre do menu
						...
						// [FS0014] - Verificar validacao de usuarios filhos 
						if(usuarioLogado.getFuncionario() != null){
						// [FS0015] - Verificar validacao de usuarios filhos a expirar
							Collection<Usuario> usuariosFilhosAExpirar = (Collection<Usuario>) Fachada.getInstancia().verificaValidacaoUsuarioFilho(usuarioLogado.getId(),false);
							
							if(usuariosFilhosAExpirar != null){						
								
								Collection<EfetuarLoginHelper> colecaoParaExibicao = new ArrayList<EfetuarLoginHelper>();
							
								for(Usuario usuario: usuariosFilhosAExpirar){
									
									EfetuarLoginHelper helper = new EfetuarLoginHelper();
								...
								if(colecaoParaExibicao.size()!=0){
									sessao.setAttribute("usuariosFilhosAExpirar", colecaoParaExibicao);
								}
							}else{
								sessao.removeAttribute("usuariosFilhosAExpirar");
							}
							
							// [FS0016] - Verificar validação de usuários filhos expirados
							Collection<Usuario> usuariosFilhosExpirados = (Collection<Usuario>) Fachada.getInstancia().verificaValidacaoUsuarioFilho(usuarioLogado.getId(),true);
							
							if(usuariosFilhosExpirados != null){
								Collection<EfetuarLoginHelper> colecaoParaExibicaoExpirados = new ArrayList<EfetuarLoginHelper>();
								
								
								for(Usuario usuario: usuariosFilhosExpirados){
									
									EfetuarLoginHelper helper = new EfetuarLoginHelper();
									
									helper.setUsuario(usuario);
									
									colecaoParaExibicaoExpirados.add(helper);
									
								}
							
								if(colecaoParaExibicaoExpirados.size()!=0){
									sessao.setAttribute("usuariosFilhosExpirados", colecaoParaExibicaoExpirados);
								}
								
							}else{
								sessao.removeAttribute("usuariosFilhosExpirados");
							}
						}else{
							sessao.removeAttribute("usuariosFilhosExpirados");
							sessao.removeAttribute("usuariosFilhosAExpirar");
...
//							}
	...
	private void bloquearSenha(String login) {
		...
		if (usuarios != null && !usuarios.isEmpty()) {
			...
			// Recupera o nÂº de vezes que o usuário foi bloqueado
			Short bloqueioAcesso = usuarioLogado.getBloqueioAcesso();

			/*
			 * Caso o usuário nunca tenha sido bloqueado seta o nÂº de bloqueios
			 * para 1(um) caso contrário incrementa o valor do nÂº de bloqueio do
			 * usuário
			 */
