...
public class AtualizarGrupoDadosGeraisAction extends GcomAction {
    ...
    public ActionForward execute(ActionMapping actionMapping,
        ...
        grupo.setIndicadorGrupoEspecial((Short)atualizarGrupoActionForm.get("indicadorGrupoEspecial"));
        grupo.setIndicadorVisualizacaoRestrita((Short)atualizarGrupoActionForm.get("indicadorVisualizacaoRestrita"));
        grupo.setIndicadorEspecialSeguranca((Short)atualizarGrupoActionForm.get("indicadorEspecialSeguranca"));
        
        Grupo grupoInferior = new Grupo();
        
        String idGrupoInferior = "";
        
	    if(atualizarGrupoActionForm.get("indicadorGrupoEspecial").equals(ConstantesSistema.NAO) && atualizarGrupoActionForm.get("idGrupoInferior") !=null && !atualizarGrupoActionForm.get("idGrupoInferior").equals("") ){
	    	
	    	if(!atualizarGrupoActionForm.get("idGrupoInferior").equals("-1")){
	    		
	    		idGrupoInferior =  atualizarGrupoActionForm.get("idGrupoInferior").toString(); 
	    		
	    		FiltroGrupo filtroGrupo = new FiltroGrupo();
	    		filtroGrupo.adicionarParametro(new ParametroSimples(FiltroGrupo.ID, idGrupoInferior));
	    		filtroGrupo.adicionarCaminhoParaCarregamentoEntidade("grupoInferior");
	            Collection colecaoGrupo = Fachada.getInstancia().pesquisar(filtroGrupo, Grupo.class.getName());
	            
	            grupoInferior = (Grupo) Util.retonarObjetoDeColecao(colecaoGrupo);
	    		
	    	}   	
	    }
        
	    grupo.setGrupoInferior(grupoInferior);
        ...
        //[FS002] - Verificar preenchimento dos campos
        if (grupo.getIndicadorVisualizacaoRestrita() == null){
        	throw new ActionServletException("atencao.naoinformado",null, "Indicador de Visualização Restrita");
        }
        ...
        //[FS0008] -  Validar seleção de grupo inferior
        
    	if (grupo.getId().toString().equals(idGrupoInferior) ) {
			throw new ActionServletException("atencao.grupo_inferior_igual");
        }
    	
    
    	if(grupoInferior != null  && grupoInferior.getId()!=null){
    	
    		Collection <Integer> listaGruposInferiores = 
    				Fachada.getInstancia().pesquisarArvoreNiveis(grupoInferior.getId());
    		
    		for(Integer integer :listaGruposInferiores){
    			
    			if(integer.equals(grupo.getId()) ){
    			
    				throw new ActionServletException("atencao.grupo_inferior_igual_loop", 
							grupo.getDescricao(), 
								grupo.getGrupoInferior().getDescricao(), 
									grupo.getGrupoInferior().getDescricao(),
										grupo.getDescricao());
