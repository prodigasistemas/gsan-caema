...
public class AtualizarSolicitacaoAcessoAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		filtroSolicitacaoAcesso.adicionarParametro(new ParametroSimples(
				FiltroSolicitacaoAcesso.ID, solicitacaoAcesso.getId()));		
		...
		Collection<PermissaoEspecial> colecaoPermissaoEspecialSelecionada  = new ArrayList();
		...
		if(solicitacaoAcesso.getFuncionario() != null){
			...
			mensagem += " O Funcionário, Matrícula: "+ solicitacaoAcesso.getFuncionario().getId()
					+ " de nome "+ solicitacaoAcesso.getNomeUsuario()+" necessita de sua liberação de acesso para o(s) grupo(s) "
	...
	private SolicitacaoAcesso setSolicitacaoAcesso(AtualizarSolicitacaoAcessoActionForm form, 
		...
		//Id
		solicitacaoAcesso.setId(Integer.parseInt(form.getIdSolicitacao()));
		...
		if(Util.verificarNaoVazio(form.getIdFuncionarioSolicitante())){
			...
			throw new ActionServletException("atencao.campo_texto.obrigatorio", null, "Usuário Solicitante");
		}
...
//				}
        ...
        if(form.getCompetenciaRetificacao() !=null
        	...
        	solicitacaoAcesso.setCompetenciaRetificacao(null);
        }
        
		// data cadastramento
		if(form.getDataCadastramento() != null){
			
			solicitacaoAcesso.setDataCadastramento(Util.converteStringParaDate(form.getDataCadastramento()));
		}
		...
		if(form.getIdUsuarioResponsavel()!= null && !form.getIdUsuarioResponsavel().equals("")){
			...
			if (colecaoUsuario != null && !colecaoUsuario.isEmpty()) {
				Usuario usuarioRevalidador = (Usuario) Util.retonarObjetoDeColecao(colecaoUsuario);
				...
				boolean possuiPermissao = 
		    			Fachada.getInstancia().verificarPermissaoEspecialAtiva(
		    					PermissaoEspecial.REVALIDAR_USUARIO,usuarioRevalidador);
				if (!possuiPermissao){
					throw new ActionServletException("atencao.usuario_sem_permissao_revalidar_soli_acesso");	
				}
				solicitacaoAcesso.setUsuarioResponsavel(usuarioRevalidador);	                
			} else {
		...
		//Competencia de Retificação
		 ...
		 //usuario representante da solicitação
		 if(form.getIdUsuarioSolicitacao() != null && !form.getIdUsuarioSolicitacao().equals("")){
			 Usuario usuario = new Usuario();
			 usuario.setId(Util.converterStringParaInteger(form.getIdUsuarioSolicitacao()));
