...
public class ExibirAtualizarLocalidadeAction extends GcomAction {
	...
	public ActionForward unspecified(ActionMapping actionMapping,
        ...
        Collection colecaoSA = (Collection)fachada.pesquisar(filtroSA, SistemaAbastecimento.class.getName());
        
        //Clonar a colecao para o campo de abastecimento secundário
        Collection colecaoSASecundario = new ArrayList();
        colecaoSASecundario.addAll(colecaoSA);
        
        sessao.setAttribute("colecaoSA", colecaoSA);
        sessao.setAttribute("colecaoSASecundario", colecaoSASecundario);
        
        //Localizando o sistema abastecimento principal e selecionando os secundários, se houver
        atualizarFormularioSistemaAbastecimento(atualizarLocalidadeActionForm,fachada, colecaoSASecundario, localidadeID);
		...
		if ((objetoConsulta != null && !objetoConsulta.trim().equalsIgnoreCase(""))
			...
			if(objetoConsulta != null && !objetoConsulta.trim().equalsIgnoreCase("")){
				...
				boolean permissaoIndicadorContratoPpp = fachada.verificarPermissaoEspecial(PermissaoEspecial.INFORMA_LOCALIDADE_CONTRATO_PPP, usuario);
				
				if(permissaoIndicadorContratoPpp){
		    		httpServletRequest.setAttribute("permissaoContratoPpp", "true");
		    	}
	...
	private void atualizarFormularioSistemaAbastecimento(
			AtualizarLocalidadeActionForm atualizarLocalidadeActionForm, Fachada fachada, Collection colecaoSA, String localidadeID) {
		
		ArrayList<String> listIdsSistemaAbs = new ArrayList<String>();
		
		//Pesquisar a Localidade Sistema Abastecimento
        Filtro filtro = new FiltroSistemaAbastecimento();
        filtro.adicionarCaminhoParaCarregamentoEntidade("sistemaAbastecimento");
        filtro.adicionarParametro(new ParametroSimples("localidade.id",localidadeID));
        Collection colecao = (Collection)fachada.pesquisar(filtro, LocalidadeSistemaAbastecimento.class.getName());
		
        //buscando o principal e marcando no formulário. Ao mesmo tempo, inserindo os secundários, se houver
        Iterator it = colecao.iterator();
        while(it.hasNext()){
        	LocalidadeSistemaAbastecimento loc = (LocalidadeSistemaAbastecimento) it.next();
        	
        	if(loc.getIndicadorPrincipalSetorAbastecimento().compareTo(ConstantesSistema.SIM) == 0){
        		atualizarLocalidadeActionForm.setIdSistemaAbastecimento(loc.getSistemaAbastecimento().getId().toString());
        		Iterator it2 = colecaoSA.iterator();
        		while(it2.hasNext()){
        			SistemaAbastecimento sis = (SistemaAbastecimento) it2.next();
        			if(sis.getId().compareTo(loc.getSistemaAbastecimento().getId()) == 0){
        				it2.remove();
        				break;
        			}
        		} 
        		...
        		listIdsSistemaAbs.add(loc.getSistemaAbastecimento().getId().toString());
        	}
        }
        
        atualizarLocalidadeActionForm.setIdsSistemasAbastecimentoSecundários(listIdsSistemaAbs.toArray(new String[listIdsSistemaAbs.size()]));
	...
	private void exibirLocalidade(String localidadeID, 
		...
		atualizarLocalidadeActionForm.setIndicadorContratoPpp(localidade.getIndicadorContratoPpp().toString());
...
//			 Pesquisa os elos realacionados com a gerência
		...
		if ((localidade.getIndicadorCoordenadaProgisRA() == null) || (localidade.getIndicadorCoordenadaProgisRA().equals(""))){
			atualizarLocalidadeActionForm.setIndicadorCoordenadaProgisRA("2");
		} else {
			atualizarLocalidadeActionForm.setIndicadorCoordenadaProgisRA(Short.toString(localidade.getIndicadorCoordenadaProgisRA()));
		}
	...
	public ActionForward atualizarListaAbastecimentoSecundario(ActionMapping actionMapping,
		 ...
		 AtualizarLocalidadeActionForm form = (AtualizarLocalidadeActionForm) actionForm;
		 Collection colecaoSA = (Collection)this.getSessao(httpServletRequest).getAttribute("colecaoSA");
