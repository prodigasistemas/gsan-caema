...
//			consultarImovelActionForm.setSituacaoCobrancaDadosComplementares(null);
			...
			consultarImovelActionForm.setMsgImovelAptoSorteio(null);
...
//				}
				...
				if (imovelNovoPesquisado) {
					...
					// Obtendo os débitos do imovel
					ObterDebitoImovelOuClienteHelper colecaoDebitoImovelSorteio = fachada
							.obterDebitoImovelOuCliente(tipoImovel.intValue(),
									idImovelDebitos.trim(), null, null,
									anoMesInicial, anoMesFinal,
									dataVencimentoDebitoI,
									new Date(), indicadorPagamento.intValue(), 
										indicadorConta.intValue(), 
										2, //indicadorDebito.intValue(), 
										2, //indicadorCredito.intValue(), 
										2, //indicadorNotas.intValue(), 
										2, //indicadorGuias.intValue(), 
					...
					if(dataSorteio != null && Util.compararData(dataAtual, dataSorteio) !=  1 && fachada.pesquisarImovelCadastradoSorteio(imovel.getId())) {
						consultarImovelActionForm.setMsgImovelAptoSorteio("IMÓVEL INSCRITO NO SORTEIO");
					}else if(dataLimiteSorteio != null && Util.compararData(dataAtual, dataLimiteSorteio) !=  1){
						if ((colecaoDebitoImovelSorteio.getColecaoContasValores()== null 
								|| colecaoDebitoImovelSorteio.getColecaoContasValores().size() == 0 )&&
					  	    (colecaoDebitoImovelSorteio.getColecaoContasValoresImovel()== null 
					  	    	|| colecaoDebitoImovelSorteio.getColecaoContasValoresImovel().size() == 0 )) {
								if(imovel.getLigacaoAguaSituacao().getId().equals(LigacaoAguaSituacao.LIGADO) || 
									...
									if(fachada.verificarImovelAptoSorteio(imovel.getId())){
										consultarImovelActionForm.setMsgImovelAptoSorteio("IMÓVEL APTO NÃO INSCRITO NO SORTEIO");
									}else{
					...
					if ((colecaoContaValores == null)&& (colecaoDebitoACobrar == null || colecaoDebitoACobrar.isEmpty())
						...
						sessao.setAttribute("valorDebitoACobrarSemJurosParcelamento", 
								valorDebitoACobrarSemJurosParcelamento);
						...
						if(sistemaParametro.getResolucaoDiretoria() != null){
							...
							Integer situacaoLigacaoAgua = new Integer(consultarImovelActionForm.getLigacaoAguaSituacaoId());
		...
		Short indicadorEmissaoExtratoNaConsulta = 
			this.getFachada().pesquisarParametrosDoSistema().getIndicadorEmissaoExtratoNaConsulta();
