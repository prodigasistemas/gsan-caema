...
public class ExibirDocumentoCobrancaItemAction extends GcomAction {
    ...
    public ActionForward execute(ActionMapping actionMapping,
    	...
    	if (cobrancaDocumentoHelper.getCobrancaDocumento().getValorDesconto() != null){
    		...
    		documentoCobrancaItemActionForm.setValorDesconto("0,00");
    	}
    	if(cobrancaDocumentoHelper.getCobrancaDocumento().getValorAcrescimos() !=null){
    		documentoCobrancaItemActionForm.setValorAcrescimo(Util.formatarMoedaReal(cobrancaDocumentoHelper.getCobrancaDocumento()
    			.getValorAcrescimos()));
    	}
    	if (cobrancaDocumentoHelper.getCobrancaDocumento().getValorDesconto() !=null && 
    		cobrancaDocumentoHelper.getCobrancaDocumento().getValorDesconto().compareTo(new BigDecimal(0)) > 0 &&
    		cobrancaDocumentoHelper.getCobrancaDocumento().getValorAcrescimos() !=null && 
    		cobrancaDocumentoHelper.getCobrancaDocumento().getValorAcrescimos()
    		.compareTo(cobrancaDocumentoHelper.getCobrancaDocumento().getValorDesconto()) < 0){
    		
    		BigDecimal sub = cobrancaDocumentoHelper.getCobrancaDocumento().getValorDesconto()
    				.subtract(cobrancaDocumentoHelper.getCobrancaDocumento().getValorAcrescimos());
    		
    		documentoCobrancaItemActionForm.setValorDescontoEfetivo(Util.formatarMoedaReal(sub));
    	}
    	else{
    		documentoCobrancaItemActionForm.setValorDescontoEfetivo("0,00");
    	}
    	...
    	if (cobrancaDocumentoHelper.getCobrancaDocumento().getMotivoNaoEntregaDocumento() != null){
    		documentoCobrancaItemActionForm.setMotivoNaoEntregaDocumento(cobrancaDocumentoHelper.getCobrancaDocumento().getMotivoNaoEntregaDocumento()
    	...
    	filtroLigacaoAgua.adicionarParametro(new ParametroSimples(FiltroLigacaoAgua.ID_IMOVEL, cobrancaDocumentoHelper.getCobrancaDocumento().getImovel().getId()));
    	filtroLigacaoAgua.adicionarCaminhoParaCarregamentoEntidade(FiltroLigacaoAgua.CORTE_TIPO);
    	filtroLigacaoAgua.adicionarCaminhoParaCarregamentoEntidade(FiltroLigacaoAgua.CORTE_MOTIVO);
    	...
    	if(colecaoLigacaoAgua != null && !colecaoLigacaoAgua.isEmpty()){
    		...
    		if (ligacaoAgua.getCorteTipo() != null && documentoCobrancaItemActionForm.getSituacaoAguaImovel() != null &&
    				documentoCobrancaItemActionForm.getSituacaoAguaImovel().equals( "CORTADO" )){
    			
    			documentoCobrancaItemActionForm.setDescricaoTipoCorte(ligacaoAgua.getCorteTipo().getDescricao());
    		
    		if (ligacaoAgua.getMotivoCorte() != null){
    			
    			documentoCobrancaItemActionForm.setDescricaoMotivoCorte(ligacaoAgua.getMotivoCorte().getDescricao());
    		}
    		
    		if(ligacaoAgua.getDataCorte() != null){
    			documentoCobrancaItemActionForm.setDataCorte(Util.formatarData(ligacaoAgua.getDataCorte()));
    		}
    		
    		if(ligacaoAgua.getNumeroSeloCorte() != null){
    			documentoCobrancaItemActionForm.setCodigoSeloCorte(ligacaoAgua.getNumeroSeloCorte().toString());
    		}
    		
    		sessao.setAttribute("documentoCorte", "SIM");
    	  } else {
    		  sessao.removeAttribute("documentoCorte");
    	  }
