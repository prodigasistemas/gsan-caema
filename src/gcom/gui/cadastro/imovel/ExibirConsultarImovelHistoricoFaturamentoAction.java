...
public class ExibirConsultarImovelHistoricoFaturamentoAction extends GcomAction {
	...
	private void setarDadosImovelNoFormESessao(ConsultarImovelActionForm consultarImovelActionForm, 
		...
		if(dataSorteio != null && Util.compararData(dataAtual, dataSorteio) !=  1 && fachada.pesquisarImovelCadastradoSorteio(imovel.getId())) {
			consultarImovelActionForm.setMsgImovelAptoSorteioHistoricoFaturamento("IMÓVEL INSCRITO NO SORTEIO");
		  	    ...
		  	    (colecaoDebitoImovel.getColecaoContasValoresImovel()== null 
					...
					if(imovel.getLigacaoAguaSituacao().getId().equals(LigacaoAguaSituacao.LIGADO) || 
						...
						if(fachada.verificarImovelAptoSorteio(imovel.getId())){
							consultarImovelActionForm.setMsgImovelAptoSorteioHistoricoFaturamento("IMÓVEL APTO NÃO INSCRITO NO SORTEIO");
						}else{
							consultarImovelActionForm.setMsgImovelAptoSorteio("");
						}
					}else{
						consultarImovelActionForm.setMsgImovelAptoSorteioHistoricoFaturamento("");
					}
			}else{
				consultarImovelActionForm.setMsgImovelAptoSorteioHistoricoFaturamento("");
			}
		...
		Collection colecaoContaImovel = Fachada.getInstancia().consultarContasImovel(imovel.getId());
		Collection colecaoContaHistoricoImovel = Fachada.getInstancia().consultarContasHistoricosImovel(imovel.getId());
		
		if (colecaoContaImovel != null && !colecaoContaImovel.isEmpty()){
			this.incluirIndicadorContaRevisao(colecaoContaImovel);
		}
		if (colecaoContaHistoricoImovel != null && !colecaoContaHistoricoImovel.isEmpty()){
			this.incluirIndicadorContaHistoricoRevisao(colecaoContaHistoricoImovel);
		}
	...
	private void limparFormSessao(
		...
		consultarImovelActionForm.setMsgImovelAptoSorteioHistoricoFaturamento(null);
	...
	//Caso a conta esteja em revisão por contrato de parcelamento exibir "PCR"
	private void incluirIndicadorContaRevisao(Collection colecao){
		...
		while(it.hasNext()){
			...
			boolean verificarParcelamento = Fachada.getInstancia().verificarRevisaoContratoParcelamento(conta.getId());
			  
	  	  	if(verificarParcelamento){
	  	  		conta.getDebitoCreditoSituacaoAtual().setDescricaoAbreviada("PCR");
	  	  	}
		}
	}
	
	//Caso a conta esteja em revisão por contrato de parcelamento exibir "PCR"
	private void incluirIndicadorContaHistoricoRevisao(Collection colecao){
				
			Iterator it = colecao.iterator();
			
			while(it.hasNext()){
				
				ContaHistorico conta = (ContaHistorico)it.next();
