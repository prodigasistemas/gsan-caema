...
//		String tipoPessoaForm =  tipoPessoa.toString() ; 
		...
		try {
			...
			if (clienteActionForm.get("indicadorBloquearDespesasPostais").equals(ConstantesSistema.SIM.toString())){
				cliente.setIndicadorBloquearDespesasPostais(ConstantesSistema.SIM);
			} else {
				cliente.setIndicadorBloquearDespesasPostais(ConstantesSistema.NAO);
			}
			
			if (clienteActionForm.get("indicadorBloquearAlterarImoveis").equals(ConstantesSistema.SIM.toString())){
				cliente.setIndicadorBloquearAlterarImoveis(ConstantesSistema.SIM);
			} else {
				cliente.setIndicadorBloquearAlterarImoveis(ConstantesSistema.NAO);
			}
			...
			if(indicadorConsultaDocumentoReceita.toString().equals(ConstantesSistema.SIM.toString())){
				...
				nomeClienteReceita = cliente.getNomeClienteReceitaFederal();
			}
			...
			if(codigoCliente != null ){
				...
				Collection<Cliente> colClienteAux = fachada.pesquisar(filtroCliente, Cliente.class.getName());
				
				if(!Util.isVazioOrNulo(colClienteAux)){
					clienteAux = (Cliente) Util.retonarObjetoDeColecao(colClienteAux);
				}
				...
				if ( clienteActionForm.get("idClienteVirtual") != null && !clienteActionForm.get("idClienteVirtual").equals("") ) {
					...
					clienteAux.setCliente(clienteAux);
					...
					RegistradorOperacao registradorOperacao = new RegistradorOperacao(
							...
							clienteImovelInserir.getId(), new UsuarioAcaoUsuarioHelper(usuario,
									UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO));
					
					Operacao operacao = new Operacao();
					operacao.setId(Operacao.OPERACAO_ATUALIZAR_CLIENTE_VIRTUAL);
