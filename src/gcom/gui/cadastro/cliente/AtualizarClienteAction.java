...
public class AtualizarClienteAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		Boolean clienteVirtualOk = false;
		...
		String nome = ((String) clienteActionForm.get("clienteNome")).toUpperCase();
		
		//Verifica se o cliente já foi atualizado
		//após a inicialização do processo atual
		//==================================================================================
		Date dataHoraAtualizarCliente = (Date)sessao.getAttribute("dataHoraAtualizarCliente");
		
		FiltroCliente filtroCliente = new FiltroCliente();
		filtroCliente.adicionarParametro(new ParametroSimples(FiltroCliente.ID,clienteAtualizacao.getId()));
		Collection colClienteCompararData = fachada.pesquisar(filtroCliente,Cliente.class.getName());
		Cliente clienteCompararData = (Cliente)Util.retonarObjetoDeColecao(colClienteCompararData);
		
		//Caso o cliente já tenha sido atualizado por outro processo,
		//Exibir a mensagem "Esse cliente foi atualizado por outro usuário. Realize uma nova atualização"
		if(Util.compararDataTime(clienteCompararData.getUltimaAlteracao(),dataHoraAtualizarCliente) > 0){
			throw new ActionServletException(
					"atencao.cliente_atualizado_outro_usu");
		}
		//==================================================================================
		...
		try {
			...
			String nomeCliente = clienteActionForm.get("nome").toString().toUpperCase();
			...
			cliente.setNomeClienteReceitaFederal(clienteActionForm.get("clienteNome").toString().toUpperCase());
...
//			 Permissao Especial Validar Acrescimos Impontualidade
			...
			if (clienteActionForm.get("diaVencimento") != null && !(clienteActionForm.get("diaVencimento").equals("")) && 
			 ...
			 cliente.setIndicadorBloquearDespesasPostais(new Short((String)clienteActionForm.get("indicadorBloquearDespesasPostais")));	 
			 cliente.setIndicadorBloquearAlterarImoveis(new Short((String)clienteActionForm.get("indicadorBloquearAlterarImoveis")));
			...
			if ( confirmado == null && 
				...
				if(consultaRF.getCpfCliente() != null && !consultaRF.getCpfCliente().equals("")){
					if(consultaRF.getNomePessoaFisica() == null || consultaRF.getNomePessoaFisica().equals("")){
						if ( clienteActionForm.get("permissaoEspecialHabilitarNome") != null  && clienteActionForm.get("permissaoEspecialHabilitarNome").toString().equals("1") ) {
							cliente.setNome(clienteActionForm.get("nome").toString().toUpperCase());
						} else {
			...
			if ( clienteActionForm.get("idClienteVirtual") != null && !clienteActionForm.get("idClienteVirtual").equals("") ) {
				...
				filtroClienteVirtual.adicionarCaminhoParaCarregamentoEntidade("foneTipo");
				filtroClienteVirtual.adicionarCaminhoParaCarregamentoEntidade("orgaoExpedidorRg");
				filtroClienteVirtual.adicionarCaminhoParaCarregamentoEntidade("pessoaSexo");
				filtroClienteVirtual.adicionarCaminhoParaCarregamentoEntidade("profissao");
				filtroClienteVirtual.adicionarCaminhoParaCarregamentoEntidade("unidadeFederacao");
		...
		if (clienteVirtualOk) {			
			...
			if ( colecaoClienteImovel != null && !colecaoClienteImovel.isEmpty() ) {
				...
				if ( !clienteImovel.getCliente().getId().toString().equals(clienteAtualizacao.getId().toString()) ) {
					...
					clienteImovel.setClienteVirtual(clienteVirtual);
					...
					clienteImovelInserir.setClienteVirtual(clienteVirtual);
					clienteVirtual.setClienteImovel(clienteImovelInserir);	
					
					
					// ------------ REGISTRAR TRANSAÇÃO ----------------
					RegistradorOperacao registradorOperacao = new RegistradorOperacao(
							Operacao.OPERACAO_ATUALIZAR_CLIENTE_VIRTUAL, clienteImovelInserir.getId(), 
							clienteVirtual.getId(), new UsuarioAcaoUsuarioHelper(usuario,
									UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO));
					
					Operacao operacao = new Operacao();
					FiltroOperacao filtroOperacao = new FiltroOperacao();
					filtroOperacao.adicionarParametro(new ParametroSimples(FiltroOperacao.ID,
						Operacao.OPERACAO_ATUALIZAR_CLIENTE_VIRTUAL));
					...
					Collection colOperacao = fachada.pesquisar(filtroOperacao, Operacao.class.getName());
					
					if(!Util.isVazioOrNulo(colOperacao)){
						operacao = (Operacao) Util.retonarObjetoDeColecao(colOperacao);
					}					

					OperacaoEfetuada operacaoEfetuada = new OperacaoEfetuada();
					operacaoEfetuada.setOperacao(operacao);
					
					clienteImovelInserir.setOperacaoEfetuada(operacaoEfetuada);
					clienteImovelInserir.adicionarUsuario(usuario,
							UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO);
					registradorOperacao.registrarOperacao(clienteImovelInserir);
					// ------------ REGISTRAR TRANSAÇÃO ----------------
