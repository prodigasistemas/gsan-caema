...
public class ExibirInserirUnidadeOrganizacionalAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		//Retorna o maior id de Unidade Organizacional existente 
		int id = this.getFachada().pesquisarMaximoIdUnidadeOrganizacional();
		// Acrescenta 1 no valor do id para setar o primeiro id vazio para o usuário
		id = (id + 1);
		String codigoUnidade = "" + id;
		if (!codigoUnidade.equals(inserirUnidadeOrganizacionalActionForm.getCodigoUnidade()) 
				&& inserirUnidadeOrganizacionalActionForm.getCodigoUnidade() != null 
				&& !inserirUnidadeOrganizacionalActionForm.getCodigoUnidade().equals("")){
			inserirUnidadeOrganizacionalActionForm.setCodigoUnidade(inserirUnidadeOrganizacionalActionForm.getCodigoUnidade());
		} else {
			inserirUnidadeOrganizacionalActionForm.setCodigoUnidade(codigoUnidade);
		}
		...
		// verifica se o usuário solicitou uma consulta de unidade de tramitacao
		String idUnidadeTramitacao = httpServletRequest.getParameter("idUnidadeTramitacao");
		String descricaoUnidadeTramitacao = httpServletRequest.getParameter("descricaoUnidadeTramitacao");

		// Seta no form os valores da pesquisa feita
		if (idUnidadeTramitacao != null && !idUnidadeTramitacao.trim().equals("") && 
			descricaoUnidadeTramitacao != null && !descricaoUnidadeTramitacao.trim().equals("")) {

			// Indica que o local de armazenagem foi encontrado
			httpServletRequest.setAttribute("idUnidadeEncontrada","true");

			inserirUnidadeOrganizacionalActionForm.setIdUnidadeTramitacao(idUnidadeTramitacao.trim());
			inserirUnidadeOrganizacionalActionForm.setDescricaoUnidadeTramitacao(descricaoUnidadeTramitacao.trim());
		}

		if (objetoConsulta != null && !objetoConsulta.trim().equals("") && 
			objetoConsulta.trim().equals("undTramit")) {

			// Faz a consulta de Unidade
			pesquisarUnidadeTramitacao(httpServletRequest, retorno,inserirUnidadeOrganizacionalActionForm);
	...
	//pesquisa a unidadeTramitacao pelo id
		private void pesquisarUnidadeTramitacao(
				HttpServletRequest httpServletRequest, ActionForward retorno,
				InserirUnidadeOrganizacionalActionForm inserirUnidadeOrganizacionalActionForm) {
			...
			String idUnidadeTramitacao = inserirUnidadeOrganizacionalActionForm.getIdUnidadeTramitacao();
			
			filtroUnidadeOrganizacional.adicionarParametro(
				new ParametroSimples(FiltroUnidadeOrganizacional.ID, idUnidadeTramitacao));
			...
			if (colecaoUnidade != null && !colecaoUnidade.isEmpty()) {
				...
				inserirUnidadeOrganizacionalActionForm.setIdUnidadeTramitacao(unidadeOrganizacional.getId().toString());
				inserirUnidadeOrganizacionalActionForm.setDescricaoUnidadeTramitacao(unidadeOrganizacional.getDescricao());

				httpServletRequest.setAttribute("corUnidadeTramitacao", "valor");	
							
			} else {
				httpServletRequest.setAttribute("corUnidadeTramitacao","exception");
				inserirUnidadeOrganizacionalActionForm.setDescricaoUnidadeTramitacao("Unidade Organizacional inexistente");
				inserirUnidadeOrganizacionalActionForm.setIdUnidadeTramitacao("");
