...
public class AtualizarUnidadeOrganizacionalAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		String idUnidadeTramitacao = form.getIdUnidadeTramitacao();
		if(idUnidadeTramitacao != null && !idUnidadeTramitacao.equals("")){
			unidadeNova.setUnidadeTramitacao(this.pesquisarUnidadeOrganizacional(idUnidadeTramitacao));	
		}else{
			unidadeNova.setUnidadeTramitacao(null);		
		}
		...
		String icUnidadeAbreRegistro = form.getUnidadeAbreRegistro();
		String idMeioSolicitacao = "-1";
		
		
		
		if(icUnidadeAbreRegistro.equals("1")){
			idMeioSolicitacao = form.getMeioSolicitacao();
		}
		
		
		if(idMeioSolicitacao != null &&  !idMeioSolicitacao.equals("")){
		...
		if(!Util.validarStringNumerica(form.getId()))
			throw new ActionServletException("atencao.integer",null,"Código da Unidade Organizacional");
		
		// [FS0015] - Verificar existência de Unidade Organizacional
		if(!form.getIdAntigo().equals(form.getId())){
			UnidadeOrganizacional verificarUnidade = this.pesquisarUnidadeOrganizacional(form.getId());
			
			//Caso o código da unidade organizacional exista
			if(verificarUnidade != null)
				throw new ActionServletException("atencao.unidade_organizacional_codigo_ja_cadastrado");
		}
		
		
		fachada.atualizarUnidadeOrganizacional(unidadeNova, this.getUsuarioLogado(httpServletRequest),new Integer(form.getIdAntigo()));
