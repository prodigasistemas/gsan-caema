...
public class FiltrarFeriadoAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
			...
			String indicadorPerene = form.getIndicadorPerene();
			...
			if (indicadorTipoFeriado.equals("1") ) {
				...
				//Indicador de Perene
				if (indicadorPerene != null && !indicadorPerene.equals("") && !indicadorPerene.equals(""+ConstantesSistema.TODOS)){
					peloMenosUmParametroInformado = true;
				}
			...
			Integer totalRegistros = fachada.pesquisarFeriadoCount(
					Short.parseShort(indicadorTipoFeriado), descricaoFeriado, dataFeriadoInicio,
							dataFeriadoFim, idMunicipio, Short.parseShort(indicadorPerene));
			...
			Collection colecaoFeriado = fachada.pesquisarFeriado(
					...
					Short.parseShort(indicadorPerene), total);
			...
			Collection relatorio =  impressao(
				Short.parseShort(indicadorTipoFeriado), descricaoFeriado, dataFeriadoInicio, 
				dataFeriadoFim, idMunicipio, pagina, 
				Short.parseShort(indicadorPerene), total);
			
			if(!Util.isVazioOrNulo(relatorio)){
	...
	public Collection impressao(Short tipoFeriado, String descricao,
			Date dataFeriadoInicio, Date dataFeriadoFim, Integer idMunicipio,
			Integer numeroPagina, Short indicadorPerene, Boolean total){
		
		Fachada fachada = Fachada.getInstancia();
		
		Collection retorno = fachada.pesquisarFeriado(
			tipoFeriado, descricao, dataFeriadoInicio, dataFeriadoFim, 
			idMunicipio, numeroPagina, indicadorPerene, total);
