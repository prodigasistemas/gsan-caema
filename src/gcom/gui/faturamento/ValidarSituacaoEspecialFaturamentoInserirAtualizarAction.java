...
public class ValidarSituacaoEspecialFaturamentoInserirAtualizarAction extends
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if ( situacaoEspecialFaturamentoActionForm.getIdFaturamentoSituacaoTipo() != null && !situacaoEspecialFaturamentoActionForm.getIdFaturamentoSituacaoTipo().equals("")) {
			this.validaInformacaoCategoriaResidencial(situacaoEspecialFaturamentoActionForm);
		}
	...
	/**
	 * Metodo reponsavel por obrigar o usuario a informar categoria residencial quando o tipo de situacao especial de faturamento possuir o indicador cobrar consumo minimo ativo.
	 * 
	 ...
	 * @param form
	 */
	private void validaInformacaoCategoriaResidencial( SituacaoEspecialFaturamentoActionForm form ) {
		
			
		FiltroFaturamentoSituacaoTipo filtro = new FiltroFaturamentoSituacaoTipo();
		filtro.adicionarParametro( new ParametroSimples(FiltroFaturamentoSituacaoTipo.ID, form.getIdFaturamentoSituacaoTipo()));
		
		FaturamentoSituacaoTipo faturamentoSituacaoTipo = (FaturamentoSituacaoTipo) Util.retonarObjetoDeColecao(
				Fachada.getInstancia().pesquisar(filtro, FaturamentoSituacaoTipo.class.getName()));
		
		boolean permite = true;
		if ( faturamentoSituacaoTipo.getIndicadorCobrarConsumoMinimo() != null && faturamentoSituacaoTipo.getIndicadorCobrarConsumoMinimo().equals(ConstantesSistema.SIM) ) {
			
			//Pesquisa realizada por imovel.
			if ( form.getIdImovel() != null && !form.getIdImovel().trim().equals("") ) {
				
				Imovel imovel = new Imovel();
				imovel.setId(Integer.valueOf(form.getIdImovel()));
				
				permite = Fachada.getInstancia().validaImovelCategoriaResidencialComIndicadorCobrarConsumoMinimo(imovel);
		...
		if ( !permite )  {
			throw new ActionServletException("atencao.faturamento_situacao_especial_imovel_residencial");
		}
	}
}
