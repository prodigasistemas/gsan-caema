 ...
 * @date 15/01/2012
 */
public class PesquisarContaRetificacaoAction extends GcomAction {
	/**
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		ActionForward retorno = actionMapping.findForward("listaContaRetificacao");
		...
		//sessao.setAttribute("idContas", "");
		
		// seta a colecao das contas selecionadas, na sessao
		if(httpServletRequest.getParameter("contas") != null){
			
			String[] idContas =(String[]) pesquisarContaActionForm.getIdContas();
			ArrayList<String> listContas = new ArrayList<String>();
			
			//Adiciona as contas selecionadas a um arraylist
			for(int i=0;i<idContas.length;i++){
				listContas.add(idContas[i]);
			}
			
			// Verifica se já havia contas selecionadas
			if(sessao.getAttribute("contasSelecionadas")!=null){
				ArrayList<String> idContasSelecionadas =(ArrayList<String>) sessao.getAttribute("contasSelecionadas");
				//Adiciona as contas previamente selecionadas a um arraylist
				for(int i=0;i<idContasSelecionadas.size();i++){
					listContas.add(idContasSelecionadas.get(i));
				}
			}
			sessao.setAttribute("contasSelecionadas", listContas);
			pesquisarContaActionForm.setIdContas(null);
			
			httpServletRequest.setAttribute("configurarRetorno", true);
		}
		...
		if(idSituacaoConta != null && idSituacaoConta.length >0){
			...
			if(sessao.getAttribute("tipoSituacao") == null){
				...
				filtroConta.adicionarParametro(new ParametroSimples(FiltroConta.DEBITO_CREDITO_SITUACAO_ATUAL_ID,
						DebitoCreditoSituacao.PARCELADA,ParametroSimples.CONECTOR_OR,2 ));
				
				filtroConta.adicionarParametro(new ParametroSimples(FiltroConta.DEBITO_CREDITO_SITUACAO_ATUAL_ID,
						DebitoCreditoSituacao.ENTRADA_DE_PARCELAMENTO, ParametroSimples.CONECTOR_OR));
			}
		}
		...
		Collection<Conta> colecaoContasImovel = null; // = fachada.pesquisar(filtroConta,Conta.class.getName());

		Map<?, ?> resultado = controlarPaginacao(httpServletRequest, retorno,
				filtroConta, Conta.class.getName());
		//colecaoContasImovel = (Collection<Conta>) resultado.get("colecaoRetorno");
		colecaoContasImovel= fachada.pesquisar(filtroConta, Conta.class.getName());

		SistemaParametro sistemaParametro = 
			this.getFachada().pesquisarParametrosDoSistema();
		
		Iterator<Conta> contaIterator = colecaoContasImovel.iterator();
		while(contaIterator.hasNext()){
			...
			// caso contrário, remove a conta da colecao
