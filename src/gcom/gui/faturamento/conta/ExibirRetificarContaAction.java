...
public class ExibirRetificarContaAction extends GcomAction {
    ...
    public ActionForward execute(ActionMapping actionMapping,
        ...
        //informa que o exibirRetificarConta foi chamado pelo checkbox AlterarContaMes
        String selecionouAlterarContaMes = httpServletRequest.getParameter("selecionouAlterarContaMes");
        
        String idConta = null;
        
        if(httpServletRequest.getParameter("contaID") != null && selecionouAlterarContaMes == null){
        	...
        	retificarContaActionForm.setObservacao("");
        }else
        {
        	idConta = sessao.getAttribute("contaID").toString();
        }
        
        boolean imovelContratoDemanda = false;
        if(httpServletRequest.getParameter("idImovel") != null){
			...
			imovelContratoDemanda = this.pesquisarImovelContratoDemanda(idImovel , fachada);
       ...
       // Collection colecaoTiposClientes = null;
        ...
        boolean retificarContaImovelContratoDemanda = fachada.verificarPermissaoEspecialSolicitacaoAcesso(
        		PermissaoEspecial.RETIFICAR_CONTA_IMOVEL_CONTRATO_DEMANDA, usuarioLogado);
        ...
        if(sessao.getAttribute("colecaoMotivoRetificacaoConta") == null){
	    	...
	    	if(!retificarContaImovelContratoDemanda || !imovelContratoDemanda){
	    		filtroMotivoRetificacaoConta.adicionarParametro(new ParametroSimplesDiferenteDe(
	    				FiltroMotivoRetificacaoConta.ID, 
	    				ContaMotivoRetificacao.RETIFICACAO_IMOVEL_CONTRATO_DEMANDA));
	    	}
        ...
        /*******
         * Exibe a mensagem de substituicao do cliente se caso a situacao atual da conta a ser retificada corresponda a normal 
         * ou corresponda a retificada ou incluida e a retificacao ou inclusao ja tenha sido contabilizada
         * E se o cliente da conta for diferente do cliente do imovel. (mensagem exibida pelo jsp)
         * 
    	...
    	if ((contaSelecao.getDebitoCreditoSituacaoAtual().getId()
		.equals(DebitoCreditoSituacao.NORMAL))
		|| ((contaSelecao.getDebitoCreditoSituacaoAtual().getId().equals(
				DebitoCreditoSituacao.INCLUIDA) || contaSelecao
				.getDebitoCreditoSituacaoAtual().getId().equals(
						DebitoCreditoSituacao.RETIFICADA)) && Util
				.compararAnoMesReferencia(contaSelecao
						.getReferenciaContabil(), sistemaParametro
							.getAnoMesFaturamento(), "<"))) {
    		
	    	boolean clienteDiferente = this.getFachada().verificarClienteImovelDiferenteClienteConta(colecaoConta);
	    	sessao.setAttribute("clienteDiferente", clienteDiferente);
    	}else{
    		sessao.setAttribute("clienteDiferente", false);
    	}
        ...
        if (idConta != null && !idConta.equalsIgnoreCase("") &&
            ...
            /** Caso o usuario logado tenha permissao especial para alterar a conta e 
             * a conta a ser retificada e do mes de faturamento. Exibi o check box Alteracao Conta do Mes
             * 
             * Anderson Cabral
             * 01/11/2012 **/
            
            boolean temPermissaoAlterarContaMes = fachada.verificarPermissaoEspecial(PermissaoEspecial.ALTERAR_CONTA_MES_NO_RETIFICAR, usuario);
            
            if (temPermissaoAlterarContaMes && contaSelecao != null && sistemaParametro.getAnoMesFaturamento() <= contaSelecao.getReferencia() && contaSelecao.getDebitoCreditoSituacaoAtual().getId() == 0) {
            	sessao.setAttribute("exibirCheckBoxAlteracaoContaMes", true);
            	if(selecionouAlterarContaMes == null){
            		retificarContaActionForm.setAlterarContaMes("false");
            	}
            }else{
            	sessao.removeAttribute("exibirCheckBoxAlteracaoContaMes");
            }            
        ...
        if(sessao.getAttribute("colecaoCategoria") != null && selecionouAlterarContaMes == null)
        {
        ...
        if(sessao.getAttribute("colecaoSubcategoria") != null && selecionouAlterarContaMes == null)
        {
        ...
        if(sessao.getAttribute("colecaoDebitoCobrado") != null && selecionouAlterarContaMes == null)
        {
        ...
        if(sessao.getAttribute("colecaoCreditoRealizado") != null && selecionouAlterarContaMes == null)
        {
        ...
        if ( contaSelecao.getConsumoTarifa() != null && selecionouAlterarContaMes == null ){
        ...
        if (reloadPage != null && !reloadPage.equalsIgnoreCase("") && !reloadPage.equalsIgnoreCase("voltar")){
        	...
        	}else if(selecionouAlterarContaMes == null){
        ...
        habilitarCampos(retificarContaActionForm.getMotivoRetificacaoID(),httpServletRequest,sistemaParametro,temPermissaoParaRetificarContaNorma, retificarContaActionForm);
	...
	public void habilitarCampos(String idMotivoSelecionado,HttpServletRequest httpServletRequest,
			SistemaParametro sistemaParametro,boolean temPermissaoParaRetificarContaNorma, RetificarContaActionForm retificarContaActionForm){
		...
		if(sistemaParametro.getIndicadorNormaRetificacao().equals(ConstantesSistema.SIM)){
			...
			if(retificarContaActionForm.getAlterarContaMes() != null && retificarContaActionForm.getAlterarContaMes().equalsIgnoreCase("true")){
				habilitaListaCategoriasEQuantidadesEconomias = "2";
				habilitaListaDebitos = "2";
				habilitaListaCreditos = "2";
			}
	...
	/**
	 * Pesquisar se o imóvel possui um contrato de demanda não encerrado 
	 * 
	 * @author Davi Menezes
	 * @date 30/05/2013
	 * 
	 ...
	 * @return
	 */
	private boolean pesquisarImovelContratoDemanda(Integer idImovel, Fachada fachada){
		boolean imovelContratoDemanda = false;
		...
		filtroContratoDemandaImovel.adicionarParametro(new ParametroSimples(FiltroContratoDemandaImovelComercialIndustrial.ID_IMOVEL, idImovel));
		filtroContratoDemandaImovel.adicionarParametro(new ParametroNulo(FiltroContratoDemandaImovelComercialIndustrial.DATA_CONTRATO_ENCERRADO));
		
		Collection<?> colecaoContratoDemandaImovel = fachada.pesquisar(
		...
		if(Util.isVazioOrNulo(colecaoContratoDemandaImovel)){
			imovelContratoDemanda = false;
		}else{
			imovelContratoDemanda = true;
		}
		
		return imovelContratoDemanda;
	}
