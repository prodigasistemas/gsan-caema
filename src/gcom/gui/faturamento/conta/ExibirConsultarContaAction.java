...
public class ExibirConsultarContaAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if (tipoConsulta.equalsIgnoreCase("conta")) {
			...
			// vai ser recuperado na geração do relatório 2Âª Via da Conta
			...
			if (idConta != null && !idConta.equalsIgnoreCase("")) {
		        ...
		        /*******
		         * Exibe a mensagem de substituicao do nome do cliente se o cliente da conta for diferente do cliente do imovel. 
		         * (mensagem exibida pelo jsp)
		         * 
		         * [UC0482] - Emitir Segunda Via Conta
	            ...
	            Collection colecaoClienteConta = this.getFachada().pesquisar(filtroClienteConta, ClienteConta.class.getName());
	                  
	            if(colecaoClienteConta != null && colecaoClienteConta.size() > 0){        	
	            	clienteConta = (ClienteConta) colecaoClienteConta.iterator().next();
	            }
	            
	            clienteImovel = this.getFachada().pesquisarClienteImovel(conta.getImovel().getId().toString(), 1);          
		    	...
		    	sessao.setAttribute("idClienteImovel", clienteImovel.getCliente().getId());				
		    	sessao.setAttribute("idClienteConta", clienteConta.getCliente().getId());
			...
			if (idContaHistorico != null && !idContaHistorico.equalsIgnoreCase("")) {
				...
				filtroContaHistorico.adicionarCaminhoParaCarregamentoEntidade("registroAtendimento");
