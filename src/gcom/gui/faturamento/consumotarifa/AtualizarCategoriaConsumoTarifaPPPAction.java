 ...
 */
public class AtualizarCategoriaConsumoTarifaPPPAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		ActionForward retorno = actionMapping
				.findForward("exibirAtualizarCategoriaConsumoTarifaPPP");

		Fachada fachada = Fachada.getInstancia();

		AtualizarCategoriaConsumoTarifaPPPActionForm form = (AtualizarCategoriaConsumoTarifaPPPActionForm) actionForm;
		...
		while (iteratorColecaoFaixa.hasNext()) {
			ConsumoTarifaFaixaPpp consumoTarifaFaixaPpp = (ConsumoTarifaFaixaPpp) iteratorColecaoFaixa
					.next();
			String parametroConsumoTarifaFaixa = "valorConsumoTarifa"
					+ consumoTarifaFaixaPpp.getUltimaAlteracao().getTime();
			consumoTarifaFaixaPpp.setValorConsumoTarifa(Util
		...
		ConsumoTarifaCategPpp consumoTarifaCategPpp = new ConsumoTarifaCategPpp();

		// Categoria
		consumoTarifaCategPpp.setCategoria(categoriaSelected);
		
		// seta a subcategoria zerada
		Subcategoria subcategoria = new Subcategoria();
		subcategoria.setId(0);
		consumoTarifaCategPpp.setSubCategoria(subcategoria);

		// Consumo mínimo
		consumoTarifaCategPpp.setNumeroConsumoMinimo(new Integer(
				form.getConsumoMinimo()));

		// Tarifa mínima
		consumoTarifaCategPpp
				.setValorTarifaMinima(Util
						.formatarMoedaRealparaBigDecimal(form
								.getValorTarifaMinima()));

		// Ultima alteração
		consumoTarifaCategPpp.setUltimaAlteracao(new Date());
		...
		if ((colecaoFaixa != null) && (!colecaoFaixa.isEmpty())) {
			...
			while (colecaoFaixaIt.hasNext()) {
				ConsumoTarifaFaixaPpp consumoTarifaFaixa = (ConsumoTarifaFaixaPpp) colecaoFaixaIt
						.next();
				if(new Integer(form.getConsumoMinimo()).intValue() > consumoTarifaFaixa.getNumeroConsumoFaixaFim().intValue() ){
					throw new ActionServletException(
		...
		while (colecaoFaixaIterator.hasNext()) {
			ConsumoTarifaFaixaPpp consumoTarifaFaixa = (ConsumoTarifaFaixaPpp) colecaoFaixaIterator
					.next();
			consumoTarifaFaixa
					.setConsumoTarifaCategPpp(consumoTarifaCategPpp);
		}
		...
		if (sessao.getAttribute("colecaoCategoria") != null) {
			...
			CategoriaFaixaConsumoTarifaPppHelper consumoTarifaHelper = new CategoriaFaixaConsumoTarifaPppHelper(
					numeroFaixasCategoria, consumoTarifaCategPpp,
			...
			if (colecaoCategoria.contains(consumoTarifaHelper)) {
				...
				if (novaCategoria != null && novaCategoria.equals("sim") && !"sim".equals(trava)) {
					...
					while (iteratorColecaoCategoria.hasNext()) {
						CategoriaFaixaConsumoTarifaPppHelper helper = (CategoriaFaixaConsumoTarifaPppHelper) iteratorColecaoCategoria
								.next();
						if (helper.getConsumoTarifaCategPpp().getCategoria()
								.getId().equals(
										consumoTarifaHelper
												.getConsumoTarifaCategPpp()
												.getCategoria().getId())) {
			...
			colecaoCategoria.add(new CategoriaFaixaConsumoTarifaPppHelper(
					numeroFaixasCategoria, consumoTarifaCategPpp,
					(Collection) sessao.getAttribute("colecaoFaixa")));
