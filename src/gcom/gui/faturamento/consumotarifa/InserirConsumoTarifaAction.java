...
public class InserirConsumoTarifaAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		String indicadorContratoDemanda = inserirConsumoTarifaActionForm.getIndicadorContratoDemanda();
		...
		if(dataVigencia!=null && !dataVigencia.equals("")){
			
			String anoMesFaturamentoDataVigencia = dataVigencia.substring(6,10)+dataVigencia.substring(3,5);
			
			fachada.verificarAnoMesDataVigenciaReferenciaFaturamentoCorrente(anoMesFaturamentoDataVigencia);
		}
		
		consumoTarifa.setIndicadorContratoDemanda(new Short (indicadorContratoDemanda));
		...
		String habilitarVoltar = (String) sessao.getAttribute("habilitarVoltar");
		...
		if (consumoTarifa.getDescricao() == null) {
			...
			if(habilitarVoltar == null || habilitarVoltar.equals("")){
				...
				if(habilitarVoltar.equals("1")){
					montarPaginaSucesso(httpServletRequest, consumoSelect
							...
							"exibirManterConsumoTarifaExistenteAction.do?idVigencia="
									+ consumoTarifaVigencia.getId().toString(),
							"Atualizar Tarifa de Consumo Inserida", 
							"Inserir Contrato de Demanda", 
							"exibirInserirContratoDemandaAction.do?menu=sim");
				}else if(habilitarVoltar.equals("2")){
					montarPaginaSucesso(httpServletRequest, consumoSelect
							.getDescricao()
							+ " de vigência "
							+ gcom.util.Util.formatarData(consumoTarifaVigencia
							...
							"exibirAtualizarContratoDemandaAction.do?menu=sim");
				}
			}
