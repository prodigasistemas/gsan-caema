...
public class EncerrarOrdemServicoAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
					...
					if (indicadorExecucao == AtendimentoMotivoEncerramento.INDICADOR_EXECUCAO_SIM) {
						...
						if (retorno.getName().equalsIgnoreCase("telaSucesso")) {
							...
							if (encerrarOrdemServicoActionForm.getTipoServicoReferenciaId() == null
								...
								//pesquisar pela ordem de serviço informada
		...
		if (retorno.getName().equalsIgnoreCase("telaSucesso")) {	
			...
			if(sessao.getAttribute("tipoSubstituicao")!=null){
				...
				montarPaginaSucesso(httpServletRequest, "Substituição de Hidrômetro para "+ sessao.getAttribute("tipoMedicaoAtual").toString() +
					" no imóvel "+sessao.getAttribute("matriculaImovel").toString()+ " efetuada com sucesso.", 
					"Efetuar outra Substituição de Hidrômetro", "exibirEfetuarSubstituicaoHidrometroAction.do",
					"gerarOrdemServicoRemocaoHidrometroAction.do?servicoTipo="+ServicoTipo.TIPO_EFETUAR_REMOCAO_HIDROMETRO, "Gerar OS de Remoção de Hidrômetro",
					"Gerar OS de Instalação da Caixa de Proteção", "gerarOrdemServicoInstalacaoCaixaProtecaoHidrometroAction.do?" +
    ...
    private Date validaDataHoraExecucao(EncerrarOrdemServicoActionForm encerrarOrdemServicoActionForm, Integer numeroOS, Usuario usuarioLogado){
		...
		if(atendimentoMotivoEncerramento.getIndicadorExecucao() == AtendimentoMotivoEncerramento.INDICADOR_EXECUCAO_NAO){
			throw new ActionServletException("atencao.msg_personalizada", "Não é possível informar data e hora de execução para o motivo de encerramento selecionado");
		}
		...
		if(encerrarOrdemServicoActionForm.getDataExecucao() == null || encerrarOrdemServicoActionForm.getDataExecucao().equals(""))
			...
			if(dataTeste.before(dataOs))
				throw new ActionServletException("atencao.msg_personalizada", "Não é permitido informar Data de Execução anterior a Data da Geração da OS");
