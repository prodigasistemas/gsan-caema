...
//		  ------  O servico Tipo foi encontrado
		...
		String indicativoObrigatoriedadeAtividade = atualizarTipoServicoActionForm.getIndicativoObrigatoriedadeAtividade();
		...
		String novaEtapa = atualizarTipoServicoActionForm.getIndicadorNovaEtapa();
		String indicadorServicoCobranca = atualizarTipoServicoActionForm.getIndicadorServicoCobranca();
		String indicadorGerarResumo = atualizarTipoServicoActionForm.getIndicadorGerarResumo();		
		 ...
		 * [RM6643]
		 */
		String indicadorTipoServicoMicromedicao = atualizarTipoServicoActionForm.getIndicadorTipoServicoMicromedicao();
		
		ServicoTipo servicoTipo = new ServicoTipo();
		
		if(atualizarTipoServicoActionForm.getConstanteFuncionalidadeTipoServico() != null && 
				!atualizarTipoServicoActionForm.getConstanteFuncionalidadeTipoServico().equals("")){
			servicoTipo.setConstanteFuncionalidadeTipoServico(Integer.parseInt(
				atualizarTipoServicoActionForm.getConstanteFuncionalidadeTipoServico()));
		}		
		...
		if(valor != null && !valor.trim().equals("")){
			servicoTipo.setValor(new BigDecimal(valor.replace(".", "").replace(",", ".")));
		}
		...
		servicoTipo.setIndicadorNovaEtapa(new Short(novaEtapa));
		servicoTipo.setIndicadorServicoCobranca(new Short(indicadorServicoCobranca));
		servicoTipo.setIndicadorGerarResumo(new Short(indicadorGerarResumo));
		 ...
		 * [RM6643]
		 */
		servicoTipo.setIndicadorTipoServicoMicromedicao(new Short(indicadorTipoServicoMicromedicao));
		...
		if (atualizarTipoServicoActionForm.getServicoTipoAtividades() != null)	{
			
			Collection colecaoServicoTipoAtividade = new ArrayList();			
			colecaoServicoTipoAtividade = atualizarTipoServicoActionForm.getServicoTipoAtividades();	
			Collection colecaoTipoAtividade = new ArrayList(); 
			Iterator<?> it = colecaoServicoTipoAtividade.iterator();
			while(it.hasNext()){
				ServicoTipoAtividade servico = new ServicoTipoAtividade();				
				servico = (ServicoTipoAtividade) it.next();
				servico.getAtividade().setIndicadorAtividadeUnica(new Short(atualizarTipoServicoActionForm.getIndicadorAtividadeUnica()));
				colecaoTipoAtividade.add(servico);
			}			
				if(!Util.isVazioOrNulo(colecaoTipoAtividade)){
					servicoTipo.setServicoTipoAtividades(colecaoTipoAtividade);		
				}
		...
		servicoTipo.setIndicadorNovaEtapa(Short.parseShort(novaEtapa));
		
		
		if(atualizarTipoServicoActionForm.getIndicadorAtividadeUnica().equals("2")
				&& Util.isVazioOrNulo(atualizarTipoServicoActionForm.getServicoTipoAtividades())){
			
			throw new ActionServletException("atencao.tipo_servico_adicionado");
			
		}
		
		Collection<ServicoPerfilTipo> colecaoServicoPerfilTipo = (Collection<ServicoPerfilTipo>) sessao.getAttribute("colecaoServicoPerfilTipo");
		
		if (colecaoServicoPerfilTipo == null || colecaoServicoPerfilTipo.isEmpty()){
			throw new ActionServletException("atencao.campo.informado", null, "Perfil do Tipo de Serviço");
		}
		
		servicoTipo.setColecaoServicoPerfilTipo(colecaoServicoPerfilTipo);
