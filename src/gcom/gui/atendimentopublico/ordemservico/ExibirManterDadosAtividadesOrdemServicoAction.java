...
public class ExibirManterDadosAtividadesOrdemServicoAction extends GcomAction{
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		Collection colecaoAtiv = (Collection) sessao.getAttribute("colecaoAtividade");
		//Concluir Manutenção
		...
		if (concluirManutencao != null && !concluirManutencao.equalsIgnoreCase("")){
			...
			else if (colecaoAtiv.size() != 0){

				throw new ActionServletException("atencao.preencher_material_horas");
			} else {
				
				throw new ActionServletException("atencao.atividade_nao_informada");
			}
