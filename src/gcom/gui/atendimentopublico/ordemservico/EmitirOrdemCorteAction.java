	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if(ordemServico != null){
			...
			if(ordemServico.getCobrancaDocumento() == null){				
				...
				cobrancaDocumento = fachada.gerarDocumentoDeCobranca(ordemServico, fachada, usuario);
				...
				Collection colecaoCobranca = fachada.pesquisar(filtroCobrancaDocumento, 
					CobrancaDocumento.class.getName());
				
				if(!Util.isVazioOrNulo(colecaoCobranca)){
					cobrancaDocumento = (CobrancaDocumento) Util.retonarObjetoDeColecao(colecaoCobranca);
				}
			}
		}		
		...
		String enderecoClienteResponsavel = fachada.pesquisarEnderecoClienteAbreviado(imovel.getId());		
		...
		String clienteNome = fachada.obterNomeCliente(new Integer(form.getMatriculaImovel()));
		...
		relatorioOrdemCorteOnline.addParametro("anoMesfaturamentoGrupo",form.getAnoMesfaturamentoGrupo());
		...
		relatorioOrdemCorteOnline.addParametro("localidade", localidade);
		relatorioOrdemCorteOnline.addParametro("cobrancaDocumento", cobrancaDocumento);			
		relatorioOrdemCorteOnline.addParametro("clienteNome", clienteNome);
