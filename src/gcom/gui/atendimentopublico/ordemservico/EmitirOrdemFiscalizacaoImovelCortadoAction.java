 ...
 * de forma individual para uma colecao de imóvel.
 * 
 * @author Arthur Carvalho
 * @data 24/02/2012
 *
 */
public class EmitirOrdemFiscalizacaoImovelCortadoAction extends ExibidorProcessamentoTarefaRelatorio {
	/**
	 * @param actionMapping
	 * @param actionForm
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		FiltrarImovelCortadoActionForm form =
			(FiltrarImovelCortadoActionForm) actionForm;
		...
		Collection<EmitirOrdemFiscalizacaoForm> colecaoDados = null;
		colecaoDados = new ArrayList<EmitirOrdemFiscalizacaoForm>();
		String[] ids = form.getIdRegistro();
		if ( ids != null && ids.length > 0 ) {
			for (int i = 0; i < ids.length; i++) {
				
				EmitirOrdemFiscalizacaoForm emitirform = new EmitirOrdemFiscalizacaoForm();
				emitirform.setMatriculaImovel(String.valueOf(ids[i]));
				
				this.getImovel(emitirform);
				this.pesquisarValores(emitirform,fachada);
				this.pesquisarEconomias(emitirform,fachada);
				this.pesquisarDadosImovel(emitirform,fachada);	 
				
				colecaoDados.add(emitirform);
			}
		} else {
				// FS0010 - verifica imóvel selecionado
				throw new ActionServletException(
						"Nenhum imóvel selecionado");
		}
		relatorio.addParametro("colecaoForm", colecaoDados);
	...
	private void pesquisarDadosImovel(EmitirOrdemFiscalizacaoForm form, Fachada fachada) {
	...
	private void pesquisarEconomias(EmitirOrdemFiscalizacaoForm form, Fachada fachada) {
	...
	private void pesquisarValores(EmitirOrdemFiscalizacaoForm form, Fachada fachada) {
	...
	private void validarExistenciaOs(EmitirOrdemFiscalizacaoForm form, Fachada fachada){
		
			ServicoTipo servicoTipo = 
				fachada.recuperaServicoTipoPorConstante(
						ServicoTipo.FISCALIZACAO_DE_CORTE);
	...
	private void getImovel(EmitirOrdemFiscalizacaoForm form) {
			...
			if (colecaoImovel != null && !colecaoImovel.isEmpty()) {		
				...
				this.validarExistenciaOs(form,fachada);
