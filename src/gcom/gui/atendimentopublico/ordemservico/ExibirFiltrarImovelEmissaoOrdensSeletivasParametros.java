...
//			}	
		...
		if (objetoConsulta != null && !objetoConsulta.trim().equalsIgnoreCase("") &&
			...
			switch (Integer.parseInt(objetoConsulta)) {
				...
				//Localicade Component
				case 6:
					pesquisarLocalidadeComponent(imovelEmissaoOrdensSeletivas, fachada, httpServletRequest);
					break;
				default:
					break;
			}
		...
		if (httpServletRequest.getParameter("pesquisaLocalidade") != null 
			...
			String localidade = imovelEmissaoOrdensSeletivas.getIdLocalidade();
			if(localidade !=null && !localidade.trim().equals("")){
				...
				if (colecaoLocalidade != null && !colecaoLocalidade.isEmpty()) {
					...
					imovelEmissaoOrdensSeletivas.setIdLocalidade("");
					imovelEmissaoOrdensSeletivas.setNomeLocalidade("");					
					...
					if(localidades !=null && !localidades.isEmpty()){
						...
						if(!localidades.contains(loca)){
							...
							throw new ActionServletException("atencao.msg_personalizada", "Localidade já foi adicionada");
						}
					}
					else{
		...
		if (httpServletRequest.getParameter("removeLocalidade") != null 
			...
			if(httpServletRequest.getParameter("tudo") !=null && httpServletRequest.getParameter("tudo").equalsIgnoreCase("SIM")){
				imovelEmissaoOrdensSeletivas.setSetoresSelecionadosComponent("");
				imovelEmissaoOrdensSeletivas.setIndiceSetoresSelecionadosComponent("");
				imovelEmissaoOrdensSeletivas.setDigitados("");				
				...
				int ind = new Integer(httpServletRequest.getParameter("ind"));
				ArrayList<Integer> indsSet = new ArrayList<Integer>();
				ArrayList<Localidade> localidades = (ArrayList<Localidade>) 
						httpServletRequest.getSession().getAttribute("colecaoLocalidadesContasEmCobrancaPorEmpresa");
				ArrayList<SetorComercial> setores = (ArrayList<SetorComercial>) 
						httpServletRequest.getSession().getAttribute("colecaoSetoresContasEmCobrancaPorEmpresa");
				
				Localidade loc = localidades.get(ind);
				for(int i = 0; i < setores.size(); i++){
					if(setores.get(i).getLocalidade().equals(loc))
						indsSet.add(i);
				}
				for(Integer indSet : indsSet){
					setores.remove(indSet);
				}
				localidades.remove(ind);
				
				String [] tempSet = imovelEmissaoOrdensSeletivas.getSetoresSelecionadosComponent().split("::");
				String [] tempIndSet = imovelEmissaoOrdensSeletivas.getIndiceSetoresSelecionadosComponent().split("::");
				String[] digitados = imovelEmissaoOrdensSeletivas.getDigitados().split("::");
				ArrayList<String> set = new ArrayList<String>();
				ArrayList<String> indSet = new ArrayList<String>();
				String setor = new String();
				String indices = new String();
				String digitadosVai = new String();
				
				for(int i = 0; i < tempSet.length; i++){
					set.add(tempSet[i]);
				}						
				for(int i = 0; i < tempIndSet.length; i++){
					indSet.add(tempIndSet[i]);
				}
				set.remove(0);
				indSet.remove(0);
				for(int i = 0;  i < set.size(); i++){
					String[] vt2 = set.get(i+1).split(",");
					if(!set.get(i).equals("") && new Integer(set.get(i)).compareTo(loc.getId()) != 0){
						setor += "::"+set.get(i)+"::"+vt2[0]+",";					
					}
					if(digitados.length > 0){
						for(int j = 0; j < digitados.length; j++){
							String[] dadosTemp = digitados[j].split("_");
							if(!digitados[j].equals("")){
								if(new Integer(dadosTemp[1]).compareTo(loc.getId()) != 0 && 
								   new Integer(dadosTemp[2]).compareTo(new Integer(vt2[0])) != 0){
									...
									digitadosVai +="::"+nome+term+"_"+dadosTemp[4];															
								}
							}
						}
					}
				...
				for(int i = 0;  i < indSet.size(); i++){
					String[] vt2 = indSet.get(i+1).split(",");
					if(!indSet.get(i).equals("") && new Integer(indSet.get(i)).compareTo(loc.getId()) != 0){
						indices += "::"+indSet.get(i)+"::"+vt2[0]+",";					
					}
					i++;
					if(i >= indSet.size()){
						break;
				...
				imovelEmissaoOrdensSeletivas.setSetoresSelecionadosComponent(setor);
				imovelEmissaoOrdensSeletivas.setIndiceSetoresSelecionadosComponent(indices);
				imovelEmissaoOrdensSeletivas.setDigitados(digitadosVai);
	...
	private void pesquisarLocalidadeComponent(ImovelEmissaoOrdensSeletivasActionForm imovelEmissaoOrdensSeletivas,
			Fachada fachada, HttpServletRequest httpServletRequest){
		
		String idLocalidade =  imovelEmissaoOrdensSeletivas.getIdLocalidade();
		...
		if(idLocalidade !=null && !idLocalidade.trim().equals("")){
			...
			if (colecaoLocalidade != null && !colecaoLocalidade.isEmpty()) {
				...
				imovelEmissaoOrdensSeletivas.setIdLocalidade(localidade.getId().toString());
				imovelEmissaoOrdensSeletivas.setNomeLocalidade(localidade.getDescricao());
			}
			else {
				imovelEmissaoOrdensSeletivas.setIdLocalidade("");
				imovelEmissaoOrdensSeletivas.setNomeLocalidade("LOCALIDADE INEXISTENTE");
