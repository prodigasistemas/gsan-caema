...
public class GerarOrdemServicoAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if(ordemServico.getServicoTipo()!=null 
			...
			if(ordemServico.getServicoTipo().getIndicadorPavimento() == ConstantesSistema.SIM.shortValue()){
				
				if (form.getIdPavimentoRua() == null ||
						form.getIdPavimentoRua().equals("") ||
							form.getIdPavimentoRua().equals(ConstantesSistema.NUMERO_NAO_INFORMADO+"")) {
					...
					pavimentoRua.setId(Integer.parseInt(form.getIdPavimentoRua()));
					ordemServicoPavimento.setPavimentoRua(pavimentoRua);
				}
				
				if(form.getMetragemPavimentoRua().equals("")){
					throw new ActionServletException(
							"atencao.campo_texto.obrigatorio", null,
							"Metragem");
				}else{
					BigDecimal metragemPavimentoRua = Util.formatarStringParaBigDecimal(form.getMetragemPavimentoRua().replaceAll(",", "."));
		        	if(metragemPavimentoRua == null){
		        		throw new ActionServletException("atencao.metragem_pavimento_rua_invalida");
		        	}
					
		        	ordemServicoPavimento.setAreaPavimentoRua(metragemPavimentoRua);
				}
				
				if (form.getIdUnidadeRepavimentadora() == null ||
						form.getIdUnidadeRepavimentadora().equals("") ||
							form.getIdUnidadeRepavimentadora().equals(ConstantesSistema.NUMERO_NAO_INFORMADO+"")) {
		...
		Integer idOrdemServico = fachada.gerarOrdemServico(ordemServico, usuario, null);
		...
		if(ordemServicoPavimento!=null){
			...
			ordemServicoPavimento.setDataGeracao(dataAtual);
			ordemServicoPavimento.setUltimaAlteracao(dataAtual);
			fachada.inserir(ordemServicoPavimento);
			os = null;
		}
