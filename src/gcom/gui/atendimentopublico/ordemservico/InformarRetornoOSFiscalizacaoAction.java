...
public class InformarRetornoOSFiscalizacaoAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if (retorno.getName().equalsIgnoreCase("telaSucesso")) {
			...
			ExecucaoOSPK execucaoOSPK = null;
			Integer idArquivoSmartphone = null;
			
			if (veioEncerrarSmartphone != null && veioEncerrarSmartphone.booleanValue()) {
				idArquivoSmartphone = (Integer) sessao.getAttribute("idArquivo");
				
				ArquivoTextoOSCobranca arquivoTextoOSCobranca = new ArquivoTextoOSCobranca();
				arquivoTextoOSCobranca.setId(idArquivoSmartphone);
				
				OrdemServico os = new OrdemServico();
				os.setId(Util.converterStringParaInteger(informarRetornoOSFiscalizacaoActionForm.getIdOrdemServico()));
				
				execucaoOSPK = new ExecucaoOSPK();
				execucaoOSPK.setArquivoTextoOSCobranca(arquivoTextoOSCobranca);
				execucaoOSPK.setOrdemServico(os);
				
				caminhoRetorno = "exibirConsultarOrdemServicoCobrancaSmartphoneAction.do?arquivoTexto=" + idArquivoSmartphone + "&idTipoOrdemServico=1";
			} else {
				caminhoRetorno = "/exibirInformarRetornoOSFiscalizacaoAction.do?menu=sim";
			}
			...
			if (!informarRetornoOSFiscalizacaoActionForm
				...
				if (informarRetornoOSFiscalizacaoActionForm.getAtendimentoMotivoEncerramento() != null) {
					...
					if (!informarRetornoOSFiscalizacaoActionForm.getAtendimentoMotivoEncerramento().equalsIgnoreCase("-1")) {
						...
						if (veioEncerrarSmartphone != null && veioEncerrarSmartphone.booleanValue()) {
							this.getFachada().atualizarSituacaoOSSmartphone(idArquivoSmartphone, numeroOS, OrdemServico.SITUACAO_ENCERRADO);
						}
