...
//					httpServletRequest.setAttribute("nomeBotao2", "Não");
			...
			if (encerrarOrdemServicoActionForm.getIndicadorExecucao() != null
				...
				Date dataEncerramento = Util.converteStringParaDateHora(encerrarOrdemServicoActionForm.getDataEncerramento());
				...
				if (indicadorExecucao == AtendimentoMotivoEncerramento.INDICADOR_EXECUCAO_NAO) {
					...
					if (indicadorExecucao == AtendimentoMotivoEncerramento.INDICADOR_EXECUCAO_SIM) {
						//[FS0014] Validar Hora de Execução.
						//[FS0015] Validar Data de Execução.
...
//								[SB0002] - Encerrar com execução e sem referência
								fachada.encerrarOSComExecucaoSemReferencia(
										...
										encerrarOrdemServicoActionForm.getCodigoRetornoVistoriaOs(),
										ordemServicoPavimento, ordemServicoBoletim,indicadorServicoAceito, dataExecucao,
								...
								fachada.encerrarOSComExecucaoComReferencia(
										...
										encerrarOrdemServicoActionForm.getIndicadorVistoriaServicoTipo(),
										encerrarOrdemServicoActionForm.getCodigoRetornoVistoriaOs(),
										ordemServicoBoletim,indicadorServicoAceito, dataExecucao, colecaoOrdemServicoFoto);
			...
			if (retorno.getName().equalsIgnoreCase("encerrarOrdemServicoPopup")) {
				...
				if(servicoTipo.getIndicadorEncAutomaticoRAQndEncOS().compareTo(ConstantesSistema.NAO) == 0){
					...
					//if (encerrarOrdemServicoActionForm.getHoraEncerramento() != null && !encerrarOrdemServicoActionForm.getHoraEncerramento().equals("")) {
						registroAtendimento.setDataEncerramento(Util.converteStringParaDateHora(encerrarOrdemServicoActionForm.getDataEncerramento()));
					/*} else {
     ...
     */
	 ...
	 private Date validaDataHoraExecucao(EncerrarOrdemServicoActionForm encerrarOrdemServicoActionForm, Integer numeroOS, Usuario usuarioLogado){
			...
			if(encerrarOrdemServicoActionForm.getHoraExecucao() == null || encerrarOrdemServicoActionForm.getHoraExecucao().equals(""))
				throw new ActionServletException("atencao.msg_personalizada", "Hora da Execução deve ser informada");						
			else{
				...
				if((hora.compareTo(0) < 0 && hora.compareTo(24) > 0) && (min.compareTo(0) < 0 && min.compareTo(24) > 0))
					throw new ActionServletException("atencao.msg_personalizada", "Hora da Execução inválida");
			}
			...
			if(encerrarOrdemServicoActionForm.getDataExecucao() == null || encerrarOrdemServicoActionForm.getDataExecucao().equals(""))
				throw new ActionServletException("atencao.msg_personalizada", "Data de Execução deve ser informada");
			else{									
				OrdemServico osConsultada = fachada.consultarDadosOrdemServico(numeroOS);
				SistemaParametro sistemaParametro = fachada.pesquisarParametrosDoSistema();
				Long timeCompareEncOs = osConsultada.getDataGeracao().getTime() - sistemaParametro.getNumeroDiasEncerramentoOrdemServico();
				Long timeCompareEncOsSel = osConsultada.getDataGeracao().getTime() - sistemaParametro.getNumeroDiasEncerramentoOSSeletiva();
				if(!Util.verificaSeDataValida(encerrarOrdemServicoActionForm.getDataExecucao(), "dd/MM/yyyy"))
					throw new ActionServletException("atencao.msg_personalizada", "Data de Execução inválida");
				...
				if(dataExecucao.after(new Date()))
					throw new ActionServletException("atencao.msg_personalizada", "Data de Execução maior que a data corrente");
				
				if(timeCompareEncOs.compareTo(dataExecucao.getTime()) > 0)
					throw new ActionServletException("atencao.msg_personalizada", "Data de Execução anterior ao permitido");
				
				if(osConsultada.getRegistroAtendimento() == null && osConsultada.getCobrancaDocumento() == null &&
				   timeCompareEncOsSel.compareTo(dataExecucao.getTime()) > 0 && 
				   !fachada.verificarPermissaoEspecial(PermissaoEspecial.ENCERRAR_ORDEM_SERVICO_SEM_ATIVIDADES,usuarioLogado))										
					throw new ActionServletException("atencao.msg_personalizada", "Data de Execução anterior ao permitido para OS seletiva");
			}
			return dataExecucao;
	    }
