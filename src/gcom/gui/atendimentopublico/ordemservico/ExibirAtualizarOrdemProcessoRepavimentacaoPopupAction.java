...
public class ExibirAtualizarOrdemProcessoRepavimentacaoPopupAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if(httpServletRequest.getParameter("anexo") != null && !httpServletRequest.getParameter("anexo").equals("")){
        ...
        // Salva na sessao para ser utilizado pelo botao de voltar na tela de anexar foto
        sessao.setAttribute("manterPaginaAux",form.getManterPaginaAux());
		...
		form.setIdRA(os.getRegistroAtendimento().getId().toString());
		form.setIdOrdemServico(os.getId().toString());
		
		String idUnidadeRepavimentadora = (String) sessao.getAttribute("idUnidadeRepavimentadora");
		if(idUnidadeRepavimentadora != null){
			form.setIdUnidadeRepavimentadora(idUnidadeRepavimentadora);
		}
		
		this.pesquisarUnidadeRepavimentadora(fachada, httpServletRequest, usuarioLogado);
		...
		if (sessao.getAttribute("collOrdemServicoPavimento") != null
			...
			if (httpServletRequest.getParameter("page.offset") != null 
	        	...
	        	sessao.setAttribute("page.offset", numeroPagina);
			}else{
				String numeroPagina = (String) sessao.getAttribute("page.offset");
				form.setManterPaginaAux(numeroPagina);
			}
	...
	private void pesquisarUnidadeRepavimentadora(Fachada fachada, HttpServletRequest request, Usuario usuarioLogado){
		...
		if(unidadeOrganizacional.getUnidadeTipo() != null && 
			...
			request.setAttribute("desabilitaOutrosServicos", false);
		}else{
			request.setAttribute("desabilitaOutrosServicos", true);
		}
	}
}
