...
public class FiltrarRegistroAtendimentoDevolucaoValoresAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		Short codigoTipoDevolucao = null;
		if (filtrarGuiaDevolucaoActionForm.getIdSolicitacaoTipoEspecificacao() != null
				&& !filtrarGuiaDevolucaoActionForm.getIdSolicitacaoTipoEspecificacao().trim().equals("")
				&& !filtrarGuiaDevolucaoActionForm.getIdSolicitacaoTipoEspecificacao().trim().equals("-1")) {
			...
			filtroSolicitacaoTipoEspecificacao.adicionarParametro(new ParametroSimples(FiltroSolicitacaoTipoEspecificacao.ID, 
				filtrarGuiaDevolucaoActionForm.getIdSolicitacaoTipoEspecificacao()));
			...
			//Caso o tipo de especificação selecionado seja com indicador de Pagamento em duplicidade (STEP_ICINFORMARPGTODUPLICIDADE = 1) colocar valor 1
			if (solicitacaoTipoEspecificacao.getIndicadorInformarPagamentoDuplicidade() != null
					&& solicitacaoTipoEspecificacao.getIndicadorInformarPagamentoDuplicidade()
						.compareTo(new Short("1")) == 0) {
				codigoTipoDevolucao = new Short(ConstantesSistema.DEVOLUCAO_PAGAMENTO_EM_DUPLICIDADE);
				
			//Caso contrário, ou seja, o tipo de especificação selecionado seja com indicador Pagamento Valor a Maior (STEP_ICINFORMARDOCPGTOAMAIOR =1) colocar valor 2
			} else if (solicitacaoTipoEspecificacao.getIndicadorInformarDocumentoPagamentoAMaior() != null
					&& solicitacaoTipoEspecificacao.getIndicadorInformarDocumentoPagamentoAMaior()
						.compareTo(new Short("1")) == 0) {
				codigoTipoDevolucao = new Short(ConstantesSistema.DEVOLUCAO_PAGAMENTO_VALOR_A_MAIOR);
			
			//Caso contrário, ou seja, o tipo de especificação selecionado seja com indicador Pagamento Indevido (STEP_ ICINFORMARDOCPGTOINDEVIDO =1) colocar valor 3)
			} else if (solicitacaoTipoEspecificacao.getIndicadorInformarDocumentoPagamentoIndevido() != null
					&& solicitacaoTipoEspecificacao.getIndicadorInformarDocumentoPagamentoIndevido()
						.compareTo(new Short("1")) == 0) {
				codigoTipoDevolucao = new Short(ConstantesSistema.DEVOLUCAO_PAGAMENTO_INDEVIDO);
			}
			...
			throw new ActionServletException(
				"atencao.campo_selecionado.obrigatorio", "Tipo de Especificação");
		}
		
		filtrarGuiaDevolucaoActionForm.setDescricaoTipoSolicitacao(
			solicitacaoTipoEspecificacao.getDescricao());
		filtrarGuiaDevolucaoActionForm.setCodigoTipoDevolucao(
			codigoTipoDevolucao.toString());
		...
		if(filtrarGuiaDevolucaoActionForm.getNumeroRA() != null && !filtrarGuiaDevolucaoActionForm.getNumeroRA().equals("")){
			...
			Collection<RegistroAtendimento> colecaoRegistroAtendimento = fachada
			...
			if(registroAtendimento.getSolicitacaoTipoEspecificacao() != null
					...
					&& registroAtendimento.getSolicitacaoTipoEspecificacao().getIndicadorInformarDocumentoPagamentoAMaior().equals(ConstantesSistema.NAO) 
					&& registroAtendimento.getSolicitacaoTipoEspecificacao().getIndicadorInformarDocumentoPagamentoIndevido().equals(ConstantesSistema.NAO)){
		...
		helper.setCodigoTipoDevolucao(codigoTipoDevolucao);
