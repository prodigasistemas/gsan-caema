...
public class FiltrarRegistroAtendimentoAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		Collection<Integer> colecaoSolicitacaoTipoGrupo = new ArrayList();
		if(filtrarRegistroAtendimentoActionForm.getGrupoAtendimento() != null && 
			...
			String [] tipoGrupo = filtrarRegistroAtendimentoActionForm.getGrupoAtendimento();
			for(int i = 0; i < tipoGrupo.length; i++) {
				if(new Integer(tipoGrupo[i]).intValue() != ConstantesSistema.NUMERO_NAO_INFORMADO) {
					colecaoSolicitacaoTipoGrupo.add(new Integer(tipoGrupo[i]));
					//passar a coleção de tipo grupo por parâmetro
					parametroInformado = true;
				}
			}
		}
		...
		if(colecaoSolicitacaoTipoGrupo.size() < 2 && 
				filtrarRegistroAtendimentoActionForm.getTipoSolicitacao() != null && 
		...
		if (parametroInformado) {
			...
			filtroRA.setColecaoTipoSolicitacaoGrupo(colecaoSolicitacaoTipoGrupo);
