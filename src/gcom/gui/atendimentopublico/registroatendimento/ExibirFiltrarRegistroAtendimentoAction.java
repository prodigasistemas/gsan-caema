...
public class ExibirFiltrarRegistroAtendimentoAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		//Grupo Atendimento
		getGrupoAtendimentoCollection(sessao, fachada);
		
		// Tipo Solicitação
		if(filtrarRegistroAtendimentoActionForm.getGrupoAtendimento() != null && 
				filtrarRegistroAtendimentoActionForm.getGrupoAtendimento().length > 0){
			getSolicitacaoTipoCollection(sessao, fachada, filtrarRegistroAtendimentoActionForm);
			
			if(filtrarRegistroAtendimentoActionForm.getMudouGrupo() != null && 
					filtrarRegistroAtendimentoActionForm.getMudouGrupo().equals("true")){
				filtrarRegistroAtendimentoActionForm.setTipoSolicitacao(null);
				filtrarRegistroAtendimentoActionForm.setMudouGrupo("false");
			}
		} else {
			filtrarRegistroAtendimentoActionForm.setSelectedGrupoAtendimentoSize("0");
	...
	private void getGrupoAtendimentoCollection(HttpSession sessao, Fachada fachada) {
		...
		Collection<?> colecaoGrupoAtendimento = fachada.pesquisar(filtroSolicitacaoTipoGrupo, 
	 ...
	 * @param fachada
	 */
	private void getSolicitacaoTipoCollection(HttpSession sessao, Fachada fachada, FiltrarRegistroAtendimentoActionForm filtrarRegistroAtendimentoActionForm) {
		String[] grupoAtendimento = filtrarRegistroAtendimentoActionForm.getGrupoAtendimento();
		filtrarRegistroAtendimentoActionForm.setSelectedGrupoAtendimentoSize(grupoAtendimento.length+"");
		
		if(grupoAtendimento.length == 1){
			// Filtra Solicitação Tipo
			FiltroSolicitacaoTipo filtroSolicitacaoTipo = new FiltroSolicitacaoTipo();
			filtroSolicitacaoTipo.adicionarParametro(new ParametroSimples(FiltroSolicitacaoTipo.SOLICITACAO_TIPO_GRUPO_ID, grupoAtendimento[0]));
			...
			if (!Util.isVazioOrNulo(colecaoSolicitacaoTipo)) {
				...
				sessao.setAttribute("colecaoTipoSolicitacao", new ArrayList<SolicitacaoTipo>());
			}
		}else{
			sessao.setAttribute("colecaoTipoSolicitacao", new ArrayList<SolicitacaoTipo>());
		}
