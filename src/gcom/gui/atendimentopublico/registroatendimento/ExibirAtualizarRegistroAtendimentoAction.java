...
public class ExibirAtualizarRegistroAtendimentoAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if (fachada.verificarRegistroAtendimentoDevolucaoValoresEfetuada(
				new Integer(idRegistroAtendimento))) {

			throw new ActionServletException("atencao.existe_devolucao_pagamento_ra");
		...
		if (parmsRA != null) {
			...
			//id do grupo de solicitação
			if(parmsRA[41] != null ){
				atualizarRegistroAtendimentoActionForm
						.setGrupoAtendimento(((Integer) parmsRA[41]).toString());
			}
			...
			//Indicador Informar Pagamento Duplicado
			if(parmsRA[42] != null){
				atualizarRegistroAtendimentoActionForm
						.setIndicadorInformarPagamentoDuplicidade(Util.converterObjetoParaString((Short)parmsRA[42]));
			}
			
			//Indicador Informar Documento Pag a Maior
			if(parmsRA[43] != null){
				atualizarRegistroAtendimentoActionForm
					.setIndicadorInformarDocumentoPagamentoAMaior(Util.converterObjetoParaString((Short)parmsRA[43]));
			}
			
			//Indicador Informar Documento Pag Indevido
			if(parmsRA[44] != null){
				atualizarRegistroAtendimentoActionForm
					.setIndicadorInformarDocumentoPagamentoIndevido(Util.converterObjetoParaString((Short)parmsRA[44]));
			}
			
			Collection colecaoDevolucaoValores = fachada.pesquisarDevolucaoValores(new Integer(idRegistroAtendimento),
					atualizarRegistroAtendimentoActionForm.getIndicadorInformarPagamentoDuplicidade(),
					atualizarRegistroAtendimentoActionForm.getIndicadorInformarDocumentoPagamentoAMaior(),
					atualizarRegistroAtendimentoActionForm.getIndicadorInformarDocumentoPagamentoIndevido());
			
			/*if(atualizarRegistroAtendimentoActionForm.getIndicadorInformarPagamentoDuplicidade() != null && 
					atualizarRegistroAtendimentoActionForm.getIndicadorInformarPagamentoDuplicidade().equals(ConstantesSistema.SIM.toString())){
				sessao.setAttribute("colecaoPagamentosDuplicidade", colecaoDevolucaoValores);
			}*/
			
			if(atualizarRegistroAtendimentoActionForm.getIndicadorInformarDocumentoPagamentoAMaior() != null && 
					atualizarRegistroAtendimentoActionForm.getIndicadorInformarDocumentoPagamentoAMaior().equals(ConstantesSistema.SIM.toString())){
				sessao.setAttribute("colecaoPagValorMaiorDocumento", colecaoDevolucaoValores);
			}
			
			else if(atualizarRegistroAtendimentoActionForm.getIndicadorInformarDocumentoPagamentoIndevido() != null && 
					atualizarRegistroAtendimentoActionForm.getIndicadorInformarDocumentoPagamentoIndevido().equals(ConstantesSistema.SIM.toString())){
				sessao.setAttribute("colecaoPagamentosValoresCobIndevidamente", colecaoDevolucaoValores);
			} 
