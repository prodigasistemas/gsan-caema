...
public class InserirRegistroAtendimentoDadosLocalOcorrenciaAction extends
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		String coordenadaNorte = inserirRegistroAtendimentoActionForm.getNnCoordenadaNorte();
		String coordenadaLeste = inserirRegistroAtendimentoActionForm.getNnCoordenadaLeste();
		
		if ( coordenadaLeste != null && !coordenadaLeste.equals("") ){
			
        	//Coordenada X
    		BigDecimal bigDecimal = Util.formatarStringParaBigDecimal(coordenadaLeste.replace(",", "."));
        ...
        if ( coordenadaNorte != null && !coordenadaNorte.equals("") ){
			
        	//Coordenada X
    		BigDecimal bigDecimal = Util.formatarStringParaBigDecimal(coordenadaNorte.replace(",", "."));
		...
		if ((idlocalidade != null) && (!idlocalidade.equals(""))){
			Boolean incluidasCoordenadasPROGIS = fachada.verificaInclusaoPROGIS(Integer.valueOf(idlocalidade));
        ...
        //Validações das regras de devolução de valores
		if (idImovel != null &&
				!idImovel.equals("")){
        ...
        if(sistemaParametro.getIndicadorBloqFuncInstalacaoSubtHidrometro().compareTo(ConstantesSistema.SIM) == 0){
             ...
             if (colecaoSolicitacaoTipoEspecificacao != null && !colecaoSolicitacaoTipoEspecificacao.isEmpty()) {
                 ...
                 FaturamentoSituacaoTipo imovelSitEspecialFat = null;
                 if(idImovel != null && !idImovel.isEmpty()){
                	 imovelSitEspecialFat = this.getFachada()
        ...
        // validar os dados da repavimentação
        if(inserirRegistroAtendimentoActionForm.getIndicadorPavimento() != null 
        	...
        	if(inserirRegistroAtendimentoActionForm.getIdPavimentoRuaEnviado() == null 
        			|| inserirRegistroAtendimentoActionForm.getIdPavimentoRuaEnviado().equals("")
        			|| inserirRegistroAtendimentoActionForm.getIdPavimentoRuaEnviado().equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
        		throw new ActionServletException("atencao.campo.informado", null, "Pavimento Rua");
        	}
        	
        	if(inserirRegistroAtendimentoActionForm.getMetragemPavimentoRua() == null 
        			|| inserirRegistroAtendimentoActionForm.getMetragemPavimentoRua().equals("")){
        		throw new ActionServletException("atencao.campo.informada", null, "Metragem do Pavimento Rua");
        	}
        	
        	if(inserirRegistroAtendimentoActionForm.getIdUnidadeRepavimentadora() == null 
        			|| inserirRegistroAtendimentoActionForm.getIdUnidadeRepavimentadora().equals("")
        			|| inserirRegistroAtendimentoActionForm.getIdUnidadeRepavimentadora().equals("" + ConstantesSistema.NUMERO_NAO_INFORMADO)){
        		throw new ActionServletException("atencao.campo.informada", null, "Unidade Repavimentadora");
        	}
        	
        	BigDecimal areaPavimentoRua = Util.formatarStringParaBigDecimal(inserirRegistroAtendimentoActionForm.getMetragemPavimentoRua().replaceAll(",", "."));
        	if(areaPavimentoRua == null){
