...
/*				HidrometroInstalacaoHistorico hidrometroInstalacaoHistorico = null;
				...
				if (ordemServico.getRegistroAtendimento() == null ||
					...
					//Caso HIDR_ICMACRO = 1
					if(hidrometroInstalacaoHistorico.getHidrometro().getIndicadorMacromedidor().intValue() == 1){
						retiradaActionForm.setNumeroTombamento(hidrometroInstalacaoHistorico.getHidrometro().getTombamento());
					}
				...
				if(hidrometroInstalacaoHistorico.getHidrometro().getIndicadorMacromedidor().intValue() == 2){
					retiradaActionForm.setHidrometro(hidrometroInstalacaoHistorico
							.getHidrometro().getNumero());
				}
				
				retiradaActionForm.setIndicadorMacromedidor(hidrometroInstalacaoHistorico.getHidrometro().getIndicadorMacromedidor().toString());
...
/*				if (hidrometroInstalacaoHistorico.getDataRetirada() != null) {
				...
				/**
				 * [SB0005] - Bloquear Local de Armazenagem
				...
				if(ordemServico.getServicoTipo() != null){
					...
					filtroServicoTipo.adicionarParametro(new ParametroSimples(FiltroHidrometroLocalArmazenagem.ID, ordemServico.getServicoTipo().getId()));
				...
				if(servicoTipo != null && servicoTipo.getConstanteFuncionalidadeTipoServico().intValue()
						== ServicoTipo.TIPO_RETIRADA_HIDROMETRO && retiradaActionForm.getIdHidrometroSituacao() != null && 
						retiradaActionForm.getIdHidrometroSituacao().equals(HidrometroSituacao.EM_MANUTENCAO.toString())){
												
					FiltroHidrometroLocalArmazenagem filtroHidrometroLocalArmazenagem = new FiltroHidrometroLocalArmazenagem();						
					filtroHidrometroLocalArmazenagem.adicionarParametro(new ParametroSimples(FiltroHidrometroLocalArmazenagem.INDICADOR_OFICINA, HidrometroLocalArmazenagem.INDICADOR_OFICINA));
					...
					HidrometroLocalArmazenagem hidrometroLocalArmazenagem = (HidrometroLocalArmazenagem) colecaoHidrometroLocalArmazenagem.iterator().next();
						
					if(hidrometroLocalArmazenagem != null){
						retiradaActionForm.setHidrometroLocalArmazenagem(hidrometroLocalArmazenagem.getId().toString());
						retiradaActionForm.setBloquearLocalArmazenamento(ConstantesSistema.SIM.toString());
					}else{
						retiradaActionForm.setBloquearLocalArmazenamento(ConstantesSistema.NAO.toString());
					}
				}else{
					retiradaActionForm.setBloquearLocalArmazenamento(ConstantesSistema.NAO.toString());
					retiradaActionForm.setHidrometroLocalArmazenagem("-1");
				}
				...
				if(retiradaActionForm.getQuantidadeParcelas() == null || 
						retiradaActionForm.getQuantidadeParcelas().equals(""))
					retiradaActionForm.setQuantidadeParcelas(qtdeParcelas.toString());
				
				if(calculaValores != null && calculaValores.equals("S"))
					calcularValorPrestacao(retiradaActionForm,
							ordemServico, valorDebito, sistemaParametro);
				else{				
...
//		 Fim da Verificacao se o hidrometro esta extraviado.
	...
	private void calcularValorPrestacao(
			EfetuarRetiradaHidrometroActionForm retiradaActionForm,
			OrdemServico ordemServico, BigDecimal valorDebito,
...
//		 Mudar isso quando tiver esquema de segurança
		...
		if(sessao.getAttribute("colecaoHidrometroSituacao") == null){
			...
			filtroHidrometroSituacao.adicionarParametro(new ParametroSimples(FiltroHidrometroSituacao.INDICADOR_BLOQUEAR_SUBSTITUICAO, ConstantesSistema.NAO));
			
			filtroHidrometroSituacao.adicionarParametro(new ParametroSimplesDiferenteDe(FiltroHidrometroSituacao.ID, HidrometroSituacao.INSTALADO));
