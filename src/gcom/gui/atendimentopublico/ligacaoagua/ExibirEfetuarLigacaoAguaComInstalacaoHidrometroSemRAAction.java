...
/*		boolean temPermissaoMotivoNaoCobranca = fachada
		...
		if(form.getIndicadorMedicaoTelemedido() == null){
			form.setIndicadorMedicaoTelemedido(String.valueOf(ConstantesSistema.NAO));
		}

		if (form.getMatriculaImovel() != null && !form.getMatriculaImovel().trim().equals("")) {
			
			//Verifica se o Cliente é desconhecido
			Integer idImovel = Integer.parseInt(form.getMatriculaImovel());
			...
			if(clienteDesconhecido != null){
				throw new ActionServletException("atencao.nao.permitido.ligacao.agua", null , form.getMatriculaImovel());
			}
		...
		if (tombamento != null && !tombamento.trim().equals("")) {
			...
			//[FS0016] - Verificar Situação do Tombamento
