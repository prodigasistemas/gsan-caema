...
//		 Usuario logado no sistema
		...
		/*
		 * Colocador por Davi Menezes em 22/08/2012
		 * 
		 * [SB 0003] - Reabilitar hidrômetro para ligação
		 */
		if(imovel != null && imovel.getLigacaoAgua() != null){
			if(imovel.getLigacaoAgua().getIndicadorPermanenciaHidrometro() != null &&
					imovel.getLigacaoAgua().getIndicadorPermanenciaHidrometro().equals(ConstantesSistema.SIM)){
				
				FiltroLigacaoAgua filtroLigAguaComp = new FiltroLigacaoAgua();
				filtroLigAguaComp.adicionarParametro(new ParametroSimples(FiltroLigacaoAgua.ID, imovel.getId()));
				Collection<LigacaoAgua> colLigAgua = fachada.pesquisar(filtroLigAguaComp, LigacaoAgua.class.getName());				
				
				LigacaoAgua ligAguaComp = colLigAgua.iterator().next();
				
				LigacaoAgua ligacaoAgua = 
						fachada.atualizarDataRetiradaHidrometroHistorico(imovel.getId(), ligAguaComp);
