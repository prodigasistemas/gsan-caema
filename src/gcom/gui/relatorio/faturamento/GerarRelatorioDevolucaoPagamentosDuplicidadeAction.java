...
import gcom.atendimentopublico.registroatendimento.FiltroCobrancaIndevidaMotivo;
...
public class GerarRelatorioDevolucaoPagamentosDuplicidadeAction extends ExibidorProcessamentoTarefaRelatorio {
	...
	public ActionForward execute(ActionMapping actionMapping,
        ...
        // Tipo de Especificacao
        String tipoEspecificacao = " -- ";
		if ( form.getTipoEspecificacao() != null && 
				!form.getTipoEspecificacao().equals("-1")) {
			
			helper.setTipoEspecificacao(form.getTipoEspecificacao());
			peloMenosUmParametroInformado = true;
			
			FiltroSolicitacaoTipoEspecificacao filtroSolicitacaoTipoEspecificacao = new FiltroSolicitacaoTipoEspecificacao();
			filtroSolicitacaoTipoEspecificacao.adicionarParametro(new ParametroSimples(FiltroSolicitacaoTipoEspecificacao.ID, form.getTipoEspecificacao()));
			Collection colecaoTipoEspecificacao = this.getFachada().pesquisar(filtroSolicitacaoTipoEspecificacao, SolicitacaoTipoEspecificacao.class.getName());
			SolicitacaoTipoEspecificacao solicitacaoTipoEspecificacao = (SolicitacaoTipoEspecificacao) Util.retonarObjetoDeColecao(colecaoTipoEspecificacao);
			
			tipoEspecificacao = solicitacaoTipoEspecificacao.getDescricao();
		}
		
		// Motivo Devolucao
		String motivoDevolucao = " -- ";
		if ( form.getMotivoDevolucao() != null && 
				!form.getMotivoDevolucao().equals("-1")) {
			
			helper.setMotivoDevolucao(form.getMotivoDevolucao());
			peloMenosUmParametroInformado = true;
			
			FiltroCobrancaIndevidaMotivo filtroCobrancaIndevidaMotivo = new FiltroCobrancaIndevidaMotivo();
			filtroCobrancaIndevidaMotivo.adicionarParametro(new ParametroSimples(FiltroCobrancaIndevidaMotivo.ID, form.getMotivoDevolucao()));
			Collection colecaoCobrancaIndevidaMotivo = this.getFachada().pesquisar(filtroCobrancaIndevidaMotivo, CobrancaIndevidaMotivo.class.getName());
			CobrancaIndevidaMotivo cobrancaIndevidaMotivo = (CobrancaIndevidaMotivo) Util.retonarObjetoDeColecao(colecaoCobrancaIndevidaMotivo);
			
			motivoDevolucao = cobrancaIndevidaMotivo.getDescricao();
		}
		
		// Tipo de Documento
		String tipoDocumento = "TODOS";
		if (form.getTipoDocumento() != null && !form.getTipoDocumento().equals("")){	
			helper.setTipoDocumento(form.getTipoDocumento());
	    	//Conta
	    	if (form.getTipoDocumento().equals("1")){
				tipoDocumento = "CONTA";
	    	} else
	    	//Guia de Pagamento
	    	if (form.getTipoDocumento().equals("2")){
				tipoDocumento = "GUIA DE PAGAMENTO";
	    	} else
	    	//Debito a Cobrar
	    	if (form.getTipoDocumento().equals("3")){
				tipoDocumento = "DEBITO A COBRAR";
	    	}
		...
		relatorio.addParametro("tipoEspecificacao", tipoEspecificacao);
		relatorio.addParametro("motivoDevolucao", motivoDevolucao);
		...
		relatorio.addParametro("tipoDocumento", tipoDocumento);
