...
public class GerarRelatorio2ViaContaAction extends
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if (sessao.getAttribute("idContaHistorico") != null && !sessao.getAttribute("idContaHistorico").equals("")){
			...
			String codigoClienteSueriorParam = httpServletRequest.getParameter("codigoClienteSuerior");
			if (codigoClienteSueriorParam != null && !codigoClienteSueriorParam.equals("")){
					
				codigoClienteSuperior = new Integer((String) httpServletRequest.getParameter("codigoClienteSuperior"));
				sessao.setAttribute("codigoClienteSuperior", codigoClienteSuperior);
			}else if (sessao.getAttribute("codigoClienteSuperior") != null){				  
				codigoClienteSuperior = new Integer("" + sessao.getAttribute("codigoClienteSuperior"));
		...
		if (colecaoConta != null && !colecaoConta.isEmpty()){
			...
			/**
			 * Atualizar a quantidade de Impressao Convencional em Conta
			 * 
			 * @author Davi Menezes
			 * @date 12/03/2012
			 */
			if(conta.getQuantidadeImpressaoConvencional() != null){
				conta.setQuantidadeImpressaoConvencional(conta.getQuantidadeImpressaoConvencional() + 1);
			}else{
				conta.setQuantidadeImpressaoConvencional(1);
			}
			...
			/**
			 * Atualizar a quantidade de Impressao Convencional em Conta Histórico
			 * 
			 * @author Davi Menezes
			 * @date 12/03/2012
			 */
			if(contaHistorico.getQuantidadeImpressaoConvencional() != null){
				contaHistorico.setQuantidadeImpressaoConvencional(contaHistorico.getQuantidadeImpressaoConvencional() + 1);
			}else{
				contaHistorico.setQuantidadeImpressaoConvencional(1);
			}
			
			fachada.atualizar(contaHistorico);
		}
		/*
		 * --Erivan Sousa--
		...
		/* RM5405 - Registrar  segunda via de conta impressa em tabela do sistema */
		if (conta!=null){
		...
		if(sistemaParametro.getCodigoEmpresaFebraban()
			...
			String tipoConta = (String) httpServletRequest.getParameter("tipoConta");
			relatorio2ViaConta.addParametro("tipoConta", tipoConta);
