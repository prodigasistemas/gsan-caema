...
public class RelatorioGerarRelatorioOSAcompanhamentoCobrancaResultado extends TarefaRelatorio {
	...
	public Object executar() throws TarefaException {
		...
		// Montar Beans
		RelatorioDataSource ds = montarBeans(colecaoImovelOS, fachada, tipoServico, filtrouGerencia,comando);
	...
	private RelatorioDataSource montarBeans(@SuppressWarnings("rawtypes") Collection colecaoImovelOS, Fachada fachada, String tipoServico,
			boolean filtrouGerencia,String comando) {
		...
		while (it.hasNext()) {
			...
			@SuppressWarnings("rawtypes")
			Collection colecaoOrdemServicoEmpresaCobranca = fachada.obterColecaoEmpresaCobrancaContaporImovel(id, new Integer(tipoServico),comando);
	...
	public int calcularTotalRegistrosRelatorio() {
		...
		Iterator it = colecaoImovelOS.iterator();
		Collection colecaoBusca = new ArrayList();
		
		while(it.hasNext()){
			HashMap map = (HashMap)it.next();
			
			colecaoBusca.add(map);
			
			if(contador != 0 && contador % 100 == 0){
				retorno += Fachada.getInstancia().obterTotalOSColecaoImovelTipoServico(colecaoBusca, new Integer(tipoServico));
				colecaoBusca.clear();
			}
