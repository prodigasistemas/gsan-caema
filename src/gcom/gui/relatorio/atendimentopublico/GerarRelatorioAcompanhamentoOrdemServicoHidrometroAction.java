...
//		httpServletRequest.setAttribute("telaSucessoRelatorio",true);
		...
		if(situacaoOrdemServico.equalsIgnoreCase("PENDENTES")){
			 dataEncerramentoInicial = null;
		...
		if (dataExecucaoInicial != null && !dataExecucaoInicial.trim().equals("")) {
			dataExecucaoInicial += " 00:00:00";
		}
		
		if (dataExecucaoFinal != null && !dataExecucaoFinal.trim().equals("")) {
			dataExecucaoFinal += " 23:59:59";
		}
		...
		Collection<Integer> colecaoCapacidadeHidrometro = null;
		if (form.getHidrometroCapacidade() != null && 
			form.getHidrometroCapacidade().length > 0) {
			
			colecaoCapacidadeHidrometro = new ArrayList();
			
			String[] array = form.getHidrometroCapacidade();
			for (int i = 0; i < array.length; i++) {
				if (new Integer(array[i]).intValue() != ConstantesSistema.NUMERO_NAO_INFORMADO) {
					colecaoCapacidadeHidrometro.add(new Integer(array[i]));
				}
			}			
		}
		...
		if (Util.converterStringParaInteger(tipoRelatorioAcompanhamento).equals(
			...
			if(colecaoCapacidadeHidrometro != null && colecaoCapacidadeHidrometro.size() != 0)
				relatorio.addParametro("colecaoCapacidadeHidrometro", colecaoCapacidadeHidrometro);
			...
			relatorio.addParametro("dataExecucaoInicial", dataExecucaoInicial);
			relatorio.addParametro("dataExecucaoFinal", dataExecucaoFinal);
