...
public class GerarRelatorioFuncionalidadesOperacoesPorGrupoAction extends
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		// Form
		GerarRelatorioFuncionalidadesOperacoesPorGrupoActionForm form = (GerarRelatorioFuncionalidadesOperacoesPorGrupoActionForm) actionForm;

		FiltrarRelatorioFuncionalidadeOperacoesPorGrupoHelper helper = new FiltrarRelatorioFuncionalidadeOperacoesPorGrupoHelper();
		...
		if (botao != null && !botao.equals("")) {
			sessao.setAttribute("botaoPesquisar", botao);
		}

		Collection<Integer> colecaoGrupos = new ArrayList<Integer>();

		if (sessao.getAttribute("botaoPesquisar") != null
				&& !sessao.getAttribute("botaoPesquisar").equals("")) {
				...
				for (int i = 0; i < idsGrupos.length; i++) {
					if (!idsGrupos[i].equals("")) {
						...
						filtroGrupo.adicionarParametro(new ParametroSimples(
								FiltroGrupo.ID, idsGrupos[i]));

						Collection colecaoGrupo = fachada.pesquisar(
								filtroGrupo, Grupo.class.getName());

						if (colecaoGrupo != null && !colecaoGrupo.isEmpty()) {
							Grupo grupo = (Grupo) Util
									.retonarObjetoDeColecao(colecaoGrupo);
		...
		relatorio = new RelatorioFuncionalidadesOperacoesPorGrupo(
