...
public class InformarAcertoDocumentosNaoAceitosAction extends GcomAction {
    ...
    public ActionForward execute(ActionMapping actionMapping,
        ...
        if ((form.getContasSelecao() == null || form.getContasSelecao().length == 0)
        		...
        		&& (form.getDocumentosCobrancaSelecao() == null || form.getDocumentosCobrancaSelecao().length == 0)) {
        ...
        InformarAcertoDocumentosNaoAceitosHelper helper = this.montarHelper(sessao, form, usuarioLogado);
    ...
    public InformarAcertoDocumentosNaoAceitosHelper montarHelper(HttpSession sessao,
    		InformarAcertoDocumentosNaoAceitosActionForm form, Usuario usuario){
		...
		Collection<CobrancaDocumentoHelper> colecaoCobrancaDocumentoSelecionados = 
			new ArrayList<CobrancaDocumentoHelper>();
		...
		Collection<CobrancaDocumentoHelper> colecaoCobrancaDocumentoValores = 
			(Collection<CobrancaDocumentoHelper>) sessao.getAttribute("colecaoDocumentoCobranca");	
		...
		if(colecaoCobrancaDocumentoValores != null && !colecaoCobrancaDocumentoValores.isEmpty()
				&& form.getDocumentosCobrancaSelecao() != null && form.getDocumentosCobrancaSelecao().length > 0){
			Iterator iterator = colecaoCobrancaDocumentoValores.iterator();
			
			while(iterator.hasNext()){
				CobrancaDocumentoHelper cobrancaDocumentoHelper = (CobrancaDocumentoHelper) iterator.next();
				
				for(String idDocumentoCobranca : form.getDocumentosCobrancaSelecao()) {
					
					if(cobrancaDocumentoHelper.getCobrancaDocumento().getId()
							.compareTo(Integer.parseInt(idDocumentoCobranca)) == 0){
						colecaoCobrancaDocumentoSelecionados.add(cobrancaDocumentoHelper);
					}
				}
			}
    	...
    	helper.setColecaoCobrancaDocumentoValores(colecaoCobrancaDocumentoSelecionados);
