...
public class ExibirInformarAcertoDocumentosNaoAceitosDebitosAction extends GcomAction {
    ...
    public void carregarDebitosImovel(HttpSession sessao, String idImovel, InformarAcertoDocumentosNaoAceitosActionForm form) {
		...
		//Criando uma lista para adicao de todas colecaoCobrancaDocumentoHelper
		Collection<CobrancaDocumentoHelper> colecaoCobrancaDocumentoHelper = new ArrayList<CobrancaDocumentoHelper>();
		...
		if (colecaoDocumentoCobranca != null
			...
			boolean documentoCobrancaPago = true;
			...
			while (colecaoDocumentoCobrancaIterator.hasNext()) {
				...
				documentoCobrancaPago = this.pesquisarDocumentosCobrancaPagos(cobrancaDocumento.getId(), fachada);
				
				if(!documentoCobrancaPago){
					valorTotalDocumentoCobranca = valorTotalDocumentoCobranca.add(cobrancaDocumento.getValorDocumento());
			...
			valorTotalDebito = valorTotalDebito.add(valorTotalDocumentoCobranca);
			
		}
		
		if (colecaoContaValoresTotal.isEmpty() && colecaoDebitoACobrarTotal.isEmpty()
				&& colecaoGuiaPagamentoValoresTotal.isEmpty() && colecaoCobrancaDocumentoHelper.isEmpty()) {
			...
			sessao.setAttribute("valorDocumentoCobranca", Util.formatarMoedaReal(valorTotalDocumentoCobranca));
    ...
    public void removerAtributosSessao(HttpSession sessao, InformarAcertoDocumentosNaoAceitosActionForm form) {
		...
		sessao.removeAttribute("valorDocumentoCobranca");
    ...
    public boolean pesquisarDocumentosCobrancaPagos(Integer idDocumentoCobranca, Fachada fachada){
    	boolean documentoCobrancaPago = true;
    	
    	FiltroPagamento filtroPagamento = new FiltroPagamento();
    	filtroPagamento.adicionarParametro(new ParametroSimples(FiltroPagamento.ID_COBRANCA_DOCUMENTO, idDocumentoCobranca));
    	...
    	if(!Util.isVazioOrNulo(colecaoPagamentos)){
    		documentoCobrancaPago = true;
    	}else{
    		FiltroPagamentoHistorico filtroPagamentoHistorico = new FiltroPagamentoHistorico();
    		filtroPagamentoHistorico.adicionarParametro(new ParametroSimples(FiltroPagamentoHistorico.ID_COBRANCA_DOCUMENTO, idDocumentoCobranca));
    		
    		Collection<?> colecaoPagamentosHistorico = fachada.pesquisar(filtroPagamentoHistorico, PagamentoHistorico.class.getName());
    		if(!Util.isVazioOrNulo(colecaoPagamentosHistorico)){
    			documentoCobrancaPago = true;
    		}else{
    			documentoCobrancaPago = false;
    		}
    	}
    	
    	return documentoCobrancaPago;
    }
