    ...
    public ActionForward execute(ActionMapping actionMapping,
		...
		if (form.getIdTipoDocumento().trim().compareTo(DocumentoTipo.CONTA.toString()) == 0) {
			...
			if(form.getSituacaoDebito().equals("1")){
				...
				//Caso não exista o débito
				if(contaEncontrada == null) {
					ContaHistorico contaHistorico = fachada.pesquisarContaHistoricoDigitadaParcelamento(
							form.getIdImovel(), form.getReferenciaConta());
			...
			if(form.getSituacaoDebito().equals("1")){
				...
				guiaPagamentoEncontradaHelper = fachada
					.pesquisarGuiaPagamentoDigitadaParcelamento(form.getIdImovel(), form.getIdGuiaPagamento());
				
				//Caso não exista o débito
				if(guiaPagamentoEncontradaHelper == null) {
					throw new ActionServletException("atencao.pesquisa_inexistente", "Guia de Pagamento");
				}
			}
			...
			if(form.getSituacaoDebito().equals("1")){
				...
				debitoACobrarEncontradoHelper = fachada
						.pesquisarDebitoACobrarParceladoImovelDigitado(form.getIdImovel(), form.getIdDebitoACobrar());
				
				//Caso não exista o débito
				if(debitoACobrarEncontradoHelper == null) {
					throw new ActionServletException("atencao.pesquisa_inexistente", "Débito a Cobrar");
				}
			}
