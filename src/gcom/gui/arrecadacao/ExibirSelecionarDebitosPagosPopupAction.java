...
public class ExibirSelecionarDebitosPagosPopupAction extends GcomAction {
    ...
    public ActionForward execute(ActionMapping actionMapping,
		...
		if (referenciaContaDigitadoEnter != null
			...
			if(Util.validarAnoMes(referenciaContaDigitadoEnter)){
				throw new ActionServletException("atencao.referencia_conta_invalida");
			}

			// Recupera a conta do imóvel com a referência informada
			Conta contaEncontrada = null;
			if(form.getSituacaoDebito() != null){ 
				if(form.getSituacaoDebito().equals("1")){
					contaEncontrada = fachada.pesquisarContaDigitada(
						codigoImovel, referenciaContaDigitadoEnter);
				}else {
					contaEncontrada = fachada.pesquisarContaDigitadaParcelamento(
						codigoImovel, referenciaContaDigitadoEnter);
				}
			}
			...
			if (contaEncontrada != null) {				
				...
				if(form.getSituacaoDebito() != null){
					if(form.getSituacaoDebito().equals("1")){
						contaHistorico = fachada.pesquisarContaHistoricoDigitada(
							form.getIdImovel(), form.getReferenciaConta());
					}else{
						contaHistorico = fachada.pesquisarContaHistoricoDigitadaParcelamento(
							form.getIdImovel(), form.getReferenciaConta());
					}
				}
		...
		if (codigoGuiaPagamentoDigitadoEnter != null
			...
			if(form.getSituacaoDebito().equals("1")){
				...
				// Pesquisa a guia de pagamento do parcelamento para o imóvel informado
				GuiaPagamentoValoresHelper guiaPagamentoEncontradaHelper = fachada
						.pesquisarGuiaPagamentoDigitadaParcelamento(codigoImovel, codigoGuiaPagamentoDigitadoEnter);
				
				if(guiaPagamentoEncontradaHelper != null) {
					form.setIdGuiaPagamento(codigoGuiaPagamentoDigitadoEnter);
					form.setDescricaoGuiaPagamento(guiaPagamentoEncontradaHelper.getGuiaPagamento().getDebitoTipo().getDescricao());
		...
		if (codigoDebitoACobrarDigitadoEnter != null
			...
			if(form.getSituacaoDebito().equals("1")){
				...
				DebitoACobrarValoresHelper debitoACobrarEncontradoHelper = fachada
						.pesquisarDebitoACobrarParceladoImovelDigitado(codigoImovel, codigoDebitoACobrarDigitadoEnter);
				
				if(debitoACobrarEncontradoHelper != null){
					form.setIdDebitoACobrar(codigoDebitoACobrarDigitadoEnter);
					form.setDescricaoDebitoACobrar(debitoACobrarEncontradoHelper.getDebitoACobrar().getDebitoTipo().getDescricao());
