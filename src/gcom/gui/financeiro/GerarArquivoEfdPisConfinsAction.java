...
public class GerarArquivoEfdPisConfinsAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if(tipoRegistroInformado != null && tipoRegistroInformado.length != 0){
			...
			tipoRegistro = new int[]{-1};
		}
		
		if(mesAnoReferencia != null && !mesAnoReferencia.equals("")){
			
			Map parametros = new HashMap();
			parametros.put("tipoRegistro", tipoRegistro);
			parametros.put("anoMesReferencia", Util.formatarMesAnoComBarraParaAnoMes(mesAnoReferencia));
			parametros.put("quebraPorMunicipio", indicadorTotalizarMunicipio == 1 ? true : false);
			parametros.put("indicadorTipoGeracao", indicadorTipoGeracao);
			
			fachada.inserirProcessoIniciadoParametrosLivres(parametros, Processo.GERAR_ARQUIVOS_EFD_PIS_CONFINS, 
...
//					);
//				
				httpServletRequest.setAttribute("telaSucesso",true);
...
//			}
		...
		montarPaginaSucesso(httpServletRequest, "O Processo "+ Processo.GERAR_ARQUIVOS_EFD_PIS_CONFINS + " Foi Adicionado",//"Gerado o Arquivo EFD-PIS/Cofins."
