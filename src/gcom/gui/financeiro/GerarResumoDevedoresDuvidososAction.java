...
import gcom.financeiro.FiltroResumoDevedoresDuvidosos;
...
public class GerarResumoDevedoresDuvidososAction extends GcomAction {
	...
	public ActionForward execute(ActionMapping actionMapping,
		...
		if(form.getAnoMesReferenciaContabil() == null || form.getAnoMesReferenciaContabil().equals("")){
			form.setAnoMesReferenciaContabil(form.getAnoMesReferencia());
		}
        ...
        form.setAnoMesReferencia(form.getAnoMesReferenciaContabil());
        
        SistemaParametro sistemaParametro = fachada.pesquisarParametrosDoSistema();
        Integer anoMesFaturamento = sistemaParametro.getAnoMesFaturamento();
        
        if(Integer.parseInt(anoMesReferenciaContabilFormatado) >= anoMesFaturamento){
        	throw new ActionServletException("atencao.ano_mes_contabil_maior_que_ano_mes_faturamento");
        }
        
        FiltroResumoDevedoresDuvidosos filtro = new FiltroResumoDevedoresDuvidosos();
        filtro.adicionarParametro(new ParametroSimples(FiltroResumoDevedoresDuvidosos.ANO_MES_REFERENCIA_CONTABIL, 
        		anoMesReferenciaContabilFormatado));
        filtro.adicionarParametro(new ParametroSimples(FiltroResumoDevedoresDuvidosos.TIPO_PERDA, form.getIdTipoPerda()));
        
        Collection colecaoResumoDevedoresDuvidosos = fachada.pesquisar(filtro, ResumoDevedoresDuvidosos.class.getName());
        
        if(!Util.isVazioOrNulo(colecaoResumoDevedoresDuvidosos)){
        	
        	indicadorReprocessamento = "1";
        	
        	retorno = montarPaginaConfirmacao("atencao.existe_resumo_gerado_parametros_informados",
				httpServletRequest, actionMapping);
        }else{
        	indicadorReprocessamento = "2";
        }
        
        String confirmado = httpServletRequest.getParameter("confirmado");
		
		if(retorno.getName().equalsIgnoreCase("telaConfirmacao") && confirmado == null){
			return retorno;
		}
        ...
        if ( form.getIndicadorTipoGeracao() != null && form.getIndicadorTipoGeracao() != "" && form.getIndicadorTipoGeracao().equals(String.valueOf(ConstantesSistema.NAO))) {
        	idProcesso = Processo.GERAR_RESUMO_DEVEDORES_DUVIDOSOS_PERDAS_SOCIETARIAS_SIMULACAO;
        }
		...
		if ( form.getIdTipoPerda().equals(String.valueOf(PerdasTipo.PROVISAO_PERDAS_SOCIETARIAS) )) {
			...
			if ( form.getIndicadorTipoGeracao() != null && form.getIndicadorTipoGeracao() != "" && form.getIndicadorTipoGeracao().equals(String.valueOf(ConstantesSistema.NAO))) {
				tipoPerdaDescricao = " da Provisão de Perdas Societárias - Simulação";
		    } else {
