...
public class ControladorUsuarioSEJB implements SessionBean {
	 ...
	 */
	public Integer inserirUsuario(Usuario usuario, Integer[] idGrupos, Usuario usuarioLogado, String idSolicitacaoAcesso)
			throws ControladorException {
...
//				registradorOperacao.registrarOperacao(usuarioGrupo);
				try {
					...
					UsuarioGrupoPK pkRetorno =  (UsuarioGrupoPK) repositorioUtil.inserir(usuarioGrupo);
					retorno = pkRetorno.getUsuarioId();
		...
		enviaEmailUsuarioESolicitante(usuario, idSolicitacaoAcesso);
	...
	/**
	 * Envia email ao usuário informando login e senha 
	 * e ao solicitante informando que o usuário foi autorizado
	 * 
	 * @author Amelia Pessoa
	 * @date 23/04/2012
	 * 
	 * @param usuario, idSolicitacaoAcesso
	 *  
	 * @throws ControladorException
	 */
	public void enviaEmailUsuarioESolicitante(Usuario usuario,
		...
		//RM 7146 - Envia email para o solicitante
		
		//Busca solicitação
		SolicitacaoAcesso solicitacaoAcesso = null;
		if (idSolicitacaoAcesso != null && !idSolicitacaoAcesso.equals("")){
			
			FiltroSolicitacaoAcesso filtroSolicitacaoAcesso = new FiltroSolicitacaoAcesso(); 
			filtroSolicitacaoAcesso.adicionarParametro(new ParametroSimples(FiltroSolicitacaoAcesso.ID, idSolicitacaoAcesso));
			Collection<SolicitacaoAcesso> colecaoSolicitacaoAcesso = getControladorUtil().pesquisar(
		...
		if (solicitacaoAcesso!=null){
			...
			filtroUsuarioSolicitante.adicionarParametro(new ParametroSimples(FiltroUsuario.FUNCIONARIO_ID, 
					solicitacaoAcesso.getUsuarioSolicitante().getId()));
			Collection<Usuario> colecaoUsuarioSolicitante = getControladorUtil().pesquisar(
			...
			if (!Util.isVazioOrNulo(colecaoUsuarioSolicitante)) {
				...
				if (usuarioSolicitante!=null){
					...
					if (usuarioSolicitante.getDescricaoEmail() != null
						...
						String mensagem = "O usuário de Login: " + usuario.getLogin() + ", solicitado por você, foi autorizado.";
						EnvioEmail envioEmail = getControladorCadastro().pesquisarEnvioEmail(EnvioEmail.INSERIR_USUARIO);
...
//		operacaoEfetuada.setOperacao(operacao);
		...
		Date dataCadastramentoFinal = usuario.getDataCadastroFim();
		Date dataAtual = new Date();
		int numeroDiasExpiracaoAcesso = 0;
		
		if(sistemaParametro.getNumeroDiasExpiracaoAcesso() != null){
			 numeroDiasExpiracaoAcesso = sistemaParametro.getNumeroDiasExpiracaoAcesso().intValue();
		}
		...
		int numeroDiasMSGExpiracao = 0;
		if(sistemaParametro.getNumeroDiasMensagemExpiracao() != null){
			numeroDiasMSGExpiracao = sistemaParametro.getNumeroDiasMensagemExpiracao().intValue();
		}
		...
		if (!(permissoesEspeciais == null && permissoesCheckBoxVazias == null)) {
			...
			if (permissoesEspeciais != null && permissoesEspeciais.length != 0) {
				...
				for (int i = 0; i < permissoesEspeciais.length; i++) {
					...
					Collection colecao = getControladorUtil().pesquisar(FiltroPermissaoEspecial, PermissaoEspecial.class.getName());
...
//					PermissaoEspecial permissaoEspecial = (PermissaoEspecial) Util.retonarObjetoDeColecao(colecaoUsuario);
	 ...
	 * 
	 * [UC0231] - Manter Usuário [SB0008] - Selecionar Restrinções (nÂ°2)
	 * 
	 * @author Sávio Luiz
	 * @date 17/07/2006
	 */
	...
	/**
	 * [UC0146] Manter Conta
	 * [SB0012] Â– Determinar competência de retificação de consumo
	 * 
	 * @author Vivianne Sousa
	 * @date 16/02/2011
	 */
	 ...
	 * @author Amélia Pessoa
	 * @date 03/01/2012
	 * @param usuarioAfastamento
	 * @throws ControladorException
	 */
	public void inserirAfastamento(UsuarioAfastamento usuarioAfastamento)throws ControladorException{
		usuarioAfastamento = atualizarSituacao(usuarioAfastamento);
		Integer id = (Integer) getControladorUtil().inserir(usuarioAfastamento);
		usuarioAfastamento.setId(id);
			
		/* [SB0006] Atribuir Permissões */
		if (usuarioAfastamento.getUsuarioEspelho()!=null){			
			atribuirPermissoes(usuarioAfastamento);			
		}
	}
	 ...
	 * @throws ControladorException
	 */
	public void atualizarAfastamento(UsuarioAfastamento usuarioAfastamento)throws ControladorException{
		try {
			//Atualiza situacao
			this.atualizarSituacao(usuarioAfastamento);
			
			//Atualiza usuario espelho
			removerPrivilegioUsuarioEspelho(usuarioAfastamento);
			
			atribuirPermissoes(usuarioAfastamento);
			
			//Atualiza objeto usuarioAfastamento
			this.getControladorUtil().atualizar(usuarioAfastamento);			
		} catch (ControladorException ex) {
			throw new FachadaException(ex.getMessage(), ex,
					ex.getParametroMensagem());
		}
	}
	 ...
	 * @throws ControladorException
	 */
	private void atribuirPermissoes(UsuarioAfastamento usuarioAfastamento) throws ControladorException {
		
		/* 1. Grupos */
		FiltroUsuarioGrupo filtroUsuarioGrupo = new FiltroUsuarioGrupo();
		filtroUsuarioGrupo.adicionarParametro(new ParametroSimples(FiltroUsuarioGrupo.USUARIO_ID, usuarioAfastamento.getUsuarioAfastado().getId()));
		filtroUsuarioGrupo.adicionarCaminhoParaCarregamentoEntidade(FiltroUsuarioGrupo.GRUPO);
		Collection<UsuarioGrupo> colecaoUsuarioGrupo = getControladorUtil().pesquisar(filtroUsuarioGrupo, UsuarioGrupo.class.getName());
		if (colecaoUsuarioGrupo!=null && usuarioAfastamento.getUsuarioEspelho()!=null){
			filtroUsuarioGrupo = new FiltroUsuarioGrupo();
			filtroUsuarioGrupo.adicionarParametro(new ParametroSimples(FiltroUsuarioGrupo.USUARIO_ID, usuarioAfastamento.getUsuarioEspelho().getId()));
			Collection<UsuarioGrupo> colecaoUsuarioEspelhoGrupo = getControladorUtil().pesquisar(filtroUsuarioGrupo, UsuarioGrupo.class.getName());
			UsuarioGrupo aux = null;
			for (UsuarioGrupo ug: colecaoUsuarioGrupo){ 
				if (!colecaoUsuarioEspelhoGrupo.contains(ug)){
					aux = new UsuarioGrupo();
					aux.setComp_id(new UsuarioGrupoPK(ug.getGrupo().getId(), usuarioAfastamento.getUsuarioEspelho().getId()));
					aux.setUsuarioAfastamento(usuarioAfastamento);
					aux.setUltimaAlteracao(new Date());
					getControladorUtil().inserir(aux);
				}
			}
		}
		...
		filtroUsuarioPemissaoEspecial.adicionarParametro(new ParametroSimples(
				FiltroUsuarioPemissaoEspecial.USUARIO_ID, usuarioAfastamento.getUsuarioAfastado().getId()));
		filtroUsuarioGrupo.adicionarCaminhoParaCarregamentoEntidade(FiltroUsuarioPemissaoEspecial.PERMISSAO_ESPECIAL);
		Collection<UsuarioPermissaoEspecial> colecaoUsuarioPermissaoEspecial = getControladorUtil().pesquisar(
				filtroUsuarioPemissaoEspecial, UsuarioPermissaoEspecial.class.getName());		
		if (colecaoUsuarioPermissaoEspecial!=null && usuarioAfastamento.getUsuarioEspelho()!=null){			
			UsuarioPermissaoEspecial aux = null;
			for (UsuarioPermissaoEspecial upe: colecaoUsuarioPermissaoEspecial){				
				if (!getControladorPermissaoEspecial()
						.verificarPermissaoEspecialAtiva(upe.getComp_id().getPermissaoEspecialId(), usuarioAfastamento.getUsuarioEspelho())){
					aux = new UsuarioPermissaoEspecial();					
					aux.setComp_id(new UsuarioPermissaoEspecialPK(usuarioAfastamento.getUsuarioEspelho().getId()
							, upe.getComp_id().getPermissaoEspecialId()));
					aux.setUsuarioAfastamento(usuarioAfastamento);
					aux.setUltimaAlteracao(new Date());
					getControladorUtil().inserir(aux);
				}				
	 ...
	 * @throws ControladorException
	 */
	public UsuarioAfastamento atualizarSituacao(UsuarioAfastamento usuarioAfastamento) throws ControladorException {
		if (usuarioAfastamento.getIcAfastamentoTemporario()==1){ //Afastamento temporário
			SistemaParametro sistemaParametro = getControladorUtil().pesquisarParametrosDoSistema();
			int dias = Util.obterQuantidadeDiasEntreDuasDatas(usuarioAfastamento.getDtAfastamentoInicial(), usuarioAfastamento.getDtAfastamentoFinal());
			if (dias > sistemaParametro.getNumeroDiasBloqueiaSenha()){
				FiltroUsuarioSituacao filtroUsuarioSituacao = new FiltroUsuarioSituacao();
				filtroUsuarioSituacao.adicionarParametro(new ParametroSimples( FiltroUsuarioSituacao.DESCRICAO, "TEMPORARIAMENTE AFASTADO"));
				Collection colecaoUsuarioSituacao = Fachada.getInstancia().pesquisar(filtroUsuarioSituacao, UsuarioSituacao.class.getName());
				
				usuarioAfastamento.getUsuarioAfastado().setUsuarioSituacao((UsuarioSituacao) Util.retonarObjetoDeColecao(colecaoUsuarioSituacao));
				Fachada.getInstancia().atualizar(usuarioAfastamento.getUsuarioAfastado());
			}
		} else { //Afastamento Definitivo
			FiltroUsuarioSituacao filtroUsuarioSituacao = new FiltroUsuarioSituacao();
			filtroUsuarioSituacao.adicionarParametro(new ParametroSimples( FiltroUsuarioSituacao.DESCRICAO_ABREVIADA, "INATIV"));
			Collection colecaoUsuarioSituacao = Fachada.getInstancia().pesquisar(filtroUsuarioSituacao, UsuarioSituacao.class.getName());
			
			usuarioAfastamento.getUsuarioAfastado().setUsuarioSituacao((UsuarioSituacao) Util.retonarObjetoDeColecao(colecaoUsuarioSituacao));
			Fachada.getInstancia().atualizar(usuarioAfastamento.getUsuarioAfastado());
			usuarioAfastamento.setDtAfastamentoFinal(Util.converteStringParaDate("31/12/9999"));
		}
		return usuarioAfastamento;
	}
	
	/* [UC0291] Bloquear/Desbloquear Acesso
	 * [SB001] Pesquisar Afastamento 
	 * RM 3892 - Inclusão de dados de afastamento do usuário
	 */
	public UsuarioAfastamento pesquisarAfastamentoVigente(Integer idUsuario) throws ControladorException { 
		UsuarioAfastamento retorno=null;
		
		//Verifica afastamento atual 
		FiltroUsuarioAfastamento filtroUsuarioAfastamento = new FiltroUsuarioAfastamento();
		filtroUsuarioAfastamento.adicionarParametro(new ParametroSimples(FiltroUsuarioAfastamento.USUARIO_AFASTADO_ID, idUsuario));
		filtroUsuarioAfastamento.adicionarCaminhoParaCarregamentoEntidade(FiltroUsuarioAfastamento.USUARIO_ESPELHO);
		filtroUsuarioAfastamento.adicionarCaminhoParaCarregamentoEntidade(FiltroUsuarioAfastamento.USUARIO_AFASTADO);
		filtroUsuarioAfastamento.adicionarCaminhoParaCarregamentoEntidade(FiltroUsuarioAfastamento.USUARIO_AFASTAMENTO_MOTIVO);		
		filtroUsuarioAfastamento.adicionarCaminhoParaCarregamentoEntidade(FiltroUsuarioAfastamento.USUARIO_AFASTADO_SITUACAO);		
		Collection<UsuarioAfastamento> colecaoUsuarioAfastamento = Fachada.getInstancia().pesquisar(filtroUsuarioAfastamento, UsuarioAfastamento.class.getName());
		if (colecaoUsuarioAfastamento!=null && !colecaoUsuarioAfastamento.isEmpty()){
			for (UsuarioAfastamento ua:colecaoUsuarioAfastamento){
				int dtInicial = Util.compararData(ua.getDtAfastamentoInicial(), new Date());
				int dtFinal = Util.compararData(ua.getDtAfastamentoFinal(), new Date());
				if (dtFinal!=-1){
					retorno = ua;
				}
			}
	 ...
	 * [SB0007] â€“ Liberar acesso no retorno de bloqueio.
	 * @param usuarioLogado
	 * @author Amélia Pessoa
	 * @date 05/01/2012
	 */	
	public String[] liberarAcesso(Usuario usuarioLogado)throws ControladorException {
		if (usuarioLogado!=null){
			UsuarioAfastamento usuarioAfastamento = this.pesquisarAfastamentoVigente(usuarioLogado.getId());
			if (usuarioAfastamento!=null){
				if (usuarioAfastamento.getIcAfastamentoTemporario()==2){ //Usuario INATIVO
					return new String[] {"1"};
				} else if (usuarioAfastamento.getUsuarioAfastado().getUsuarioSituacao().getId().intValue()==UsuarioSituacao.TEMPORARIAMENTE_AFASTADO.intValue()) { 
					//Usuario TEMPORARIAMENTE AFASTADO
					return new String[] {"2", Util.formatarData(usuarioAfastamento.getDtAfastamentoFinal())};
				}
			} else {
				UsuarioAfastamento usuarioUltimoAfastamento = pesquisarUltimoAfastamento(usuarioLogado.getId());
				if (usuarioUltimoAfastamento!=null){
					if (usuarioUltimoAfastamento.getUsuarioEspelho()!=null){
						removerPrivilegioUsuarioEspelho(usuarioUltimoAfastamento);
					}			
					//Atualizar a situação do usuário para ATIVO
					FiltroUsuarioSituacao filtroUsuarioSituacao = new FiltroUsuarioSituacao();
					filtroUsuarioSituacao.adicionarParametro(new ParametroSimples( FiltroUsuarioSituacao.DESCRICAO, "ATIVO"));
					Collection colecaoUsuarioSituacao = Fachada.getInstancia().pesquisar(filtroUsuarioSituacao, UsuarioSituacao.class.getName());
					
					usuarioLogado.setUsuarioSituacao((UsuarioSituacao) Util.retonarObjetoDeColecao(colecaoUsuarioSituacao));
	...
	/* Método Auxiliar para pesquisar afastamento anterior
	 * RM 3892 - Inclusão de dados de afastamento do usuário
	 */
	public UsuarioAfastamento pesquisarUltimoAfastamento(Integer idUsuario) throws ControladorException{ 
		UsuarioAfastamento retorno=null;
		FiltroUsuarioAfastamento filtroUsuarioAfastamento = new FiltroUsuarioAfastamento();
		filtroUsuarioAfastamento.adicionarParametro(new ParametroSimples(FiltroUsuarioAfastamento.USUARIO_AFASTADO_ID, idUsuario));
		filtroUsuarioAfastamento.adicionarCaminhoParaCarregamentoEntidade("usuarioEspelho");
		filtroUsuarioAfastamento.adicionarCaminhoParaCarregamentoEntidade("usuarioAfastado");
		Collection<UsuarioAfastamento> colecaoUsuarioAfastamento = Fachada.getInstancia().pesquisar(filtroUsuarioAfastamento, UsuarioAfastamento.class.getName());
		Collection<UsuarioAfastamento> anteriores = new ArrayList<UsuarioAfastamento>();
		if (colecaoUsuarioAfastamento!=null && !colecaoUsuarioAfastamento.isEmpty()){
			for (UsuarioAfastamento ua:colecaoUsuarioAfastamento){
				if (ua.getDtAfastamentoFinal().before(new Date())){
					anteriores.add(ua);
				}
			}
		} else {
		...
		if (anteriores.size()==1){
			return (UsuarioAfastamento) Util.retonarObjetoDeColecao(anteriores);
		} else {
			UsuarioAfastamento ultimo = (UsuarioAfastamento) Util.retonarObjetoDeColecao(anteriores);
			for (UsuarioAfastamento ua:anteriores){
				if (ua.getDtAfastamentoFinal().after(ultimo.getDtAfastamentoFinal())){
					ultimo = ua;
				}
			}
			retorno = ultimo;
	...
	/**
	 * [UC0287] Efetuar Login
	 * [SB0008] â€“ Remover privilégios usuário espelho
	 * @param usuarioLogado
	 * @author Amélia Pessoa
	 * @throws ControladorException 
	 * @date 05/01/2012
	 */
	public void removerPrivilegioUsuarioEspelho(UsuarioAfastamento usuarioAfastamento) throws ControladorException{
		/* 1. Grupos */
		FiltroUsuarioGrupo filtroUsuarioGrupo = new FiltroUsuarioGrupo();
		filtroUsuarioGrupo.adicionarParametro(new ParametroSimples(FiltroUsuarioGrupo.USUARIO_AFASTAMENTO_ID, usuarioAfastamento.getId()));
		Collection<UsuarioGrupo> colecaoUsuarioGrupo = getControladorUtil().pesquisar(filtroUsuarioGrupo, UsuarioGrupo.class.getName());
		
		if (colecaoUsuarioGrupo!=null){
			for (UsuarioGrupo ug: colecaoUsuarioGrupo){ 
				getControladorUtil().remover(ug);
			}
		}		
		...
		FiltroUsuarioPemissaoEspecial filtroUsuarioPemissaoEspecial = new FiltroUsuarioPemissaoEspecial();
		filtroUsuarioPemissaoEspecial.adicionarParametro(new ParametroSimples(
				FiltroUsuarioPemissaoEspecial.USUARIO_AFASTAMENTO_ID, usuarioAfastamento.getId()));
		Collection<UsuarioPermissaoEspecial> colecaoUsuarioPermissaoEspecial = getControladorUtil().pesquisar(
				filtroUsuarioPemissaoEspecial, UsuarioPermissaoEspecial.class.getName());
		
		if (colecaoUsuarioPermissaoEspecial!=null){
			for (UsuarioPermissaoEspecial upe: colecaoUsuarioPermissaoEspecial){ 
				getControladorUtil().remover(upe);
			}
		}
	}
	 ...
	 * [FS-0011] â€“ Clonar usuário GSAN na base
	 * @author Amélia Pessoa
	 * @date 06/01/2012
	 * @param solicitacaoAcesso
	 */
	public void clonarUsuario(SolicitacaoAcesso solicitacaoAcesso, Usuario usuarioLogado, Usuario usuarioCadastro)throws ControladorException {
		try{
			
			//Altera situacao da solicitacao de acesso
			FiltroSolicitacaoAcessoSituacao filtroSAS = new FiltroSolicitacaoAcessoSituacao();
			filtroSAS.adicionarParametro(new ParametroSimples(FiltroSolicitacaoAcessoSituacao.ID, 
							SolicitacaoAcessoSituacao.CAD_POR_AUTORIZACAO));
			Collection<SolicitacaoAcessoSituacao> colecaoSolicitacaoAcessoSituacao = 
				getControladorUtil().pesquisar(filtroSAS,SolicitacaoAcessoSituacao.class.getName());
			...
			if(solicitacaoAcesso.getUsuario() == null || solicitacaoAcesso.getUsuario().equals("")){
			...
			//Criação de novo user no BD
			criarUsuarioBD(solicitacaoAcesso.getLogin(), usuarioBD,usuarioCadastro.getSenha());
					
			//Gravação da operação na tabela de histórico.
			UsuarioBDHistorico usuarioBDHistorico = new UsuarioBDHistorico(usuarioLogado, usuarioCadastro, new Date());
			this.getControladorUtil().inserir(usuarioBDHistorico);
	...
	/**
	 * Pesquisa os grupos marcados pro usuário em sua solicitação de acesso
	 * @param id
	 * @return Integer[] id's dos grupos
	 * @throws ControladorException 
	 */
	public Integer[] pesquisarSolicitacaoAcessoGrupo(Integer id) throws ControladorException {
		try {
			Collection<Integer> colecao =  repositorioUsuario.pesquisarSolicitacaoAcessoGrupo(id);
			Integer[] retorno;
			retorno = new Integer[colecao.size()];
			int i=0;
			for (Integer sag : colecao){
				retorno[i] = sag;
				i++;
			}
			return retorno;
	...
	/**
	 * [UC1093] Manter Solicitação de Acesso
	 * Criação de novo usuário e concessão da permissão de acesso
	 * @author Amelia Pessoa
	 * @date 09/01/2012
	 * @param login
	 * @param senha
	 */
	public void criarUsuarioBD(String login, Usuario usuarioBD,String senha) throws ControladorException {
		try {
			//Verifica se usuario já existe
			if (!repositorioUsuario.verificaUsuarioExisteBanco(login)){
				repositorioUsuario.criarUsuarioBD(login, usuarioBD,senha);
			}			
		} catch (ErroRepositorioException ex) {
			sessionContext.setRollbackOnly();
			throw new ControladorException("erro.sistema", ex);
	...
	/**
	* [UC0287] -  Efetuar Login
	 * [FS00014] - Verificar validacao de usuarios filhos
	 * @author Fernanda Almeida
	 * @date 25/01/2012
	 */
	public Collection<Usuario> verificaValidacaoUsuarioFilho(Integer idFuncionario, boolean expirado) throws ControladorException {
		try{
			return repositorioUsuario.verificaValidacaoUsuarioFilho(idFuncionario,expirado);
		} catch (ErroRepositorioException ex) {
			throw new ControladorException("erro.sistema", ex);
		}
	}
	...
	/**
	 * [UC0291] Bloquear/Desbloquear Acesso
	 * RM 3892 - Excluir de dados de afastamento do usuario
	 * @author Amelia Pessoa
	 * @date 17/02/2012
	 * @param usuarioAfastamento
	 * @throws ControladorException
	 */
	public void excluirAfastamentoVigente(UsuarioAfastamento usuarioAfastamento) throws ControladorException {
		usuarioAfastamento = pesquisarAfastamentoVigente(usuarioAfastamento.getUsuarioAfastado().getId());
		if (usuarioAfastamento!=null){
			//Altera a data do afastamento pra que ele saia de vigência
			usuarioAfastamento.setDtAfastamentoFinal(Util.subtrairNumeroDiasDeUmaData(new Date(), 1));
			usuarioAfastamento.setUltimaAlteracao(new Date());
			getControladorUtil().atualizar(usuarioAfastamento);
		}
	}
	 ...
	 * @date 30/08/2012
	 */
	public BigDecimal pesquisarCompetenciaRetificacaoUsuario(Usuario usuarioLogado)
			throws ControladorException{
		try{
			return repositorioUsuario.pesquisarCompetenciaRetificacaoUsuario(usuarioLogado);
		} catch (ErroRepositorioException ex) {
			throw new ControladorException("erro.sistema", ex);
		}
	}
	 ...
	 * @date 25/09/2012
	 */
	public Collection pesquisarGruposUsuarioComVisualizacaoRestrita(Integer idUsuario,Short indicadorGrupoEspecial)
			throws ControladorException{
		try{
			return repositorioUsuario.pesquisarGruposUsuarioComVisualizacaoRestrita(idUsuario,indicadorGrupoEspecial);
		} catch (ErroRepositorioException ex) {
			throw new ControladorException("erro.sistema", ex);
		}
	}
	...
	/**
	 * FS0018 - Verificar se Usuário Solicitante é do Grupo Especial Segurança
	 * @param usuario
	 * @author Amelia Pessoa
	 * @date 16/10/2012
	 * @return boleano
	 */
	public boolean isGrupoEspecial(Integer idUsuario) throws ControladorException{
		try{
			Collection<Grupo> gruposEspeciais = repositorioUsuario.pesquisarGruposEspeciais(idUsuario);
			if (gruposEspeciais==null || gruposEspeciais.size()==0){
				return false;
			}
			return true;
		} catch (ErroRepositorioException ex) {
	...
	/**
	 * O sistema emite email informando de que foi incluída uma solicitação de acesso a um 
	 * grupo especial para o Usuário que foi informado como usuário autorizador e 
	 * Para todos usuários que façam parte do grupo especial de segurança
	 * 
	 * @param solicitacaoAcesso
	 * @param tipoEmail
	 * @param idGrupoEspecial
	 * @author Amelia Pessoa
	 * @date 16/10/2012
	 */
	public void enviarEmailGrupoEspecialSeguranca(SolicitacaoAcesso solicitacaoAcesso, Integer tipoEmail, Integer idGrupoEspecial) throws ControladorException{
		
		EnvioEmail envioEmail = Fachada.getInstancia().pesquisarEnvioEmail(tipoEmail);
		...
		filtroGrupo.adicionarParametro(new ParametroSimples(FiltroGrupo.ID, idGrupoEspecial));
		Collection<Grupo> colecaoGrupo = getControladorUtil().pesquisar(filtroGrupo, Grupo.class.getName());
		Grupo grupo = (Grupo) Util.retonarObjetoDeColecao(colecaoGrupo);
		
		//Obter Funcionario
		FiltroFuncionario filtroFuncionario = new FiltroFuncionario();		
		filtroFuncionario.adicionarParametro(new ParametroSimples(FiltroFuncionario.ID, solicitacaoAcesso.getFuncionarioResponsavel().getId()));
		Collection<Funcionario> colecaoFuncionario = getControladorUtil().pesquisar(filtroFuncionario, Funcionario.class.getName());
		Funcionario funcionario = (Funcionario) Util.retonarObjetoDeColecao(colecaoFuncionario);
		
		mensagem += " e de nome "+ solicitacaoAcesso.getNomeUsuario()+" solicitou acesso para o grupo especial "
				+ grupo.getDescricao()+ ", sendo o funcionário responsável pela autorização: "
				+ funcionario.getNome()+".";
		
		String titulo = "Solicitação de Acesso a grupo especial.";
		
		//Busca usuarios que façam parte do grupo especial de segurança
		Collection<Usuario> usuariosGrupoSeguranca=null;
		try {
			usuariosGrupoSeguranca = repositorioUsuario.pesquisarUsuariosGruposEspeciais();
		} catch (ErroRepositorioException e1) {
			e1.printStackTrace();
		}
		
		//Envia email para cada um deles
		if (usuariosGrupoSeguranca!=null){
			for (Usuario usuario : usuariosGrupoSeguranca){
				try {
					ServicosEmail.enviarMensagem(envioEmail.getEmailRemetente(), usuario.getDescricaoEmail(),
							titulo, mensagem);
				} catch (ErroEmailException e) {
					e.printStackTrace();
				}
