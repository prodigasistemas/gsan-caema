...
public class RelatorioResumoDadosCas extends TarefaRelatorio {
	...
	public Object executar() throws TarefaException {
		...
		Collection<Object> colecaoAnormalidadeHidrometro = 
				fachada.pesquisarAnormalidadeHidrometroResumoDadosCas(filtro);
		...
		BigDecimal totalLigacoesAnormalidade  = new BigDecimal(0);
		BigDecimal totalLigacoesAtivas = new BigDecimal(0);
		
		Long sumTotalLigacoesAnormalidade = new Long(0);
		Long sumTotalLigacoesAtivas = new Long(0);
		...
		if (colecaoResumoLigacaoEconomia != null && !colecaoResumoLigacaoEconomia.isEmpty()) {
		    ...
		    Iterator iteratorRAH = colecaoAnormalidadeHidrometro.iterator();
			...
			while (iteratorRELE.hasNext() || iteratorRECA.hasNext() 
				...
				if(objRELE[0] != null && ((Long)objRELE[0]).compareTo(new Long(0)) != 0 
						&& objRELE[6] != null && ((Long)objRELE[6]).compareTo(new Long(0)) != 0){
				...
				if(colecaoResumoConsumoAgua != null && colecaoResumoConsumoAgua.size() != 0 && iteratorRECA.hasNext()){
					...
					if ( objRECA[11] != null && relatorioBean.getTotalLigacoesFaturadasAgua() > 0 ) {
						BigDecimal valorLigacoesFaturadasAguaHidrometradas = new BigDecimal((Long) objRECA[11]);
						relatorioBean.setIndiceHidrometracaoFaturado(  Util.dividirArredondando(valorLigacoesFaturadasAguaHidrometradas,
								new BigDecimal(relatorioBean.getTotalLigacoesFaturadasAgua())));
					} else {
						relatorioBean.setIndiceHidrometracaoFaturado( new BigDecimal(0) );
					}
				}else{
				...
				Object[] objRAH = null;
				if(colecaoAnormalidadeHidrometro != null && colecaoAnormalidadeHidrometro.size() != 0 && iteratorRAH.hasNext()){
					objRAH = (Object[])iteratorRAH.next();
					totalLigacoesAnormalidade = new BigDecimal((Long)objRAH[0]);
					
					if((Long)objRAH[1] != null && ((Long)objRAH[1]).compareTo(new Long(0)) != 0){
						totalLigacoesAtivas = new BigDecimal((Long)objRAH[1]);
						relatorioBean.setPercentualAnormalidadeHidrometro(Util.dividirArredondando(totalLigacoesAnormalidade, totalLigacoesAtivas));
					}
				}
				else{
					relatorioBean.setPercentualAnormalidadeHidrometro(new BigDecimal(0));
				}
				...
				if(relatorioBean.getTotalArrecadacaoAtual().compareTo(new BigDecimal(0))!=0 
					...
					if (filtro.getOpcaoTotalizacao() == TOTALIZACAO_ESTADO_POR_GERENCIA_REGIONAL || filtro.getOpcaoTotalizacao() == TOTALIZACAO_ESTADO_POR_UNIDADE_NEGOCIO){
						...
						if(objRAH!= null){					
							//BigDecimal ligacoesAnormalidade = new BigDecimal(0);
							sumTotalLigacoesAnormalidade += (Long)objRAH[0];
							sumTotalLigacoesAtivas += (Long)objRAH[1];
						}
			...
			if (filtro.getOpcaoTotalizacao() == TOTALIZACAO_ESTADO_POR_GERENCIA_REGIONAL || filtro.getOpcaoTotalizacao() == TOTALIZACAO_ESTADO_POR_UNIDADE_NEGOCIO
				...
				totalLigacoesAnormalidade = new BigDecimal(sumTotalLigacoesAnormalidade);
				totalLigacoesAtivas = new BigDecimal(sumTotalLigacoesAtivas);
				
				if(totalLigacoesAnormalidade.compareTo(new BigDecimal("0")) != 0 
						&& totalLigacoesAtivas.compareTo(new BigDecimal("0")) != 0){
					relatorioBean.setPercentualAnormalidadeHidrometro(Util.dividirArredondando(totalLigacoesAnormalidade, totalLigacoesAtivas));					
				}else{
					relatorioBean.setPercentualAnormalidadeHidrometro(new BigDecimal(0));
				}
