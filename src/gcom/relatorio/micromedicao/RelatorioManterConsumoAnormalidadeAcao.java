...
import gcom.relatorio.faturamento.conta.Relatorio2ViaContaBoletoBancarioBean;
...
public class RelatorioManterConsumoAnormalidadeAcao extends TarefaRelatorio {
	...
	public Object executar() throws TarefaException {
		...
		//FiltroConsumoAnormalidadeAtividadeAcao filtroConsumoAnormalidadeAcao = (FiltroConsumoAnormalidadeAtividadeAcao) 
		...
		/*Collection colecaoConsumoAnormalidadeAcao = fachada.pesquisar(filtroConsumoAnormalidadeAcao,
				ConsumoAnormalidadeAtividadeAcao.class.getName());*/
		...
		if (colecaoConsumoAnormalidadeAcao != null && !colecaoConsumoAnormalidadeAcao.isEmpty()) {
			...
			while (consumoAnormalidadeAcaoIterator.hasNext()) {
				...
				ConsumoAnormalidadeAtividadeAcao consumoAnormalidadeAcao = (ConsumoAnormalidadeAtividadeAcao) consumoAnormalidadeAcaoIterator.next();				
				...
				int idConsumoAnormalidade = (int) consumoAnormalidadeAcao.getId();
				...
				filtroMensal.setCampoOrderBy(FiltroConsumoAnormalidadeAtividadeAcaoMensal.ORDEM_MES);
				...
				Collection<RelatorioManterConsumoAnormalidadeAcaDetailsoBean> details = new ArrayList<RelatorioManterConsumoAnormalidadeAcaDetailsoBean>();
				

				
				Iterator consumoAnormalidadeMesalIterator = colecaoAnormalidadeAcaoMensal.iterator();			
				
				// laço para criar a coleção de parâmetros da analise
				while (consumoAnormalidadeMesalIterator.hasNext()) {
					
					ConsumoAnormalidadeAtividadeAcaoMensal consumoAnormalidadeMes = (ConsumoAnormalidadeAtividadeAcaoMensal) consumoAnormalidadeMesalIterator.next();
					
					String descricaoConsumoACobrar = consumoAnormalidadeMes.getLeituraAnormalidadeConsumo().getDescricaoConsumoACobrar();

					String serviçoTipo="";
					if(consumoAnormalidadeMes.getServicoTipo()!=null){
						serviçoTipo = consumoAnormalidadeMes.getServicoTipo().getDescricao();
					}
					
					String tipoEspecificacao = "";
					if(consumoAnormalidadeMes.getSolicitacaoTipoEspecificacao()!=null){
						tipoEspecificacao = consumoAnormalidadeMes.getSolicitacaoTipoEspecificacao().getDescricao();	
					}
					
					String ordemMes = "";
					if(consumoAnormalidadeMes.getCodigoOrdemMes()!=null){
						ordemMes = consumoAnormalidadeMes.getCodigoOrdemMes().toString();
					}
					
					String mensagemConta = "";
					if(consumoAnormalidadeMes.getDescricaoContaMensagem() !=null){
						mensagemConta = consumoAnormalidadeMes.getDescricaoContaMensagem().toString();
					}
					
					
					String indicadorGerarCarta = "";
					if( consumoAnormalidadeMes.getIndicadorGeracaoCarta().toString().equals("1")){
						indicadorGerarCarta = "Sim";
					}else{
						indicadorGerarCarta = "Não";
					}
					
					String numeroFatorConsumo = consumoAnormalidadeMes.getNumerofatorConsumo().toString();
					
					RelatorioManterConsumoAnormalidadeAcaDetailsoBean detalhe = 
							new RelatorioManterConsumoAnormalidadeAcaDetailsoBean
							(descricaoConsumoACobrar,serviçoTipo,tipoEspecificacao,ordemMes,indicadorGerarCarta,numeroFatorConsumo, mensagemConta);
