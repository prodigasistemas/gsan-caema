...
public class RelatorioRelacaoOrdensServicoEncerradasPendentes extends TarefaRelatorio {
	...
	public Object executar() throws TarefaException {
		...
		String dataExecucaoInicial = (String) getParametro("dataExecucaoInicial");
		String dataExecucaoFinal = (String) getParametro("dataExecucaoFinal");
		...
		String[] colecaoCapacidadeHidrometro = null;
		if(getParametro("colecaoCapacidadeHidrometro") != null)
			colecaoCapacidadeHidrometro =  getParametro("colecaoCapacidadeHidrometro").toString().replaceAll("[^0-9,]", "").split(",");
		...
		Collection colecaoDadosHelper = fachada
				.pesquisarOrdemServicoGerarRelatorioAcompanhamentoHidrometro(
						firma, tipoOrdem, situacaoOrdemServico, idGerenciaRegional, colecaoCapacidadeHidrometro,
						idLocalidadeInicial, idLocalidadeFinal, dataEncerramentoInicial, dataEncerramentoFinal,
						dataExecucaoInicial, dataExecucaoFinal,
		...
		if (colecaoDadosHelper != null && !colecaoDadosHelper.isEmpty()) {
			...
			while (iColecaoDados.hasNext()) {
				...
				// Imovel Selecionado
				relatorioBean.setImovel(Util.retornaMatriculaImovelFormatada((Integer) helper.getIdImovel()));
				// Localidade
				...
				if (helper.getIdMotivoEncerramento() != null){
					relatorioBean.setIdMotivoEncerramento(helper.getIdMotivoEncerramento().toString());
				}
				...
				if (dataExecucaoInicial != null && !dataExecucaoInicial.equals("") &&
					...
					relatorioBean.setPeriodoExecucao(dataExecucaoInicial + " à " + dataExecucaoFinal);
				}else {
					relatorioBean.setPeriodoExecucao("");
				}
				// Nome Localidade
				...
				if(helper.getNumeroHidrometro() != null){
					relatorioBean.setNumeroHidrometro(helper.getNumeroHidrometro());
				}else{
					relatorioBean.setNumeroHidrometro("");
				}
				
				//Tipo Caixa
				if(helper.getTipoCaixa() != null){
					relatorioBean.setTipoCaixa(helper.getTipoCaixa());
				}else{
					relatorioBean.setTipoCaixa("");
				}
				
				//KIT
				Short indTrocaRegistro = helper.getIndicadorTrocaRegistro();
				if(indTrocaRegistro != null){
					if(indTrocaRegistro == 1){
						relatorioBean.setIndicadorTrocaRegistro("Sim");
					}else{
						relatorioBean.setIndicadorTrocaRegistro("Não");
					}
				}else{
					relatorioBean.setIndicadorTrocaRegistro("");
				}
			...
			parametros.put("numeroRelatorio", "R0732");
