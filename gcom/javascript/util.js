...
/**
 * funcao que manipula select multiplo movendo os dados de um select para outro e valida se foi selecionado algum item
 ...
 * @since 10/04/2012
 * *********/
function MoverDadosSelectMenu1PARAMenu2ComMensagem(nomeForm, nomeCampoMenu1, nomeCampoMenu2, msgSelectVazio, msgItemNaoSelecionado) {
  ...
  if(m1len > 0){
	  if(selected){
		  ...
		  alert(msgItemNaoSelecionado);
	  }
  }else{
	  alert(msgSelectVazio);
  }
  
}

/**
 * funcao que manipula select multiplo movendo os dados de um select para outro e valida se existe algum item na lista de origem
 * nomeForm - Nome do form a ser manipulado
 * nomeCampoMenu1 - Nome do campo do select 1 que contem os dados disponoveis
 * nomeCampoMenu2 - Nome do campo do select 2 que contem os dados cadastrados
 * 
 ...
 * @since 10/04/2012
 * *********/
function MoverTodosDadosSelectMenu1PARAMenu2ComMensagem(nomeForm, nomeCampoMenu1, nomeCampoMenu2, msgSelectVazio) {
 ...
 * @since 10/04/2012
 * *********/
function MoverDadosSelectMenu2PARAMenu1ComMensagem(nomeForm, nomeCampoMenu1, nomeCampoMenu2, msgSelectVazio, msgItemNaoSelecionado) {
 ...
 * @since 10/04/2012
 * *********/
function MoverTodosDadosSelectMenu2PARAMenu1ComMensagem(nomeForm, nomeCampoMenu1, nomeCampoMenu2, msgSelectVazio) {
...
function botaoAvancarTelaEsperaComando(uri){
	document.forms[0].action=uri;
...
function verificaDataSemMensagem (mydata) {
	dia = mydata.value.substring(0,2);
    mes = mydata.value.substring(3,5); 
    ano = mydata.value.substring(6,10); 
...
function calculaValorParcela() {
	...
	*/
	...
	function verificarAtributosDataInicialFinal(campoInicio, campoFim, nomeCampo){
		var msg = "";
	...
	*/
	...
	/**
	 * @autor: Amelia Pessoa
	 * Aceita numeros e . (ponto) como separador de decimal
	 * Ex.: onkeypress="return isBigDecimal(event);" 
	 */
	function isBigDecimal(evento) {
		  var tecla = null;
...
function comparaDataComDataAtual(data1, operador ){
	...
	/**
	 * Função que verifica se o campo
	 * possui uma hora no formato válido
	 * @autor: Raimundo Martins
	 * */
	function validarCampoHoraComMensagemLimpandoCampo(campo,nomeCampo){
		if( !isBrancoOuNulo(campo.value)){			
			if(!validaHoraMinuto(campo.value)){
				campo.value = '';
 ...
 */
	...
	/**
	 * Metodo responsavel por validar campo texto Caso o usuario copie e cole algum caracter indevido ao tirar o foco do campo o caracter é removido.
	 * O usuario so pode digitar numeros
	 * 
	 * @author Arthur Carvalho
	 * @date 24/01/2012
	 ...
	 * @returns {Boolean}
	 */
		function campoTextoSemAcento(evento, campo) {
	...
	function mascaraEnderecoMac(campo, teclapres){
		var tecla = teclapres.keyCode;
		var vr 	  = campo.value;
		var tam   = vr.length + 1;
		...
		if(tecla != 14){
			...
			if(tam == 9)
				campo.value = vr.substr(0, 2) + ':' + vr.substr(3, 2) + ':' + vr.substr(6, 2) + ':';
			if(tam == 12)
				campo.value = vr.substr(0, 2) + ':' + vr.substr(3, 2) + ':' + vr.substr(6, 2) + ':' + vr.substr(9, 2) + ':';
			if(tam == 15)
				campo.value = vr.substr(0, 2) + ':' + vr.substr(3, 2) + ':' + vr.substr(6, 2) + ':' + vr.substr(9, 2) + ':' + vr.substr(12, 2) + ':';
		}
	}
	...
	function mascaraProcessoJudicial(v){
		v=v.replace(/\D/g,"");	                
	...
	function mascaraAnoMesReferencia(v){
        v=v.replace(/\D/g,"")
	...
	function mascaraAlfaNumericoComEspaco(v){
		 v=v.replace(/[^a-zA-Z0-9\s]/g,"")
		 return v
	}
	
	function mascaraCampoData(v){
		v=v.replace(/\D/g,"");
        v=v.replace(/^(\d{2})(\d)/,"$1/$2");
		v=v.replace(/(\d{2})(\d)/,"$1/$2");
		return v;
	...
	function mascaraTextAreaSemEnter(v){
		v=v.replace(/\n/g,"");
		return v;
	}
	
	
	function validarPorcentagemComMsg(obj,msg){
		var multiplier = Math.pow(10,2);
		var a = Math.round(parseFloat(obj.value.replace(",", ".")) * multiplier);
		var b = Math.round(100 * multiplier);
		if(a > b){
			obj.value = '';
			alert(msg);
		}
	...
	/**
	 * Metodo responsavel por se todas as incricoes do imovel foram informadas corretamente no submit.
	 * @author Arthur Carvalho
	 * Somente utilizar quando incluir o jsp filtro_inscricao_inicial_final.jsp
	 */
	function validarInscricaoInicialFinal() {
		var form = document.forms[0];
		...
		if ( form.numeroRotaInicial.value != null && form.numeroRotaInicial.value != '' &&
				(form.numeroQuadraInicial.value == null || form.numeroQuadraInicial.value == '') ) {
					alert("Informe a Quadra Inicial");
					return false;
		}
		
		if ( form.numeroSequencialRotaInicial.value != null && form.numeroSequencialRotaInicial.value != '' &&
				(form.numeroRotaInicial.value == null || form.numeroRotaInicial.value == '') ) {
					alert("Informe a Rota Inicial");
					return false;
		}
		
		
		if ( form.numeroSequencialRotaFinal.value != null && form.numeroSequencialRotaFinal.value != '' &&
			(form.numeroSequencialRotaInicial.value == null || form.numeroSequencialRotaInicial.value == '') ) {
				alert("Informe o Sequencial da Rota Inicial");
				return false;
		}
		...
		if ( form.numeroSequencialRotaInicial.value != null && form.numeroSequencialRotaInicial.value != '' &&
				(form.numeroRotaFinal.value == null || form.numeroRotaFinal.value == '') ) {
	...
	/**
	 * Mascara que formata numero inteiro com milhar Ex: 00.000
	 * @author Rômulo Aurélio
	 * */
	
	function mascaraInteiroComMilhar(v){	
	...
	function isCampoNumericoNegativoComVirgula(evento) {
		  var tecla = null;
	      ...
	      if (tecla == 8 || tecla == 13 || tecla == 45 || tecla == 44){ // backspace ou enter ou menos(-) ou virgula(,)
		        return true;
	      }
