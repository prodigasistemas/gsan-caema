...
<%@ page import="gcom.micromedicao.Leiturista" %>
		...
		<tr>
			...
			<table width="100%" border="0">
				...
				<tr>
					...
					<td align="left">
						<select name="idLeiturista">
							<option value="-1">&nbsp;</option>
							<% Collection<Leiturista> col = (Collection<Leiturista>) session.getAttribute("collectionLeiturista");
							for (Leiturista leiturista: col){
									if(leiturista.getCliente() != null){%>
									<option  value="<%=leiturista.getId()%>"><%=leiturista.getCliente().getNome()%></option>
									<%}else if(leiturista.getFuncionario() != null){%>%>
									<option  value="<%=leiturista.getId()%>"><%=leiturista.getFuncionario().getNome()%></option>
							<%	} }%>
