	...
	function selecionar(){
	 ...
	 function habilitarPesquisaConta(){
	    ...
	    if(form.idTipoDocumento.value == CONTA){
	    	...
	    	if(form.situacaoDebito[0].checked == true){
   				...
   				redirecionarSubmit('exibirPesquisarContaAction.do?idImovel='+form.idImovel.value+ '&limpaForm=1&situacaoConta=pe&caminhoRetornoTelaPesquisaConta=exibirSelecionarDebitosPagosPopupAction');
   			}
	    }
	 }
	 ...
	 function habilitarPesquisaGuiaPagamento(form){
	    ...
	    if(form.idTipoDocumento.value == GUIA_PAGAMENTO){
			...
			var situacaoDebito = "1";
			if(form.situacaoDebito[0].checked){
				situacaoDebito = "1";
			}else if(form.situacaoDebito[1].checked){
				situacaoDebito = "2";
			}
			
   			redirecionarSubmit('exibirPesquisarGuiaPagamentoAction.do?idImovel='+form.idImovel.value+ '&situacaoDebito='+situacaoDebito+ '&limpaForm=1&caminhoRetornoTelaPesquisaGuiaPagamento=exibirSelecionarDebitosPagosPopupAction');
	 ...
	 function habilitarPesquisaDebitoACobrar(form){
	    ...
	    if(form.idTipoDocumento.value == DEBITO_A_COBRAR){
	    	...
	    	if(form.situacaoDebito[0].checked){
	   		...
	   		redirecionarSubmit('exibirPesquisarDebitoACobrarAction.do?idImovel='+form.idImovel.value+ '&situacaoDebito='+situacaoDebito+ '&caminhoRetornoTelaPesquisaDebitoACobrar=exibirSelecionarDebitosPagosPopupAction');
      ...
      <table width="100%" border="0">
       ...
       <table width="100%" border="0">
	        ...
	        <tr>
	        	...
	        	<td colspan="2">
	        		<html:radio property="situacaoDebito" value="1" /><strong>Pago</strong>
					<html:radio property="situacaoDebito" value="2" /><strong>Parcelado</strong>		
				</td>
	        </tr>
		  ...
		  <td>
		    <strong> 
		      <html:text property="referenciaConta" size="7" maxlength="7"
		      	  onkeyup="document.forms[0].descricaoReferenciaConta.value='';validaAnoMesNumerico(this);mascaraAnoMes(this, event);"
			      onkeypress="javascript:mascaraAnoMes(this,event);validaEnterString(event, '/gsan/exibirSelecionarDebitosPagosPopupAction.do', 'referenciaConta');return isCampoNumerico(event);"/>
		    </strong>mm/aaaa 
        ...
        <tr>
			  ...
			  <td width="82%" height="24">
				    <html:text maxlength="9" tabindex="1" property="idDebitoACobrar" size="9" 
					    ...
					    onkeypress="validaEnterComMensagem(event, '/gsan/exibirSelecionarDebitosPagosPopupAction.do','idDebitoACobrar', 'Debito A Cobrar');return isCampoNumerico(event);"/> 
	        ...
	        <tr>
				  ...
				  <td width="82%" height="24">
				    ...
				    <html:text maxlength="9" tabindex="1"	property="idGuiaPagamento" size="9" 
					    ...
					    onkeypress="validaEnterComMensagem(event, '/gsan/exibirSelecionarDebitosPagosPopupAction.do','idGuiaPagamento', 'Guia Pagamento');return isCampoNumerico(event);"/> 
