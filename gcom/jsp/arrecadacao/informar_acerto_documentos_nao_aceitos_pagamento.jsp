...
<script language="JavaScript" src="<bean:message key="caminho.js"/>validacao/regras_validator.js"></script>
<html:javascript staticJavascript="false"  formName="InformarAcertoDocumentosNaoAceitosActionForm"
...
<script>
	...
	function validaRequiredAvisoBancario () {
		...
		if(isNaN(form.idArrecadador.value)){
			msg = msg + 'Arrecadador só aceita números positivos.\n';
		}

		if(isNaN(form.idAvisoBancario.value)){
			msg = msg + 'Codigo do Aviso Bancário só aceita números positivos.\n';
		}
	...
	function limparMovimento() {
		var form = document.forms[0];
		if(form.idArrecadadorMov.value != ''){
	...
	function filtrarPagamentos(){
		...
		avisoBancario = form.idAvisoBancario;
		...
		if(arrecadador.value != null && arrecadador.value != "") {
			if(validaCampoInteiro(arrecadador) == false){
				msg = msg + 'Número do Arrecadador inválido. \n';
			}
		}
		
		if(avisoBancario.value != null && avisoBancario.value != "") {
			if(validaCampoInteiro(avisoBancario) == false){
				msg = msg + 'Aviso Bancário inválido. \n';
			}
		}
        ...
        <tr>
			...
			<td colspan="2">
				<html:text maxlength="3" property="idArrecadador" size="4"
					onkeypress="return isCampoNumerico(event); validaEnterComMensagem(event, 'informarAcertoDocumentosNaoAceitosWizardAction.do?action=exibirInformarAcertoDocumentosNaoAceitosPagamentoAction', 'idArrecadador','Arrecadador');"
					onkeyup="validaEnter(event, 'informarAcertoDocumentosNaoAceitosWizardAction.do?action=exibirInformarAcertoDocumentosNaoAceitosPagamentoAction', 'idArrecadador','Arrecadador');"  /> 
				...
				<a 	href="javascript:abrirPopup('exibirPesquisarArrecadadorAction.do');"><img
					...
					height="21" title="Pesquisar Arrecadador" border="0"></a>
				...
				<a href="javascript:limparPesquisaArrecadador(document.forms[0]);"> <img
				   ...
				   border="0" title="Apagar Arrecadador" /></a>
			</td>
          ...
          <td colspan="3">
            <strong>
              <html:text property="idAvisoBancario" size="9" maxlength="9" 
              		onkeyup="validaEnter(event, 'informarAcertoDocumentosNaoAceitosWizardAction.do?action=exibirInformarAcertoDocumentosNaoAceitosPagamentoAction', 'idAvisoBancario');limparAvisoBancarioTecla();" 
              		onkeypress="return isCampoNumerico(event);" /> 
			 ...
			 <a href="javascript:limparAvisoBancario();">
					<img src="<bean:message key="caminho.imagens"/>limparcampo.gif"
						border="0" title="Apagar Aviso Bancário" /> </a>
			</strong>
		...
		<tr>
			...
			<td colspan="3">
				...
				<a href="javascript:limparMovimento();">
					<img src="<bean:message key="caminho.imagens"/>limparcampo.gif" border="0" title="Apagar Movimento" /></a>
				</strong>
			</td>
		</tr>
        <tr>
