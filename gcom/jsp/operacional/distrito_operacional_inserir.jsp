...
<script language="JavaScript">
	...
	$(document).ready(function(){
		...
		$('[name=sistemaAbastecimento]').change(function() {
		  var theForm = $("form[name=InserirDistritoOperacionalActionForm]");
		  var params = theForm.serialize();
		  var actionURL = 'exibirInserirDistritoOperacionalAction.do?action=atualizarListaSubsistemaAbastecimento'
			$.ajax({
				type:"POST",
				url:actionURL,
				data:params,
		...
		$('[name=subsistemaAbastecimento]').change(function() {
		  var theForm = $("form[name=InserirDistritoOperacionalActionForm]");
		  var params = theForm.serialize();
		  var actionURL = 'exibirInserirDistritoOperacionalAction.do?action=atualizarListaSetorAbastecimento'
			$.ajax({
				type:"POST",
				url:actionURL,
				data:params,
	...
	function adicionarSetorAbastecimento(){
		  ...
		  if(msg != ""){
			  ...
			  document.forms[0].action = "exibirInserirDistritoOperacionalAction.do?action=inserirSetorAbastecimento";
	...
	function removerSetorAbastecimento(id){
		 var theForm = $("form[name=InserirDistritoOperacionalActionForm]");
		  var params = theForm.serialize();
		  var actionURL = 'exibirInserirDistritoOperacionalAction.do?action=removerSetorAbastecimento&idSetorARemover='+id
			$.ajax({
				type:"POST",
				url:actionURL,
				data:params,
	...
	/*function validarForm() {
		...
		if($("input[name=idSetorAbastecimentoPrincipal]").is(":checked")){
			if(validateInserirDistritoOperacionalActionForm(form)){	     
				form.action = "/gsan/inserirDistritoOperacionalAction.do"	     
...
<body leftmargin="5" topmargin="5" onload="setarFoco('descricao');atualizarCoresTabela();">
					...
					<tr>
						<td colspan="3">
							<table id=header width="100%" border="0" bgcolor="#90c7fc">
								...
								<bean:define name="InserirDistritoOperacionalActionForm" 
											 property="setoresSelecionados" 
											 id="setoresSelecionados" />
							<DIV STYLE="overflow: auto; width: 100%; height: 140; padding:0px; margin: 0px ">						 
								<TABLE border="0" width="100%" bgcolor="#90c7fc" id="tabelaSetoresAbastecimento">
									<tr id="trTabelaClonar" style="display:none">
										<td width=68 align="center">
										 ...
										 <td align="center" width="70">
											<input type="radio" name="idSetorAbastecimentoPrincipal"  class="radioClone" value="${setor.id}" />
										</td>
										<td>
										</td>						
									</tr>
									<c:if test="${InserirDistritoOperacionalActionForm.setoresSelecionados != null }">
				...
				<tr>
					<td width="40%" align="left">
						<input type="button"
							...
							onClick="javascript:window.location.href='/gsan/exibirInserirDistritoOperacionalAction.do?menu=sim';">
						<input type="button"
