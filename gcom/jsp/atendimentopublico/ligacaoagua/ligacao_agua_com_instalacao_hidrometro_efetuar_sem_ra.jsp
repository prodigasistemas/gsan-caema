...
<script language="JavaScript">
	...
	function validaForm() {
		...
		if (validateEfetuarLigacaoAguaComInstalacaoHidrometroSemRAActionForm(form) && validaDebito()){
	 		...
	 		if( (form.indicadorTipoHidrometro[0].checked && form.numeroHidrometro.value != '') ||
	 			(form.indicadorTipoHidrometro[1].checked && form.numeroTombamento.value != '')){
				...
				if(form.indicadorTipoHidrometro[1].checked){
				...
				if(form.localInstalacao.value =='-1'){
					submeter = false;
					alert('Informe Local de Instalação');
				}else
				if(form.protecao.value =='-1'){
					submeter = false;
		...
		if (comparaDataComDataAtual(form.dataLigacao.value, ">")){
			submeter = false;
			alert("A data de ligação não deve ser maior que a data atual!");
	...
	function recuperarDadosPopup(codigoRegistro, descricaoRegistro, tipoConsulta) {
		...
		if(varTipoConsulta == 'tombamento'){		
    		form.numeroTombamento.value = codigoRegistro;
	      	form.action='exibirEfetuarLigacaoAguaComInstalacaoHidrometroSemRAAction.do?tombamento=ok';
	      	form.submit();
    	}
	...
	function reload(funcao){	
		...
		if(funcao == 2)
			form.numeroHidrometro.value = "";
		form.action='exibirEfetuarLigacaoAguaComInstalacaoHidrometroSemRAAction.do';
      	form.submit();
	}
	...
	function limparNumeroTombamento(){	
		var form = document.forms[0];
		form.numeroTombamento.value = "";
	}
	...
	function compararData (){
		...
		var dataLigacao = form.dataLigacao.value;
		
		if (comparaDataComDataAtual(dataLigacao, ">")){
			alert("A data de ligação não deve ser maior que a data atual!");
			form.dataLigacao.value = "";
		}
	}
...
<body leftmargin="5" topmargin="5"
	onload="javascript:setarFoco('${requestScope.nomeCampo}');desabilitaCamposNumeroLacreOnClick();">
									...
									<table border="0" width="100%">
										<tr>
											...
											<td colspan="2"><strong><b> <html:text property="dataLigacao"
												size="10" maxlength="10" onkeyup="mascaraData(this, event)" onblur="compararData()" />
											</b></strong> <a
								...
								<tr bgcolor="#cbe5fe">
									...
									<table border="0" width="100%">
										...
										<tr>
											...
											<td><strong> 
												<html:radio property="indicadorTipoHidrometro" value="1" onchange="reload(1)"/><strong>Micromedidor 	
												<html:radio	property="indicadorTipoHidrometro" value="2" onchange="reload(2)"/>Macromedidor</strong>	
											</strong></td>
										</tr>										
										<c:choose>
											<c:when test="${EfetuarLigacaoAguaComInstalacaoHidrometroSemRAActionForm.indicadorTipoHidrometro == '1'}">
												<tr>
													...
													<a
														href="javascript:abrirPopup('exibirPesquisarHidrometroAction.do?limparForm=true',600,640);"><img
											...
											<c:otherwise>											
												<tr>													
													<td width="37%" class="style3"><strong>Tombamento:
																	</strong></td>
													<td width="58%"><html:text property="numeroTombamento"
														size="13" maxlength="10"
														onkeypress="validaEnterString(event, 'exibirEfetuarLigacaoAguaComInstalacaoHidrometroSemRAAction.do?tombamento=ok', 'numeroTombamento','Tombamento');" />
													<a
														href="javascript:pesquisarHidrometroTombamento()">
													<img src="/gsan/imagens/pesquisa.gif" title="Pesquisar Tombamento"
										...
										<tr>
											<td width="25%"><strong>Telemedido:<font
												color="#FF0000">*</font></strong></td>
				                   			<td height="24" width="25%">
						               			<html:radio property="indicadorMedicaoTelemedido" value="1"><strong>Sim</strong></html:radio>
						               			<html:radio property="indicadorMedicaoTelemedido" value="2"><strong>Não</strong></html:radio>
						               		</td>
										</tr>
					...
					<table width="100%">
						...
						<tr>
							<td width="40%" align="left"><input type="button"
								...
								onclick="window.location.href='/gsan/exibirEfetuarLigacaoAguaComInstalacaoHidrometroSemRAAction.do?menu=sim'"
