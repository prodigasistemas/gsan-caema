...
jQuery(document).ready(function() {
	$("textarea[name='observacaoEncerramento']").val($("[name='observacaoEncerramentoHidden']").val());
 ...
 * Data:09/09/2013
 * Observacao: Essa funcao é usada quando o adicionar uma foto a tela mostrar preenchida
 */
function carregarPaginaAposAdicionarImagem(indicadorAdicionarFoto,idMotivoEncerramento){
	var form = document.forms[0];
	if(indicadorAdicionarFoto==1){
			for(var a=0;form.idMotivoEncerramento.options.length;a++){
				if(form.idMotivoEncerramento.options[a].value==idMotivoEncerramento){
					form.idMotivoEncerramento.value=idMotivoEncerramento;
					if(form.verificadorReloadPagina.value!=""){
						carregarCampos(1);
					}
			}
		}
...
var bCancel = false;
   ...
   function caracteresespeciais () { 
    ...
    function required () { 
     ...
     this.aa = new Array("idMotivoEncerramento", "Informe Motivo do Encerramento.", new Function ("varName", " return this[varName];"));
    } 
	...
	function carregarCampos(opcao){
		...
		if (form.indicadorExecucao.value == form.indicadorExecucaoSim.value){
			...
			form.horaExecucao.value = "";
	  	}
		redirecionarSubmit('exibirEncerrarOrdemServicoAction.do?carregarCampos=OK&indicadorAdicionarFoto='+opcao);
	  ...
	  if(validateEncerrarOrdemServicoActionForm(form)){
	  	if (form.indicadorExecucao.value == form.indicadorExecucaoSim.value){
		  	if(form.dataExecucao.value == ""){
				alert("Data de Execução deve ser informada");
				return false;
			}
		  	else if(form.horaExecucao.value == ""){
				alert("Hora da Execução deve ser informada");
				return false;
			} 
		  	if(form.mostrarAlert.value != null && form.mostrarAlert.value != '') {
...
function adicionarArquivo(){
	...
	var form = document.forms[0];
	form.target = "";
	form.action = "exibirEncerrarOrdemServicoAction.do?addFoto=OK&indicadorAdicionarFoto=1";
...
<body leftmargin="3" topmargin="3"
	onload="window.focus();javascript:setarFoco('${requestScope.nomeCampo}');javascript:telaAtencao('${requestScope.atencaoIndeferimento}');carregarPaginaAposAdicionarImagem(${indicadorAdicionarFoto},${idMotivoEncerramento});">
<div id="formDiv">
<html:form action="/encerrarOrdemServicoAction"
	...
	<html:hidden property="observacaoEncerramentoHidden" />
	<html:hidden property="idMotivoEncerramentoHidden" />
	<html:hidden property="verificadorReloadPagina" />
								...
								<tr bgcolor="#cbe5fe">
									...
									<table border="0" width="100%">
										<tr>
											...
											<td>
											<html:text property="dataEncerramento" size="20" readonly="true" /> dd/mm/aaaa
											<%-- <html:text property="dataEncerramento" size="10"
										...
										<tr>
											...
											<td width="69%"><html:select property="idMotivoEncerramento"
												tabindex="2" onchange="carregarCampos(3);">
										...
										<logic:notEmpty name="EncerrarOrdemServicoActionForm"
											...
											<logic:equal name="EncerrarOrdemServicoActionForm"
												...
												<tr>
													<td><b>Data de Execução: <font color="#ff0000">*</font></b></td>
													<td>
													<html:text property="dataExecucao" size="10" maxlength="10" tabindex="1" 
														onkeyup="mascaraDataNova(this, event);" 
														onkeypress="return isCampoNumerico(event);"
														onchange="validarCampoDataComMensagemLimpandoCampo(document.forms[0].dataExecucao, 'Data de Execução')"/>
												...
												<tr>
													<td><b>Hora de Execução: <font color="#ff0000">*</font></b></td>
													<td>
														<html:text property="horaExecucao" size="5"	maxlength="5" tabindex="1" 
														onkeyup="mascaraHora(this, event);" 
														onkeypress="return isCampoNumerico(event);"
														onchange="validaHoraMinuto(document.forms[0].horaExecucao.value)"/>
													</td>
												</tr>
					...
					<table width="100%">
						<tr>
							...
							<div><input name="ButtonVoltar" type="button"
								...
								onclick="javascript:redirecionarSubmit('${sessionScope.retornoTela}')">							
								<input name="ButtonLimpar" type="button" class="bottonRightCol" value="Desfazer"
									onclick="window.location.href='/gsan/exibirEncerrarOrdemServicoAction.do?numeroOS=${sessionScope.numeroOS}&retornoTela=exibirConsultarDadosOrdemServicoAction.do?numeroOS=${sessionScope.numeroOS}'"
									 	/>
							</div>
							</td>							
							<td>
