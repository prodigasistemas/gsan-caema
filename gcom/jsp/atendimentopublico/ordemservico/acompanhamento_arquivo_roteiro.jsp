	...
	<head>
		...
		<script type="text/javascript">
			function facilitador(objeto){
				if (objeto.id == "0" || objeto.id == undefined || objeto.id == ""){
					objeto.id = "1";
					marcarDesmarcarTodosLista(objeto,true);
				}
				else{
					objeto.id = "0";
					marcarDesmarcarTodosLista(objeto,false);
				}
			}
			function marcarDesmarcarTodosLista(objeto,booleano){
				var count = 0;
				var elementChecked;
				for(indice = 0; indice < form.idsRegistros.length; indice++){
					if (form.idsRegistros[indice].type == "checkbox" && form.idsRegistros[indice].checked == true) {
						elementChecked =  form.idsRegistros[indice].value;
						if(count > 0){
							objeto.id = "0";
				...
				for(i = 0; i <form.idOsProgramacaoAcompanhamentoServico.length; i++){
					var elemento = document.forms[0].idOsProgramacaoAcompanhamentoServico[i];
					
					var myString = new String(elemento.value);
					
					var splitString = myString.split("___");
					
					chave = splitString[0];
					if (elemento.type == "checkbox" && elemento.value == chave+"___"+elementChecked ){
...
*/
				...
				$(document).ready(function(){ 
					$('[name=idsRegistros]').change(function(){
						if($('[name=idsRegistros]:checked').size() > 0){
							$('#cadastrarMensagem, #incluirOs, .todos').removeAttr('disabled');
						}else{
							$('#cadastrarMensagem, #incluirOs, .todos').attr('disabled', true);
						}
					});
		
					$('[name=idOsProgramacaoAcompanhamentoServico]').change(function(){
						if($('[name=idOsProgramacaoAcompanhamentoServico]:checked').size() > 0){
							$('#remanejarOsEquipe, #informarSituacaoOS, #atualizarOsEquipe, #reordenarSequencialOS, #excluirOsEquipe').removeAttr('disabled');
						}else{
							$('#remanejarOsEquipe, #informarSituacaoOS, #atualizarOsEquipe, #reordenarSequencialOS, #excluirOsEquipe').attr('disabled', true);
						}
					});
		...
		<logic:notPresent name="exibirBotoes" scope="request">
			<script type="text/javascript">
				$(document).ready(function(){
					$('#cadastrarMensagem, #incluirOs, #remanejarOsEquipe, #informarSituacaoOS, #atualizarOsEquipe, #reordenarSequencialOS, #excluirOsEquipe,.todos').attr('disabled', true);
		...
		<script type="text/javascript">
			...
			function remanejarOs(){
				chave = selecionouOrdemServicoSemRestricao();
				if(chave != false){
					myString = new String(chave);
					splitString = myString.split("___");
					...
					var url = 'exibirAcompanhamentoArquivosRoteiroRemanejarOrdemServicoAction.do?veioAcompanhamentoRoteiro=true&chaves='+obterValorCheckboxMarcadoPorNome("idOsProgramacaoAcompanhamentoServico")+'&chaveArquivo='+chaveArquivo;
					abrirPopup(url,400,600);
				}
			}
			...
			function excluirOs(){
				...
				if(chave != false){
					...
					var url = 'acompanhamentoArquivosRoteiroExcluirOrdemServicoAction.do?chaveArquivo='+chaveArquivo;
					form.action = url;
				    form.submit();
				}
			}
			...
			function selecionouOrdemServicoSemRestricao(){
				var form = document.forms[0];
			    var selecionados = form.elements['idOsProgramacaoAcompanhamentoServico'];
				var jaSelecionado = false;
				var retorno = false;
			...
			function abrirEtapas(id){
			
				
				var url = 'exibirEtapasOSAcompanhamentoServicoAction.do?chave='+id;
				abrirPopup(url,330,720);
																											...
																											<logic:iterate name="acompanhamentoArquivosRoteiro"
																												...
																												if (cont2 % 2 == 0) {%>
																													...
																													<tr bgcolor="#FFFFFF">
																														...
																														<td width="5%">
																															<div align="center">
																																<logic:present name="osProgramacaoAcompServico" property="idOsProgramacaoAcompanhamentoServico">
																																	...
																																	<html:multibox property="idOsProgramacaoAcompanhamentoServico" 
																														...
																														<!-- RM 9304 -->
																														<logic:present name="osProgramacaoAcompServico" property="hint">
																															<td width="8%" align="center"
																																onmouseover="this.T_BGCOLOR='whitesmoke';this.T_FONTCOLOR='black';return escape('${osProgramacaoAcompServico.hint}'); ">
																																<a href="javascript:abrirPopup('exibirConsultarDadosOrdemServicoPopupAction.do?numeroOS=${osProgramacaoAcompServico.idOrdemServico}&acompanhamento=sim',480,650);"')"><font color="#FFA500"><strong>${osProgramacaoAcompServico.idOrdemServico}</strong></font></a>
																															</td>
																								                     	</logic:present>
																								                     	<logic:notPresent name="osProgramacaoAcompServico" property="hint">
																									                     	<td width="8%" align="center">
																									                     	<a href="javascript:abrirPopup('exibirConsultarDadosOrdemServicoPopupAction.do?numeroOS=${osProgramacaoAcompServico.idOrdemServico}&acompanhamento=sim',480,650);"')">${osProgramacaoAcompServico.idOrdemServico}</a>
																									                     	</td>
																								                     	</logic:notPresent>																					                     		
																														<!-- Fim RM 9304 -->
																														...
																														<!-- RM 6655 -->
																															<c:choose>
																																<c:when test='${osProgramacaoAcompServico.dsSituacao == "ENCERRADO"}'>
																																	<c:choose>
																																		<c:when test='${osProgramacaoAcompServico.icNovaEtapa == "1"}'>
																																			<div align="center">
																																				<a style="color:#FF0000" href="javascript:abrirEtapas(${osProgramacaoAcompServico.idOrdemServico});">
																																			${osProgramacaoAcompServico.dsSituacao}/PENDENTE</a></div>
																																		</c:when>
																																		<c:otherwise>
																																			<div align="center">																																				
																																				${osProgramacaoAcompServico.dsSituacao}</div>
																																		</c:otherwise>
																																	</c:choose>
																																</c:when>
																																<c:otherwise>
																														...
																														<logic:present name="osProgramacaoAcompServico" property="hint2">
																															<td width="15%" align="center"
																																onmouseover="this.T_BGCOLOR='whitesmoke';this.T_FONTCOLOR='black';return escape('${osProgramacaoAcompServico.hint2}'); ">
																																<font color="#FF0000">${osProgramacaoAcompServico.dsServicoTipo}</font>
																															</td>
																														</logic:present>
																														<logic:notPresent name="osProgramacaoAcompServico" property="hint2">
																															<td width="15%">
														...
														<tr>
															<td align="left">
								                        		...
								                        		<input name="ButtonAtualizarOs"
								                        			...
								                        			>
								                        		<input name="ButtonExcluirOs"
								                        			type="button"
								                        			class="bottonRightCol"
																	onClick="javascript:excluirOs();"
								                        			value="Excluir OS"
								                        			tabindex="15"
								                        			id="excluirOsEquipe"
								                        			>
															...
															<td colspan="2" align="left">
																...
																<input name="ButtonReordenarSequencial"
																	...
																	onClick="javascript:reordenarSequencial();"
								                      				...
								                      				>
								                      			<input name="ButtonTodos"
								                      				type="button" 
								                      				class="bottonRightCol" 
																	onClick="javascript:facilitador(this);"
								                      				value="Todos" tabindex="14" name="todos"
								                      				>
															</td>
														</tr>
													</table>
