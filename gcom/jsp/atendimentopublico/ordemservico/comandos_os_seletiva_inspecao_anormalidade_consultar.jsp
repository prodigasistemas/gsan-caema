...
<SCRIPT LANGUAGE="JavaScript">
	...
	function gerarTxtCorrecaoAnormalidade(){
		var form = document.forms[0];
	...
	function gerarTxtMicromedicao(){
		var form = document.forms[0];
	    var selecionados = form.elements['idRegistro'];
		var retorno = false;

		if (selecionouComando()) {
	  		form.action = 'exibirGerarArquivoTextoOrdensServicoSmartphoneAction.do?comando='+comandoSelecionado()+'&identificadorOS=2';	  		
	    	form.submit();
	  	}
	}
		...
		<tr>
			...
			<table width="100%" border="0" align="center" cellpadding="0"
				...
				<tr>
					...
					<c:choose>
						<c:when test="${not empty ConsultarComandosOSSeletivaInspecaoAnormalidadeActionForm.idTipoOS}">
							<td class="parabg">Consultar Comandos de OS Seletiva de Micromedição</td>
						</c:when>
						<c:otherwise>
			...
			<table width="100%" border="0" >
				...
				<tr>
					...
					<td><html:text maxlength="9" property="idEmpresa" size="6"
						tabindex="14" 
					...
					<a href="javascript:pesquisarEmpresa();"><img
						...
						<html:text property="nomeEmpresa" size="25" maxlength="25"
							readonly="true"
				...
				<tr>
					...
					<td>
						...
						<logic:present name="localidadeInexistente" scope="request">
							<html:text property="nomeLocalidade" size="28" maxlength="30" readonly="true" 
								style="border: 0pt none ; background-color:#EFEFEF; color: #ff0000" />
						</logic:present> 
						<logic:notPresent name="localidadeInexistente" scope="request">
							<html:text property="nomeLocalidade" size="28"
				...
				<tr>
					...
					<!-- validaEnterComMensagem(event, 'exibirConsultarComandosOSSeletivaInspecaoAnormalidadeAction.do', 'localidadeFinal', 'Localidade Final'); -->
						...
						<logic:present name="localidadeFinalInexistente" scope="request">
							<html:text property="nomeLocalidadeFinal" size="28" maxlength="30" readonly="true" 
								style="border: 0pt none ; background-color:#EFEFEF; color: #ff0000" />
						</logic:present> 
						<logic:notPresent name="localidadeFinalInexistente" scope="request">
							<html:text property="nomeLocalidadeFinal" size="28"
								maxlength="30" readonly="true"
								style="background-color:#EFEFEF; border:0; color: #000000" />
						</logic:notPresent> 
				...
				<tr>
					...
					<logic:present
						...
						<html:text property="descricaoSetorComercialOrigem" size="28"
							maxlength="30" readonly="true"
							style="border: 0pt none ; background-color:#EFEFEF; color: #ff0000" />
					</logic:present> 
					<logic:notPresent
				...
				<tr>
					...
					<td><html:text maxlength="3" property="codigoSetorComercialDestino"
						...
						<html:text property="descricaoSetorComercialDestino" size="28"
							maxlength="30" readonly="true"
							style="border: 0pt none ; background-color:#EFEFEF; color: #ff0000" />
							...
							<c:choose>
								<c:when test="${not empty ConsultarComandosOSSeletivaInspecaoAnormalidadeActionForm.idTipoOS}">
									<input name="Button" type="button" class="bottonRightCol"
										...
										onclick="window.location.href='<html:rewrite page="/exibirConsultarComandosOSSeletivaInspecaoAnormalidadeAction.do?idTipoOS=2&menu=sim"/>'">
								</c:when>
								<c:otherwise>
									<input name="Button" type="button" class="bottonRightCol"
										value="Desfazer" align="left"
							...
							<c:when test="${not empty ConsultarComandosOSSeletivaInspecaoAnormalidadeActionForm.idTipoOS}">
								<td align="right">
									<input type="button" name="encerrar"
										class="bottonRightCol" value="Gerar TXT Micromedição"
										onClick="javascript:gerarTxtMicromedicao();" />	
								</td>
