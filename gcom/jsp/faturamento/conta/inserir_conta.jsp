...
function habilitacaoCamposEsgoto(listBoxEsgoto, consumoFaturadoEsgoto, percentualEsgoto){
 ...
 function limparForm(){
  ...
  function validarInsercao(form){
  	  var VOLUME_AGUA_CONTRATO_DEMANDA = document.getElementById("VOLUME_AGUA_CONTRATO_DEMANDA").value;
  	  if (validarForm(document.forms[0], true) != false){
	  	if(VOLUME_AGUA_CONTRATO_DEMANDA != '' && form.consumoAgua.value != ''){
	  		if(parseInt(form.consumoAgua.value) < parseInt(VOLUME_AGUA_CONTRATO_DEMANDA)){
	  			var msg = 'Consumo de água informado menor que o volume de água contratado através do contrato de demanda(' + VOLUME_AGUA_CONTRATO_DEMANDA + ') m³. Confirma ?';
	  			if(confirm(msg)){
...
<div id="formDiv"><html:form action="/exibirInserirContaAction" method="post">
	...
	<INPUT type="hidden" ID="VOLUME_AGUA_CONTRATO_DEMANDA" value="${sessionScope.volumeAguaContratoDemanda}" />
