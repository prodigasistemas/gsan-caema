  ...
  if (validarForm(document.forms[0], true) != false){
	  	form.substituirClienteConta.value = true;
	  	 if(document.getElementById("CLIENTE_DIFERENTE").value == "true"){
			var x;
			var sim=confirm("Deseja substituir o(s) cliente(s) vinculado(s) à conta pelo(s) cliente(s) atualmente vinculado(s) ao imóvel?");
			if (sim==true){
				form.substituirClienteConta.value = true;
			}
			else{
...
function verificarAlterarContaMes(idConta) {

	var form = document.forms[0];
	var url = "";

	if (form.alterarContaMesCheck.checked == true) {
		form.alterarContaMes.value = 'true';	
		form.action = "/gsan/exibirRetificarContaAction.do?selecionouAlterarContaMes=true";
	} else {
		form.alterarContaMes.value = 'false';
		form.action = "/gsan/exibirRetificarContaAction.do?selecionouAlterarContaMes=false";
	}

	form.submit();
}

function checkAlterarContaMes() {

	var form = document.forms[0];
	
	if (form.alterarContaMes.value.toUpperCase() == 'TRUE') {
		form.alterarContaMesCheck.checked = true;
		
		form.situacaoAguaConta.disabled = true;
		form.situacaoEsgotoConta.disabled = true;
	} else {
		form.alterarContaMesCheck.checked = false;
		habilitacaoCamposTela();
	}
}
...
<body leftmargin="5" topmargin="5" onload="limitTextArea(document.forms[0].observacao, 300, document.getElementById('utilizado'), document.getElementById('limite'));habilitacaoConsumoEsgotoEPercentualColeta();habilitacaoCamposTela();checkAlterarContaMes();" >

<div id="formDiv">
<html:form action="/exibirRetificarContaAction" method="post" >

<input type="hidden" id="SELECIONOU_ALTERAR_CONTA_MES" value="${requestScope.selecionouAlterarContaMes}" />	
<input TYPE="hidden"  name="alterarContaMes" value="<bean:write name="RetificarContaActionForm" property="alterarContaMes"/>"/>
          ...
          <td colspan="4">
				...
				<tr bgcolor="#cbe5fe">
					<td colspan="4" width="100%" align="center">
						...
						<tr> 
							<td height="10"><strong>Mês e Ano da Conta:</strong></td>
          					<td height="10">
          						<html:text property="mesAnoConta" size="8" readonly="true" style="background-color:#EFEFEF; border:0"/>
          						<span style="padding-left: 70px;">
	          						<logic:present name="exibirCheckBoxAlteracaoContaMes">
	          							<span style="vertical-align: 0px;">
		          							<input width="5" type="checkbox" name="alterarContaMesCheck" checked="<bean:write name="RetificarContaActionForm" property="alterarContaMes"/>" value="<bean:write name="RetificarContaActionForm" property="alterarContaMes"/>"
				   									onclick="javascript:verificarAlterarContaMes('${sessionScope.contaID}')" />
		          						</span>
		          						<span style="vertical-align: 4px;">
		          							<strong>Alteração conta mês</strong>
		          						</span>
	          						</logic:present>
          						</span>
							</td>
			    ...
			    <tr bgcolor="#cbe5fe">
					<td width="100%" align="center">
						...
						<table width="100%" border="0">
							<tr>
						        <td>
									<html:textarea property="observacao" cols="40" rows="4" style="min-height: 60px;min-width: 400px; max-width: 400px; max-height: 100px;"
										onkeyup="limitTextArea(document.forms[0].observacao, 300, document.getElementById('utilizado'), document.getElementById('limite'));"/><br>
