...
function emitirSegundaViaConta(comTaxa, semCodigoBarra){
	var clienteImovelId = document.getElementById("ID_CLIENTE_IMOVEL").value;
	var clienteContaId  = document.getElementById("ID_CLIENTE_CONTA").value;
	var url = "";
	var continuar = true;
	
	if(!comTaxa && semCodigoBarra){		
		url = 'gerarRelatorio2ViaContaAction.do?contaSemCodigoBarras=1&idClienteImovel='+clienteImovelId+'&idClienteConta='+clienteContaId;
	}else if(comTaxa && semCodigoBarra){
		if (confirm('A impressão da 2ª Via de Conta irá gerar taxa de cobranca. Confirma?')){
			url = 'gerarRelatorio2ViaContaAction.do?contaSemCodigoBarras=1idClienteImovel='+clienteImovelId+'&idClienteConta='+clienteContaId;
		}else{
			continuar = false;
		}	
	}else if(!comTaxa  && !semCodigoBarra){
		url = 'gerarRelatorio2ViaContaAction.do?idClienteImovel='+clienteImovelId+'&idClienteConta='+clienteContaId;
	}else if(comTaxa && !semCodigoBarra){
		if(confirm('A impressão da 2ª Via de Conta irá gerar taxa de cobranca. Confirma?')){
			url ='gerarRelatorio2ViaContaAction.do?idClienteImovel='+clienteImovelId+'&idClienteConta='+clienteContaId;
		}else{
	...
	if(continuar){
		if(document.getElementById("CLIENTE_DIFERENTE").value == "true"){
			if(confirm("Caso deseje emitir a segunda via em nome do cliente que está vinculado à conta (opção padrão), clique no botão 'OK'. Caso deseje a conta em nome do atual cliente vinculado ao imóvel, clique no botão 'Cancelar'.")){
...
<input type="hidden" id="CLIENTE_DIFERENTE" value="${sessionScope.clienteDiferente}" />
<input type="hidden" id="ID_CLIENTE_IMOVEL" value="${sessionScope.idClienteImovel}" />
<input type="hidden" id="ID_CLIENTE_CONTA" value="${sessionScope.idClienteConta}" />
<input type="hidden" id="TIPO_CONTA" value="${requestScope.tipoconta}" />
	...
	<tr>
		...
		<logic:present name="conta" scope="session">
			<table width="100%" border="0">
				...
				<tr>
					...
					<td>
						<logic:equal name="conta" property="indicadorContaAlterada" value="1">
							<input type="text" 
								align="middle"
								value="Conta Alterada" 
								name="indicadorContaAlterada"
								size="20" 
								readonly="readonly"
								style="background-color:#EFEFEF; border:0" />
						</logic:equal>
				...
				<tr>
					<td><strong>Descrição da Retifica&ccedil;&atilde;o:</strong></td>
					<td colspan="1">
						<logic:present name="conta" property="descricaoObservacaoRetificacao">
							<html:text name="conta"
								property="descricaoObservacaoRetificacao"
								size="35" 
								readonly="true"
								style="background-color:#EFEFEF; border:0" />
						</logic:present> 

						<logic:notPresent name="conta" property="descricaoObservacaoRetificacao">
							<input type="text" 
								name="descricaoObservacaoRetificacao" 
								size="35"
								readonly="true" 
								style="background-color:#EFEFEF; border:0" />
						</logic:notPresent>
				...
				<logic:present name="conta" property="registroAtendimento">
					<tr>
						<td><strong>Número da RA:</strong></td>
						<td><strong>
							<a href="javascript:abrirPopup('exibirConsultarRegistroAtendimentoPopupAction.do?numeroRA='+<bean:write name="conta" property="registroAtendimento.id"/>, 400, 800);">
								<bean:write name="conta" property="registroAtendimento.id"/>&nbsp;
							</a>
						</strong></td>
					</tr>
				</logic:present>
				...
				<logic:present name="contaPaga" scope="session">
					...
					<tr>
						<td colspan="4"><strong>Dados do Pagamento:</strong></td>
					</tr>
		...
		<logic:present name="contaHistorico" scope="session">
			<table width="100%" border="0">
				...
				<tr>
					<td><strong>Observação da Retifica&ccedil;&atilde;o:</strong></td>
					<td colspan="1">
						<logic:present name="contaHistorico" property="descricaoObservacaoRetificacao">
							<html:text name="contaHistorico"
								property="descricaoObservacaoRetificacao"
								size="35" 
								readonly="true"
						...
						<logic:notPresent name="contaHistorico" property="descricaoObservacaoRetificacao">
							<input type="text" 
								name="descricaoObservacaoRetificacao" 
								size="35"
								readonly="true" 
				...
				<logic:present name="contaHistorico" property="registroAtendimento">
					<tr>
						<td><strong>Número da RA:</strong></td>
						<td><strong>
							<a href="javascript:abrirPopup('exibirConsultarRegistroAtendimentoPopupAction.do?numeroRA='+<bean:write name="contaHistorico" property="registroAtendimento.id"/>, 400, 800);">
								<bean:write name="contaHistorico" property="registroAtendimento.id"/>&nbsp;
							</a>
						</strong></td>
					</tr>
				</logic:present>
									...
									<tr bgcolor="#FFFFFF">
										...
										<td width="6%" align="center">
				                        	...
				                        	<logic:equal name="contaClienteAnterior"  value="1" property="indicadorNomeConta" >
				                        		<input type="radio" name="contaClienteAnterior" checked="checked" disabled="disabled" />
				                        	</logic:equal>
				                        	<logic:notEqual name="contaClienteAnterior" value="1" property="indicadorNomeConta" >
				                        		<input type="radio" name="contaClienteAnterior" disabled="disabled"  />
				                        	</logic:notEqual>
				                        	</div>
										</td>
										<td width="10%">
					...
					<logic:present name="emitirSegundaVia" scope="request">
						<logic:present name="contaSemCodigoBarras" scope="request">
							...
							<input type="button" 
								...
								class="bottonRightCol"
								onclick="javascript:emitirSegundaViaConta(false, true);" />
							</logic:present>
							...
							<input type="button" 
								...
								value="Emitir 2ª Via de Conta"
								class="bottonRightCol"
								onclick="javascript:emitirSegundaViaConta(true, true);" />
							</logic:notPresent>
						</logic:present> 
						...
						<logic:notPresent name="contaSemCodigoBarras" scope="request">
							...
							<input type="button" 
								...
								value="Emitir 2ª Via de Conta"
								class="bottonRightCol"
								onclick="javascript:emitirSegundaViaConta(false, false);" />
							</logic:present>
							<logic:notPresent name="naoCobrarTaxa">
							<input type="button" 
								name="" 
								value="Emitir 2ª Via de Conta"
								class="bottonRightCol"
								onclick="javascript:emitirSegundaViaConta(true, false);" />
							</logic:notPresent>
						</logic:notPresent>
					</logic:present> 
