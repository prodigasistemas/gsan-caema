  ...
  if(validaForm(form)){
  	...
  	if(form.quatidadeConta.value == "" || form.quatidadeConta.value == 0){
    	...
    	if (form.idGrupoFaturamento != null && form.idGrupoFaturamento.value.length > 0){
    		urlCancelamentoConta = urlCancelamentoConta + "&idGrupoFaturamento=" + form.idGrupoFaturamento.value;
        	if (form.codigoClienteSuperior != null && form.codigoClienteSuperior.value.length > 0){
        		urlCancelamentoConta = urlCancelamentoConta + "&codigoClienteSuerior=" + form.codigoClienteSuperior.value;
        	}
    	}
...
function habilitarBotoes(){
	...
	if (idGrupoFaturamento != null && idGrupoFaturamento.value.length > 0 && form.codigoClienteSuperior == null){
...
function concederCredito(){
  ...
  if(validaForm(form)){
  	...
  	if(form.quatidadeConta.value == "" || form.quatidadeConta.value == 0){
    	...
    	var urlConcederCredito = "/gsan/exibirConcederCreditoConjuntoContaAction.do?mesAno="+ form.mesAnoConta.value;
    	urlConcederCredito = urlConcederCredito + "&mesAnoFim="+ form.mesAnoContaFinal.value; 
    	urlConcederCredito = urlConcederCredito + "&indicadorContaPaga="+ indicadorContaPaga;
    	urlConcederCredito = urlConcederCredito + "&inicio=sim";   
    	
    	if (form.idGrupoFaturamento != null && form.idGrupoFaturamento.value.length > 0){
    		urlConcederCredito = urlConcederCredito + "&idGrupoFaturamento=" + form.idGrupoFaturamento.value;
    	}
    	
    	if (form.codigoClienteSuperior != null && form.codigoClienteSuperior.value.length > 0){
    		urlConcederCredito = urlConcederCredito + "&codigoCliente=" + form.codigoClienteSuperior.value;
    	}
    	else if (form.codigoCliente != null && form.codigoCliente.value.length > 0){
    		urlConcederCredito = urlConcederCredito + "&codigoCliente=" + form.codigoCliente.value;
    	}
    	
    	if (form.dataVencimentoInicial.value.length > 0){
    		urlConcederCredito = urlConcederCredito + "&dataVencimentoContaInicial=" + form.dataVencimentoInicial.value;
    		
    		if (form.dataVencimentoFinal.value.length > 0){
    			urlConcederCredito = urlConcederCredito + "&dataVencimentoContaFinal=" + form.dataVencimentoFinal.value;
    		}
    		else{
    			urlConcederCredito = urlConcederCredito + "&dataVencimentoContaFinal=" + form.dataVencimentoInicial.value;
    		}  
    	}
    	
    	abrirPopup(urlConcederCredito, 460, 630);  
...
<logic:present name="mensagemSuceso">
	<body leftmargin="5" topmargin="5" onload="javascript:alert('${requestScope.mensagemSuceso}');habilitarBotoes();habilitarIntervalo();">
</logic:present>
			...
			<table width="100%" border="0">
            	  ...
            	  <!--                  DADOS DA CONTA                        -->
                	...
                	<tr bgcolor="#cbe5fe"> 
                  	  <td width="100%" align="center"> 
						  ...
						  <tr>
							<td height="10" width="160">
								<strong>Data Vencimento Conta:</strong>
							</td>
							<td width="403">
						  ...
						  <tr>
							...
							<logic:equal name="usuarioPodeAtualizarRetificarContasPagas" value="true" >
								...
								<td><strong> 
									<html:radio property="indicadorContaPaga" value="1" onclick="limparContas(document.forms[0]);" />SIM 
									<html:radio	property="indicadorContaPaga" value="2" onclick="limparContas(document.forms[0]);" />NÃO
									<html:radio	property="indicadorContaPaga" value="3" onclick="limparContas(document.forms[0]);" />AMBOS								
								</strong></td>												
							</logic:equal>
							...
							<tr>													
								<td height="10" width="160">
									<strong>Valor Total Contas:</strong>
								</td>
								<td width="400">
									<html:text  property="valorTotalContas" 
												size="7" 
												readonly="true" 
												style="background-color:#EFEFEF; border:0; color: #000000"/>
								</td>
				...
				<tr>
					<td colspan="3" height="5">
						...
						<gsan:controleAcessoBotao
							name="Button" value="Conceder Crédito"
							onclick="javascript:concederCredito(document.forms[0]);" url="retificarConjuntoContaAction.do" style="width: 170px"/>
