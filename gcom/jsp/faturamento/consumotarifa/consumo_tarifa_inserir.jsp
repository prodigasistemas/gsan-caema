...
</script><html:javascript staticJavascript="false"  formName="InserirConsumoTarifaActionForm" />
...
function abrirCategoria(){
  	...
  	if ((form.descTarifa.value == '') && (form.slcDescTarifa[form.slcDescTarifa.selectedIndex].value == '-1')){
  	  ...
  	  if (validateRequired(form) && validateCaracterEspecial(form) && validateDate(form)){
  	    if(form.indicadorContratoDemanda[0].checked){
  	    	indicador = '1';
  	    }else{
  	    	indicador = '2';
  	    }
  	    
  	    window.open('exibirInserirCategoriaConsumoTarifaAction.do?limpa=1&parametroVigencia='+form.dataVigencia.value+'&parametroDescricao='+form.descTarifa.value+'&parametroSelect='+form.slcDescTarifa.value+'&parametroIdLigacaoAguaPerfil='+form.idLigacaoAguaPerfil.value+'&parametroIdTarifaTipoCalculo='+form.idTarifaTipoCalculo.value+'&parametroIndicador='+indicador, 'pesquisar','location=no,screenY=0,screenX=0,menubar=no,status=no,toolbar=no,scrollbars=yes,resizable=no,width=630,height=400');
  	  }
  	}
 }
			...
			<table width="100%">
				...
				<tr>
					<td colspan="3"><strong> <font color="#ff0000"> 
						...
						<logic:present name="habilitarVoltar" scope="session" >	
							<logic:equal name="habilitarVoltar" value="1">
								<input name="Submit23" class="bottonRightCol" value="Voltar"
								type="button"
								onclick="window.location.href='/gsan/exibirInserirContratoDemandaAction.do'">
							</logic:equal>
							<logic:equal name="habilitarVoltar" value="2">
								<input name="Submit23" class="bottonRightCol" value="Voltar"
								type="button"
								onclick="window.location.href='/gsan/exibirAtualizarContratoDemandaAction.do'">
							</logic:equal>
						</logic:present>
