...
$("#grid").mousemove(function(){
	...
	$("#grid tbody tr").each(function(){
		...
		if(selected != 'undefined' && selected !=null && selected == "true"){
...
<SCRIPT LANGUAGE="JavaScript"><!--
	...
	function habilidarDesabilitarPercImoveis(){
		var form = document.forms[0];
		if(form.qtdImoveisMaximaComando.value == ''){
			for(var i = 0;i < form.percentualImoveisFaixa.length; i++){
				//if(apagarRegistros)
				form.percentualImoveisFaixa[i].value = "";
				form.percentualImoveisFaixa[i].disabled = true;
			}
		}
		else{
			for(var i = 0;i < form.percentualImoveisFaixa.length; i++){
				form.percentualImoveisFaixa[i].value = form.hiddenPercentual[i].value.replace(".",",");
				form.percentualImoveisFaixa[i].disabled = false;
			}
		}
	}
	
	function habilitarDesabilitarBotaoSelecionar(){
		var form = document.forms[0];
	...
	function recarregarPagina(){
		var form = document.forms[0];
		if(form.qtdImoveisMaximaComando != null){
			form.qtdImoveisMaximaComando.value = '';
		}
		form.action = "exibirInformarContasEmCobrancaAction.do?limparEmpresa=ok";
		form.submit();
	}
	
	function carregarCamposEnter(event,obj){
		var form = document.forms[0];
	...
	function validarBlur(){
		var form = document.forms[0];
		if(form.qtdImoveisMaximaComando.value > 0){
			habilidarDesabilitarPercImoveis();
			document.forms[0].percentualImoveisFaixa[0].focus();								
		}
		else{
			if(form.qtdImoveisMaximaComando.value != ''){
				if(form.qtdImoveisMaximaComando.value == 0){
					alert('Zero (0) não é um valor válido para esse campo.');
					form.qtdImoveisMaximaComando.value = '';
					habilidarDesabilitarPercImoveis();
				}else{
					alert('Valor inválido para esse campo.')
					form.qtdImoveisMaximaComando.value = '';
					habilidarDesabilitarPercImoveis();
				}
			}
	...
	function limparQtdImoveisEnter(event){ 
		var form = document.forms[0];
		var codigo = event.keyCode;
		if (codigo == 13) {
			if (form.qtdImoveisMaximaComando != null)
...
<html:form action="/exibirInformarContasEmCobrancaAction"
	...
	<table width="770" border="0" cellspacing="5" cellpadding="0">
		<tr>
			...
			<table width="100%" border="0">
				...
				<tr>
					...
					<td><html:text maxlength="9" property="idImovel" size="9"
						...
						onkeyup="somente_numero_zero_a_nove(this);validaEnterComMensagem(event, 'exibirInformarContasEmCobrancaAction.do', 'idImovel', 'Imóvel'); limparImovelTecla();bloqueiaDados();" 
						onblur="habilitarDesabilitarBotaoSelecionar();recarregarPagina();"/>
					<a
					...
					</logic:notPresent> <a href="javascript:limparImovel();limparTotalizacao();recarregarPagina();"> <img
				...
				<tr>
					...
					<td><html:text maxlength="9" property="idCliente" size="9"
						...
						onkeyup="somente_numero_zero_a_nove(this);validaEnterComMensagem(event, 'exibirInformarContasEmCobrancaAction.do', 'idCliente', 'Cliente'); limparClienteTecla();bloqueiaDados();" 
						onblur="habilitarDesabilitarBotaoSelecionar();recarregarPagina();"/>
					<a
						...
						<html:text property="nomeCliente" size="40" maxlength="40"
							readonly="true"
							style="background-color:#EFEFEF; border:0; color: #000000" />
					</logic:notPresent> <a href="javascript:limparCliente();limparTotalizacao();recarregarPagina();"> <img
				...
				<tr>
					<td width="30%"><strong>Não Gerar com as Ações de Cobrança Selecionadas:</strong></td>
					<td><html:select property="idsAcoesCobrancaNaoGeracao" tabindex="3" multiple="mutiple" size="4">
						<logic:notEmpty name="colecaoAcoesCobrancaNaoGeracao">
							<option value="<%=""+ConstantesSistema.NUMERO_NAO_INFORMADO%>">&nbsp;</option>
							<html:options collection="colecaoAcoesCobrancaNaoGeracao"
				...
				<tr>
					<td width="30%"><strong>Gerar Exclusivamente com as Situações de Cobrança Selecionadas:</strong></td>
					<td><html:select property="idsCobrancaSituacao" tabindex="3" multiple="mutiple" size="4">
				...
				<tr>
					...
					<tr>
						...
						<td><html:text maxlength="3" property="idLocalidadeOrigem" size="3"
							...
							tabindex="4" onkeyup="javascript:somente_numero_zero_a_nove(this);replicarCampo(form.idLocalidadeDestino, form.idLocalidadeOrigem);limparLocalidadeOrigemTecla();bloqueiaDados();" 
					...
					<tr>
						...
						<td><html:text maxlength="3" property="codigoSetorComercialOrigem" size="3"
							...
							tabindex="5" onkeyup="javascript:somente_numero_zero_a_nove(this);replicarCampo(form.codigoSetorComercialDestino, form.codigoSetorComercialOrigem);limparSetorComercialOrigemTecla();"
							onchange="javascript:limparTotalizacao();" />
					...
					<tr>
						...
						<td><html:text maxlength="4" property="codigoQuadraInicial" size="3"
							...
							tabindex="8"
							onkeyup="javascript:somente_numero_zero_a_nove(this);replicarCampo(form.codigoQuadraFinal, form.codigoQuadraInicial);limparQuadraInicialTecla();" 
							onchange="javascript:limparTotalizacao();" />
					...
					<tr>
						...
						<td><html:text maxlength="3" property="idLocalidadeDestino"
							...
							tabindex="6" onkeyup="somente_numero_zero_a_nove(this);limparLocalidadeDestinoTecla();" 
					...
					<tr>
						...
						<td><html:text maxlength="3" property="codigoSetorComercialDestino"
							...
							onchange="javascript:limparTotalizacao();" 
							tabindex="7" onkeyup="somente_numero_zero_a_nove(this);limparSetorComercialDestinoTecla();" /> <a
					...
					<tr>
						...
						<td><html:text maxlength="4" property="codigoQuadraFinal" size="3"
							...
							tabindex="8" onkeyup="javascript:somente_numero_zero_a_nove(this);limparQuadraFinalTecla();" 
							onchange="javascript:limparTotalizacao();" />
				...
				<tr>
					...
					<td><html:text maxlength="9" property="idEmpresa" size="10"
						tabindex="14"
						onkeyup="somente_numero_zero_a_nove(this);limparQtdImoveisEnter(event); validaEnterComMensagem(event, 'exibirInformarContasEmCobrancaAction.do?limparEmpresa=ok', 'idEmpresa', 'Empresa'); limparEmpresaTecla();" 
						onchange="javascript:limparTotalizacao();"
						onblur="recarregarPagina()" />
					<a href="javascript:pesquisarEmpresa();"><img
						...
						<html:text property="nomeEmpresa" size="40" maxlength="40"
							readonly="true"
							style="background-color:#EFEFEF; border:0; color: #000000" />
					</logic:notPresent> <a href="javascript:limparEmpresa();limparTotalizacao();recarregarPagina();"> <img
				...
				<tr>
					...
					<td><strong> <html:text maxlength="7" property="referenciaInicial"
						size="7" tabindex="8"
						onkeyup="somente_numero(this);mascaraAnoMes(this, event); replicarCampo(document.forms[0].referenciaFinal, document.forms[0].referenciaInicial);"
						onchange="javascript:limparTotalizacao();" />
				...
				<tr>
					...
					a</strong> <html:text maxlength="10" property="dataVencimentoFinal"
						tabindex="11" size="10" onkeyup="somente_numero(this);mascaraData(this, event);" 
						onchange="javascript:limparTotalizacao();" /> <a
				...
				<tr>
					...
					<td><strong> <html:text property="quantidadeContasInicial" size="14"
						...
						onkeyup="somente_numero_zero_a_nove(this);replicarCampo(document.forms[0].quantidadeContasFinal, document.forms[0].quantidadeContasInicial);"
						onchange="javascript:limparTotalizacao();" 
			...
			<table width="100%" >
				<logic:present name="colecaoEmprCobFai" scope="session">
					<tr>
						<td width="30%"><strong>Quantidade de Imóveis Máxima do Comando:<font color="#FF0000">*</font></strong></td>
						<td><html:text maxlength="9" property="qtdImoveisMaximaComando" size="10"
							tabindex="14" onkeypress="return isCampoNumerico(event);"
							onblur="validarBlur(); return isCampoNumerico(event);"
							onkeyup="somente_numero_zero_a_nove(this);carregarCamposEnter(event,this)" />
						</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
					...
					<tr>
						<!-- <td  align="center" bgcolor="#99CCFF"><strong>Faixa de Cobrança</strong></td>-->
						<td  align="center" bgcolor="#99CCFF"><strong>Descrição</strong></td>
						<td  align="center" bgcolor="#99CCFF"><strong>Qtd. Mínima</strong></td>
						<td  align="center" bgcolor="#99CCFF"><strong>Perc. da Faixa</strong></td>
						<td  align="center" bgcolor="#99CCFF"><strong>Perc. de Imóveis</strong></td>
					</tr>
					
					<% int cont = 0; %>
					<logic:iterate name="colecaoEmprCobFai" id="emprCobFaixa">
						...
						<tr bgcolor="#FFFFFF">
						  <%}%>
							...
							<td align="center">
								<strong>${emprCobFaixa.descricao}</strong>
							</td>
							<td align="center">
								<strong>${emprCobFaixa.numeroMinimoContasFaixa}</strong>
							</td>
							<td align="center">
								<strong>${emprCobFaixa.percentualFaixa}</strong>
							</td>
							<td align="center">
								<html:text maxlength="6" 
	   									   property="percentualImoveisFaixa" 
	   									   size="6"  
	   									   tabindex="6" 
	   									   onkeyup="formataValorMonetario(this, 5);"
	   									   value="${emprCobFaixa.percentualImovelFaixa}"/>
	   							<input type="hidden" value="${emprCobFaixa.percentualImovelFaixa}" name="hiddenPercentual" />		   
							</td>
						</tr>
					</logic:iterate>
					<script language="javascript">javascript:habilidarDesabilitarPercImoveis();</script>
				</logic:present>
			...
			<table width="100%" >
				...
				<tr>
					...
					<td><strong> <html:text maxlength="10" property="dataInicioCiclo"
						size="10" tabindex="8" onkeyup="somente_numero(this);mascaraData(this, event);" />
					</strong> <a
       				...
       				<td colspan="4">
       					<strong>
         					...
         					<html:text property="idServicoTipo" size="7" maxlength="4" 
									onkeyup="somente_numero_zero_a_nove(this);validaEnterComMensagem(event, 'exibirInformarContasEmCobrancaAction.do?tipoPesquisa=servicoTipo', 'idServicoTipo' ,'Tipo de Serviço');" />
				...
				<td colspan="2" ><input name="Button" type="button" class="bottonRightCol"
						...
						onclick="window.location.href='<html:rewrite page="/exibirInformarContasEmCobrancaAction.do?menu=sim"/>'">
	...
	<script language="javascript">javascript:habilitarDesabilitarBotaoSelecionar();</script>
