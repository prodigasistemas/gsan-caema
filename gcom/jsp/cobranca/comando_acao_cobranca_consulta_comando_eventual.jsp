...
<!-- Begin
	...
	function alterarDataPrevista() {
		abrirPopupComSubmit('exibirAlterarDataPrevistaEncerramentoComandoAcaoCobrancaPopupAction.do?tipoComando=2', 480, 300, 'eventual');
	}
	
	function cancelarComandoAcaoCobranca() {
		botaoAvancarTelaEsperaComando('/gsan/cancelarComandoAcaoCobrancaAction.do?tipoComando=E');
	} 
										...
										<div align="center">
											<logic:present name="possuiPermissao" scope="session">
												<% if(cobrancaAcaoAtividadeComando.getCobrancaAtividade().getId().equals(CobrancaAtividade.EMITIR) 
														&& cobrancaAcaoAtividadeComando.getRealizacao() != null
														&& cobrancaAcaoAtividadeComando.getDataEncerramentoRealizada() == null) { %>
													<html:checkbox property="idsRegistros" value="${cobrancaAcaoAtividadeComando.id}" />
												<% } %>
											</logic:present>
										</div>
										</td>
