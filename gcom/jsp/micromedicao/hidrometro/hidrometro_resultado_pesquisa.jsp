	...
	<tr>
		...
		<table width="100%" border="0" bgcolor="#90c7fc">
			<tr align="left" bgcolor="#90c7fc">
				<td width="16%">
				<logic:notEmpty name="tipoMacromedidor" scope="session">
					<div align="center"><strong>Tombamento</strong></div>
				</logic:notEmpty>
				
				<logic:empty name="tipoMacromedidor" scope="session">
				...
				<c:forEach items="${sessionScope.colecaoHidrometros}"
					...
					<pg:item>
						...
						<%}%>
							...
							<logic:notEmpty name="tipoMacromedidor" scope="session">
								<c:if test="${!empty hidrometro.tombamento}" var="naoVazio">
									<logic:equal name="enviarDadosComParametro" value="SIM">
										<a href="javascript:enviarDadosParametros('exibirEfetuarSubstituicaoHidrometroAction' ,'${hidrometro.tombamento}', '${hidrometro.hidrometroMarca.descricao} FAB.: ${hidrometro.anoFabricacao }&comboSituacaoHidrometro=sim', 'hidrometro');">${hidrometro.tombamento}</a>
									</logic:equal>
									<logic:notEqual name="enviarDadosComParametro" value="SIM" >
										<a
											href="javascript:enviarDados('${hidrometro.tombamento}', '${hidrometro.hidrometroMarca.descricao} FAB.: ${hidrometro.anoFabricacao }', 'hidrometro');">${hidrometro.tombamento}</a>
									</logic:notEqual>
								</c:if>	
								<c:if test="${!naoVazio}">
									&nbsp;							
