	...
	<tr>
		...
		<table width="100%" align="center" bgcolor="#99CCFF">
			<tr align="left" height="23" bgcolor="#90c7fc">
				...
				<logic:iterate name="colecaoCliente" id="cliente" type="Cliente">
					<pg:item>
						<%cont = cont + 1;
						if (cont % 2 == 0) {%>
						<tr bgcolor="#cbe5fe">
									...
									<logic:present name="caminhoRetornoTelaPesquisaCliente" scope="session" >
										<logic:notEqual name="cliente" property="indicadorUso" value="<%=ConstantesSistema.INDICADOR_USO_DESATIVO.toString()%>">           
											<a href="javascript:enviarDadosParametros('<bean:write name="caminhoRetornoTelaPesquisaCliente" scope="session"  />','<bean:write name="cliente" property="id"/>', 
												'<%=cliente.getNome().replace("'", "\\'")%>', 'cliente');">
									...
									<logic:notPresent name="caminhoRetornoTelaPesquisaCliente" scope="session" >
										<c:choose>
											<c:when test="${tipoConsulta != null}">
												<!-- Configura o retorno pra cpf ou cnpj. Caso tenha cpf retorna o cpf caso contrario retorna cnpj-->
												<c:choose>
													<c:when test="${cliente.cpf != null}">
														<bean:define id="cpfCnpjCliente" name="cliente" property="cpfFormatado"></bean:define>
													</c:when>
													<c:when test="${cliente.cnpj != null}">
														<bean:define id="cpfCnpjCliente" name="cliente" property="cnpjFormatado"></bean:define>
													</c:when>
												</c:choose>
												<logic:notEqual name="cliente" property="indicadorUso" value="<%=ConstantesSistema.INDICADOR_USO_DESATIVO.toString()%>">            
													<a href="javascript:enviarDadosConsultaContrParcel('<bean:write name="cliente" property="id"/>', '<bean:write name="cliente" property="nome"/>', '<c:out value="${cpfCnpjCliente}"></c:out>', '<c:out value="${tipoConsulta}"></c:out>');">
														<bean:write name="cliente" property="nome" /> 
													</a>
									            </logic:notEqual>
									            <logic:equal name="cliente" property="indicadorUso" value="<%=ConstantesSistema.INDICADOR_USO_DESATIVO.toString()%>">            
