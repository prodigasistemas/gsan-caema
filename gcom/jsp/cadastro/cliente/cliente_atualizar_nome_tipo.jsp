...
function redirecionarSubmitAtualizar(id) {
	...
	redirecionarSubmit('atualizarClienteWizardAction.do?destino=1&action=exibirAtualizarClienteNomeTipoAction&idCliente=' + id);
}

function validateClienteActionForm(form) {
	
	$("input[@name='indicadorBloquearAlterarImoveis']").val($("input[@name='indicadorBloquearAlterarImoveisCampo']:checked").val());
	$("input[@name='indicadorBloquearDespesasPostais']").val($("input[@name='indicadorBloquearDespesasPostaisCampo']:checked").val());
...
function required () {
	...
	this.ac = new Array("clienteNome", "Informe Nome na Receita Federal", new Function ("varName", " return this[varName];"));
}
...
function habilitarPemissaoEspecial() {
	var form = document.forms[0];
	
	if ( form.permissaoEspHabilitarExcluirReceitaFed.value != "2" ) {
		form.btExcluirCpfCnpj.disabled = false;
	} else {
		form.btExcluirCpfCnpj.disabled = true;
	}
	
}

function excluirCpfCnpj(){
	var form = document.forms[0];

	if (confirm("Confirma remoção?")){
		form.action = "atualizarClienteWizardAction.do?destino=1&action=exibirAtualizarClienteNomeTipoAction&excluirCpfCnpj=S";
		form.submit();
	}
}
...
<body leftmargin="5" topmargin="5"
	onload="setarFoco('${requestScope.nomeCampo}');validarDiaVencimentoTecla(); verificarCampoDesabilitarCampos('${desabilitarCampos}');habilitaNome(); habilitarPemissaoEspecial();">
<div id="formDiv"><html:form action="/atualizarClienteWizardAction"
	...
	<html:hidden property="permissaoEspHabilitarExcluirReceitaFed" />
			...
			<table width="100%" border="0">
				...
				<logic:present name="indicadorNomeFantasia" scope="session">
					...
					<tr>
						...
						<html:text maxlength="100" property="clienteNome" size="50" tabindex="1" onkeyup="replicarNome();" />
						...
						<td align="right" width="100%">
							<input id="btExcluirCpfCnpj" type="button" class="bottonRightCol"
							value="Excluir CPF/CNPJ" tabindex="11" style="width: 150px"
							title="Excluir registro da consulta CPF/CNPJ feita na Receita Federal"
							 onclick="excluirCpfCnpj()">
						</td>
				...
				<tr>
					...
					<td><html:hidden property="tipoPessoaAntes" /> <html:select 
						property="tipoPessoa" tabindex="5" style="width:300px" >
						<html:option
				...
				<tr>
					...
					<logic:present name="permissaoDebitoOriundoEntregaPostal" scope="session">
						<td>
						<html:radio property="indicadorBloquearDespesasPostaisCampo"
							value="<%=ConstantesSistema.SIM.toString()%>" tabindex="9"/>
						<strong>Sim</strong> <html:radio property="indicadorBloquearDespesasPostaisCampo" 
							value="<%=ConstantesSistema.NAO.toString()%>" tabindex="9" />
						<strong>Não</strong>
						</td>
					</logic:present>
					...
					<logic:notPresent name="permissaoDebitoOriundoEntregaPostal" scope="session">
						...
						<html:radio property="indicadorBloquearDespesasPostaisCampo"
							value="<%=ConstantesSistema.SIM.toString()%>" tabindex="9" disabled="true"/>
						<strong>Sim</strong> <html:radio property="indicadorBloquearDespesasPostaisCampo" disabled="true"
							value="<%=ConstantesSistema.NAO.toString()%>" tabindex="9" />
						<strong>Não</strong>
						</td>
					</logic:notPresent>
					<html:hidden property="indicadorBloquearDespesasPostais"/>
				</tr>
				...
				<tr>
					...
					<logic:present name="permissaoIndicadorBloqueioImoveisAssociados" scope="session">
						<td>
						<html:radio property="indicadorBloquearAlterarImoveisCampo"
							value="<%=ConstantesSistema.SIM.toString()%>" tabindex="9"/>
						<strong>Sim</strong> <html:radio property="indicadorBloquearAlterarImoveisCampo" 
							value="<%=ConstantesSistema.NAO.toString()%>" tabindex="9" />
						<strong>Não</strong>
						</td>
					</logic:present>
					...
					<logic:notPresent name="permissaoIndicadorBloqueioImoveisAssociados" scope="session">
						...
						<html:radio property="indicadorBloquearAlterarImoveisCampo"
							value="<%=ConstantesSistema.SIM.toString()%>" tabindex="9" disabled="true"/>
						<strong>Sim</strong> <html:radio property="indicadorBloquearAlterarImoveisCampo"
							value="<%=ConstantesSistema.NAO.toString()%>" tabindex="9" disabled="true"/>
						<strong>Não</strong>
						</td>
					</logic:notPresent>
					<html:hidden property="indicadorBloquearAlterarImoveis"/>
				</tr>
